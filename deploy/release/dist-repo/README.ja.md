...
| ![3 ペイン表示。チャットミラー、shell セッションに繋がった稼働中ターミナル、作業ツリーの変更一覧とコミット欄](docs/img/split-ja.webp) | ![コミットグラフ。ブランチのレーン、ref チップ、コミットごとの作者と時刻](docs/img/scm-ja.webp) |
| **ペイン分割** — ミラー・ターミナル・変更一覧を並べて表示。各ペインは別タブへ切り離せます。 | **git はコンソールの中で** — コミットグラフ、差分、stage とコミットを作業コピー / worktree ごとに。 |
