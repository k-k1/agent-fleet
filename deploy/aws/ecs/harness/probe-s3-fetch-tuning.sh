...
cat >&2 <<'MIERA'
probe-s3-fetch-tuning.sh has not been run since ADR 0077 moved the engine off Managed Instances.
...
say "cluster=$CLUSTER bucket=$BUCKET key=$KEY size=$SZ configs=$CONFIGS"
