...
sweep() {
  ...
  #
  # ⚠️ It sits BEHIND the `ec2 wait` above, which can take ~40s — so a run killed during
  # the sweep terminates the instance and leaves the role and profile behind (measured:
  # af-armpush-673597 survived a mid-sweep interrupt). They cost nothing, but they
  # accumulate. If a run was interrupted, check with
  #   aws iam list-roles --query 'Roles[?starts_with(RoleName,`af-armpush`)].RoleName'
  # and unwind by hand in this same order.
