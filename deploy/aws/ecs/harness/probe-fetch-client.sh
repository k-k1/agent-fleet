...
cat >&2 <<'MIERA'
probe-fetch-client.sh has not been run since ADR 0077 moved the engine off Managed Instances.
...
say "cluster=$CLUSTER bucket=$BUCKET key=$KEY size=$SZ s5cmd=$S5V"
