...
> `readup` が塞いだ穴（2026-09-08）: 頁が 1 つ入った瞬間、読んでいたブロック（turn 182）が
> **turn 122 として描き直され**（＝React の key が変わり subtree ごと再マウント）、閉じていた
> 作業過程が**開いた状態で復活**し、高さが **4,427px → 91,296px**、読者は `182@-3752px` から
> 転写の先頭へ飛んだ（3/3 決定的）。`groupTurns` がブロックに**最初の行の idx** を付けるため、
> 頁の境目がブロックの内側に落ちると同じブロックが別の名前になる。修正後は同じ形で
> `4,427 → 8,905px` / 再マウント 0 / 開いた作業過程 0（`useStableBlockIds` と
> `captureMarkBelow`）。
