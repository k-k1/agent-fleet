...
export function sameTarget(view: View, target: OpenTarget): boolean {
  ...
  switch (t.kind) {
    ...
    // The studio (ADR 0100 decision 10). null matches null: the studio-less pane is one pane,
    // and without this a second open would lay a copy of the same localStorage draft beside it.
    case "imagegen": return c.kind === "imagegen" && c.studioId === t.studioId;
  }
}
