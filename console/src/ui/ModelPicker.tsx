...
// applies). claude/codex render as segmented buttons (small fixed catalogs);
// opencode as a <select> — its live provider/model list is unbounded in count and
// id length. Callers gate on caps.model and re-resolve the value when the kind
...
export function ModelPicker({ kind, model, onChange }: ModelPickerProps) {
  ...
  if (kind === "opencode") {
    // A stored last-used model can be missing from the fetched list (provider since
    // disconnected, or the list hasn't loaded yet): keep it visible as the selected
    // option rather than showing 既定 while actually sending the stored id.
