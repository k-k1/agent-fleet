...
// The work trace reuses the mirror's .mt-work verbatim, so the "close" control at the bottom
...
// ChatSteps renders an assistant turn's work trace (docs/log/19 §separation): the narration the model
...
export function ChatSteps({ steps, defaultOpen, live }: { steps: ChatStep[]; defaultOpen?: boolean; live?: boolean }) {
  ...
  // The heading (summary). After the bottom "close" collapses it, scroll the heading back
  // if it has drifted off the top of the screen.
...
// Flatten a turn's work trace into an ordered list of parts (narration text / tool name), then
...
// ChatToolRun renders a run of consecutive tool/mcp calls in the work trace. A lone call shows
// as a plain chip (as the mirror does for output-less traces); two or more fold into one
// collapsed "N tools · tally" summary that expands to the individual calls. Mirrors
