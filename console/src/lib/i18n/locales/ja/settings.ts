...
export const settings = {
  ...
  "agents.fleet_observe": "セッションからのフリート観測",
  "agents.note_fleet_observe":
    "セッションが、自分の置かれたフリートの状況を見て、気づいたことをあなたのメモキューに残せるようにします。開くのは4つだけです。**他セッションの状態**（作業中／入力待ち／停止中など。指示を渡した相手がまだ動いているかを確認できます）、**コンテキスト使用量**（自分の文脈が詰まってきたことに気づいて、続きを新セッションへ引き継ぐ判断ができます）、**メモキューの閲覧と追加**（頼んだ範囲の外で見つけた不具合や古い記述を、作業を中断せずあなた宛に残せます）。\n**動かす操作は一切増えません。** 他セッションへの指示、質問への回答、プランの承認、セッションの起動・停止・削除、メモの送信はこれまでどおりフリート・オペレーターと Console だけのものです。他セッションが質問やプランで止まっていることは見えますが、**その本文は渡らず**、代わりに答えることもできません（あなたの承認を肩代わりさせないためです）。\n**反映は次に起動するセッションから**で、すでに動いているセッションは再起動するまで変わりません。既定 OFF。",
  "agents.fleet_spawn": "セッションからのセッション起動",
  "agents.note_fleet_spawn":
    "セッションが、別のセッションを起こして仕事を渡し、自分が起こしたものだけを世話できるようにします。増えるのは **create_session** と、起動先を選ぶための読み取り（作業コピー一覧・選べるモデル一覧・各 CLI の残り枠）、それに**自分が起こしたセッションの出力を読む・停止する・終わったら停止を予約する・再開する**の 4 つだけです。指示を送ることも、質問に答えることも、削除・アーカイブすることもできません（自分の子であってもです）。自分が起こしていないセッションには一切触れません。\n**上限は組み込みです。** 子は 1 セッションあたり同時 3 本まで（停止やアーカイブでは枠は空かず、削除で空きます）、この方法で起こされたセッションはさらに子を起こせません、生シェルのセッションは起こせません。子は必ず**新しい worktree** で始まるので、親と作業コピーを共有しません。子の最初の指示にはミラーで「どのセッションが書いたか」のバッジが付き、エージェントには「利用者の指示ではない」と伝わります。\n**これは、誰も見ていないところで実資源を使う唯一のセッション設定です** — 子 1 本はこのワークスペースのメモリを丸ごと 1 エージェント分使い、消費トークンはあなたの請求に載り、使用量では「無人の消費」として記録されます。**反映は次に起動するセッションから**で、すでに動いているセッションは再起動するまで変わりません。既定 OFF。",
  "agents.note_fleet_spawn_needs_observe":
    "**セッションからのセッション起動**は、上の「セッションからのフリート観測」が ON のときだけ設定できます。起こした相手が終わったかどうかを知る手段が状態の確認しかないため、起動だけを単独では開けません。",
  "agents.image_generation": "画像生成",
  "agents.note_image_generation":
    "セッションが、プロンプトから画像を生成できるようにします（generate_image ツール）。生成に使うのは、いま接続済みの CLI 自身の画像生成です（下の優先順位を参照）。どの種別のセッションにも出ますが、**そのセッション自身の CLI の経路だけは選べません** — Codex セッションから Codex 経路、Antigravity セッションから Antigravity 経路を使うのは、同じ CLI をもう一度起動して枠を二重に払うだけだからです（その経路の絵は、そのセッション自身の内蔵ツールで作れます）。使える経路が 1 つも残らないときだけ、ツールごと出ません。生成物は会話をまたいで残り、Console のファイルビューアからも開けます（30 日で自動削除）。\n**画像 1 枚ごとに、生成に使った側のプラン利用枠を消費します**（Codex なら ChatGPT プラン、Antigravity なら Gemini/Antigravity プラン。テキストのターンより 3〜5 倍速く減り、ChatGPT Free では画像生成そのものが使えません）。プロンプトはその提供元へ送られます。**寸法・背景・枚数は希望であって保証ではなく**、実際に何が返ったかは結果の warnings に入ります（Codex 経路で 1024x1024 を頼んで 1254x1254 が返ることが実測されています）。縦横比だけは Antigravity 経路で実際に効きます（16:9 を頼んで 1376x768 の実測）。**依頼のときに提供元を名指しできます**（「Codex で作って」「両方で比べたい」）。名指しした場合は次送りせずその 1 つだけを使い、比較のために 2 回頼めば 2 つのプランがそれぞれ減ります。\n**反映は次に起動するセッションから**で、すでに動いているセッションは再起動するまで変わりません。既定 OFF。",
  "agents.image_provider_order": "画像生成の優先順位",
  "agents.note_image_provider_order":
    "画像を生成する提供元を試す順です。上から順に、サインイン済みでその操作ができるものが選ばれ、**呼び出しに失敗したら次へ送ります**（枠切れは実際に呼ぶまで分からないため）。並べ替えが意味を持つのは、**消費されるプランが提供元ごとに違う**からです — Codex は ChatGPT プラン、Antigravity は Gemini/Antigravity プランを消費します。**絵そのものも別のサービスです**（Codex 経路＝GPT Image、Antigravity 経路＝Gemini の画像生成）。\n既定は Antigravity が先です。縦横比の指定が実際に通るのはこちらだけで、Codex 経路は寸法をいっさい選べないためです。\nここで決まるのは**既定の行き先**で、依頼のときに「Codex で作って」「両方で作って見比べたい」と伝えれば、エージェントはその提供元を名指しできます（そのときは次送りせず、指定した側だけを使います）。",
  ...
  "keys.palette.mode_sessions": "セッション",
  "keys.palette.placeholder_sessions": "セッションを検索…",
  "keys.palette.sessions_empty": "セッションがありません",
