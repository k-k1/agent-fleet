...
export const settings = {
  ...
  "agents.note_image_generation":
    "セッションが、プロンプトから画像を生成できるようにします（generate_image ツール）。Codex セッションには CLI 内蔵の画像生成があるため出しません — この設定が効くのは claude / opencode など、それ自体では画像を作れない種別です。生成物は会話をまたいで残り、Console のファイルビューアからも開けます（30 日で自動削除）。\n**画像 1 枚ごとに、接続している ChatGPT プランの利用枠を消費します**（テキストのターンより 3〜5 倍速く減り、ChatGPT Free では画像生成そのものが使えません）。生成に使うのは Codex の内蔵ツールで、プロンプトは OpenAI へ送られます。**寸法・背景・枚数は希望であって保証ではなく**、実際に何が返ったかは結果の warnings に入ります（1024x1024 を頼んで 1254x1254 が返ることが実測されています）。\n**反映は次に起動するセッションから**で、すでに動いているセッションは再起動するまで変わりません。既定 OFF。",
