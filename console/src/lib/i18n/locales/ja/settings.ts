...
export const settings = {
  ...
  "agents.note_stopped_archive":
    "停止したままのセッションが一覧に残る期間です。過ぎるとアーカイブへ移ります。削除はされず、アーカイブからいつでも戻せます。削除ロック中のセッションは一覧に残ります。停止したままの子セッションはそれまで枠を持ち続け、「しない」ではアーカイブか削除をするまで持ち続けます。反映は次の一覧更新からで、短くすると新しい期間を過ぎているセッションはその場でまとめてアーカイブへ移ります。「既定」は配備の設定値で、管理者が変えていなければ 7 日です。",
  ...
  "agents.note_fleet_spawn":
    "セッションが、別のセッションを起こして仕事を渡し、自分が起こした子だけを世話（一覧・出力の確認・停止・再開）できるようにします。指示・回答・削除はどのセッションにもできません。子の本数は下で決めます（枠は削除・アーカイブで空きます）。孫と生シェルは起こせません。子 1 本がエージェント 1 つ分のメモリと利用枠を使い、消費はあなたの請求に載ります。反映は次に起動するセッションから。既定 OFF。",
  ...
  "agents.note_image_provider_order":
    "画像生成で試す提供元の順番です。上から順に使用可能なものが選ばれ、失敗時は次へ送ります。既定はこの配備が自前で動かすエンジン（Agent Fleet (self-hosted)）が先で、外部サービスはその後です。自前のエンジンはこの配備が払う GPU を使い、外部サービスは利用者本人のプランの利用枠を消費します。ここに並んでいない提供元が後から増えたときも、自前のエンジンは先頭に、外部サービスは末尾に入ります（ここで並べたものの順番は変わりません）。",
  "agents.image_kind_comfy": "ComfyUI",
  "agents.image_kind_openai_compat": "OpenAI 互換",
  "agents.image_provider_order_fallback":
    "既定の並びを表示しています。ワークスペースを起動すると、この配備が実際に持つ画像エンジンがそれぞれの名前で並びます。",
  "agents.rate_limit_resume": "利用上限リセット後の自動再開",
  "agents.note_claude_rate_limit_resume":
    "Claude セッションが利用上限で中断したとき、上限が解ける時刻に、そのセッションへ「続きを進めて」と1回だけ送る再開予約を作ります。予約は使用後に削除され、繰り返し実行はしません。待機中にワークスペースが停止しても、予約時刻に起動して届けます。その時刻に人が既にセッションを動かしていれば見送ります。利用上限に達したときと、実際に自動再開したときは通知センターにも表示します。Console から直接起動したセッションも対象です。既定 ON。\nOFF にしても、上限メニュー（「1. リセットまで待つ / 2. 管理者に増枠を依頼」）は自動確定します。メニュー中は通知・報告・入力を受け付けず、自動で選ぶのは待つ側（無課金）だけです。2 を選びたい場合は、メニュー表示中に自分で選んでください。\nこのスイッチは Codex（マネージド）にも効きます（Codex カードの同じ項目と同じ設定です）。",
  "agents.note_codex_rate_limit_resume":
    "Codex（マネージド）のターンが利用上限で失敗したとき、上限が解ける時刻に、そのセッションへ「続きを進めて」と1回だけ送る再開予約を作ります。Codex には Claude のような上限メニューが無いので、行うのは予約だけです。再開時刻は Codex 自身が記録している利用枠の窓（5時間／週）のうち、使い切っている窓のいちばん早いリセット時刻を使います。読み取れる時刻が無いときは予約しません（当てずっぽうに起こしても同じ上限に当たるだけだからです）。実行方式が ターミナル（CLI）の Codex は対象外です — どのセッションが上限で止まったのかを知る手がかりが残らないためです。Claude カードの同じ項目と同じ設定です。既定 ON。",
  ...
  "agents.muse_connect_note": "承認リンクとコードを表示します。ブラウザで承認してください。サブスク契約ならこちら。",
  ...
  "agents.muse_install_note": "約 299MB をホームボリュームに落とします（数分）。",
  "agents.muse_installing": "Muse Code を導入中…（約 299MB・数分）。終わると自動でサインイン画面になります。",
