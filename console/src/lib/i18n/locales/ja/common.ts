...
export const common = {
  ...
  // 取得中。動的 kind のカタログは Agent が CLI か daemon に問い合わせるので少し遅れて
  // 出る。その間ピッカーは「既定」だけ＝下の空の文言と同じ絵になるため、決着するまでは
  // こちらを出す（`useModelCatalogSettled`）。
  "ui.model_loading": "モデル一覧を取得しています…",
  // 動的 kind のカタログが解決した結果、選べるモデルが 1 つも無かったとき。Agent が
  // 理由（`reason`）を返す場合は下の 2 つを使い、これは **原因が分からないとき専用**。
  // **原因は断定しない**: このピッカーは「まだログインしていない」「認証はあるが
  // プロバイダへ到達できない」「プランが既定のみ（Copilot Free は Auto だけ＝空が正常）」
  // を区別できない。**どの場合にも真である一文**にしてあるので、断定形へ書き換えないこと。
  "ui.model_default_only": "既定のモデルのみ利用できます（接続とプランをご確認ください）",
  // 原因が分かっている 2 つ。どちらもこのワークスペース自身の設定なので、接続やプランを
  // 見に行かせるのは誤誘導になる——名指してよい。
  "ui.model_none_hidden": "選べるモデルを設定ですべて除外しています（設定 > エージェント > 使わないモデル）",
  "ui.model_none_route": "いまの課金の選択では、選べるモデルが 1 つもありません（設定 > エージェント > opencode）",
  // 取得そのものが届かなかったとき。ワークスペース起動直後は Agent がまだ待ち受けておらず
  // CP が 502 を返す——**接続やプランの問題ではない**ので、そこへ人を送ってはいけない。
  // 数回の再試行を使い切ってから出す。
  "ui.model_unreachable": "モデル一覧を取得できませんでした。ワークスペースの起動直後かもしれません（開き直すと再取得します）",
  "ui.cancel": "キャンセル",
  "ui.run": "実行",
  "ui.running": "実行中…",
  "ui.processing": "処理中…",
