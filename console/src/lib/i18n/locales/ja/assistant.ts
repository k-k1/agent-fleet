...
export const assistant = {
  ...
  "assistant.note_output_language":
    "アシスタント・チャットの回答言語です。「入力に合わせる」は、渡した文章や質問の言語に合わせて返します。日本語／English を選ぶと、他言語の文章でもその言語で回答します（翻訳アシスタントは対象外）。この設定が変えるのはチャットの回答だけで、読み上げの言語は「読み上げ」タブで設定します。",
  "assistant.agent_order": "エージェント優先順位",
  "assistant.note_agent_order":
    "アシスタント・チャットを動かす CLI の優先順位です。上から順に見て、最初に接続済みの CLI が使われます（未接続のものは飛ばされます）。モデルは下で CLI ごとに指定します。反映はビルトインアシスタントの新しい会話から（カスタムアシスタントの明示設定が優先）。タイトルや返信候補などの AI 補助生成は別の優先順位で動きます（「AI補助」タブ）。",
