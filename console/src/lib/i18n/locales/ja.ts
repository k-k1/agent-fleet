...
export const ja = {
  ...
  "assistant.rate_limit_resume": "利用上限リセット後の自動再開",
  "assistant.note_rate_limit_resume":
    "claude のセッションが利用上限でターンを切られたとき、上限が解ける時刻に「続きを進めて」と自動で送って作業を再開させます。予約は定時実行のスケジュールとして登録されるので（設定 > 定時実行の一覧に出ます）、待っている間にワークスペースが停止していても時刻になれば起こして届きます。その時刻に人が既にそのセッションを動かしていた場合は見送ります。既定 ON。\nOFF にしても、上限メニュー（「1. リセットまで待つ / 2. 管理者に増枠を依頼」）の自動確定は行います — メニューが出ている間セッションは通知も報告も入力も受け付けられず、自動で選ぶのは待つ側（無課金）だけだからです。2 を選びたいときはメニューが出ている間に自分で選んでください。",
