...
export const ja = {
  ...
  "git.bb_token_hint_pr": " は任意です（作業項目レールに PR を出す場合のみ。clone / push には要りません）。",
  ...
  "tenant.git_oauth_bb_scopes": "Bitbucket は認可 URL にスコープを載せないので、コンシューマの Permissions がそのまま権限になります。Account: Read と Repositories: Read/Write（clone / push 用）に加え、作業項目レールに PR を出すなら Pull requests: Read も入れてください。後から足した場合、既に接続済みのメンバーは接続し直しが必要です（古い権限がトークンに焼かれているため）。",
  ...
  "wi.load_failed": "作業項目を読み込めませんでした",
  "wi.query_failed": "「{label}」の取得に失敗しました",
  "wi.empty": "該当する作業項目はありません",
  ...
  "tracker.intro": "左ペインの「作業項目」に課題を出すための接続です。GitHub の Issue / PR は設定 > Git の接続をそのまま使うので、ここに出るのは Jira だけです。資格情報はワークスペース内に暗号化保存され、Control Plane には渡りません。",
  ...
  "ops.jira_hint": "作業項目レールに Jira の課題を出すための接続です。API トークンは Atlassian のアカウント設定で発行できます（読み取り権限だけで足ります）。メールとトークンはこのコンテナの暗号化ストアに保存され、Control Plane には渡りません。保存前に接続を検証します。",
