...
export const ja = {
  ...
  "keys.kt.quickRepliesNote":
    "オンにすると、コンポーサーの上によく使う短い返信（OK・進めて・commit など）と直近の回答に沿った候補をチップで表示します。クリックで入力欄に差し込み、Ctrl・⌘・Alt のいずれかを押しながらクリックで即送信。キーボードだけでも使えます: 入力欄が空のとき Tab で候補へ移り、Tab／Shift+Tab は「入力欄→候補→入力欄」と一巡（✨ボタンも候補の一員として含みます）、←／→ は候補内だけを循環。Enter で差し込み、Ctrl（⌘）+Enter で送信（Enter 送信の設定なら役割が入れ替わります）、✨上の Enter は候補生成、Escape で入力欄へ戻ります。候補はあなたが送った短文から学習します。既定はオン。",
  ...
  "noti.kind_answer_ready": "回答が返りました",
  ...
  "rp.delete_workcopy_title": "ワーキングコピーを削除",
