...
export const ja = {
  ...
  "agents.kiro_install_desc": "Kiro CLI はこのワークスペースにまだ導入されていません。初回のみ導入が必要です。",
  "agents.kiro_install": "Kiro CLI を導入",
  "agents.kiro_install_note": "約 855MB をあなたのホームボリュームにダウンロードします（数分かかります）。",
  "agents.kiro_installing": "Kiro CLI を導入中です…（約 855MB・数分かかります）。完了すると自動でサインイン画面に切り替わります。",
  "agents.kiro_install_error": "導入に失敗しました。回線状況を確認して再試行してください。",
  "agents.kiro_install_failed": "Kiro CLI の導入開始に失敗しました: {msg}",
  ...
  "keys.kt.quickRepliesNote":
    "オンにすると、コンポーサーの上によく使う短い返信（OK・進めて・commit など）と直近の回答に沿った候補をチップで表示します。クリックで入力欄に差し込み、Ctrl・⌘・Alt のいずれかを押しながらクリックで即送信。キーボードだけでも使えます: 入力欄が空のとき Tab で候補へ移り、Tab／Shift+Tab で次／前の候補、Enter で差し込み、Ctrl（⌘）+Enter で送信（Enter 送信の設定なら役割が入れ替わります）、Escape で入力欄へ戻ります。候補はあなたが送った短文から学習します。既定はオン。",
  ...
  // コンポーザ
  "mirror.suggest_hint": "クリックで入力欄へ・Ctrl/⌘/Alt＋クリックで即送信（入力欄が空なら Tab で候補へ移動）",
