...
export const ja = {
  ...
  "agents.note_claude_rate_limit_resume":
    "Claude セッションが利用上限で中断したとき、上限が解ける時刻に、そのセッションへ「続きを進めて」と1回だけ送る再開予約を作ります。予約は使用後に削除され、繰り返し実行はしません。待機中にワークスペースが停止しても、予約時刻に起動して届けます。その時刻に人が既にセッションを動かしていれば見送ります。Console から直接起動したセッションも対象です。既定 ON。\nOFF にしても、上限メニュー（「1. リセットまで待つ / 2. 管理者に増枠を依頼」）は自動確定します。メニュー中は通知・報告・入力を受け付けず、自動で選ぶのは待つ側（無課金）だけです。2 を選びたい場合は、メニュー表示中に自分で選んでください。",
