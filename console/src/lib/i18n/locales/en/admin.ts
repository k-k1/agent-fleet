...
export const admin: Record<keyof typeof jaAdmin, string> = {
  ...
  "clean.reason.cache_no_store": "The session store is missing, so session folders are not judged (only chat folders are covered; survey again once the store is back)",
  ...
  "clean.reason_hint.cache_no_store": "The session store is missing, so session folders are not judged; only chat folders are covered",
