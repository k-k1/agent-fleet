...
export const settings: Record<keyof typeof jaSettings, string> = {
  ...
  "agents.fleet_observe": "Fleet observation from sessions",
  "agents.note_fleet_observe":
    "Lets a session look at the fleet it is part of, and leave what it noticed in your memo queue. It opens four things and no more: **another session's state** (working / waiting for input / stopped, so a session can check whether the peer it handed work to is still going), **context usage** (so a session notices its own context filling up and hands the rest over instead of pushing on), and **reading and adding memos** (so a bug it had to step around, or a stale document it passed, reaches you without interrupting the work).\n**Nothing that acts is added.** Instructing another session, answering its questions, approving its plans, starting, stopping or deleting sessions, and sending the memo queue all stay with the fleet operator and Console. A session can see THAT a peer is stopped on a question or a plan, but **not the text of it**, and it cannot answer on your behalf — that would stand in for your approval.\n**The change applies to sessions started from now on**; sessions already running keep their current tools until restarted. Default OFF.",
  "agents.fleet_spawn": "Starting sessions from sessions",
  "agents.note_fleet_spawn":
    "Lets a session start another session and hand it a task, then look after the ones it started. It adds **create_session** and the reads that go with choosing where to launch (working copies, selectable models, each CLI's remaining plan quota), plus **reading the output of, stopping, booking a stop for, and resuming the sessions it started itself** - and nothing else. A session cannot instruct, answer for, delete or archive any session, not even its own children, and it cannot touch sessions it did not start.\n**Limits are built in.** At most three children per session (stopping or archiving one does not free the slot - you delete it), a session that was itself started this way cannot start more, and raw shell sessions cannot be started at all. A child begins in a NEW worktree, so it never shares a working copy with its parent. Its first instruction is labelled in the mirror with the session that wrote it, and the agent is told it did not come from you.\n**This is the one session setting that spends real resources with nobody watching** - each child is a whole agent's memory on this workspace, and its tokens are billed to you and recorded as unattended. **The change applies to sessions started from now on**; sessions already running keep their current tools until restarted. Default OFF.",
  "agents.note_fleet_spawn_needs_observe":
    "**Starting sessions from sessions** needs fleet observation above. Watching a session's state is the only way a session can tell whether the one it started has finished, so starting is not offered on its own.",
  "agents.image_generation": "Image generation",
  "agents.note_image_generation":
    "Lets a session generate an image from a prompt (the generate_image tool). The picture is made by one of the CLIs you already have connected (see the order below). Every kind of session gets the tool, but **not the route that runs its own CLI** — taking the Codex route from a Codex session, or the Antigravity route from an Antigravity session, only starts the same CLI a second time and pays for it twice (that picture is available from the session's own built-in tool). Only when no route is left does the tool itself disappear. Results persist across turns and open in Console's file viewer; they are swept after 30 days.\n**Every image spends the included usage of whichever plan produced it** — the ChatGPT plan for Codex, the Gemini/Antigravity plan for Antigravity. It goes 3-5x faster than a text turn, and image generation is not available at all on ChatGPT Free. The prompt is sent to that provider. **Size, background and count are requests, not guarantees**: what actually came back is listed in the result's warnings (asking Codex for 1024x1024 and getting 1254x1254 was measured). The aspect ratio is the one exception — it really does take effect on the Antigravity route (16:9 measured as 1376x768). **You can name the provider in the request** (\"make it with Codex\", \"make it on both so I can compare\"): a named one is used on its own with no fall-through, and comparing two costs one image on each plan.\n**The change applies to sessions started from now on**; sessions already running keep their current tools until restarted. Default OFF.",
  "agents.image_provider_order": "Image provider order",
  "agents.note_image_provider_order":
    "The order providers are tried in. The first one that is signed in and can do the requested operation is used, and **a call that fails falls through to the next** — running out of quota only shows up once the call is made. The order matters because **each provider spends a different plan**: Codex spends the ChatGPT plan, Antigravity spends the Gemini/Antigravity one. **The picture comes from a different service too** — GPT Image on the Codex route, Gemini image generation on the Antigravity one.\nAntigravity is first by default: it is the only one where a requested aspect ratio takes effect, while the Codex route lets you choose no dimensions at all.\nWhat this sets is the DEFAULT destination. Ask for one in the request — \"make it with Codex\", \"make it on both so I can compare\" — and the agent can name the provider itself; a named one is used on its own, with no fall-through.",
