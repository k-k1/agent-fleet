...
export const settings: Record<keyof typeof jaSettings, string> = {
  ...
  "agents.note_image_generation":
    "Lets a session generate an image from a prompt (the generate_image tool). Codex sessions do not get it — they already have the CLI's own built-in image generation — so this is for the kinds that cannot make a picture on their own, such as claude and opencode. Results persist across turns and open in Console's file viewer; they are swept after 30 days.\n**Every image spends the connected ChatGPT plan's included usage** — 3-5x faster than a text turn, and image generation is not available at all on ChatGPT Free. Generation runs through Codex's built-in tool, so the prompt is sent to OpenAI. **Size, background and count are requests, not guarantees**: what actually came back is listed in the result's warnings (asking for 1024x1024 and getting 1254x1254 was measured).\n**The change applies to sessions started from now on**; sessions already running keep their current tools until restarted. Default OFF.",
