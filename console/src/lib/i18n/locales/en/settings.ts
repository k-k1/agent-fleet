...
export const settings: Record<keyof typeof jaSettings, string> = {
  ...
  "agents.note_fleet_observe":
    "Lets a session see other sessions' state, its own context usage, and the memo queue. It adds no ability to act — instructing other sessions and so on. Applies to sessions started from now on. Default OFF.",
  ...
  "agents.note_image_provider_order":
    "The order providers are tried in for image generation. The first signed-in one that can do the request is used, and a failed call falls through to the next. Antigravity is first by default.",
