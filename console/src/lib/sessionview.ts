...
// LABEL_TAG_RE matches the leading tag the Agent puts on a label
// (workspace/agent/internal/session/label.go): both the old `[AF] ` and the newer
// `[AF:<name>] ` that carries the session name. The label is baked into meta at creation
// time, so across an upgrade both shapes are on screen at once — stripping only one would
// leave the tag on older sessions' rows.
...
// working / idle / question chip as the owner's own rail, from the same code, or the two
...
export interface SessionState {
  ...
  /** What is running: "process" | "subagent" | "shell". Unknown/unset falls back to the generic wording. */
...
// (no reason recorded) — those keep the plain stopped chip.
...
export const stateInfo = (s: SessionState): StateInfo => {
  if (!s.alive) {
    ...
    // clean stopped row; the reason detail rides the row tooltip.
    const ex = exitLabel(s);
    if (ex) return { cls: "off warn", icon: "warning", text: ex.text };
    // An interaction was still awaiting an answer when the session was stopped
    // (docs/log/75). Saying only "stopped" makes a row holding an unanswered question
    // indistinguishable from one that was merely put away — and since a session waiting on a
    // human can be stopped too (docs/log/75 P2), without this the question is, from the
    // user's side, silently lost. It takes the question colours (the attention side) but
    // keeps `off`, so "not running right now" still reads.
    switch (s.carried) {
      case "question":
  ...
  switch (s.state) {
    ...
    // and showing the rate-limited wording here would park the user on a reset that never
    // comes. The
    ...
    // own, an expired login never does — it needs re-authentication now. NOT "idle" for the same
    ...
    default:
      ...
      // (spinner + the background-running label alongside idle). When the agent could name
      // WHAT is running, say that instead — "subagent running" tells the user their Task
...
// Folds the remaining time until a future instant into "1h20m" / "12m". Past, empty and
// unparsable values all give "".
//
// That "" is a requirement: the keep-awake pin's badge (docs/log/75) has to disappear once
// the pin expires — leaving it up lets users assume they are still protected. The same
// applies to the admin screen's scheduled stop: a negative remaining time says nothing (the
// next sweep either stops the session or records why it did not).
