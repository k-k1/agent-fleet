...
# 08. フリート・オペレーター — 複数セッションをチャットで指揮する

[English](08-organising.md) | 日本語

[07 チャットとメモ](07-chat-memo.ja.md) のアシスタントチャットと
...
- **セッションを停止・再開する** — 不要になった・暴走している・リソースを空けたいセッションを畳み、必要になったら会話履歴ごと再開できます。停止はどれを止めるか確認してから実行し、停止中のセッションは Console からも再開できます。オペレーターができるのは**再開可能な「停止」だけ**で、セッションを消す破壊的な削除は Console 側の操作に限られています。
  - まだ作業中のセッションには「**終わったら止めて**」と頼めます。走っているターンは最後までやり切り、報告が届いてから停止するので、作業が途中で切れません。並列で走らせた数本を、終わったものから順に畳ませるときに使います（新しい指示を送ると予約は解除されます）。
...
- **終わったら止める**こともできます。詳細・編集の **「終わったら停止」**（オペレーターに
  ...
  （報告が先、停止はそのあと。再開できます）。夜間や早朝の発火のために起こしたワークスペース
  が、アイドル自動停止の時間が来るまで起きっぱなしになるのを避けられます。
