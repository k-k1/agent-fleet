...
- **対象は「自分では絵を作れない」種別**です。claude や opencode のセッションに出ます。
  **codex セッションには出ません** — Codex CLI には内蔵の画像生成があり、二重に通す意味が
  ないためです。
- **生成には接続済みの Codex（ChatGPT ログイン）を使います。** Codex を接続していないと
  この機能は出ません。**画像 1 枚ごとに、そのプランの利用枠を消費します**（テキストのやり取り
  より 3〜5 倍速く減ります）。ChatGPT Free では画像生成そのものが使えません。
- **プロンプトは OpenAI に送られます。** 機密を含む指示で画像を作らせないでください。
- **寸法・背景・枚数は希望であって保証ではありません。** 1024×1024 を頼んで 1536×1024 が
  返ることが実測されています。実際に何が返ったかはエージェントの手元に警告として渡るので、
  仕上がりの寸法が重要なときは、出来上がりを自分で確認してください。
...
- 生成物は `~/.cache/agent-fleet/generated/` の下に残り、ファイルペインからも開けます。
  30 日で自動的に消えます。
- カードが出るのは claude と opencode のセッションです。ほかの種別では、パスは返るものの
  会話の中に絵は出ません（エージェントが読める形では届いています）。
