...
- そのあとは通常の起動ダイアログが開きます（エージェント・モデル・最初の指示が入った状態）。
  **最初の指示に入るのはキー・タイトル・URL と「本文をどこで読むか」**で、本文そのものは
  貼られません。
- ブランチの既定は `feature/<キー>` です（テンプレートは設定タブ）。
...
- 主ボタンは **GitHub / Bitbucket で開く** です。差分もレビューのやり取りも Agent Fleet 側には
  無いので、読むのは元のページです。
- **セッションを始める**導線は畳まれた中にあります（人の PR をレビューするために作業コピーが
  要るときのためで、着手済みの印もそのまま働きます）。
- **本文（説明）はここでも出ません。**
...
- **既定でチェックが入るのは、消しても失うものが無い行だけ**です（親に取り込み済み・未コミット無し・
  ...
  「それでも消す」の確認**になります。
- **稼働中のセッションがある行・削除ロック中の行はチェックできません**。理由が行の下に出るので、
  先に停止するかロックを解除してください。
...
- **開き方**: 操作バーの **「セッション一覧」**（右に分割／下に分割／全て閉じる の隣）、
  左ペイン上部のレイアウト図にある同じボタン、またはリーダーキー **`g` → `s`**。
...
- **カードをクリックするとそのセッションがグリッドの隣に開きます**。一覧が置き換わることは
  ...
  移ります。分割中は、どのペインで開いているかがカード上の番号バッジで分かります。
...
- **1 枚のカードに出るもの**: 右上に状態（入力待ち／進行中…／停止など）、下の行に
  ...
  左ペインのリポジトリ行と同じ見方）、いちばん下に種類・モデル・コンテキスト使用率・
...
- 並び順は「あなたが必要かどうか」です。答えを待っているセッションが先頭、次に残りの稼働中を
  新しい順、最後に停止中。同じ段の中では、眺めている間に順番が入れ替わることはありません。
...
- **どの種別のセッションにも出ます**（claude や opencode はもちろん、codex や agy にも）。
  ...
  出ません。
- **生成には接続済みの CLI 自身の画像生成を使います。** 使えるのは **Codex（ChatGPT
  ログイン）** と **Antigravity（agy）** で、**設定 > エージェント > セッション >
  「画像生成の優先順位」** で試す順を決められます。上から順に、サインイン済みのものが
  使われ、**呼び出しに失敗したら次に送ります**。どちらも接続していないとこの機能は出ません。
- **どちらで作るかは、依頼のときに名指しできます。**「Codex で作って」「同じプロンプトで
  両方作って見比べたい」と伝えれば、エージェントはその提供元を指定します。名指しされた側は
  次送りされず、そこだけで作られます（利用者が選んでいないサービスに課金しないため）。
  **比較すれば 2 つのプランがそれぞれ 1 枚分減ります。** 何も言わなければ上の優先順位に従います。
- **画像 1 枚ごとに、生成に使った側のプラン利用枠を消費します**（Codex なら ChatGPT
  プラン、Antigravity なら Gemini/Antigravity プラン。テキストのやり取りより 3〜5 倍速く
  減ります）。ChatGPT Free では画像生成そのものが使えません。**どちらのプランが減るかは
  順番で決まる**ので、片方の枠を使いたくないときは順番を入れ替えてください。
- **プロンプトはその提供元（OpenAI または Google）に送られます。** 機密を含む指示で画像を
