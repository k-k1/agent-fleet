...
**停止中のまま 7 日経ったセッションは、自動でアーカイブへ移ります。**（期間は [設定 → エージェント](12-settings.ja.md#エージェント) で変えたり止めたりできます。短くすると、新しい期間を過ぎているセッションが次の一覧更新でまとめて移ります） 終わった仕事で普段の
...
ターミナルの中でモデル・effort・plan モードを切り替えていた場合は、起動時の設定に戻らず、
**会話に記録された最後の設定で再開します**。ただし claude / codex / kiro 以外では、モデルを
切り替えたあと 1 通もメッセージを送っていないと CLI 自身がその切り替えを記録しないため、
起動時のモデルで再開します。
