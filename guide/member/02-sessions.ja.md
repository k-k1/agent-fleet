...
- **対象は「自分では絵を作れない」種別**です。claude や opencode のセッションに出ます。
  **生成に使う CLI と同じ種別のセッションには出ません** — 生成先が Codex なら codex
  セッションに、Antigravity なら agy セッションに出ません。その CLI には内蔵の画像生成が
  あり、同じものをもう一度起動して枠を二重に払うだけだからです。
