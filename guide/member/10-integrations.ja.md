...
登録したプロファイルは **`~/.aws/config`** にも書き出されます。`aws --profile <名前>`・`AWS_PROFILE=<名前>`・
AWS SDK・ビルドツール（Gradle、Maven、CDK、Terraform など）から、設定を写し直さずに名前で選べます。
...
- ファイル末尾の **管理ブロック**（`# agent-fleet` で始まる 2 行の目印の間）に入ります。編集は設定画面で行って
  ...
  そのまま残ります。書き出すのは**アカウントとロール**の両方があるプロファイルだけです。無いと
  `aws --profile <名前>` は SSO を使わず、ワークスペース自身のロールで黙って動いてしまうためです。`~/.aws/config` の `[DEFAULT]` の行がプロファイルを壊す場合（違うリージョン、すべてのプロファイルに
