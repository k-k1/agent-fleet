...
- ファイル末尾の **管理ブロック**（`# agent-fleet` で始まる 2 行の目印の間）に入ります。編集は設定画面で行って
  ...
  そのまま残ります。同じ名前のプロファイルを自分で定義済みなら（`~/.aws/config` でも `~/.aws/credentials` でも）、
...
- プロファイルの**短命な**資格情報を、そのコマンド 1 つの環境変数にだけ渡します。`af-aws-exec` 自身は資格情報を
  ...
  ロールのキャッシュを持ちます）。
- プロファイルには設定画面で**アカウントとロール**を入れておく必要があります。`role_arn`・`source_profile`・
  `credential_process`・静的キーも持つプロファイルは断ります。AWS CLI がその SSO ログインを使わなくなるためです
  （`[DEFAULT]` セクションのキーも数えます。CLI はそれをすべてのプロファイルに適用するためです）。資格情報は、
