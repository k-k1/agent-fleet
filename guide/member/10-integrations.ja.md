...
- ファイル末尾の **管理ブロック**（`# agent-fleet` で始まる 2 行の目印の間）に入ります。編集は設定画面で行って
  ください。ブロックの中は書き直されます。ブロックの外はあなたのもので、そのまま残ります。同じ名前のプロファイルを
  自分で定義済みなら、**あなたの定義が使われ**、こちらからは書き出しません。
...
- プロファイルの**短命な**資格情報を、そのコマンド 1 つの環境変数にだけ渡します（ファイルへの書き込みも表示も
  しません）。実行する身元（ARN）を表示します。
