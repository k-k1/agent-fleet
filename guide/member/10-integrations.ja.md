...
- コマンドには**空の AWS 設定ファイルと資格情報ファイル**を渡すので、そのコマンドが見つけられる身元は上の資格情報
  だけです。Terraform の `profile = "staging"`、`cdk deploy --profile`、スクリプト内の `AWS_PROFILE` のように
  ツール側で別のプロファイルを指定していると、黙ってそちらで動くのではなく「profile could not be found」で失敗します。
  リージョンは `AWS_REGION` で渡します。`~/.aws` の他の設定が必要なツールでは `--keep-aws-config` を付けてください。
- `--account <ID>` を付けると、そのプロファイルがその AWS アカウントでない限り実行しません。デプロイするスクリプト・
  手順書・エージェントへの指示には付けておくと、プロファイル名を間違えても何も起きる前に止まります。
- 1 つの名前が 2 つを指す場合は断ります。2 つの設定ラベルが同じ名前になる場合と、設定のプロファイルと同じ名前を
  `~/.aws` で自分で定義していて、アカウントかロールが違う場合です。
