...
| フリート俯瞰図 — 1 セッション 1 レーン、横軸が時間 | メンバー | セッション一覧 → 俯瞰図、またはリーダー `g f` | [02 セッション](../member/02-sessions.ja.md#セッション同士の関係を時間で見るフリート俯瞰図) |
| 作業コピー配下のセッションをまとめて停止 | メンバー | リポジトリ／worktree の右クリック → 配下のセッションを停止 | [02 セッション](../member/02-sessions.ja.md#セッションを止める片付ける) |
...
| セッションに画像を作らせる（`generate_image`）| メンバー | スタジオに付けたセッション以外で、頼むだけ | [02 セッション](../member/02-sessions.ja.md#セッションに画像を作らせる) |
| セッションから別のセッションを起こす（子セッション）— 別のエージェントに仕事を渡し、起こしたものの面倒を見る | メンバー | 設定 → エージェント → セッション、あとはどのセッションでも頼むだけ | [02 セッション](../member/02-sessions.ja.md#セッションから別のセッションを起こす子セッション) |
...
| 編集と部分描き直し — 参照画像・マスク画像・指示編集のモデル | メンバー | 画像生成 → 上級 → 操作 | [04 ファイル](../member/04-files.ja.md#画像生成) |
| LoRA と、選ぶ前に見えるトリガー語 | メンバー | 画像生成 → LoRA | [04 ファイル](../member/04-files.ja.md#画像生成) |
| 画像生成のスタジオ — エージェントと会話で下書きを直し、生成は人が押す（編集履歴・版・絵の履歴）| メンバー | 画像生成 → エージェントを付ける | [04 ファイル](../member/04-files.ja.md#スタジオ-エージェントと会話で下書きを直す) |
| 画像生成の知識のメモ — モデル・系統ごとに 4 節、`~/imagegen-knowledge/` | メンバー | 画像生成 → メモ、またはファイルツリー | [04 ファイル](../member/04-files.ja.md#スタジオ-エージェントと会話で下書きを直す) |
| 画像ギャラリー — フォルダの画像をカードで | メンバー | ファイルツリー →「ギャラリーで開く」、またはセッションの「生成した画像 (N)」| [04 ファイル](../member/04-files.ja.md#画像ギャラリー) |
| ギャラリーのカードのメニュー — コピー・名前の変更・削除・生成したセッションへ | メンバー | ギャラリーのカード → 右クリック | [アイコン・バッジ・メニュー](../member/badges-and-menus.ja.md#画像ギャラリーのカード) |
...
| 未読の通知があるセッションの赤いドット | メンバー | 左ペイン・タブ | [02 セッション](../member/02-sessions.ja.md#状態を読む-バッジと通知) |
