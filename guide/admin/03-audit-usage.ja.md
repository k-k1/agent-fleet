---
audience: "何が起きたか・いくらかかったかを見るテナント管理者"
updated: "2026-09"
---

# 03. 監査ログと利用状況

[English](03-audit-usage.md) | 日本語

「誰が何をしたか」を追える監査ログと、「どれだけ使われたか」を集計する稼働時間。どちらも
テナント設定のレールから、自分のテナントの範囲で確認できます。

## 監査ログ（「監査」）

**「監査」** は、テナント内で起きた **変更操作の記録** です。1 行が 1 操作で、次の列が並びます。

- **時刻** — いつ行われたか。
- **操作** — 何をしたか（`fs.*` = ファイル操作、`git.*` = コミットやチェックアウトなど、`repo.*` =
  リポジトリの作成・削除、`session.*` = セッションの作成・fork・停止、`engine.*` = セルフホストの
  推論エンジン など）。
- **実行者** — 誰が（メールアドレス）。人の操作のほか、外部の Claude クライアント（MCP）や
  システム動作（自動停止など）が主体のこともあります。
- **対象** — どのファイル・リポジトリ・セッションに対してか。

上部の検索欄に「操作 / 対象 / ユーザー」を入れると絞り込めます。ログは自動更新されないので、
最新を見たいときは更新ボタンを押してください。右肩に件数が出ます。

### 何が記録され、何が記録されないか

記録されるのは **変更・破壊をともなう操作だけ** です。ここは意図的な線引きなので、覚えておいてください。

- **記録される** — ファイルのアップロード／作成／改名／削除、git のコミット・チェックアウト・取得、
  リポジトリの clone・削除、セッションの作成・fork・停止、といった「状態を変える」操作。
  セルフホストの推論エンジンもここに書きます——誰も頼んでいないのにお金を使うからです。
  `engine.<エンジン>.offer` は買った GPU インスタンス 1 台につき 1 行で、詳細に実際に買えた
  インスタンスの型と時間単価が入ります。`engine.<エンジン>.box` は Control Plane が自分の
  インスタンスに対してしたこと（止めた・入れ替えた）、`engine.<エンジン>.ingest` と
  `engine.<エンジン>.model.complete` はモデルの取り込みと揃え、`engine.hf_token` と
  `engine.civitai_token` は API トークンの登録と削除です。これらの行はテナントではなく配備に
  属するので、配備管理者の監査画面に出ます。
- **記録されない** — 単なる**読み取り**（ファイルを開いて見ただけ、一覧を眺めただけ）は既定で残りません。
  そして **ターミナルの生の入出力（画面に流れる文字そのもの）は保存しません**。パスワードやトークンが
  紛れ込むリスクを避けるための設計です。

したがって「そのメンバーが端末で具体的に何をタイプしたか」までは監査ログでは追えません。追えるのは
「いつ・誰が・どのファイルやセッションに対して・どんな種類の変更をしたか」です。記録範囲の設計意図は
開発者向けの資料にまとまっています。

## 稼働時間（「稼働時間」）

**「稼働時間」** は、メンバーごとの**ワークスペース稼働時間**を集計します。ここで数えるのは
「インフラをどれだけ占有したか」＝ワークスペースが起動していた時間であって、Claude の利用料ではありません。
Claude は各メンバーが自分のサブスクリプション（席）でログインする「持ち込み（BYO）」なので、運用者側の
コストになるのは占有時間の方だ、という考え方です。この値は約 5 分ごとのサンプリングなので、多少の誤差を
含む概算です。

- **期間** — 「開始」「終了」で日付を区切り、「適用」で反映します。
- **メンバー別** — メンバーごとの稼働時間が棒グラフで並び、上部に「合計稼働」と「メンバー」数が出ます。
- **CSV** — 「CSV」ボタンで、表示中の期間・範囲のデータをそのまま書き出せます（費用の按分＝ showback や
  社内報告に使えます）。

テナントを切り替える選択肢は super_admin にしか出ません。あなたの稼働時間の画面は常に自分のテナント範囲です。

---

## FAQ

**Q. メンバーの追加は招待制ですか、自動ですか？**
デプロイの設定次第です。自動加入（既定）なら許可されたログインは初回に自動で入り、招待制なら管理者が
追加した人だけが入れます。モードの切替は情シスの領分です。詳しくは [01-members.md](01-members.ja.md) の
「自動加入と招待制」を参照してください。

**Q. 上限に達したメンバーはどうなりますか？**
今動いているものが止められることはありません。新しく起動しようとした操作だけが弾かれ、Console に
「上限に達しています」と表示されます。片付ければ続けられます。詳しくは [02-limits.md](02-limits.ja.md) の
「上限に達するとメンバー側で何が起きるか」を参照してください。

**Q. 自分に見えるのは自分のテナントだけですか？**
はい。メンバーもセッションも監査も稼働時間も、すべて自分のテナントの範囲だけです。テナントを切り替える
選択肢は表示されません。デプロイ全体を見られるのは super_admin だけです。

**Q. メンバーがターミナルで打ったコマンドの中身まで監査で見られますか？**
いいえ。ターミナルの生の入出力は保存されません。監査で追えるのは変更操作（ファイル・git・セッション等）の
種類・実行者・対象・時刻です。

**Q. 稼働時間に Claude の利用料は含まれますか？**
含まれません。ここで数えるのはワークスペースの稼働時間（インフラ占有）です。Claude の料金は各メンバーの
サブスクリプション側です。

**Q. あるメンバーを昇格させて管理者にしたいのですが、ボタンがありません。**
tenant_admin のあなたには権限付与ができません。super_admin だけがロールを付与できます。情シス／
デプロイ管理者に依頼してください（[01-members.md](01-members.ja.md) の「ロールの意味」）。

**Q. 退職者のメンバーを削除したいのですが。**
あなたの権限でできます。メンバー詳細の「操作」から **メンバーを外す → ワークスペースを強制停止 →
home を掃除** の順です（[01-members.md](01-members.ja.md) の「メンバーを外すには」）。

**Q. 外部通信（egress）の統制はどこですか。**
egress（外部通信）統制は super_admin 専用です。通信の制御が必要になったら情シスへ相談してください
（[operator/README.md](../operate/README.ja.md)）。

**Q. ワークスペースを強制停止すると、そのメンバーの作業は消えますか？**
消えません。コンテナがいったん止まるだけで、home の中身（リポジトリや設定）は残ります。メンバーは
Console から起動し直せます。

**Q. メンバーから「エージェント CLI のバージョンが何週間も同じ」と言われました。正常ですか？**
**設定 → 環境** を開き、ツールのバージョン表の下の行「上流のリリース監視: 最終成功 …」を見てください。
上流のリリースは毎日監視していて、この配備が焼き込むバージョンはその監視が新版を報告したときにだけ
動きます。この行が新しく、警告が付いていなければ、本当にバージョンが動いていないだけで、することは
ありません。警告が付いているとき（特定の取得元を読めていない／48 時間以上成功していない）は、
バージョンが止まっていることは上流について何も語っていません。見直すべきは配備管理者側の監視です
（[operate/](../operate/README.ja.md)）。行そのものが **出ていない** ときは、この配備から GitHub に
届かないので何も分からない、という意味で、それ自体は異常ではありません。

---

- 前に戻る: [02 資源上限とセッション](02-limits.ja.md)
- ガイド全体の索引: [../README.md](../member/README.ja.md)
