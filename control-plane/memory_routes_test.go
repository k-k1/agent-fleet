...
// エージェントメモリの版管理（docs/39 P1）の中継登録を固定する。CP は明示許可リスト
...
// 手動 snapshot は変更操作なので監査に載る（docs/39 の監査要件。restore / import /
// export は P2・P3 で同じ表へ足す）。target は URL 由来のみで body は読まない。
func TestMemorySnapshotIsAudited(t *testing.T) {
	...
	// 読み取り系は監査しない。
	if _, _, ok := auditActionTarget(httptest.NewRequest(http.MethodGet, "/api/agents/memory/snapshots", nil)); ok {
		t.Error("GET snapshots should not be audited")
	}
}
