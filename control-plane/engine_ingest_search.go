...
// interesting thing is at the top because the order is by downloads.
...
// engineSearchHF asks Hugging Face.
func engineSearchHF(ctx context.Context, q, kind string) ([]engineSearchHit, *apiError) {
	...
	expand := []string{"gated", "downloads", "likes", "cardData", "lastModified"}
	if kind == "gguf" {
...
func engineSearchCivitai(ctx context.Context, q, kind string) ([]engineSearchHit, *apiError) {
	if kind == "gguf" {
...
func (a engineAdminAPI) searchIngest(w http.ResponseWriter, r *http.Request, _ store.Identity) {
	...
	if q == "" {
		writeAPIErr(w, &apiError{http.StatusBadRequest, errCodeEngineBadBody, "q is required"})
		return
	}
	...
	case "civitai":
		hits, aerr = engineSearchCivitai(r.Context(), q, kind)
	case "", "hf":
		hits, aerr = engineSearchHF(r.Context(), q, kind)
	default:
