...
func (e *lfsEnv) addMember(t *testing.T, email, key, role string) string {
	...
	return mintGitToken(gitSignKey(e.c.mgr.master32), mem.ID)
}
...
func TestLFSLockLifecycle(t *testing.T) {
	...
	// Create.
	w := e.lockCall(e.c.handleLFSLockCreate, "POST", locksURL, e.token, map[string]any{"path": path, "ref": map[string]any{"name": "refs/heads/main"}}, map[string]string{})
	if w.Code != 201 {
	...
	w = e.lockCall(e.c.handleLFSLockCreate, "POST", locksURL, e.token, map[string]any{"path": path}, map[string]string{})
	if w.Code != 409 {
	...
	// List shows it.
	w = e.lockCall(e.c.handleLFSLocksList, "GET", locksURL, e.token, nil, map[string]string{})
	if w.Code != 200 {
	...
	w = e.lockCall(e.c.handleLFSLocksVerify, "POST", locksURL+"/verify", e.token, map[string]any{}, map[string]string{})
	var vOwner struct {
	...
	w = e.lockCall(e.c.handleLFSLocksVerify, "POST", locksURL+"/verify", bob, map[string]any{}, map[string]string{})
	var vBob struct {
		Ours   []map[string]any `json:"ours"`
		Theirs []map[string]any `json:"theirs"`
	}
	...
	w = e.lockCall(e.c.handleLFSUnlock, "POST", unlockURL, bob, map[string]any{}, map[string]string{"id": lockID})
	if w.Code != 403 {
		t.Fatalf("bob unlock: want 403 got %d", w.Code)
	}
	// force by a non-admin is still refused.
	w = e.lockCall(e.c.handleLFSUnlock, "POST", unlockURL, bob, map[string]any{"force": true}, map[string]string{"id": lockID})
	if w.Code != 403 {
	...
	w = e.lockCall(e.c.handleLFSUnlock, "POST", unlockURL, admin, map[string]any{"force": true}, map[string]string{"id": lockID})
	if w.Code != 200 {
...
func TestLFSLockOwnerUnlockAndReadOnly(t *testing.T) {
	...
	w := e.lockCall(e.c.handleLFSLockCreate, "POST", locksURL, e.token, map[string]any{"path": "a.bin"}, map[string]string{})
	var created struct {
		Lock struct {
			ID string `json:"id"`
		} `json:"lock"`
	...
	json.Unmarshal(w.Body.Bytes(), &created)
	w = e.lockCall(e.c.handleLFSUnlock, "POST", locksURL+"/"+created.Lock.ID+"/unlock", e.token, map[string]any{}, map[string]string{"id": created.Lock.ID})
	if w.Code != 200 {
	...
	w = e.lockCall(e.c.handleLFSLockCreate, "POST", locksURL, viewer, map[string]any{"path": "b.bin"}, map[string]string{})
	if w.Code != 403 {
		t.Fatalf("viewer create: want 403 got %d", w.Code)
	}
	// But a viewer CAN list/verify (read).
	w = e.lockCall(e.c.handleLFSLocksList, "GET", locksURL, viewer, nil, map[string]string{})
	if w.Code != 200 {
		t.Fatalf("viewer list: want 200 got %d", w.Code)
	}

	// Unlock of a missing lock → 404.
	w = e.lockCall(e.c.handleLFSUnlock, "POST", locksURL+"/nope/unlock", e.token, map[string]any{}, map[string]string{"id": "nope"})
	if w.Code != 404 {
