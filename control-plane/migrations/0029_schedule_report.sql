-- Scheduled execution (docs/38): per-schedule completion-report opt-in. report=1 passes
-- the schedule's owner_conv as report_to on a fire so the session's completion rides the
-- docs/30 seam back to the operator/assistant conversation. Default 0 = do not report
