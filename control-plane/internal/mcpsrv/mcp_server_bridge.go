...
func (a ServerAPI) mcpTokenMembership(r *http.Request) (store.MembershipView, *APIError) {
	...
	mv, ok, err := a.cp.Store().GetMembershipByID(r.Context(), mid)
	if err != nil {
...
func (a ServerAPI) WithMCPToken(h func(http.ResponseWriter, *http.Request, store.MembershipView)) http.HandlerFunc {
