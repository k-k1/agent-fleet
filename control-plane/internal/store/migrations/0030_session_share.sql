-- Live session sharing. Transcript bodies remain in the owner workspace.
...
CREATE INDEX idx_session_share_recipient ON session_share(recipient_membership_id);
CREATE TABLE shared_session_catalog (
    id                  TEXT PRIMARY KEY,
...
CREATE INDEX idx_shared_catalog_owner ON shared_session_catalog(owner_membership_id);
CREATE TABLE session_share_proposal (
    id                      TEXT PRIMARY KEY,
    tenant_id               TEXT NOT NULL REFERENCES tenant(id),
...
CREATE INDEX idx_share_proposal_owner ON session_share_proposal(owner_membership_id, status)
