...
- **ベンダーアイコン**（AWS・GCP・Azure・Kubernetes・ラック機器…）は、セット単位で Control Plane が 1 回だけ取得して全員分をキャッシュします。通信が制限された環境では取得できないことがあり、その場合は図形の大きさ・色・枠・ラベルは残り、中のアイコンの図案だけが空になります（図そのものは開きます）。運用者が事前に投入しておくこともできます（[運用 02](../guide/operator/02-operations.ja.md)）。
