...
- **朗読中の告知差し挟み＋朗読の声の即時切替（2026-07-12）** ✅: ①長い朗読（ファイル・
  ...
  二重再生なし）。②朗読ビューの声セレクトを再生中に変えると、いま読んでいる文から新しい
  声で読み直す（続きから切替。ReaderView の `startFrom(active, voice)` — setSetting 直後の
  古いクロージャを避けるため上書きを直接渡す）。あわせてミラーのターンフッターの読み上げ
...
- **裸のハッシュの省略読み（2026-07-12）** ✅: バッククォートで括られていない生ハッシュ
  （`f437e17` 等）も省略読み（`ttsAbbrevCode` に相乗り・新設定なし）。地の文に混ざるため
