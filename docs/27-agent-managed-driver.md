...
> 暫定対処の `[notice] hide_rate_limit_model_nudge` トグルは main マージ済み（`0e6dd0e`）。
...
P1 実装（2026-07-15）で観測対象を `contextCompaction` の item lifecycle（`597f8c4`）から
...
アタッチする `codexObserver` を追加した——**597f8c4 の圧縮検知はアタッチなしでは本番で発火しない
実装だった**（P1 で修正）。
...
1. **thread スコープ通知はアタッチ必須（本設計全体に効く最重要事実）**。`item/*`・`turn/*`・
   ...
   帰結: 597f8c4 の圧縮検知はアタッチなしでは不発だった（P1 で修正）。P2/P3 の Driver 購読も
