...
```jsonc
{
  ...
  "kind": "claude",               // 実行エージェント種別
  "model": "haiku",               // CLI に渡した/報告されたモデル
...
- `claudeResult` / `streamLine` に `total_cost_usd` を足す。
- `oneShotHeadless` は現在 usage を全部捨てている（`_` で受けている）ので受け取る。
...
```
GET /usage/series?from=&to=&bucket=day|hour&by=feature|kind|model|trigger&include=session,aux
 → { buckets:[{t, series:{<key>:{spend,in,out,cread,ccreate,calls,cost_usd}}}],
     totals:{...}, coverage:{<kind>:"exact|partial|none"}, unmeasured_calls:N }
```
...
- **チャート構成**（dataviz スキルの規約に従って実装する）:
  ...
  2. **内訳**（期間合計）: feature 別と kind 別の横棒2枚。kind の色は `tokens.css --kind-*` を流用
     （agent-display-naming の1ソース規約）。feature 用のカテゴリ色は新規に定義する。
  3. **表**: feature × kind で calls / spend / 平均 / cost、ソート可。
  4. 期間 24h / 7d / 30d、cache_read 表示トグル、feature・kind フィルタ。
  5. **未計測バナー**: 「cursor/agy はトークンを報告しないため回数のみ」を常時明示。
...
| P1 | 台帳 + 補助呼び出し計装（UI 無し） | 13箇所タグ付け・go test 緑・実 CLI で1行記録を実測 |
