...
### 5-c. 単価カタログ（P2a・2026-08-31）

§5-b の内蔵表は **Anthropic の一次単価だけ**なので、codex / opencode の消費が「値付け不可」で
残った。実測でこのワークスペースの台帳（668M spend）を測ると **92.7% しか値付けできていない**。

**単価表を自分で育てない。** models.dev のカタログが既にワークスペースの中にある —
`~/.cache/opencode/models.json`（opencode が自動更新。212 provider / 7,493 モデル・**単位は
$/1M でこちらと同じ**・ライセンス MIT）。これを読むだけで **99.3%** になる（残りは
`<synthetic>` 0.54%・上流も価格を持たない `fugu` 0.13%・モデル名が空 0.02%）。実装は
`workspace/agent/usage_catalog.go`。

- ★**同じモデル id が 20〜34 provider に載っていて価格が違う**（再販・ルータ経由。ある
  再販業者の `claude-opus-4-8` は $1.5/$9.25 で、一次の $5/$25 と別物）。**名前だけで引くと
  静かに嘘の金額**になるので、引く provider は台帳の `kind` から決める:
  `claude→anthropic` / `codex→openai` / `opencode→opencode` / `copilot→github-copilot`。
  索引に入れるのも順序表に載せた一次 provider だけで、再販系は**そもそも引けない**。
- **opencode の消費はゲートウェイ価格で換算する**（2026-08-31 決定）。利用者が実際に払うのは
  opencode の値段であって、元プロバイダの定価ではない。したがって `kind` は単価の次元でもある
  ＝**推定は `kind` を落とす前に引く**。
- 一次が anthropic の時だけ内蔵表を優先する（人が公表値を確認して置いた表なので、コミュニティ
  由来のカタログより信頼が高い。**実測では models.dev の anthropic 値と完全一致した**）。
- **best-effort に徹する。** ファイルが無い・形が変わった・壊れている、のどれでも「カタログ
  無し」に落ちて内蔵表だけで動く。上流の内部ファイル名で検査を書いて壊れた前例がある
  （memory: copilot-session-db-moved）。探す順は `AF_USAGE_CATALOG` → `usage/catalog.json`
  （opencode を使わないワークスペース向けの置き場）→ opencode のキャッシュ。**こちらから
  models.dev を取りに行くことはしない**（egress が閉じたデプロイがある。必要になったら
  `usage/catalog.json` に置く運用でよい）。
- `tiers` / `context_over_200k`（長文の割増）は**使わない** — 台帳はターンごとの入力長を
  持たないので当てられない。下段固定＝**推定は下振れしうる**。
- 応答に `prices`（モデル → 実効単価＋出所）と `catalog`（origin・モデル数・**取得日**）を
  載せ、画面はツールチップと「計測できている範囲」に出す。推定額は保存していないので
  **カタログが更新されると過去の額も変わる**。どの時点の単価かを言わずに額だけ動かすのは
  黙って嘘をつくのに近い。
- 同じモデル名でも kind で単価が違う行（`gpt-5.6-luna` は codex なら openai 定価、opencode なら
  ゲートウェイ価格）は、モデル軸の表に単価を1つしか出せない。消費の大きい方を代表にして
  `ambiguous` を立て、ツールチップで「エージェントによって単価が違う」と言う。
- テストの隔離は **HOME だけでは足りない**（`XDG_CACHE_HOME` も差し替える。開発機の実カタログを
  読むと期待値が環境で動く＝環境を検査するテストになる。memory: ci-runner-xdg-not-home）。
- 端数の言い方: 値付け漏れが四捨五入で 0% になる時に「消費の 0% は…」と書くと自己矛盾する
  （実データで踏んだ: 57k / 34.1M）→ 1% 未満は専用の文言。

**残（P2b・未実装）**: 利用者が単価を手入力・上書きする口（`usage/prices.json` ＋
`GET/PUT /usage/prices` ＋設定 UI ＋ [79](79-settings-export-import.md) のセクション）。
効果は残り 0.7%（上流が価格を持たないモデル）と、交渉レート・自社ゲートウェイの反映。
