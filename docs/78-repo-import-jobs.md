...
| `svnRepoEntry` | 行ごとに `svn info` ×2 ＋ `svn status`（全走査・無制限） | 走行中の checkout と `wc.db` を奪い合う。`L` を 未コミット と表示 |
...
- **中止は確認を挟む**。取り込みは実例で 11.4 GB あり、誤クリック 1 回の代償が大きい。文言は VCS で
  分ける —— svn は「途中まで残り 更新 で続きから取れる」、git は「再開できないのでやり直し」。
...
## 10. 大きな作業コピー（実例: 11.4 GB）

- **ディスクは content の約 2 倍**。svn 1.8+ は `.svn/pristine` に全ファイルの原本を（無圧縮で）持つ。
  実測: 20 MB の内容 → 作業ファイル 20 MB ＋ pristine 21 MB。同一内容は SHA1 で重複排除される。
  つまり 11.4 GB が「内容」なら home に **23 GB 前後**要る。取り込み前に `df -h ~` を見ること。
- **一覧の dirty 判定は作業コピー全体を走査する**（`svn status`）。実測は 20k ファイルで 0.24 秒
  （ローカルディスク・キャッシュ温）だが、コストはファイル数に比例し、**EFS では 1 stat が
  ネットワーク往復**になる。よって:
  - `svnDirtyTimeout`（20 秒）で打ち切り、**打ち切ったときは直近の実測値を使う**（測れなかったものを
    「変更なし」と報告しない — docs/63 §63.9 と同じ規律）。
  - 同じ作業コピーへの同時要求は **1 本の走査に相乗り**させる。一覧は 60 秒ポーリングに加えて画面の
    操作でも refresh されるので、素直に書くと全走査が重なる。
  - **TTL キャッシュにはしない**。手動の 更新 を押した直後に古い判定を返すのは、この画面では嘘に近い
    （既存テストがこれを検出した）。
- ジョブの上限 6 時間は、11.4 GB でも常識的な回線なら足りる（5 MB/s で約 38 分）。足りなくなるのは
  実効 0.5 MB/s を切るときで、そのときは中止して原因を見るべき状況である。

## 11. 残っている宿題

- **実機確認**（acrt）。ここまでの検証は単体テスト・Go の実 svn テスト（`svnadmin` で作った file:// リポジトリ）・
