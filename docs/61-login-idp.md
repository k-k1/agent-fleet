...
| **P5**（実装済み）| テナント定義の **GitHub**（§61.15）: `kind` / `allowed_orgs`・行から組む GitHub アダプタ・承認の対象を **(org, ドメイン)** に読み替え・org 追加で再承認・**規則 1.5**（`realm` で「同じ IdP アカウントを別のボタンから」を結合） | `0041`（pg `0024`）|
...
- 別 IdP 同士（email だけ同じ）の結合。**本人の同意で紐づける導線**（ログイン済みの本人が
  「この方式を自分のアカウントに紐づける」を押す）なら安全に全経路が通るが、新しい面と
  セッション付きの API が要る。§61.5 の「別 email の結合はしない」とは別の話（同じ email）。
- 規則 1.5 を OIDC でどこまで使うか。realm は汎用に作ってあるが、Entra のペアワイズ `sub` の
  ように、同じ issuer でもアプリ登録が違えば当たらない IdP がある。`oid` のような
  IdP 固有の安定クレームを realm 側の subject に使うかは、要望が出てから。
- env の GitHub とテナントの GitHub が同じ画面に並ぶときのボタン文言。id は衝突しないが
  既定ラベルは同じ「GitHub でサインイン」になる。当面はラベルの書き分けと、
  `hidden_providers`（§61.15.9）で運用する（§61.11.1 の 2 つ目の穴と同じ）。
- `hidden_providers` は**そのテナントのログイン画面**にしか効かない。素の `/login` は
  テナントを知らないので、デプロイ共通の方式はそこに出続ける（出さないと、そもそも
  誰も入れない）。テナントの入口を `/login/<slug>` に寄せる案内で足りるかは運用を見てから。
