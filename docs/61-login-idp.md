...
でもない*ことだったが、P7 でそこに**既定テナントの `hidden_providers` を適用**すると根拠ごと
消える。以下の運用回避は P7-1 で不要になる。
...
- → **(b) の面に順序を書き、UI にも守らせる**: 新しい行を有効にしても**古い行は止めない**。
  ...
  `GET /api/me/login-methods` があるので「まだ古い方式しか持たない人が N 人います」は
  出せる。★ この順序は §61.17.5 末尾の「順序を UI が守る」と同じ性質の規則で、
