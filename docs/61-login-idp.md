...
| **P7**（未実装）| **サインイン方式をテナントへ寄せる**（§61.17）: env の方式を**既定テナントの方式**として扱い、他テナントはそれを**参照**する・テナントの面を統合リスト＋2 トグルにして `allowed_providers` / `hidden_providers` の自由入力を画面から外す・素の `/login` を既定テナントのページとして描画（§61.15.13 が閉じる）。★ **provider id の形と identity 層は 1 行も触らない**（§61.17.3） | 無し |
...
★ **後日撤回する見込み（P7・§61.17.6）。** 「変えられない」の根拠は*素の `/login` がどのテナント
でもない*ことだったが、P7 でそこを**既定テナントのページ**として描画すると根拠ごと消える。
以下の運用回避は P7-1 で不要になる。

- テナント設定に出している `loginURL` の**直下に一文**を置いた。「隠す指定をしたなら、
  この URL を配らないと隠した意味が無い」が、隠す欄と同じ画面で読める。
- 管理者ガイド（`docs/guide/admin/`）とオペレーターガイド（`docs/guide/operator/01-install`）に
  **「隠しても素の `/login` には出る。テナントの人には `/login/<slug>` を配る」**を明記した。
...
- ★ **だから「デプロイの既定方式を全部やめる」は選べない。** 2 つの一行が止める:
  ...
  （`resolver.go:56`・決定 31）ので、全方式がテナント定義になると `SUPER_ADMIN_EMAILS` に
  載っていても**誰も super_admin になれない**。加えて `AUTH=oauth` は provider ゼロで
  `log.Fatalf`（`main.go:361`・`oauthConfigured()`）。承認する super_admin が居ないので
  最初のテナント行も有効化できない（決定 30）＝循環する。
...
「既定テナントの方式」を素直に `t:default:google` へ改名すると、**2 つ壊れる**:

| 触ってしまうもの | 実測 | 壊れ方 |
|---|---|---|
| 決定 32-3 の固定 | `checkTenantProvider` は `parseTenantProviderID(prov)` の**文字列の形**で「テナント定義セッション」を自テナントに固定する（`resolver.go:224`） | 共有の Google で入った人が**既定テナント以外を一切使えなくなる**（兼務も部署テナントも全滅） |
| 決定 31 の役割ガード | 同じ判定で `roleHint` を空にする（`resolver.go:56`） | その入口からは**新規に super_admin になれない**（既存行は降格しない＝`UpsertIdentity` は never downgrade。「昇格できない」だけ、という非対称） |
...
- `pairwise` かつ `oid` が使えるなら **`link_claim` を必須**にする（保存時 400）。今の UI は
  初期値が「既定（`sub` で見分ける）」なので、そのままでは事故る。
- `pairwise` で名乗れる安定クレームが無い場合は**保存を拒否しない**。分裂は起きるが、
  §61.16（本人の同意で紐づける）が後から救える — `email_taken` の画面がその導線を案内する。
  代わりに**保存時と承認画面の両方に警告を出す**（誰に着地するかが変わる＝承認者が見るべき情報）。
...
- 自前の行（作成・編集可・承認が要る）と、参照した既定テナントの方式（バッジ「デプロイ共通」・
  編集不可・外すのは自由）が同じリストに並ぶ。**その画面が門の全体を映す**ようになる。
- ［方式を追加］は 2 択 — ①既定テナントの方式から選ぶ（＝参照・(a)）②新しく作る（(b)）。
  ①の選択肢が、今まで置き場の無かった「このデプロイで使えるサインイン方法」そのもの。
...
★ **実装上の罠 2 つ**（どちらも「空＝全部」という既存の意味から出る）:

- **最後の 1 つは OFF にできない。** `allowed_providers` は空＝全部受け入れ（§61.9.4）なので、
  全部 OFF にすると保存の結果は「全部 ON」になる。トグルを全部落とせる UI は、
  **絞ったつもりで全開にする**。最後の 1 行は `disabled` にして理由を出す。
- **未設定のテナントは「全部 ON」と描く。** そして最初のトグル操作で明示リストへ固める
  （空 → 明示は互換のある方向で、意味は変えない）。

★ **順序を UI が守る。** 先に絞ってからテナント管理者を招くと、その人が入れない。
参照を外す操作は、**そのテナントに active な自前の行が 1 つ以上あるときだけ**許す。
最初の行は super_admin が作って承認できる（§61.17.1）ので、循環はしない。

### 61.17.6 素の `/login` は既定テナントのページになる
...
- ★ **「全部隠したら無視」の安全弁は既定テナントでこそ効く**（`loginButtons`）。既定テナントの
  ボタンが 0 になると、どのテナントにも属さない人（新任 super_admin・招待前の人）が
  入れなくなる。
...
| 段階 | 内容 | スキーマ |
|---|---|---|
| **P7-0** | テナントの「サインイン方式」を統合リストにする（自前の行＋参照行・2 トグル）。参照の追加は既定テナントの方式から選ぶ。管理モーダルの登録簿にも既定テナントの行を出す | 無し |
| **P7-1** | 素の `/login` を既定テナントのページとして描画（決定 42）。§61.15.13 の運用回避を撤去 | 無し |
| **P7-2** | 新規インストールの既定を `AF_PROVISION=invite` に。§61.10.2 を「super_admin だけが入れる状態から始める」に一本化 | 無し |
| **P7-3**（任意）| (b) の `link_claim` 必須化と discovery による `pairwise` 判定 | 無し |

★ **意味の反転はしない。** 既存テナントの「空＝全部」はそのまま。厳密になるのは、
新規テナント作成時に選んだ方式を**明示で書き込む**分だけ。

★ **`AF_PROVISION` の既定を倒すのは破壊的変更**（P7-2）。`auto` で回っている単一テナントの
会社が、更新した瞬間に super_admin 以外締め出される。**membership が 0 行の新規インストール
だけ** `invite` にするか、インストールガイドの既定に留める。
