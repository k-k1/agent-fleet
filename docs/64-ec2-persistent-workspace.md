...
いない見積もりだった。** <prod-deployment> 本番（`af-af-ecs-platform`・スロット 4 台・全員 m7i.large）で
...
- **新規箱の初回配置が 1 サイクル空振りする。** 上の表の +30.3s、および 08-26 10:38 の回で
  ...
  当該スロットは登録済みなのにまだ配置エンジンから見えていない。`waitSlotRegistered`
  （`status=ACTIVE && agentConnected`）を満たしても配置可能とは限らない、ということになる。
