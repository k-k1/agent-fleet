...
| **`UpdateService` の分割**（§64.39.6） | ⚠️ **速さでは主張しない。** 40% の +40 秒と引き換えに、退役待ちを確定で払う |
| mount のポーリング（§64.38） | 1 回 1.5 秒。誤差 |

### 64.39.6.1.1 待ちを無くす道は在る（未実装）

サービスの `deploymentConfiguration` は既定のまま **`maximumPercent: 200` /
`minimumHealthyPercent: 100`** である（`UpdateService` の応答で確認）。**200% だからこそ
desired 1 のサービスがタスクを 2 本走らせられる。**

`maximumPercent: 100` / `minimumHealthyPercent: 0` にすれば、**ECS はこのサービスに 2 本目を
立てられなくなる**——退役待ちも要らなくなり、§64.39 の欠陥は構造的に消える。ワークスペースの
サービスにとって 2 本同時は**常に害**（Service Connect が両方に振る）なので、意味論としても
そちらが正しい。

⚠️ **未検証**。既存サービスには `UpdateService` で配り直す必要があり、「デプロイ中に running
0 になってよい」を明示的に認めることでもある（実際には Stop→Start で既にそうなっている）。
**次にやるならここで、そのとき退役待ちは丸ごと削れる。**
...
同期実行される**（`Start` の `deferred` 分岐）ので、**待ちをそこに置いてはいけない**。CP の
前段の ALB は既定 60 秒で切れ、それが `watchReady` の「90 秒待って 504」の正体である。
最初は待ちを 25 秒に切って収めようとしたが、**実機では 53〜55 秒かかった**ので収まらない。
いまは**後半ごと背景に出している**（§64.39.8）。
なお**その経路はタスク定義が再利用された回はこの待ちに入らない**（＝再 wake の大半）。
...
### 64.39.7 ⚠️ これはレースであって、必ず起きることではない（測って分かった）
