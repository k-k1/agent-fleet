...
- ⚠️ **その AZ に容量が無いと、そのデプロイでは新規 home が作れない。** `runSlot` は
  `InsufficientInstanceCapacity` を他の AZ で撮り直さないので、既存ユーザーは動いたまま
  **新規だけが止まる**。sandbox では起こせない（容量不足を作れない）ので**未実測の設計上の穴**として残す。
