...
**この 2 つは別の現象ではなく、同じ 1 つの原因の 2 つの見え方**だった。原因は CP 側にある。
...
**①`UpdateService(td=新)` を `launch()` の先頭で先に出し、②`UpdateService(desiredCount=1)` を
最後に出す。**間の待ちは「`ACTIVE` な旧デプロイが無くなるまで（実測 10〜23 秒・上限付き）」。

これが只で済むのは、**その 10〜23 秒が既に払っている待ちと丸ごと重なる**からである
（§64.38.3 の内訳: インスタンス起動→ECS 登録 **18s** ＋ mount **2〜4s**）。配置制約の
`ec2InstanceId` もネットワーク設定の AZ も `launch()` に入った時点で確定しているので、
①を先に出すのに足りない情報は無い。

- ⚠️ **①は `desiredCount` を触らない**ので、走っていないワークスペースを起こしてしまうことはない
- ⚠️ mount に失敗して隔離に落ちた場合、サービスは「隔離された箱に pin されたタスク定義」を
  `desiredCount=0` で指したまま残る。**何も走らないので無害**で、次の Start が上書きする
- ⚠️ 上限（たとえば 30 秒）を置き、超えたら**そのまま ②を出す**。今より悪くはならない
