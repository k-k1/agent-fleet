...
> **残す宿題**: 上限に張り付いた状態で「休眠中の**型違い**の箱を terminate して、
> 正しい型を建て直す」ところまではやっていない。運用方針（使えない人を作らない）から言えば
> そちらが正しいが、`Ec2SlotTerminateAfterSec`（§64.32）が入って定常の箱数が上限から離れた
> 今は、この窓は「上限にほぼ張り付いた配備」でしか開かない。現状は**黙って詰まる**のを
> **明示的に失敗する**に変えたところまで。
