...
### 64.39.6.1 これで起動は速くなるのか——経路によって違う

**待ちが只になるのは、それが「既に払っている待ち」と重なるときだけ**である。重なりは経路で違う。

| 経路 | ①②の間に既にある待ち | 期待できる短縮 |
|---|---|---|
| 停止スロットを起こす | 起動待ち ~19s ＋ ECS 登録 ~18s ＋ mount 2〜4s ＝ **約 40s** | **ほぼ 40 秒まるごと**（待ちは完全に隠れる） |
| 新しいスロットを建てる | ECS 登録 **18s** ＋ mount 2〜4s ＝ 約 20s | **ほぼ 40 秒**（同上） |
| **running の空き箱へ載せ替え** | `waitSlotRegistered` は初回ポーリングで返る ＋ mount ＝ **約 3s** | ⚠️ **待ちが 10〜20 秒露出する**。正味 **20〜30 秒** |
| 同じスロットへ復帰（タスク定義を再利用） | — | **変わらない**（今も 1 本しか立たない） |

⚠️ **「40 秒」は本番配備の実測値**（+39.7〜40.7s・n=4）である。検証台（Fargate・小さい
イメージ）では 2 本目まで 7 秒しか空かなかったので、**この数字は配備の条件込み**であり、
**適用後に測り直すこと**。
