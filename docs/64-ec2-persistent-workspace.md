...
**併せて直す価値があるもの（別件）**: 4 の「CP 再起動で中身が同じでも新リビジョン」は、
指紋を**タスク定義のタグに刻んで読み戻す**（`RegisterTaskDefinition` の `tags` ＋
`DescribeTaskDefinition --include TAGS`）ようにすれば消える。プロセスローカルのキャッシュを
配備全体で共有できる形にする、というだけの話で、デプロイのたびに全員が払っている 40 秒が
**イメージが本当に変わったときだけ**になる。
