...
を最後に出す。**間の待ちは「`ACTIVE` な旧デプロイが無くなるまで」で、上限 30 秒
（`ec2DeploymentRetireBudget`）。実装は `pointServiceAt` / `waitOldDeploymentRetired` /
`scaleUpService` の 3 つ。
...
⚠️ **`ec2DeploymentRetireBudget` は 25 秒で、上げてはいけない。** 「同じ running スロットへ
再 attach する」経路だけは `launch()` が背景に回らず **Start の HTTP リクエスト内で同期実行**
される（`Start` の `deferred` 分岐）ので、待ちがそのまま応答時間になる。CP の前段の ALB は
既定 60 秒で切れ、それが `watchReady` の「90 秒待って 504」の正体である。
...
### 64.39.6.3 ⚠️ Fargate 側（`runtime_ecs`）は同じ形のまま
...
無条件で新リビジョンを登録する（再利用の仕組みが無い）。**構造は同じなので同じことが起きる
はず**で、Fargate は**イメージキャッシュを持たない**（`watchReady` の注記）ぶん、無駄に立つ
タスクがフル pull を払う可能性がある。

**触っていない。** ADR 0045 のファイル一覧が「**`runtime_ecs.go`（Fargate）は変更しない**」と
明示している面であり、実利用者の居る配備は `ecs-ec2` なので**検証する手段が無い**。
**未検証の推測として記録するにとどめる。**
