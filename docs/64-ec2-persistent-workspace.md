...
### 64.40.2 ⚠️ 引き金は**メモリ枯渇ではなかった**——root ボリュームが 3 時間 上限に張り付いていた

**最初の見立て（メモリ枯渇）は誤りだった。** 隔離した箱を起動し直して root ボリュームの
ログを読んだところ（§64.40.7）、**永続ジャーナルが事故当時のぶんを丸ごと保持していて**、
そこには **OOM killer の発火が 1 件も無い**。`hung_task` も `page allocation failure` も
`reclaim` 系の警告も 0 件。swap はそもそも無効（zram は「メモリが多すぎる」で自分を無効化）。
`docker inspect` も全コンテナで **`OOMKilled=false`**。

実際に起きていたのは **EBS スループットの飽和**である。

| 指標 | 値 |
|---|---|
| **root** ボリューム (`vol-…`・gp3 40GiB) の read | **39.34 GB / 5 分を 3 時間、有効数字 4 桁まで不変** |
| ↑ を秒速に直すと | **131 MB/s ＝ gp3 の既定スループット 125 MiB/s ちょうど** |
| home ボリューム (gp3 50GiB) の read | ピーク 38.3 GB / 5 分（write は 0・キュー長 6.7） |
| インスタンス合計 (`AWS/EC2 EBSReadBytes`) | **≈196 MB/s を維持**（30 秒粒度・`SampleCount=10`/5 分） |
| `EBSByteBalance%` | **99% → 70% へ単調に減少**（帯域クレジットを食い続けている） |
| `EBSIOBalance%` | 99% のまま（＝大きな順次読み出しで、IOPS は問題ではない） |

★ **3 時間ぶん 20 個以上の連続した 5 分バケットが、すべて 39.34 GB。**
これは「重い処理」ではない——**出口の無いループがデバイスを上限に張り付かせ続けている**形である。
利用者も Agent も既に居ない（13:34 に全部死んでいる）のに、**私が強制停止する 16:10 まで
1 秒も緩まなかった。**

root ボリュームが読まれているのは、そこに `/var/lib/docker`＝**コンテナの overlayfs** が
あるからで、home（利用者のファイル）とは別のデバイスである。**両方が同時に上限に張り付いた。**

⚠️ **ページキャッシュは 8 GiB しかない箱で毎分 40 GB を読めば何の役にも立たない。**
だから dockerd や containerd や ECS エージェントの実行ページまで追い出され、
それを読み直す I/O が同じ飽和したデバイスの後ろに並ぶ。**メモリは「足りなくなった」のではなく
「意味を失った」**——だから OOM killer は出番が無く、それでも箱は動けなくなった。

箱が実際に飢えていたことは、ログの側からも直接測れる:

- containerd が **`04:24:58` に生成したイベントがジャーナルに載ったのは `04:26:20`＝82 秒遅れ**
- dockerd: **`Could not send KILL signal to container process`**（＝プロセスが D 状態）
- ECS エージェントは 30 秒ごとの warn を出し続けるので、その**欠落がそのまま飢餓の目盛り**になる:
  `04:24:26→04:26:27` が **2.0 分**、`04:26:58→04:34:33` が **7.6 分**、
  そして **04:34:33 を最後に 3 時間 1 行も書けなかった**（05 時台・06 時台の行数は 0）
...
### 64.40.2.2 ⚠️ この訂正は対策の中身を変える

- **コンテナにメモリのハードリミットを付けても、この事故は防げない。** 制約はメモリではなかった。
  むしろ cgroup のメモリ上限はそのグループのページキャッシュも縛るので、
  **同じワークロードで再読み込みが増え、飽和が悪化する側**である。効くのは
  **I/O のスロットリング（`io.max`）か、root ボリュームのスループット引き上げ**の方
- **ホストのメモリ計測を足しても、この事故は見つけられない。** メモリは正常に見えたはずである。
  悲鳴を上げていたのは **`VolumeReadBytes` が上限に張り付き・`VolumeQueueLength`・
  `EBSByteBalance%` の単調減少**で、**これらは最初から CloudWatch に在る。**
  足りないのは計測ではなく、**在る計測に対する警報**である
...
### 64.40.6 まだ残っている穴（§64.40.2.2 の訂正を反映）

- **在る計測に警報が無い。** `VolumeReadBytes` / `VolumeQueueLength` / `EBSByteBalance%` は
  最初から CloudWatch に在り、今回の異常は**どれを見ても一目で分かる**（次点の利用者の 55 倍、
  3 時間 不変）。誰も見ていなかっただけである
- **ワークスペースのコンテナに I/O の上限が無い。** メモリと違い、**こちらは実際に効く**——
  ただし「利用者が払っている箱より小さく縛らない」という `MemoryReservation` の設計判断と
  同じ緊張があるので、上限そのものより「片方の暴走が箱の管理系まで道連れにしない」ことを
  どう担保するかを先に決めるべきである
- **root ボリュームは 40 GiB gp3 の既定スループット（125 MiB/s）のまま。** §64.31 で 100→40 GiB に
  縮めたときの結論は「gp3 はサイズ非依存（3000 IOPS / 125 MiB/s）なので縮めても遅くならない」
  だった。それは正しいが、**その 125 MiB/s は 1 台の暴走で使い切れる量**でもある
- **原因プロセスは特定できていない。** ジャーナルは残っていたがプロセス表は残らない。
  何が読み続けていたのかは、利用者の home を漁らない限り分からない
