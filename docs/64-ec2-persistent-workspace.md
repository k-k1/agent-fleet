...
| | 引き金 | 動作 | 可逆 |
|---|---|---|---|
| **退避** | 休眠 N 日の自動スイープ（`AF_ECS_EC2_HIBERNATE_AFTER_SEC`・**既定 0 ＝ オフ**） | スロット返却 → `CreateSnapshot` → 完了待ち → ボリューム削除 | ✅ 次の Start で復元 |
...
⚠️ **デプロイ単位の設定であって、テナント単位ではない。** 決定 13-2 は「テナント設定で有効化」と
書いたが、この層では守れない——掃除ループは EC2 のタグから世界を導出しており、テナントも CP の
DB も見えない（ADR 0012）。テナント単位にするなら引き金を reaper（テナントを知っている）へ
上げることになり、機能そのものより大きな変更になる。
...
### 64.19.4 まだ実機に当たっていないもの

- **`AF_WS_KEEP` の symlink 化**。sandbox のイメージは `workspace:0.8.0` で、まだ入っていない。
- **`bake-golden.sh` 本体**（E2E は同じ AWS 呼び出しを Go 側から実行しただけで、スクリプトは未実走）。
- **golden から作った home で実際にタスクを起こすところ**。確認したのはボリュームの出自までで、
  「boot-install を省ける」という効果そのものは §64.13 の実測に依っている。
