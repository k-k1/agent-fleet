...
| **1 台に 2 ユーザーを同時**（別デバイス・別 bind mount） | 成立（両タスク RUNNING・残 CPU/メモリも期待どおり） |
...
1. **root ボリュームをユーザー間で共有する。** イメージ層の共有は狙いどおりだが、**前のユーザーの
   コンテナ書き込み層と `/tmp` が同じ root に残る**。ECS の掃除は
   `ECS_ENGINE_TASK_CLEANUP_WAIT_DURATION` が握っていて**既定 3 時間**（本計測では 1m に落とした）。
   詰めないと「前の人のコンテナが残っているスロット」を次の人に渡すことになる。
2. **カーネル分離が弱くなる。** Fargate はタスク毎に microVM だが、プールに複数ユーザーを同居させると
   1 カーネルの共有になる。**同時に 1 スロット 1 ユーザー（排他）**にすれば元の分離水準に戻せる——
   その場合ホットスロット数 ≒ 同時稼働ユーザー数なので、**Fargate で同時に走らせているのと同じ量の
   compute しか払わない**（余るのは待機スロットの分だけ）。
...
5. **1 台に載る上限は EBS のアタッチ数**。`DescribeInstanceTypes` で確認すると m7i 系は
   **`MaximumEbsAttachments = 32`・`AttachmentLimitType = dedicated`**（ENI とは別枠）。
   root を除いて 31 ユーザー分まで刺さる計算で、実際の上限は CPU/メモリ側で先に来る。
   ただし awsvpc を使う場合は ENI 側（m7i.large は 3・4xlarge で 8）が先に効く。
6. **AZ ごとにプールが要る**（EBS は AZ 固定）。スロットが空いている AZ にユーザーのボリュームが
