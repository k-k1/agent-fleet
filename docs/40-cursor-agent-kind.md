...
| 項目 | 決定 |
|---|---|
| 対話ログイン | `NO_OPEN_BROWSER=1 agent login` が URL を標準出力に出す（実測）→ claude/agy と同じ start/complete 連携フロー（URL 抽出→Console 表示→ユーザーがブラウザで承認→完了検知）。OSC-8 汚染（agy 77837bf）に注意して URL 抽出 |
...
- ヘッドレス経路の workspace trust は `--trust`。TUI 初回の trust プロンプト有無は要プローブ
  （出るなら copilot 同様に設定ファイル事前追記 or `--trust` 相当で毎回スキップ、agy 5ec911a 教訓で起動毎に再固定）。
...
- **TUI**: claude と同型の working マーカー（beforeSubmitPrompt=turn 開始、stop=turn 終了）を
  ...
  `LiveState` の一次ソース。報告消失の教訓（ce8ca69）を踏まえ同じ seam に乗せる。
...
- TUI 文字列（スピナー/フッタ）には依存しない（false-idle 第3〜6次の教訓。週次リリースでドリフト前提）。
  paneMode 分岐（9526fff 教訓）用のフッタは §Track 0 実測結果 に採取済み。
...
| 質問/許可 | `session/request_permission`（allow-once/allow-always/reject-once — 公式 docs）→ Interaction(question)。`cursor/ask_question`（blocking 拡張）も question に写像。防御実装（agy 538bbeb 教訓） | docs |
...
| 完了報告 | runTurn 境界で `MarkTurnStart/End`（notify seam、b156557/330aa47 教訓） | — |
...
tmux 検証は `-L cursor-probe` 専用ソケット隔離を遵守（102670e 教訓）。
...
1. `workspace/agent/internal/agents/cursor/` 新設（テンプレ: copilot）:
   ...
   - `hooks.go` — `~/.cursor/hooks.json` へ AF 状態フック（stop/beforeSubmitPrompt）を起動前に配線
     （起動毎に再固定、5ec911a 教訓。ユーザー既存 hooks.json とのマージ規則を決める）
   - `sids.go` — sid-store（slotSid → cursor chatId。create-chat 事前採番）
   - `transcript.go` — JSONL 転写パーサ（Turn.Idx 単調採番 — aa341b4 教訓・pending 検知・ツール正規化）
...
3. paneMode（`session_io.go`）: フッタ実測パターンで分岐（9526fff 教訓・Track 0 で採取）。
   tmux 内の改行は Ctrl+J（公式 docs — Shift+Enter 非対応）→ 投入経路 5 つ
   （launch-seed / send_to_session / /turn / 保留中 / paste）を監査（87156b7 教訓）
...
5. entrypoint で `cli-config.json` の `channel:"static"` を起動毎再固定（channel 鍵のみ・
   他キー保存・JSON でなければ触らない — 5ec911a 教訓）。
...
3. Console:
   ...
   - `agents/registry.ts` — descriptor（**caps 全項目を明示決定**した — chat/transcript/model/
     ...
     false、fork は TUI 限定で false、contextBar/imagePaste は v1 未配線で false=c527735/57d9c9e
...
4. availability: registry の `available` 述語＝`supported!==false && connected`（agy/copilot
   同型）。conns 未取得時は表示（rail の null=show-all）＝66f9720 教訓に整合。
...
| 過去の指摘 | cursor での対応 |
|---|---|
| resume ID 取得不能（agy 280945c） | AF 側採番の chat ID を `--resume` に渡して構造的に回避（copilot `--session-id` と同型。当初計画の `create-chat` 事前採番は Track A で自己採番 v4 UUID へ変更 — §Track A の実測反映） |
| auth URL の OSC-8 汚染（agy 77837bf） | login start/complete フローの URL 抽出で同対策を最初から適用 |
| tmux kill-server 全滅（agy 102670e） | probe/E2E は `-L cursor-probe` 専用ソケット隔離 |
| Turn.Idx 未採番（aa341b4） | transcript.go 単調採番＋単調増加テスト |
| paneMode 分岐漏れ（9526fff） | Track 0 でフッタ実測 → 実装。改行 Ctrl+J の投入経路監査（87156b7）込み |
| MCP ツール欠落（1c8b195） | mcp_stdio.go / CP mcp.go 総ざらい済（Track C — create_session driver・list_models whitelist・各 usage 記述） |
| turn 終端未検出→報告不発（b156557/ce8ca69） | hooks stop を status seam に乗せ、managed は MarkTurnStart/End |
| caps 明示漏れ（c527735/57d9c9e） | descriptor caps 全項目を明示決定済（Track C — effort/fork/contextBar/imagePaste は根拠付きで false） |
| 権限「発生しない前提」崩壊（agy 538bbeb） | request_permission / ask_question を防御実装 |
| conns ローディング中の誤露出（66f9720） | connsDone ゲート |
| 設定の一回きり固定（agy 5ec911a） | hooks.json / trust / auto-update 封殺は起動毎に再固定 |
...
| インストーラ既存あり no-op（agy b3ac0e7） | tarball 直展開のため非該当だが、焼き込み検証を e2e-smoke に追加 |
