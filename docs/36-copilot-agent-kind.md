...
- **セッション UUID は AF 側で外部採番**する（`--session-id <uuid v4>`、RFC4122 v4 必須・実測）。
  ...
  agy の「resume UUID が取れない」問題（46271bb）は構造的に発生しない。
...
- Transcript() はこれを正規化（**Turn.Idx 単調採番**、1ccb63e 教訓）。managed でも CLI 自身が書くため
...
| 質問/許可 | `session/request_permission`（server-initiated request、allow_once/allow_always/reject_once・実測）→ `Interaction`（Kind:"question"）に写像し waiting_interaction。**allow-all 運転でも防御的に実装**（agy 3aaebf6 教訓: 「発生しない前提」を信用しない）。ask_user は ACP では平文テキスト+end_turn に落ちる（実測）→ 通常チャット往復で成立 | probe2/3 |
| 完了報告 | runTurn 境界で `agents.MarkTurnStart` / `MarkTurnEnd(sid, terminal)`（notify seam、0c80377/451ff8b 教訓） | — |
...
1. `workspace/agent/internal/agents/copilot/` 新設（テンプレ: codex/agy）:
   ...
   - `trust.go` — config.json `trustedFolders` 事前追記（起動毎、agy 5a19080 教訓で毎回再固定）
...
3. **paneMode**（`session_io.go`）: フッタ実測パターンで分岐（1ab3eb9 教訓）:
...
3. 自己更新 opt-in（`AF_AGENT_SELF_UPDATE`）に copilot 追加（npm 系なので claude/codex と同経路。
   6111248 の「既存あり no-op」問題は npm には無い）。
...
2. Console:
   ...
   - `agents/registry.ts` — descriptor（`managedDriver:true`、`tuiMemoryCost`、**caps 全項目を明示決定**
     （f49e162/41c78b0 教訓）、色・アイコン・表示順を最初に確定（5bf9800 教訓））
...
3. availability: `connsDone` 分離（c6514b5 教訓）、選別は `caps.runsInDir`（238f253 教訓）。
...
|---|---|
| resume UUID 取得不能（46271bb） | `--session-id` 外部採番で構造的に回避 |
| auth URL の OSC-8 汚染（1e8a71d） | 専用 auth フロー無し（該当なし） |
| tmux kill-server 全滅事故（910cc9a） | E2E は `-L` 専用ソケット隔離を本計画の実測でも遵守済。live テストも 3 点隔離 |
| 子プロセス reap 漏れ（b83cdbc） | 独自 spawn は managed 子のみ・必ず `cmd.Wait()`。他は `agents.Flow` 経由 |
| Turn.Idx 未採番（1ccb63e） | transcript.go で単調採番＋単調増加テスト |
| paneMode 分岐漏れ→初回プロンプト消失（1ab3eb9） | フッタ実測パターンで実装（本計画 §Track A-3） |
| 投入経路の監査漏れ（bb81e62） | launch-seed / send_to_session / /turn / 保留中 / paste の 5 経路を実装時に監査 |
| MCP ツール欠落（413b696） | mcp_stdio.go / CP mcp.go 総ざらいを Track C チェック項目に |
| turn 終端未検出→報告不発（0c80377） | TUI=LiveState（events.jsonl）、managed=runTurn 境界で MarkTurnStart/End |
| caps.imagePaste 放置（f49e162）/ caps 反転漏れ（41c78b0） | registry caps 全項目を descriptor 実装時に明示決定（表で記録） |
| 権限プロンプト「発生しない前提」崩壊・halt が承認を踏む（3aaebf6/79fcd90） | request_permission を防御実装（managed）。TUI GracefulStop は Escape 先行 |
| conns ローディング中の誤露出（c6514b5） | connsDone ゲート |
| telemetry 一回きり固定（5a19080） | trust/設定は起動毎に再固定。remote-export/remote も起動フラグで毎回オフ |
| /usage プラン別形式差（6210d4c） | v1 は /usage スクレイプ非採用（events.jsonl の usage_checkpoint / result usage を利用） |
| インストーラ既存あり no-op（6111248） | npm 導入のため非該当。自己更新は claude/codex 同経路 |
| 焼き込み再配布ライセンス（ff8c708） | LICENSE.md 実測: 無改変＋実質機能要件で焼き込み可の読み。パッケージング配布物は agy 同様 boot-install 検討 |
| 質問カードキー列（843e1e0/7c09dcb） | managed は /respond で構造化（キー列非依存）。TUI 保留カードは v1 非対応（allow-all 運転） |
| チャット分離 HOME（81cd5bc） | Track D（チャット統合時に COPILOT_HOME 分離を適用） |
