...
- [50-mirror-skill-picker.md](50-mirror-skill-picker.md) — ミラーのスキルピッカー: セッションの worktree ＋ユーザーレベル `.claude/skills`・`commands` をセッション単位 API で認識し、コンポーザーの「/」補完＋「/」ボタンでキーボード / マウス / タップいずれでも 1〜2 操作で呼ぶ（✅ 実装済み・実機目視待ち。設計判断は [decisions/0034](decisions/0034-mirror-skill-picker.md)）
