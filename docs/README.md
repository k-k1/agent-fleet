...
- [60-user-instructions.md](60-user-instructions.md) — ユーザー指示: フリート方針（イメージ焼き込み）とプロジェクト指示（コミットされる）の間に「その人の層」を作る。正本は `~/.config/agent-fleet/user-notes.md` 1 本で、各 CLI の global 指示位置へマーカー合成で配る（entrypoint の `cp -f` 全消しを置換し、1 ファイル 1 ライターへ）。**利用者の追記が毎起動で消えていた**こと・**agy/cursor/kiro/copilot はフリート方針すら読んでいない**ことを実測で確認し、置き場の罠（`~/.claude/CLAUDE.md` は claude ではなく opencode が読む）と codex のバイト予算（フリート方針だけで 32 KiB 予算の 91%）も記録（設計確定・未実装。設計判断は [decisions/0042](decisions/0042-user-instructions.md)）
...
- [0042-user-instructions.md](decisions/0042-user-instructions.md) — ユーザー指示は AF が所有する本文 1 本とし、各 CLI の global 指示へマーカー合成で配る（配布軸＝自動で書く側・kind 別本文は作らない・claude だけ native な user memory 層に独立ファイルで置く・上限 8 KB）（設計 [60](60-user-instructions.md)・[0040](decisions/0040-project-mcp.md) の別軸）
