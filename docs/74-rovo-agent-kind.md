# 74. Rovo Dev エージェント種別（kind=rovo・第9種）— Track 0 プローブ第1段（認証前）
...
そこが覆ると設計も変わる。ADR は採用が決まってから `decisions/0055` で起票する。
...
※ 起票時は docs/70・ADR 0051 で採番したが、develop 側で 70〜73 と 0051（[70](70-slot-instance-classes.md) が P1 で使う予約）が
先に埋まっていたため **74 / 0055** へ振り直した。
...
ADR は採用が決まってから `decisions/0055` で起票する（現時点で ADR は作らない＝状態「検討中」の ADR は前例が無い）。
