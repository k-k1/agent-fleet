...
- 編集面へのfocus移動は「モード選択ごとの世代番号」を単位とする。編集面を表示する選択のたびに
  番号を進め、CodeMirrorのready後に一度だけ消費する。ready前の選択も取りこぼさない。要求は
  自分の選択より長生きさせず、(a) 編集面が表示されなくなった時点、(b) 要求を出したコミットで
  編集面が表示されていない場合（同一バッチ内で後続の選択に追い越された）、(c) ファイルが
  変わった時点、のいずれでも破棄する。ファイルを開いただけ・行引用で開いただけではfocusを
  移さない。
- **既知の限界（v1で対応を見送る）:** 上記の破棄契機は「表示される面の組」の変化とファイル
  変更に基づくため、**面を変えない操作は未消費の要求を破棄しない**。次の窓が残る: (1) CodeMirror
  のready完了前に `split` を選ぶ（要求が立つ）。(2) ready前に、同じ `split` のままpreview
  rendererを切り替える（面の組は変わらないので要求は生き残り、focusはrendererボタンにある）。
  (3) readyが完了し、残っていた要求が消費されてfocusがrendererボタンから編集面へ移る。
  編集面はペインが編集可能になった時点でマウントされ、通常はモードを選べるようになる前に
  readyが完了しているため、この窓に入るにはreadyの遅延とその最中のrenderer操作が重なる必要が
  ある。影響はfocus位置だけで、バッファ・保存・表示状態は変わらず、Shift+Tabで復帰できる。
  解消するには、面を変えない操作でも未消費の要求を破棄するか、要求に「その後ユーザーが別の
  コントロールを操作したか」を持たせる必要があり、将来の改善候補とする。
...
からなる。読み取り専用のレビュー担当セッションとのペアで5ラウンド・13件の指摘を修正し、うち1件
（§5の既知の限界）はユーザー判断で対応を見送った。
