...
| fs | `GET /api/fs/{tree,file,download,changes,linemarks}`・`PUT /api/fs/file`・`POST /api/fs/{upload,mkdir,newfile,rename,delete,suggest-edit}` | 中継 | [04](04-agent.ja.md) / [docs/44](../decisions/0027-markdown-code-editor.ja.md) |
| connections | `GET /api/connections`・git `PUT/DELETE /api/connections/git/{host}`（+ GitHub Device / Bitbucket OAuth / claude / codex / opencode）・`GET /api/git-oauth`（自テナントでどの OAuth ボタンを出せるか）| 中継。ただし **git プロバイダの OAuth は両方とも CP**（GitHub の device flow `start`/`poll`・Bitbucket の `start` と callback・`/api/git-oauth`）＝[71](../decisions/0052-tenant-git-oauth.ja.md) | [08](08-integrations.ja.md) |
...
| schedules | `GET /api/schedules`・`GET …/{id}/runs`・`PATCH/DELETE …/{id}`・`POST …/{id}/{pause,resume,run-now}`（Console は一覧・管理のみ。作成は MCP ツール経由）| CP（DB + scheduler）| [docs/38](../decisions/0021-scheduled-execution.ja.md) |
...
| agent memory | `GET /api/agents/memory/{roots,snapshots,diff,tree,export}`・`POST …/{snapshots,restore,import,import/apply}`・`PUT …/settings` | 中継 | [docs/39](../decisions/0022-agent-memory-management.ja.md) |
| MCP レジストリ | `GET/POST /api/mcp-servers`・`PUT/DELETE …/{id}`・`POST …/{test,tenant-refresh}`・`POST …/{id}/enabled`・`PUT …/{id}/secrets` | 中継（実体は Agent）| [docs/48](../decisions/0031-mcp-registry.ja.md) |
| usage 時系列 | `GET /api/usage/series`（機能別トークン台帳の時系列）| 中継 | [docs/46](../decisions/0029-usage-accounting.ja.md) |
...
| admin | `GET /api/admin/{tenants,sessions,usage,audit,host,egress*}`・`POST /api/admin/{tenants,memberships,stop-workspace,clean-home}`・`PUT /api/admin/{tenants/{slug}/limits,user-limits,membership-role,egress/mode}`・git プロバイダ OAuth `GET /api/admin/tenants/{slug}/git-oauth`＋`PUT/DELETE …/git-oauth/{provider}`（tenant_admin・承認なし・[71](../decisions/0052-tenant-git-oauth.ja.md)）| CP（super_admin / tenant_admin gate）| [03](03-control-plane.ja.md) |
| MCP | `POST /mcp`（Streamable HTTP JSON-RPC・Bearer PAT・authGate 除外）| CP | [03](03-control-plane.ja.md) / [decisions/0006](../decisions/0006-mcp-unified.ja.md) |
...
- `GET /api/workspace/stats` の応答は CP がホストから cgroup v2 を直読みして組む（`metrics.go`）:
  ...
  （コンテナ内子プロセスの OOM kill／コンテナ丸ごとの OOM 落ち）で、設計は [26](../decisions/0014-agent-exit-recording.ja.md)。
...
| **browser** | `/api/browser/pages*` → Agent `/browser/pages*`、`/ws/browser` → Agent同名 | membership/running検査後にAgent bearerだけを付与。REST本文/応答とWS textは解釈せず、binary JPEGはlatest-onlyで中継。visible viewerだけがworkspaceをwarm維持。wire v1は[設計31](../decisions/0018-container-browser-pane.ja.md) |
