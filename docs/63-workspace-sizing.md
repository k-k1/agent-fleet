...
> 決定済み: **ディスク軸は「21〜200 GiB は ephemeral storage、200 GiB 超は ECS 管理 EBS」・既定は
> 20 GiB（＝無料枠のまま）**（§63.3）。
> 未決: CPU/メモリの表現方法（数値 2 軸 か 名前付きサイズ か）と、`~` の置き場（§63.5）。
...
- **「Elastic throughput に変えれば速くなる」は否定された。** 小ファイルには効かない。
  導入するとしても理由は「逐次帯域」と「バーストクレジットの崖への保険」であって、ビルドの速度ではない。
- **「キャッシュ類だけローカルへ移す」案の効果も、実測済みの数字としてほぼ無い。** 今回の計測は
  まさにその配置（npm キャッシュはローカル・書き込み先だけ EFS）であり、それでも `npm ci` は
  9.4 倍遅かった。**支配項は生成物の書き込み**であって、キャッシュの読み出しではない。

残る選択肢は「生成物をどこへ書くか」に絞られる。

| | 内容 | 効果 | 危険 |
|---|---|---|---|
| 現状維持 | 全部 EFS | — | ビルドが遅いまま |
| `~/repos` ごとローカル | 未コミットの作業もローカル | 大 | **アイドル停止で作業が消える。採らない** |
| **生成物だけローカル** | `node_modules` / `target` / `.venv` / `dist` などをローカルへ逃がす（symlink） | 大（`npm ci` 自体は 3s 側になる） | 再生成が毎起動必要。誰が symlink を張るかの設計が要る |

3 つ目が本命だが、「誰がいつ symlink を張るか」（クローン導線・entrypoint・利用者の手作業）が
未設計なので、本書では起票のみとし、ディスク軸の実装とは分けて扱う。
