...
- 「再発行」ボタンを添える（URL を配ってしまったときに、その場で捨てられる道）。
  ⏸ **未実装。** 今は固定を一度 OFF にして起動し直すと予約が捨てられる（実装は
  `rotatePreviewSlug`）ので、逃げ道はあるが 1 手多い。
...
| **P1 Console** | ✅ | プレビューのポップオーバー（発行済み URL・コピー）・Workspace 設定（許可ポート / slug 固定 / 公開モード）。⏸ **「再発行」ボタンは未実装**（§4.1） |
