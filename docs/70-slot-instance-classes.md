...
❌ **残る弱点: 配置できない理由が Console から見えない。** ECS はイベントに理由を書いて
いるのに CP は黙って `starting` を返し続ける。今回それを突き止められたのは
`describe-services` を直接読んだからで、運用者に同じことはさせられない。
...
| **P5** ◐ | 実 AWS で端から端まで（§70.14・<dev-deployment>）。**arm64 の golden が probe 込みで publish され**、**x86 → arm の切替も home 自己修復ごと通った**（§70.14.5）。残 = **agy の OAuth と実セッション** | 実運用に出せる |
