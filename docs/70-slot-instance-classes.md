...
- m6i = 同じ x86 の 1 世代前。**−4.8% しか下がらず**、アーキ問題を一切引き起こさない代わりに
  ほとんど何も得しない。**単独で導入する理由は無い**（クラスの選択肢としては安い）。

⚠️ **P0 の計測項目**（§70.14）: この 4 種で **このリポジトリ自身**を回す。
...
1. **どのファミリをクラスとして出すか。** §70.3.1 の実測待ち。現時点の見立ては
   `standard = m7i` / `arm = m8g`（−11% で速い）または `m7g`（−19%）/ `econ = m6g`（−24%・遅い）。
