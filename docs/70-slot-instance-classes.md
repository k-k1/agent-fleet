...
### 70.3.1 「一番安いのが一番良い」とは限らない（未実測）

価格は実測したが、**性能は測っていない**。世代差は一般論として次のとおりで、
開発 Workspace の支配項（`npm ci`・`tsc`・`go build`・エージェント CLI の起動）は
**シングルスレッド寄り**である。

- m6g = Graviton2（2020）。コア当たりは m7i より体感で明確に遅い部類。**−24% の対価は速度**。
- m7g = Graviton3。m7i に対しコア当たりは同等〜やや下、価格は −19%。
- m8g = Graviton4。**m7i より安く（−11%）、かつコア当たりは m7i 以上**という位置づけ。
- m6i = 同じ x86 の 1 世代前。**−4.8%** しか下がらないが、**前提条件がゼロ**である（後述）。

### 70.3.2 m6i の位置づけ（起票時の判断を訂正）

起票時、m6i は「−4.8% しか下がらないので単独で導入する理由は無い」と書いた。**これは
割引率だけを見た誤りである。** 正しくは、m6i は**この表で唯一、前提条件を 1 つも持たない
選択肢**である。

| | m6i | m7g / m6g / m8g |
|---|---|---|
| arm64 の workspace イメージ（§70.9） | **不要** | 必須（P1・未） |
...
| 性能の未知数（§70.3.1） | 世代差 1 つ分 | 世代 ＋ アーキ |

つまり **m6i は「今日そのまま出せる唯一のクラス」** で、−4.8% は「リスクゼロで得られる
4.8%」である。開発 Workspace の費用は 1 人あたり月 $16〜95（§70.3）なので、
20 人なら年 $150〜550 になる。

そして本質的に、**どれを選ぶかは利用者の判断であって、こちらが代わりに決めることではない**。
「速度より費用」を選ぶ人・チーム・時期は普通にある（学習用のテナント、CI を回さない
レビュー専用のメンバー、繁忙期を過ぎた部署）。**その判断のために存在するのがこの機能**で
あって、割引率が小さいという理由で選択肢から外すのは、この機能そのものの否定になる。

**決め**: m6i を選択肢として載せる。⚠️ ただし**既定にはしない**——既定の変更は選んでいない
全員を動かすことなので、それは別の判断である（§70.14-2）。

⚠️ **P0 の計測項目**（§70.14）: この 5 種（m7i / m6i / m8g / m7g / m6g）で **このリポジトリ自身**を回す。
`console/npm ci`（20,905 files）・`console/npm run build`・`(cd control-plane && go test ./...)`・
`chromium --headless` のスクショ 1 枚。[63](63-workspace-sizing.md) §63.4 と同じ流儀で、
同一手順・同一 home サイズ・同一イメージで比較する。**この数字が出るまで「m6g を既定にする」
という選択はしない。**
...
1. **どのファミリをクラスとして出すか。** arm64 側（m8g / m7g / m6g のどれを `arm` と
   `econ` に割り当てるか）は §70.3.1 の実測待ち。**x86_64 側は決まっている**——
   `standard = m7i` と `econ-x86 = m6i` で、後者は前提条件がゼロなので P0 も P1 も待たずに
   出せる（§70.3.2）。
