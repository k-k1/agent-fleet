...
- 関連: [0045-ec2-persistent-workspace.md](0045-ec2-persistent-workspace.ja.md) 決定 8
  ...
  今回はそれを明示的にひっくり返す） /
  [0044-workspace-sizing.md](0044-workspace-sizing.ja.md) 決定 3（**既定オフで出した機能は
  存在しないのと同じ**） /
...
- 付与は home のアタッチが成功した地点、除去は `releaseSlot` のデタッチ成功後と
  `quarantineSlot`。掃除経路で「`af-membership` があるのにボリュームが付いていないインスタンス」を
...
- **掃除経路は双方向に直す。** 古いタグ（解放が半分で終わったインスタンス）は消し、抜けたタグ
  （アタッチ直後に落ちた／このコード以前のインスタンス）は**ボリュームから写す**。前者は人を過剰請求し、
...
- **空きプールのスロット時間も共有**（`af-role=slot` かつ `af-membership` 無し）。
  「誰も使っていないのに回っているインスタンス」の実費が初めて数字で見える。
...
- **追記（2026-08-25・[docs/67](../log/67-member-cloud-cost.md) §67.17）**: **組織のメンバー
...
[0044](0044-workspace-sizing.ja.md) 決定 3 の「誰もやらない手順は存在しない手順」そのもの。
...
## 決定 15 — 共有の内訳に **`af-role` の軸**を足す。推論エンジンと TTS は「別枠」であって按分ではない

ADR 0071 決定 9 は「エンジンのインスタンス時間は `af-role=engine-llm` / `engine-image` の
費用配分タグで**コンポーネント費用として表示し、按分しない**」と決めていた。**タグは打たれて
いたが、読む側が無かった**——CP が CE に投げる唯一のリクエストは `GroupBy=(af-membership,
SERVICE)` で、`af-role` は決定 1 の許可リストで有効化だけされて誰にも使われていなかった。

結果として、共有バケットの内訳は AWS のサービス名しか持たない:

| 実体 | 共有カードでの見え方 | 混ざる相手 |
|---|---|---|
| エンジンの GPU（MI の EC2） | `Amazon EC2 - Compute` | **空きスロットプール** |
| TTS（Fargate） | `Amazon Elastic Container Service` | **CP 自身のタスク** |

**実測（af-sandbox・2026-09-01〜08、CE 1 リクエスト）**: `engine-llm` $5.18・`engine-image`
$0.73・`tts-engine` $0.17 で、**請求全体の 19%** が「見えないまま共有に入っていた」。
人に紐づく分が 22.3%（決定 4）なのだから、同じ桁の塊である。

- **2 本目の CE リクエストを足す。** GroupBy は CE の上限が 2 軸で、1 本目が
  (af-membership, SERVICE) で使い切っている。既存の表から導出することは**できない**——
  エンジンの GPU 時間も空きスロットも「membership 空 ＋ EC2-Compute」で、どんな算術でも
  分けられない。$1.2/月 → **$2.4/月**。
- **これは按分ではない。** 決定 2（見積を出さない）も決定 4（共有を配らない）も**そのまま
  生きている**。同じ請求を別の軸で切っただけで、`engine_hourly`（ADR 0071 決定 13）の稼働秒に
  単価を掛ける案は今回も採らない。実費が $0.01 で取れるのに見積を並べる理由がない。
- ⚠️ **フィルタが本体。** `af-membership` が **ABSENT** の行だけを引く。付けないと、
  利用者が握っているスロットは `af-membership` と `af-role=slot` の**両方**を持つので、
  その人の割り当て額と役割別の `slot` に**二重で出て、共有カードの合計が共有バケットを超える**。
  実測で `slot` の EC2-Compute は $3.33（無フィルタ）→ **$0.12**（ABSENT）に落ちた。差の
  $3.21 は誰かが握っていた時間で、それは per-member 側が既に答えている。
- ⚠️ **予約メンバーシップは Or で足し戻す**（決定 13）。golden の種と probe は
  `af-membership` を持つが取り込みで SHARED へ畳まれるので、ABSENT だけだと 2 つの表の
  合計がその分ずれる。`Or[ABSENT, EQUALS(予約 ID…)]`。⚠️ **CE は空の `Values` を拒否する**ので、
  予約 ID が無いデプロイでは Or ごと落として素の ABSENT にする（実測でこの形が通ることを確認）。
- **`role=''` は欠測ではなく答え。** NAT・ALB・RDS・Route53・税は `af-role` を持てない
  （NAT のバイトは「誰が要求したか」を知らない）。**エンジンの隣に置く残差**として出す。
  ⚠️ 実測では sandbox の残差 $22.25 の最大項が NAT $8.59（bytes $5.61＋hours $2.98・
  `EC2 - Other` の中）で、決定 4 の「NAT は割れない」がそのまま効いている。**モデルの取り込みが HF から引く GB も
  ここに入る**（S3 からの再取得はゲートウェイ経由で NAT を通らない）。
- **2 段で見せる。** グループ合計（エンジン／音声合成／誰も使っていないワークスペース資源／
  基盤）を先に、役割ごとの行はその下の折り畳みへ。グループ分けは **CP の
  `runtime.CostRoleGroup` 1 か所**で決める（画面側でも分けると 2 つの合計が食い違う）。
  ⚠️ **知らない役割は「その他」**であって基盤の残差に混ぜない——新しいタグ値が NAT や RDS に
  静かに合流したら、二度と誰も問わない。
- **失敗したら 1 本目だけ残す。** 2 本目は追加の内訳であって請求そのものではないので、
  落ちても per-member の取り込みは完了しており、`meta.error`（画面が数字の隣に出す文言）にも
  しない。旧 CP・失敗中の CP では応答にキーごと出さず、画面はサービス別だけを描く。
- ⚠️ **S3 と CloudWatch Logs にタグが 1 つも無かった**（`ModelsBucket` / `LogGroup`）。
  **有効化と同じで、タグも遡らない**——だから設計の結論より先に打った（決定 1 と同じ順序）。
  MI の管理手数料とデータボリュームは**サービスのタグを継いでいた**（`engine-llm` の
  $0.37 が ECS、$0.08 が EC2-Other で返ってきた）ので、そこは追加不要と実測で確認した。
...
- **円換算** — 決定 6。
- **役割別を `engine_hourly` の稼働秒 × 単価で出す** — 決定 15。決定 2 の型そのもので、
  しかも表が既に在るぶん書けてしまうのが罠。
- **既存の 1 本を (af-role, SERVICE) に差し替える** — 決定 15。membership 軸を失う。
  CE の 2 軸上限は動かせないので、2 本目を足すしかない。
- **役割別だけ 12 時間おきにして $0.6/月 に抑える** — 取得の新鮮さが画面の中で 2 つに
  割れる。$0.6/月 は、そのわかりにくさに見合わない。
