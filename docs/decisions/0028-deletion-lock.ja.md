...
## 追記（2026-09-24）— 「worktree の自動 prune」は無くなった

背景と決定 2 が挙げる自動削除の経路のうち、停止 TTL の prune は ADR 0097 で「棚へ移す」になり、
セッションが尽きた worktree の自動 prune（`MaybePruneWorktree`）は [ADR 0101](0101-session-delete-via-trash.ja.md)
決定 3 で廃止された。worktree を消すのは人の操作（掃除②・作業コピーの削除・MCP `delete_worktree`）だけで、
ロックは今までどおりそのすべてに効く。セッションのロックも、削除（ごみ箱へ移す）と、worktree の削除の
巻き添え（棚へ・ごみ箱へ移す）の両方に効く。
