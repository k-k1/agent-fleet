# 0075. エンジンのインスタンスは、必要な VRAM を満たす提案のうち上から買う——Spot とオンデマンドを 1 本の一覧に並べる
...
- 関連: [0074-engine-instance-classes.ja.md](0074-engine-instance-classes.ja.md)
  ...
  一度も呼ばない）・決定 4（段の変更は次のインスタンスから／退場待ち）・決定 5（Describe → 写す → Update と
...
`UnfulfillableCapacity` を返した（0074「インスタンスは取れなかった。しかも第 3 のエラーコードだった」）。
**誰もオンデマンドへ落ちてくれない。**

**3. インスタンスを選ぶのは人で、CP は価格を知らない。** ADR 0045 決定 21 と ADR 0074 決定 1 が
...
`draining` も 0074 決定 4 の退場待ちも、**こちらが desired を 0 にしたインスタンス**を見る仕組みである。
Spot の中断は誰も desired を動かしていないのにインスタンスが消える。コントローラはこれを**既に見ている**
...
- **エンジン表の 1 行には capacity provider が 1 つしか無い**（`engines.go` の
  ...
  自分のインスタンスかの照合（`box()`）が、どちらもこの 1 つの名前で引いている。
...
- **エンジン表のうち生きたまま運べるのは梯子と capacity provider 名**（`engine_table_reload.go`）。
  ...
  「provider の名前が変わると、走っている CP はインスタンスを見失う」——段の適用が消えた旧名へ飛んで 400、
  `box` は `null`、それでもパネルは「l4 で起動中」と言った）が、**#542（2026-09-11）が生きたまま
  採る形にした**。その形は「宛先」と「照合」の名前を `engineECS` の**1 つの欄**に置き、改名で
  インスタンスのキャッシュと最後に適用した段を忘れる、というものである——決定 3 はこの 1 欄を 2 欄に広げる。
...
| `get-spot-placement-scores` は**型の組で答えが変わる**: 1 型で 1/10、3 型（g6+g5+g6e）で 9/10。9 を記録した **42 秒後**にインスタンスが来た | 同上 |
...
| **価格が付いていることは在庫の証拠ではない**——買えなかった 17 分のあいだ、価格履歴は $0.563〜0.577 を返し続けた | 0074「インスタンスは取れなかった」 |
| クォータは別枠: オンデマンド `L-DB2E81BA`、Spot `L-3819A6DF`（既定 0）。acrt は 64/64、af-sandbox は 8/8 | 0074「追試」「Spot は買えた」 |
| `describe-instances` は MI のインスタンスを**列挙できない**が、**ID を知っていれば返す**（`InstanceLifecycle: spot` を読む唯一の経路） | 0074「Spot であることの証明は」 |
| 中断後のインスタンスは EBS/instance store が新品——S3 からの取り直しは実測 6.62 GiB / 66 秒（102.7 MiB/s） | 0071 実測 |
| コールドスタート: image 165〜197 秒、llm 527〜586 秒 | 0071 実測 |
| `mode: on` から Spot のインスタンスが ACTIVE まで **42 秒**、running 281 秒、warm 307 秒、1 枚生成 403 秒 | 0074「実測（すべて `mode: on` の 06:01:16Z から）」 |
...
- **価格が無い行が「買えない行」になってしまう。** 0074 が決めた規約は「実測があるものは実測、
  ...
  （0074 で買えた Spot のインスタンスは 403 秒しか走っていない）。価格を並びの鍵にすると、いちばん試して
  ほしい行が最後に回る。
...
- **必要 VRAM が `unknown` のときは絞らない。** 一覧の先頭から試す。0074 決定 6 の
  「不明を大丈夫と描かない」は**警告についての決定**であって、買うインスタンスを止める決定ではない
...
- **エンジン表の行は provider を 2 つ持つ**ことになる——`capacityProvider`（オンデマンド・
  ...
  `draining` の判定と `box()` の照合は、**どちらの名前でも自分のインスタンスと認める**必要がある
  ——片方だけを見ていると、もう一方で買ったインスタンスが `box: null` として見えなくなる
...
### 5. 規則 2 ——予算の中にインスタンスが来なければ、次の提案へ。**失敗コードで待ち方を変える**

提案 1 つあたりの予算 `<役>OfferBudgetSec`（既定 **180 秒**）は、**desired を 1 にしてから
container instance が ACTIVE になるまで**を測る。インスタンスが来たらこの時計は止まり、あとは通常の
...
- 一覧を**一周したら諦める**。コントローラの既存の cooldown（既定 900 秒・失敗のたびに倍・
  ...
  早く伸びる）。
- **提案を移るたびに監査ログに 1 行**（`engine.<役>.offer`）。「なぜ高いインスタンスで走っているのか」は、
...
- **中断は失敗として数えない。** コントローラの `failures`（cooldown を倍にする数）は
  ...
  期限までインスタンスを得られなければ、それは起動の失敗であり、今までどおり数える**。
...
- **固定してインスタンスが来ないときは、サービスイベントがそのまま出る**（0074 決定 1 の最終項と同じ立場）。
...
🔴 **この決定は 2026-09-19 に置き換わった**（ADR 0077 決定 9 と、その改訂節「役の禁止を、管理者の
受け入れに置き換えた」）。禁じた理由——中断は会話の途中で来て、そのあとにコールドスタートが続く——は
そのままだが、それは役の性質ではなく配備ごとの判断だと読み直し、**管理者が役ごとに中断を受け入れた
ときだけ `spot` の行が候補になる**形にした。ADR 0077 が provider を廃した以上、ここの「テンプレートで
担保する」半分は最初から残っていない。

### 10. TTS は provider を作らず、`FARGATE_SPOT` ↔ `FARGATE` の strategy に同じ規則を当てる

TTS は Fargate なので買うインスタンスが無い。提案の行は**型を持たず、購入形態と価格だけ**になる。
...
- 🔴 **実際に Spot だったかの証明は `describe-instances` を ID で引くしかない**（0074 実測。
  MI のインスタンスは列挙できないが ID なら返り、`InstanceLifecycle: spot` はそこにしか無い）。
...
🔁 **反証されたら変える条件**: 「CP が選んだ提案」と「実際に買われたインスタンス」が食い違う例が
...
|---|---|---|
| capacity provider（4 欄） | 保存時と**毎回の起動直前**（冪等） | 何も動かない。次に買うインスタンスの仕様が変わるだけ |
...
| service の strategy（**CloudFormation が書く**） | service の資源に触る更新のたび——タスク定義が変わるリリースは毎回 | **宣言の provider へ戻り、新しいデプロイが起きる。** 走っている Spot のインスタンスはオンデマンドのインスタンスに置き換わり、CP はそれに関与しない |

running 0 に限る理由は、(c) が正しくても残る——走っているインスタンスと strategy が食い違う状態を
...
🔴 **service については窓が「状態」ではなく「インスタンス」になる**——リリースのデプロイは走っている
Spot のインスタンスをオンデマンドのインスタンスで置き換えるので、リリースの直後は宣言の provider（オンデマンド）で
...
- **要求ごとにインスタンスを選ぶ**（この生成は安いインスタンス、この会話は大きいインスタンス）。却下: インスタンスは 1 役 1 台で、
  コールドスタートは image 165〜197 秒・llm 527〜586 秒である。要求ごとに選ぶには複数台を
  並べることになり、G 系クォータ（8〜64 vCPU）と 0071 決定 2（2 役を同じインスタンスに載せない）の外に出る。
  未解決 4 に残す。
- **Spot の 2 分前予告を受けて、中断の前にオンデマンドのインスタンスを建てておく。** 予告を受け取るのは
...
- **0074 決定 3「未宣言・未選択なら ECS を一度も呼ばない」は継承する。** 提案が 1 つも
  ...
  変わらない。
- **0074 決定 4「段の変更は次のインスタンスから。停止 → 退場を待つ → 起動」も不変**である。
...
値打ちである**。⚠️ **1.57 は算術であって請求の実績ではない**——0074 で買えた Spot のインスタンスは
...
| コード | 何を指すか | 対応 | 出所 |
|---|---|---|---|
| `UnfulfillableCapacity` | 「あなたの**要求の設定**のせいだ」。AZ を変えろとも後で試せとも言わない。実測では 17 分・4 回すべて同じ | **予算を待たずに次の提案へ。** 待っても直る形をしていない | 0074「インスタンスは取れなかった」 |
...
「オンデマンドの枠が退場待ちのインスタンスで埋まっているとき、Spot の枠は空いている」を意味する。
...
- **ゲートウェイ側の `AF_ENGINE_WAKE_TIMEOUT`（既定 900 秒）は変えない**（0071 決定 5）。
  ...
  次の要求が温まったインスタンスに当たる、という形になる。これは 0071 決定 5 が既に選んでいる形と同じである
...
- したがって**規則 2 のフォールバックは、枠の観点でも本物である**——オンデマンドの枠が
  退場待ちのインスタンスで埋まっている（0074 P1 の実測: ECS の登録解除から 5 分以上 `VcpuLimitExceeded` が
...
2. 中断を断定するために `DescribeTasks`（と IAM 1 つ）を足す価値があるか。検出はインスタンスの消失だけで
...
4. **要求ごとにインスタンスを選ぶ**（却下案）。1 役 1 台という前提が動いたら再検討する。
...
8. **`--models-max` との関係。** 0074 決定 11 が「タスク定義は段に追随させない」と決めているので、
   ...
   「大きいインスタンスなら 2 つ載る」が自然な期待になるが、それは 0074 未解決 6 と同じ場所
...
10. 🔴 **孤児のインスタンス。** Spot の提案からオンデマンドへ移った（決定 4 (b)）あと、Spot の provider が
    **途中まで進めていた起動**は取り消されるのか、それとも遅れてインスタンスが来て、載せるタスクの無いまま
    `scaleInAfter` まで課金されるのか（0071 実測: GPU の退場は 427〜463 秒）。MI の provider は
    「タスクが置けない」を見てインスタンスを買いに行くので、移った直後に前の provider のインスタンスが到着する形は
...
| 3 | desired 0 → 1 で Spot の提案が選ばれ、インスタンスが **`InstanceLifecycle: spot`** で来る | `ecs describe-container-instances` の `ec2InstanceId` → `describe-instances --instance-ids` |
| 4 | 規則 2 のフォールバック | Spot の提案に**わざと買えない型**（在庫の無い型か、綴りの違う型）を書いて、予算後にオンデマンドのインスタンスが来ること。**これが P0 の陽性対照である。** 移った**あと**も旧 provider の `describe-container-instances` を数分見て、孤児のインスタンス（未解決 10）が来るかを記録する |
| 5 | `box()` が**どちらの provider のインスタンスも**自分のものと認める | `GET /api/admin/engines` の `box` が `null` にならない（0074 の改名のときはここが `null` になった） |
...
1. desired 1・running 0・インスタンスなしが「中断」として扱われ、**cooldown を倍にしない**ことがテストで
   言える（決定 6）。
...
- **本命は AWS FIS の Spot 中断注入**（`aws:ec2:send-spot-instance-interruptions`）。
  ...
  🔴 **未検証**: MI のインスタンスを FIS のターゲットにできるかは測っていない。`describe-instances` は
  MI を**列挙できない**（ID なら返る）ので、**タグやフィルタでターゲットを解決する FIS が
  このインスタンスを見つけられるかは分からない**。ID で直接指定できるなら通る見込みだが、それも
...
- **代替の陽性対照（FIS が使えなかったとき）**: `ec2 terminate-instances` を **ID 指定で**叩く。
  ...
  中断そのものではないが、**「需要があるのにインスタンスが消えた」は同じ形**であり、決定 6 の検出は
  それで測れる。⚠️ 予告つきの経路（`stoppedReason` に何が入るか）はこれでは測れない
  ——未解決 2 はその場合 P2 へ送る。
- **中断の実費を 1 回ぶん実測する**: 中断 → 新しいインスタンス → モデルの取り直し → warm までの秒数。
...
- **R2. 「中断を指す語彙が無い」は誤り。** `engine_control.go` の `noteReplacement` が「desired 1 の
  ...
  この遷移に「インスタンスも消えた」を足す形に直した。あわせて、ECS は CP が何もしなくても同じ provider で
...
- **R7. service の strategy は CloudFormation のものである。** テンプレートが宣言している以上、
  ...
  デプロイを起こす——走っている Spot のインスタンスはオンデマンドで置き換わり、CP は関与しない。起草の
...
- **R8. 孤児のインスタンス。** Spot からオンデマンドへ移ったあと、Spot の provider が途中まで進めていた起動が
...
| 6 | `noteReplacement` の遷移＋インスタンスの消失／ECS の置き直しとの関係／立て直し失敗は数える | R2 |
...
| 未解決 | 1 を書き直し／10（孤児のインスタンス）を追加 | R1・R8 |
...
| 18:44:05 / 18:45:58 | `list-container-instances` → `describe-container-instances` | 4 台。**どれも `capacityProviderName` が無く**、登録は 09-06・09-07・当日 10:30——どちらの provider のインスタンスでもない。**新しいインスタンスは 1 台も来ていない** |
...
- **(b) 緑。service は無傷。** 拒まれた呼び出しは 1 欄も動かさなかった（`updatedAt` が控えのまま）。
  ...
  `desiredCount` は最初から最後まで 0、`runningCount`/`pendingCount` も 0、インスタンスは 1 台も来なかった。
...
2. 🔴 **決定 4 (a) は「`forceNewDeployment` を渡さない」と書いているが、strategy が今と違う値に
   ...
   呼び出しはインスタンスを買うので、この $0 の試験では測れない**（実機 3 で desired 0 → 1 を測るときに
...
どのコードでもない（🔴2）、起動のたびに**インスタンスを 2 台買う**（🔴3）。3 点とも本文の決定は変えず、
...
| 3 | 🟢（2 回目で） | `mode: on` の **26 秒後**に Spot のインスタンス（`g6e.xlarge`・`InstanceLifecycle: spot`・1a）、**5 分 30 秒**で `state: running` / `warm: true` / `offer: {id: spot3, buy: spot}`。1 回目は既定の予算 180 秒に切られて失敗（🔴1）。⚠️ **画像は 1 枚も生成していない**——外から engine ゲートウェイを叩く口が無く（トークンは Workspace 内部の `/internal/engine/token`）、他人のセッションを駆動するのは採らなかった。Spot のインスタンスでの生成は 0074 が実測済み（403 秒で 1 枚） |
| 4 | 🟢（予算経由） | Spot を**買えない状態**にして（下の「陽性対照の作り方」）起動 → `l4`(od) へ移り、OD のインスタンス（`g6.xlarge`）が **28 秒**で来て `warm: true`。⚠️ 移った理由は失敗コードの認識ではなく**予算切れ**（🔴2）。移った**あと**に旧 provider のインスタンスが来る形（未解決 10）は観測されず（そもそも買えていない） |
| 5 | 🟢 | 3 でも 4 でも `GET /api/admin/engines` の `box` は `null` にならない（Spot: `i-0fb3e7…` `g6e.xlarge`／OD: `i-0a4cdf…` `g6.xlarge`）。**両方の provider のインスタンスを自分のものと認めている** |
...
### 🔴 1. 予算は「インスタンスが来ない」ではなく「まだ RUNNING でない」で切れる——既定 180 秒では Spot は必ず失敗する
...
**直し方（決定 5 の本文は変えない）**: 予算の終わりは「**その提案の provider にインスタンスが着いたか**」で判定する
——`describe-container-instances` にその provider のインスタンスが現れた／タスクがそのインスタンスに載った時点で予算を止め、
あとは `StartDeadlineSec` に任せる。いまの実装は `engineOfferVerdict(events, waited, budget)` が
`waited >= budget` だけを見ており、インスタンスの有無を一度も読んでいない。
...
ECS は `desiredCount` を**先に**当てて古い strategy でタスクを置きにいき、その provider がインスタンスを買う。
force で作られた新しい PRIMARY がそれを置き直すが、**買われたインスタンスは残る**。2 回目の余計なインスタンスは
...
provider は**前の要求のまま**＝宣言と実際がずれたまま Spot のインスタンスが来た。⚠️ **提案の型を書き換えても、
...
- 未解決 10（孤児のインスタンス）は**この形では出なかった**。出たのは 🔴3 の「起動時に 2 台」で、旧 provider のインスタンスは
...
| 4 | `g6e.xlarge` | Spot（実機 3 が warm に達したインスタンス） | 9m6s | $0.21 |
...
（未解決 5）。**そのうち $0.74（55%）は 🔴3 の余計なインスタンス**である。

### 後始末

- `mode: off`。**インスタンスが消えるまで見届けた**——`list-container-instances` に capacity provider を持つインスタンスは 0、
...
- **決定 5 — 予算の終わりは「その提案の provider に ACTIVE なインスタンスが現れたら」。** インスタンスが来た時点で
  予算の時計を止め、以降は `StartDeadlineSec` に任せる。⚠️ 併せて**インスタンスの照合は provider を名指しで
  引く**（`boxOn()`）——起動の前後には退場中のインスタンスと新しいインスタンスが **2 台 registered している時間帯**が
  あり、名前無しの `box()` は先に列挙された退場中のインスタンスを返しうる。`box()` は ACTIVE を優先する。
- **決定 4 (a) — 起動は 2 回の `UpdateService`。** strategy が変わるときは「strategy +
  ...
  ECS が古い strategy で先に置いてインスタンスを 2 台買う（実機 3 で 2 回再現）。確認できないうちは desired を
...
——予算はインスタンスの到着で止まるようになり（🟢）、Spot のクォータ語も 25 秒で読めるようになった（🟢）。
しかし **起動はいまもインスタンスを 2 台買い**（🔴、原因は前回の診断より一段深い）、**前の提案のイベントで
...
| (b) **インスタンスは 1 台か** | 🔴 **2 台**。前回と同じ | OD `i-0a46e6…` 23:15:36 登録 / Spot `i-05f9f2…` 23:15:47 登録。止めを刺したのは `describe-tasks`: 最初のタスクは **`startedBy: ecs-svc/9980…`＝古い deployment**。つまり **新しい PRIMARY ができても、古い deployment が `ACTIVE` のあいだに `desiredCount: 1` を入れると ECS は古い側にも 1 本置き、その provider がインスタンスを買う** |
...
| (e) `box` が ACTIVE 側を指すか | 🟢 | OD のインスタンスが `DEREGISTERING` のあいだも `box` は `i-05f9f2…`（`g6.xlarge`・`InstanceLifecycle: spot`・`status: ACTIVE`）。`null` になった瞬間は無い |
...
| (c) 孤児のインスタンスが Spot 側に来ないか | 🟢 | 数分見て、capacity provider を持つ container instance は 0。Spot は 1 台も買えていない |
| (d) インスタンスは 1 台だけか | 🟢 | **0 台**（買えないので当然）。この再走は **$0** |
...
| インスタンス | 型 | 購入形態 | 稼働 | 概算 |
|---|---|---|---|---|
| 1 | `g6.xlarge` | Spot（再走 3 が warm に達したインスタンス） | 7m34s | $0.07 |
| 2 | `g6.xlarge` | OD（🔴(b) の余計な 1 台） | 13m8s | $0.28 |

**のべ 21 分・約 $0.35**（＋MI 管理料 7.80%＝**約 $0.38**）。⚠️ **余計なインスタンスが支出の 79%** で、比率は
...
- **決定 4 (a) — 起動 2 本目の門は「新しい PRIMARY ができた」ではなく `settled()`**（deployments が
  ...
  ECS はそちらにも 1 本置き、旧 provider がインスタンスを買う（再走で再現）。**対価は起動が 2 分 35 秒以上
...
| 決定 4「strategy は running 0 のときだけ」 | **消える**（strategy が無い） | running 1 以上でインスタンスを替えない（0077 決定 5 の退場待ち） |
| 決定 5「予算と失敗コード」 | 予算は**登録待ちの上限**へ縮み、失敗コードは `CreateFleet` の応答へ移る | 一周したら cooldown／監査 1 行 |
| 決定 6「中断」 | 検出に `describe-instances` を使える（インスタンスを列挙できる） | 中断は失敗に数えない／2 回続けば飛ばす |
| 決定 11「パネルは service の strategy から言う」 | **インスタンスのタグから言う**（`af-engine-offer` / `af-engine-buy`） | EC2 の価格には訊かない（`describe-instances` は読む） |
