...
- 状態: **P0・P1・P4 実装済み・実機検証済み（2026-09-08〜09）。P5 のうち HF トークンの
  ...
  **P6（seed と 6 パラメータの撤去）は 2026-09-10 に実装済み・同日に実機検証済み**
  ...
  状態の更新（2026-09-25）: 上の「P3 … は未着手」は古い——P3 は 2026-09-11 に実装し、実機で閉じた（image 側は「H3 — P2 の残りと P3 を実機で閉じた」、llm 側は「P3 の llm 側の実装——preset の固定 LoRA」と「#513 と #518 の llm 側を実機で閉じた」）。決定 5 の後半、llm の仮想モデル id（P5 に列挙）は予定しない: ゲートウェイに要求本文を書き換えさせることになるため。preset に固定した LoRA では足りず、llm 役で要求ごとの LoRA の組を必要とする人が出たときだけ再検討する。
- Follow-ups: #964
...
- 同日、**開発配備の GPU（g6.xlarge・L4）で ComfyUI を実測した**（「実測で解けた点」節。
  ...
  同じインスタンスで描け、温まった 1 枚は 4〜11 秒、**切り替えは EBS からの読み直しで 1〜2.5 分**。
...
- 同日、**P0 を実装し実機で検証した**（「P0 の実測」節）。カタログの表・種・active set の SSM・
  ...
  空回りさせ**（実測 2）、**RUNNING のまま温まらないインスタンスを誰も止めない穴**が出て（同 4）、
...
- 同日、**P1（llm 役のルーターモード）を実装し、CPU と実機で測った**（「P1 の実測」節）。
  ...
  **3 つとも実機で通った**（同じインスタンスで 2 つの GGUF が使え、交替のリロードが 1 回の試行で答えを
  ...
  **warm は「既定モデルが `loaded`」ではなく「どれか 1 つが `loaded`」**（交替中のインスタンスが
...
- 2026-09-12、**パネルを「運用」と「モデル」の 2 画面に割り、上流の制限・ファミリーの推定・
  推奨パラメータを取り込んだ**（末尾「追記 — パネルを 2 画面に割り…」節）。決定 2・4・10・11
  の広げ方で、決定の差し替えではない。🔴 Civitai の「要ログイン」はメタデータからは分からず、
  上位 20 件のうち **13 件**が匿名 HEAD に 401 を返す（残りは 307）——予測できるフィールドは
  一つも無いので、検索の 20 件に対しても HEAD を撃つ。カタログ行に `params` を足し、comfy の
  5 テンプレートがフィールド単位で被せる。**実機（GPU）には当てていない。**

## 背景

ADR 0071 は「フリートのインスタンスで推論する」土台を P0・P1 で出荷した。llm 役は Qwen3-Coder-30B-A3B、
...
- **モデルは CloudFormation のパラメータである。** `60-engines.yaml` の `LlmModelS3Key` /
  ...
  image 役の対応物 `ImageModelS3Key` / `ImageModelFile` / `ImageModelIds` / `ImageExtraArgs`。
...
1. **モデルはスタックの資源ではなく、データである。** スタックが持つのは役の**器**——capacity
   ...
   - **二段階スタンドアップ（0071 P0 の実測）が消える。** 今は「モデルがバケットに無いと
     ...
     **コントローラは `mode=on` でもカタログが空なら起こさない**（`engineSnapshot.hasModels`、
     ...
     `running && !warmed` を 5 秒間隔で永遠に見に行くので（R6）、`sleep infinity` のインスタンスを
     `mode=on` の配備が $1.26/時で買い続ける穴がある。パネルはトグルの代わりに「有効なモデルが
     無い」を出す。消えるのは二段階であって、**安定化のために GPU インスタンスを 1 回買う 10 分は残る**
...
2. **カタログの真実は 2 層。S3 が「何があるか」、CP の DB が「どう出すか」。エンジンには聞かない。**
   ...
   - **「いまインスタンスに載せるもの」（active set）は CP が SSM に書く**: `/af-ws/engines/<key>/active`
     に、有効なモデルと LoRA の S3 キー・生成する preset の材料を JSON で。CP のタスクロールは
     **既に** `/af-ws/*` への `ssm:PutParameter` を持つ（`20-platform` の
     `SsmWorkspaceParams`）ので CP 側の IAM 追加は無く、インスタンス側は `EngineTaskRole` に
     ...
     ポリシー）と比べ、**既に持っている権限**で済み（R2(a) で確認）、値が小さく、そしてインスタンスが読む
...
3. **llm 役はルーターモードで動かし、モデルは要求の `model` で選ぶ。窓はモデル毎になる。**
   ...
   - **`LlmModelsMax` の既定は 1。** ルーターは VRAM を知らない。L4（22.9 GB）に 30B Q4
     ...
     **この設計の価格**であり、隠さず admin パネルに「モデルの交替が N 回」を出す。小さい
     モデルを複数載せる配備は運用者が 2 以上にする（カタログの `vramMiB` の和がインスタンスに入るかは
   ...
   - **`warm` の定義が変わる。** 今は `/health` ok＝重みが VRAM にある。ルーターでは
     ...
     preset の `load-on-startup = true` にする。起動時に載せないと、最初の要求が「インスタンスの起動
     527 秒＋ロード 267 秒」を払う。
     - 🔴 **「既定モデルが `loaded`」ではない**（P1 の実測 4）。`--models-max 1` で別の
       モデルに交替したインスタンスは既定を降ろしているので、既定に縛ると「答えているのに warm で
       ない」になり、`running && !warmed` が 900 秒続けば P0 で足した `unwarmed` 規則が
       **会話の最中にインスタンスを止める**。`sleeping` と `loading` は warm ではない——どちらも
       次の要求が重みを払う状態である。
...
4. **image 役は 1 チェックポイントを保持し、差し替えは「次の起動」で効く。同時に 1 つ。**
   ...
   - **停止中**（普段の状態——$1.26/時のインスタンスは寝ている）に変えれば、次の要求が新しい
...
5. **LoRA はカタログの項目であり、要求で選ぶ。エージェントが選べるのはカタログにある名前だけ。**
   - **image。** インスタンスは `image/loras/` のうち**有効なもの**を同期し、`--lora-model-dir
     ...
     - `baseModel` が合わない LoRA（SD1.5 の LoRA を SDXL に）は sd.cpp が**黙って崩れた絵**を
       ...
       もう 1 段の守りは**インスタンス**にある: 有効な LoRA しか同期しないので、enum の外の名前は
...
6. **取り込みは CP の仕事になり、Console から起動する。手打ちの `run-task` は残るが主経路ではない。**
   ...
   - **HF API から `sha256` と `license` を解決**（`?blobs=true` の `siblings[].lfs.sha256`、
     ...
     - 🔴 **gated でもメタデータは匿名で読める**（P4 の実測 1）。`gated: auto` のリポジトリでも
       ...
       「トークンはインスタンスに載せない」が、CP にも当てはまる形で成立する。
   ...
   - 🔴 **CP はマニフェストを読まない——読めない**（R3: CP のタスクロールに S3 のアクションは
     ...
     照合は `fetch` が済ませているのでそれで足りる。マニフェストは**インスタンスが読むもの**である。
...
8. **由来と使用量はモデルと LoRA を持つ。** `generate_image` の結果の `model` はチェック
   ...
   返す名前（＝カタログ id）で、`engine_hourly`（0071 決定 13）には触らない——稼働はインスタンスの
   性質で、モデルの性質ではない。
...
（19〜24 秒）。テキストエンコーダは fp8（`qwen_3_4b_fp8_mixed`・5.6 GB）。数字は同じインスタンスで
...
5. 🔴 **切り替えは毎回 EBS からの読み直しで、1〜2.5 分。** 初回と「戻ってきた」回が同じ:
   SDXL 56〜63 秒、klein 106〜110 秒、Z-Image 133〜154 秒。インスタンスの RAM は 15 GB で 3 モデル
...
7. **コールドスタートの内訳**（RunTask から）: インスタンス +33 秒で pull 開始、ECR から 5.4 GB
...
10. **インスタンスの事実**: g6.xlarge は ECS に **15,000 MiB** を登録する。**同じインスタンスに 2 本目のタスクを
    流すと `No space left`**（匿名 host volume は前のタスクの分が片づかない——0071 決定 7 の
    注のとおり）。停止直後のインスタンスに載ったタスクは pull 開始まで **9.5 分 PENDING** だった（新しい
    インスタンスなら 33 秒）。MI の回収を待つほうが早い。
...
1. **CP はカタログを種から作り、active set を publish した**（決定 2・7）。起動時のログが
   ...
   **4,096 文字（R2(c)）に対して 132〜152 文字。** テストで固定した「モデル 20・LoRA 20」は
   ...
   「S3 キーとフラグだけ」を守っても足りず、(a) LoRA は素の S3 キーだけにし（インスタンスはディレクトリを
...
2. 🔴 **サイドカーが空回りした。YAML の折り畳みブロック（`>-`）は、より深くインデントされた行を
   ...
   **サービスは steady state に達し、どこにも理由が出ない**。GPU 1 台と 10 分を払って
   ...
   シェルは、型検査もリンタも無く、唯一の反応が 10 分後の GPU インスタンスだという場所にある。
...
8. 🔴 **G 系の vCPU クォータ 8 に当たった。** 止めた直後に起こし直すと、退場中のインスタンスが 4 vCPU を
...
4. 🔴 **空のルーターでも `/health` は `{"status":"ok"}`**（R1(a) の再確認）。加えて
   ...
   交替したインスタンスは「答えているのに warm でない」になり、`running && !warmed` が 900 秒続けば
   P0 で足した `unwarmed` 規則が**会話の最中にインスタンスを止める**。
...
1. 🔴 **gated リポジトリのメタデータは匿名で読める。** FLUX.1-dev と SD3.5 Medium で確認:
   ...
   **29 ファイル分の sha256 とサイズ**を鍵無しで返し、**401 になるのは
   ...
   そのまま成立する（決定 6 の「トークンはインスタンスに載せない」を CP にも適用できた）。
...
4. **gated を本当に取り込んだ（FLUX.1-dev・23,802,932,552 バイト）。ボタンから行まで
   ...
   とも exit 0。
5. **取り込みタスクは Fargate（2 vCPU / 4 GB）で走り、GPU のインスタンスは 1 台も起きない。**
   `launchType: FARGATE`、capacity provider は無し。23.8 GB の取り込みに $1.26/時のインスタンスを
...
> ✅ **修正済み（2026-09-10）**: 区別を読む人に任せるのをやめた。ジョブ行が `code` を名乗る
> ——403 は新設の `gated_not_accepted`、401 は既存の `gated_no_token`、Civitai 由来のものは
> `civitai_login_required`——`engineIngestFailureCode` がタスクの文面（curl の `error: 403`、
> verbose の `HTTP/1.1 403`）から読む。Console はログの行を残したまま、その隣に行き先の違う
> 文を ja / en で出す（403 は HF のモデルページ、401 はトークン欄）。`resolve` 側は**コードを
> 出せない**——CP は匿名で調べるので、登録済みトークンのアカウントがそのリポジトリに同意して
> いるかを知る術が無い——ので、gated ＋トークン有りのときに `gated_needs_acceptance` の注意を
> 先に出すところまでにした。試験は 401 と 403 の両方（片方だけだと分岐の無い実装でも緑になる）。
...
3. `60-engines.yaml` に `ImageEngine`（`sdcpp`/`comfy`、既定 `sdcpp`）。image ロールのコンテナ
   ...
   - fetch サイドカーは**無改造で足りた**——PRESET_FILE が空のときの経路は元々 role を
     ...
     preset を持たないルータで、その等式を破る。** start 以外のモデルはインスタンスに降りて来ず、
...
> ✅ **修正済み（2026-09-10）**: `resolve` が CivitAI の download URL に **`HEAD` を 1 本**投げ、
> 401 / 403 なら `login_required` を立てる（`engineCivitaiAnonymous`）。パネルは `can_ingest`
> を落とし、取り込みは `civitai_login_required` で**タスクを起こす前に**断る。HF の
> `gated_no_token` と**別のコード**にしたのは、鍵が違うからではなく**無いから**である——
> gated はリポジトリの条項でトークンが満たすが、これは投稿者ごとの設定で、この配備には
> CivitAI のアカウントが無い。だから Console の文面も「トークンを登録してください」ではなく
> 「別の資産を選ぶか、手で置いて登録する」と言う。読めない向きには**開いて倒れる**——
> HEAD を嫌う CDN の 405 も、投げられなかった probe も、ログインの壁ではない（401 と 403
> だけを壁と読む）。試験は 401・403・405 と、**壁の無い資産が通ること**（陽性対照）。
>
> **CivitAI 用トークン欄は作らない（範囲外）。** 理由は 3 つ: この配備に CivitAI の
> アカウントが無く、作るなら「誰の名義で・誰が条項を引き受けるか」が決定 10 の acceptance と
> 同じ重さで要る。値の置き場は HF トークンと同じ秘密の仕組みを 2 本目として増やす話になり、
> 60-engines の壁（残り約 200 バイト）に触れる。そして踏んだのは 5 資産中 2 件で、
> **「別の資産を選ぶ」で回避できる**——回避できないと分かってから作るほうが安い。
...
> ✅ **修正済み（2026-09-10）**: 取り込み要求に `file_flag` と `attach` が付いた。前者は行の
> `file_flags`（Console に配っている正本）で検証し、後者は「既にある行の部品として足す」
> ——分割モデルを取り込みだけで組める唯一の形——を、id 重複の 409 を通す**唯一の**経路として
> 開ける（`engineAttachAllowed`: 行が無い・Flag が無い・その役割が埋まっている、の 3 つは
> ダウンロード**前**に断る）。足し込みは `AppendEngineModelFile` の 1 トランザクションで、
> 行のライセンス・ファミリー・有効状態には触れない（アップサートに戻すと、この節が
> 「既にある id を断る」理由をそのまま壊す）。Console の取り込みフォームは役割の選択欄を持ち、
> **S3 のディレクトリを役割から決める**（`engineIngestPrefix`）——`image/checkpoints/` に
> 置いた text encoder はどのローダの一覧にも出ないので、これは飾りではない。
...
> ✅ **修正済み（2026-09-10）**: `deleteModel` は行を消す**前**に**全役のカタログ**を読み、
> 他の行が指しているキーを MODE=delete に渡さなくなった（`engineKeysStillUsed`）。残した
> ファイルは「どの行が使っているか」と一緒に応答の `purge` に載る——名前の無い「残した」は
> 運用者が動けない。カタログの読みに**失敗した**ときは 1 バイトも消さない（共有の有無が
> 不明なまま消すのがこの節の事故そのもの）。全役を読むのは、同じキーを別の役の行が指せない
> と決めた覚えが無いため。共有は決定 2 が意図したもので（`text_encoders/` は SD3.5 と
> FLUX.1 で同じ T5-XXL・CLIP-L を読む）、欠落 6 が直っても消えない。試験は両方向——共有
> キーが残ることと、**最後の参照が消えたキーは実際に消えること**（陽性対照）。
...
🔴 **欠落 7——エンジンは「まだインスタンスに無いモデル」への要求を受け付けてしまう。** fetch
...
**運用者にも呼び出し側にも「まだ降りて来ていない」と分かる手掛かりが無い。** 2 台目のインスタンスでは
12 ファイル・48 GB を約 270 秒（≒180 MB/s）かけて同期しており、その 270 秒はまるごと
この窓である。要求されたモデルの files がインスタンスに揃っているかは CP が知っている事実なので、
`engine_waking` 相当で待たせるのが素直な直し方に見える。

> ✅ **修正済み（2026-09-11・実装済み・実機未検証）——欠落 7 と未解決 3 を一体で塞いだ。**
> サイドカーが**まだ降ろしていない S3 キー**を `<base>/<役>/pending` に書く（裸のキーの JSON
> 配列・Standard tier の 4,096 文字・1 ファイル降りるたびに縮み、揃ったら `[]`）。ゲートウェイは
> 要求されたモデルの `files[]` がそこに載っていれば `engine_waking` と Retry-After で返す
> ——**enum の条件は「有効」のまま変えない**。「インスタンスに在るか」は 1 台の 1 瞬間の性質で、
> カタログを読み終えたセッションの下で変わる。ダウンロードに合わせて点滅するメニューより、
> 待てる要求のほうがよい。🔴 **パラメータが無い・読めないは「同期中」ではなく「不明」**で、
> 素通しする——古いサイドカーの配備も SSM が落ちている配備も、この検査が無かった頃と同じに
> 振る舞う。未解決 3 のほうは**サイドカーを常駐させた**（`WATCH_SEC`、既定 60 秒）: active set
> が変わったら差分を降ろし、その間は同じ pending に載るのでゲートウェイの待ちがそのまま効く。
> ⚠️ ただし **llm 役は preset を起動時に読む**ので、後から有効にしたモデルはファイルが届いても
> 次の起動までルータに現れない——この役で買えるのは「次の起動が速い」だけである。image 役
> （ComfyUI は要求のたびにディレクトリを列挙する）ではファイルが届くことがそのまま解になる。
> 🔴 **実機未検証**: この配備の既定の形ではインスタンスの取得が同期より長く、窓がそもそも開かない
> （PR #507 の観測）。窓を作って確かめる（大きいモデルを 2 本目に有効化し、起動直後に要求する）
> のは次の実機セッションの宿題である。

**欠落 8——稼働中のインスタンスにモデルを足しても同期されない。これは新発見ではない**——未解決 3
（「サービス起動後の追加同期」）がそのまま出たもので、P5 に送られている既知の穴である。
ただし image 役で実際に踏むと何が起きるかは初めて測った: `flux1-dev-fp8` と `sd35-medium` を
有効化してもファイルは降りて来ず、`mode` を `off` → `on` して**インスタンスを作り直す**しかなかった。
llm 役では「次の起動まで待つ」で済むが、image 役では**有効化したモデルがインスタンスに無いまま
`generate_image` の `model` enum に出る**ので、欠落 7 と重なって「選べるのに 400 が返る」に
なる。P5 でこれを解くときは、enum に出す条件を「有効」ではなく「インスタンスにある」に寄せるか、
欠落 7 側で待たせるかのどちらかが要る。

🔴 **欠落 9——`/history` の 503 はリトライされない。** provider は `/prompt` の 503
（`engine_waking`）を待って再送するが、その後のポーリングは待たない。インスタンスがポーリング中に
入れ替わると、呼び出し側に届くのは
...
**運用上の注意（今回自分で踏んだ）**: `mode=on` でインスタンスを起こしてから `ondemand` に戻すと、
`last_demand` が古いままなので**コントローラが即座にそのインスタンスを止める**。48 GB 同期しなおしに
なるので、インスタンスを温めたいなら `ondemand` のまま要求で起こすのが正しい。
...
> ✅ **修正済み（2026-09-10）**: 検査は**両方の場所**に置いた。**有効化する場所**は拒否する
> ——`engineFilesGuard` が 409 `engine_files_missing` を返し、足りない Flag を名指しする。
> VRAM の門と違って `confirm` は無い: これは不確実な賭けではなく、`comfyBuildGraph` が HTTP を
> 叩く前に断る事実そのもので、有効にすれば `generate_image` の enum に「必ず失敗する id」が
> 並ぶだけである。**宣言する場所**は拒否しない——壊れた行を直す唯一の操作を断ると、行は
> 壊れたまま直せなくなる——代わりに `files_missing` を行に載せ、Console が不足を名指しする。
> 要求ファイルの正本は変わらず `comfy_workflows.go` のテンプレート内のガードで、CP 側の写しは
> `engine_catalog_test.go` が**そのガードを読んで**（`f.X == ""` と `resolveComfyFiles` の
> switch から組み立てて）一致を検証する。5 ファミリー分が読めなければ Fatal。
...
- **モデルごとに `60-engines` の行（サービス一式）を足す。** 壁で入らず、入っても寝ている
  サービスが役の数だけ並び、2 モデル同時に起きればインスタンスも 2 台。llm はルーターが、image は
  「次の起動」が、同じインスタンスで答える。
...
- **インスタンスが HF / Civitai から直接引く。** 0071 決定 3 のまま（速度が 4〜236 MB/s で読めず、
  トークンがインスタンスに載る）。
- **ゲートウェイでカタログを SSM から読む。** 要求ごとの SSM 呼び出し（`engines.go` が起動時
  1 回にした理由）。SSM に書くのは**インスタンスが読む active set** だけで、CP 自身は DB を読む。
...
3. ~~**サービス起動後の追加同期。**~~ **半分解けた（2026-09-11・実装済み・実機未検証）**:
   サイドカーを常駐させた（`WATCH_SEC`、既定 60 秒）。active set が変わったら差分を降ろし、
   降ろし中は `pending` に載るのでゲートウェイが `engine_waking` で待たせる（欠落 7 の修正と
   一体）。⚠️ **ルーターの側は解けていない**——`--models-dir` は付けない設計（決定 3）で、
   preset は起動時に読むので、後から有効にしたモデルは**ファイルが届いてもルータには現れない**。
   llm 役で買えるのは「次の起動が速い」だけで、モデルが使えるようになるのは次の起動である。
   image 役（ComfyUI は要求のたびに列挙する）はこれで解ける。元の文: 有効化したモデルを、
   走っているインスタンスに**次の起動を待たず**足せるか——サイドカーを常駐させて active set を
   再同期し、ルーターが `--models-dir` を再走査するか（しないなら `/models` の再読込の口が
   あるか）。無ければ「llm も次の起動」で始め、これは P4。
...
7. ~~**既定モデルの実測**（決定 10）~~ **解けた**（実測で解けた点 3〜5）。残るのは gated の
   ...
   **本体を入れて描かせたわけではない**（23.8 GB の `flux1-dev.safetensors` は L4 に載らない）。
   この 2 ファミリーの生成そのものは、ungated のミラーから入れたファイルで「P2 の残作業 4・5 を実機で
   押した」節が済ませている。
...
9. ~~**切り替えの読み直し（実測で解けた点 5）を縮められるか——P2 の前に 1 GPU 時間で 2 案を測る**~~
   **(1) は解けた（レビュー 2026-09-09 の 1）——`useLocalStorage` で交替は 276〜282 秒 →
   ...
   残らないのはインスタンスの RAM が 15 GB だからで、`ImageMemMinMiB` を 30,000 に上げれば g6.2xlarge
...
10. ~~**モデルの置き場そのものを問い直す**~~ **解けた（レビュー 2026-09-09 の 1〜3）**:
    **(a) 採用**（`useLocalStorage`——コールドスタート 527〜586 秒 → 275 秒、交替
    276〜282 秒 → 98.5 秒、既定を `true` に）、**(b) 否定**（温かいインスタンスは MI では作れない。
    ...
    **S3 は律速ではない**——0071 の未解決 1 が既に「g6.xlarge の EBS ベースライン 125 MB/s は
    ...
    - **(b) 温かいインスタンス。** 0071 決定 7(c) は**未証明のまま**で、失敗が記録されている
...
11. ~~**テナント軸をどこに入れるか**~~ **決まった（レビュー 2026-09-09 の 4）——中間案を採る。**
    ...
    **共有のインスタンスの上では 4 つが掛け算になる**: active set はエンジンに 1 つなので全テナントの
    ...
    インスタンスをテナント毎にすれば全部消えるが $1.26/時 × テナント数で、0071 が「1 インスタンスを共有して
    ...
    **「取り込める人」と「誰が同意したか」をテナント軸にする**。掛け算は起きず、使い勝手の
    ...
    - **テナント毎の S3 バケットは薦めない。** バケットは意味のある境界ではない——どの
      バケットから来てもモデルは**同じインスタンスの同じディスクに載り、同じプロセスが読む**。境界は
      GPU のインスタンスであってバケットではない。加えて共有モデルがテナント数だけ複製され、いま
...
- **P1 — llm のルーターモード。実装済み・実機検証済み（「P1 の実測」）。** preset 生成
  ...
  完了の定義と同じ観測を交替で行う）。**3 つとも実機で通った**（同じインスタンスで 2 つの GGUF が
...
- **P2 — ComfyUI（0071 P2 をここへ前倒し）。実装済み・実機検証済み（2026-09-10・
  ...
  `generate_image` に要るのは API だけで、画面は P5。**完了の定義: 同じインスタンスで
...
- **P5 — 走行中の追加同期（未解決 3）、llm の仮想モデル id（決定 5 の後半）、ComfyUI の
  ...
  **走行中の追加同期（未解決 3）は実装済み・実機未検証**（2026-09-11。サイドカーを常駐させ、
  未同期のキーを `pending` に出し、ゲートウェイが `engine_waking` で待たせる。欠落 7 と一体で
  塞いだ——その節の補遺に設計と、llm 役では preset のせいで半分しか解けないことを書いた）。
  **完了の定義: 大きいモデルを 2 本目に有効化し、起動直後に要求すると、400 ではなく
  `engine_waking` が返り、同期が終わると同じ要求が通る。** これは窓を意図的に作らないと
  測れない（この配備の既定の形ではインスタンスの取得のほうが長い。PR #507 の観測）。
...
- **R6. コントローラの現物（`engine_control.go`）。** `decideEngineAction` は `mode=on` で
  ...
  **`mode=on` の配備では管理者が「モデル無し」のインスタンスを $1.26/時で買い続け、パネルは永遠に
  「準備中」**を出す。
...
- **決定 1** — 5 点。(a) **P1 を後回しにした以上、P0 の llm 役は `-m` のままで「空で listen」
  ...
  **今日と同じく安定化のために GPU インスタンスを 1 回買う**（プレースホルダも `GPU` の
  `ResourceRequirements` を持つ）——消えるのは二段階で、初回の 10 分と $0.2 ではない。本文に
  書く。(e) 以上の条件で、二段階は本当に消える。
- **決定 2** — (a) IAM の前提は正しく（R2(a)）、インスタンス側の `GetParameter` を `60-engines` の
...
- **決定 5** — 🔴 **「CP が要求で拒否する」は、決定 4 の「ゲートウェイは本文を読まない」と
  ...
  段階で断る）と**インスタンス**（有効な LoRA しか同期しないので、無い名前はエンジンが失敗させる。
...
- **決定 6** — (a) `ecs:RunTask` は本当に無い（R3）ので必要。`iam:PassRole` は
  **`IngestTaskRole` だけ**——`ExecRole` の PassRole は `PassTaskRoles` に既にある。本文の
  ...
  足りる（sha256 の照合は `fetch` が済ませている）。マニフェストはインスタンスが読むもので、CP は
...
- **未解決 9** — **測る価値はある。1 GPU 時間で 2 つ動く。** (1) `useLocalStorage`: R9 の
  ...
  **コールドスタートの S3 → EBS 33 GB・332〜360 秒**もで、これは EBS の書き込み上限
  ...
  並べて測る: 3 モデル 27 GB がページキャッシュに残らないのはインスタンスの RAM が 15 GB だから
...
1. **決定 2 の SSM は閉じる**（R2(a)）——CP 側は追加ゼロ、インスタンス側は `60-engines` の中の
...
2. **安定はする。ただし 3 条件つき**（決定 1 の改訂 (a)(b)(c)）: 両役ともラッパー、
   ...
   安定する。二段階は消えるが、安定化のための GPU インスタンス 1 回は残る。
...
4. **拒否の場所が違う**（決定 5 の改訂）——CP は本文を読まないので Agent とインスタンスで拒む。
...
登録（12）**——を別セッションで詰めた。10 は実機で測り、11 と 12 はインスタンスを起こさずに設計として
...
測ったのは**配備の `llm` 役そのもの**——ベンチ用のインスタンスではなく、決定 3 が値段を付けた当の経路で
...
🔴 **読み取るべきは取得ではなく交替である。** 決定 3 は「1 インスタンス 1 モデル・要求で交替」に
**276〜282 秒**という値段を付け、実測 5 は image 役の切り替えを 1〜2.5 分と書いた。その値段が
**98.5 秒**になった。人が待つのはここで、コールドスタート（275 秒）は次である。

代償は 2 つとも本物だが、どちらも新しくない: インスタンスストアはインスタンスと一緒に消える——が
**EBS のデータボリュームも MI が消す**ので、コールドスタートが毎回 S3 から取るのは前と同じ。
そして `*StorageGiB` が効かなくなり、インスタンスは機種が持つものをそのまま得る——g6.xlarge で
...
### 2. 未解決 10(b)——温かいインスタンスは「未証明」ではなく**否定された**
...
**帰結**: MI の上では、温かいモデルボリュームは**ホストボリュームでは作れない**。インスタンスを保持して
...
- 🔴 **代償を隠さない**: カタログは配備で 1 つなので、**どのテナントからも全モデルの id が
  ...
  誤解した運用が生まれる。
- **テナント毎の S3 バケットは薦めない**（ADR の理由をそのまま維持——境界は GPU のインスタンスであって
...
**実測（同じインスタンス・同じタスク・cli を先に流して page cache が mount に有利にならないようにした）**:
...
なお同じ `aws s3 cp` が実測 1 では 158 MB/s、ここでは 210 MB/s だった。インスタンスと時刻で振れるということで、
...
同じインスタンス・同じタスクで、ローカルへコピーしてからのロードを対照に置いた:
...
`LlmAllowedInstanceTypes` が `g6.xlarge` 1 機種に縮んでいたためインスタンスが建たなかった。テンプレートの
...
- `LlmModelS3Key` / `LlmModelIds` / `LlmContextTokens` / `LlmMaxOutputTokens` と、image 役の
  `ImageModelS3Key` / `ImageModelIds`。**`*ExtraArgs` は残す**——あれはインスタンス側のフラグ
...
空く。P2 の残り（ファミリーごとの image-to-image グラフ）も ADR 0074 の梯子も、同じテンプレートの中に
収めなければならない。
...
~~**残っているもの。**~~ **画面も入れた（2026-09-11・下の段落）。** 以下は当時の記述:
Console の**エンジンパネル自体は super_admin のまま**である
...
**縮小版の画面（2026-09-11・実機未検証）。** `GET /api/admin/engines` を取り込みの門と
**同じ述語**に寄せ、super_admin でない呼び手には**行の部分集合**を返す（`engineTenantAdminRow` が
鍵を名指しで写すので、包含は書き方で保証され、試験が実際の行に対して確かめる）。残すのは
`key` / `api` / `provider` / `base_models` / `file_flags` と、同じく絞った `model_rows`
（id・kind・enabled・説明・ファミリー・ライセンス）だけ。封筒に `super_admin` を載せ、Console は
**その旗だけで**分岐する——「`mode` が来たか」で推測すると、欄の改名でいつか静かに 403 する
ボタンを描く。取り込みジョブの一覧も呼び手のテナントに絞った（`engine_ingest_jobs.tenant_id`。
sqlite `0063` / postgres `0048`）。🔴 **空のテナントは「絞らない」ではなく値である**——
「テナントが空でなければ絞る」と書くと、テナントを解決できなかった呼び手に運用者のジョブが全部
渡る。ジョブの照合（reconcile）だけは全件のまま——見えない行も更新しないと、誰も見ていない間に
終わったタスクが永久に `running` になる。
🔴 **置き場所は管理モーダルではなくテナント設定である。** 管理モーダルの入口は TopBar で
super_admin 専用なので、そこに足しても tenant_admin には扉が無い（実際に描いて分かった）。
描いて分かったことがもう 1 つ: **控えの無くなった説明文が 2 つ残っていた**（「選び直しは次の
起動から効きます」「『無効』にすると…」）。どちらも消えたボタンの説明で、「出ない要素」を
列挙した試験は文章を見ていない。
...
1. **テンプレート。** `LlmModelS3Key` / `LlmModelIds` / `LlmContextTokens` /
   ...
   **50,997 → 49,298 バイト**（1,699 空き、壁まで 1,902）。**空いた分は使っていない**——P2 の
   残り（ファミリーごとの image-to-image グラフ）と ADR 0074 の梯子の取り分である。
...
- ファミリーごとのテンプレート 5 つすべてに `LoraLoader` の連鎖。要求されなければノードは 1 つも
  増えないので、LoRA 無しのグラフはゴールデンと 1 バイトも変わらない。
- baseModel 不一致の拒否を **Agent 側**に置いた（レビュー決定 5）。拒否は 3 種類——名前が
  カタログに無い／ファミリーがチェックポイントと違う／LoRA がファミリーを宣言していない——で、どれも
...
ファミリーを名乗り、合わない組は組み立ての段階で名指しで断る。完了の定義の後半は「enum から消す」
...
このレーンは配備を走らせない約束なので、ここで止めた。**5 ファミリーの edit / inpaint グラフは
...
- `sdxl` と `sdxl` が「ファミリーが違うので組み合わせられない」という、コードが出せない形の
  エラー文を貼った。
- 自動要約（compact）をまたぐと、**明示的に禁止した `loras` 引数を毎回足すようになった**。
  これのせいで欠落 7 の観測は 1 回無駄になっている——足された LoRA がファミリー不一致になり、
...
指定された seed を各ファミリーのサンプラーへそのまま入れる。理由と、なぜ seed だけが provider 中立の
...
- 「同じ prompt・同じ seed で LoRA の有無が絵を変える」——**seed を固定できるようになった。
  実機での確認は未**。次に配備する回に、P2 の残り（5 ファミリーの edit / inpaint）とまとめて H3 として
...
- **土台が無い LoRA。** 土台が無効・不在なら固定せず、パネルの行に `lora_base_missing` を
  出す。有効で・インスタンスにファイルもあって・何もしない行は、効いている行と見分けが付かない。
...
## H3 — P2 の残りと P3 を実機で閉じた（2026-09-11・開発配備）

配備 `0.18.1-dev-b6feea43`（#507 と #509 を含む）で image 役を 1 回起こし、約 30 分
（01:19:18Z mode=on → 01:23:36Z までに running → 01:49:33Z stopped）。**全部緑。**

### P3 の完了の定義は閉じた

`sdxl-base-1.0`・同じ prompt・**同じ seed 4242** で 3 枚:

| | sha256 | バイト |
|---|---|---|
| LoRA 無し | `2d086024d086c76d48800eb830531aac0f020e4524a1cad9a4cb3044b90e0366` | 1,550,948 |
| LoRA 無し（同じ引数で再送） | `2d086024…`（**同一**） | 1,550,948 |
| `pixel-art-xl` weight 1 | `66f7cde8b315f86d18cb56397dce5e3b894351d6a90d27fe370186975a527aed` | 1,509,900 |

つまり **seed を固定すると同じ絵が返り、LoRA を足すと同じ seed のまま絵が変わる。**
「同じ prompt・同じ seed で LoRA の有無が絵を変える」——**満たした**。もう一方の
「SD1.5 の LoRA が SDXL で enum に出ない」は 2026-09-11 の補遺のとおり名指しの拒否で
満たしている。**P3 の完了の定義は両方とも閉じた。**

**キャッシュの警告も実機で出た。** 2 枚目（同一グラフの再送）の warnings に、0069 の追記が
設計したとおりの 1 行が入っていた——`this picture came from the engine's cache, not from a
new generation …`。3 枚目（LoRA 付きでグラフが変わる）には出ていない。ComfyUI の
`execution_cached` を読む判定が、当たったときだけ鳴ることを含めて実機で確認できた。

この 3 枚は互いに矛盾しない形で取れている（同一ハッシュ・同一バイト数・キャッシュ警告が
再送のときだけ・グラフが変われば別ハッシュ）。駆動役が捏造していたらこの組み合わせは出ない。

### P2 の残り（ファミリーごとの image-to-image）も実機で通った——10/10

入力は上の 1 枚（1024×1024）、マスクは中央 512×512 の白い正方形。
...
すべて 1024×1024（入力画像の寸法。設計どおり）、warnings は空。**SD3.5 のときのような
「ゴールデンは緑だがグラフが誤っていた」は起きなかった。** 上流 v0.34.0 の `nodes.py` /
`comfy_extras` と突き合わせて書いた 3 点——`LoadImage` の値は入力ディレクトリの列挙なので
先に `/upload/image` する・マスクは red チャンネル・klein だけ `SplitSigmasDenoise` の
第 2 出力——は、どれか 1 つでも誤っていれば ComfyUI が検証で `Value not in list` か
ノードエラーを返すので、10 回すべて通ったことがそれらの妥当性の証拠になる。

### 🔴 実機でしか出なかった罠——広告されているツールが自分の呼び出しを拒む

H3 の最初の 30 分はこれで潰れた。`generate_image` はクライアントのツール一覧に載っている
のに、呼ぶと
`この対話セッション用サーバーでは許可されていないツールです: generate_image`
が返る（権限の問題に見える文面）。

機構: `mcpx` の `mcpStdioCall` は呼び出し側でも `mcpStdioToolAdvertised()` を**評価し直す**。
それは `mcpStdioToolList()` → `mcpImageGenAdvertise()` → Agent の `/imagegen/status` という
**生きた HTTP 往復（タイムアウト 3 秒）**に落ちる。status は各 provider の `Ready()` を呼び、
フリートのエンジンではそれが `engineToken` → CP への往復（タイムアウト 15 秒）になる。
**CP が重い瞬間に tools/call が来ると 3 秒を超え、載っているはずのツールが「許可されていない」
と答える。** 今回は CP を `--force-new-deployment` で入れ替えた直後がその瞬間だった。

広告の再評価は「推測した名前で未広告のハンドラに届かせない」ための境界で、それ自体は要る。
ただし `generate_image` には**同じ関数の数行下に専用の門が既にある**
（`if !mcpImageGenEnabled && p.Name == mcpToolGenerateImage`）ので、呼び出し側で
ネットワーク越しの再評価まで要るかは設計の判断になる。直し方を決めるのはこの補遺の仕事では
ないので、機構と再現条件だけ書いておく。

### 駆動役の話（前回の補遺の続き）

🔴 **opencode の managed セッションは、この用途には使えない。** 新しく 2 本立てて 2 本とも
捏造した: 呼んでいないツールの返り値を作文する、リポジトリのどの版にも存在しない英文の
エラーを貼る（`{"error":"The requested model sdxl-base-1.0 is not available for provider
comfy"}` — `git log --all -S` で不在を確認）、**存在しないパスに対する sha256 を貼る**、
`bash` を実行していないのに「done」と答える（ファイルが作られていないことを
`GET /api/fs/file` で確認した）。

**kind=claude のセッションに替えたら一発で通り、以降 13 回の呼び出しがすべて素直に返った。**
実機の数字を取るなら claude のセッションで駆動する。これは前回の補遺の「駆動役は測定器では
ない」の続きだが、**エージェントの種類で結果が変わる**ところまでは前回書けていなかった。
...
`upload`）。実際に走ったのは image の fetch と ingest の 2 つで、**llm の fetch だけはインスタンスが
取れず未実行**である。
...
（インスタンスの取得が残りの同期より長い）。**有効なモデルを増やして `keys.rest` を伸ばすと開く。**
...
（12.3 GB）が最後まで残る。**窓の長さは「start 以外のモデルの合計バイト数 − インスタンスの取得時間」で
...
インスタンスが走っている最中に `juggernaut-xl-v9`（7.1 GB）を有効化した。**再起動は無い。**
...
llm のインスタンスは**取れなかった**。サービスのイベントをそのまま写す:
...
**解消（2026-09-11・#537）。** CP 側は 1 行残るようになった——`pendingGuard` が断るときに
`(役, モデル)` ごと 60 秒に 1 回まで（`enginePendingLogEvery`）ログを書く。**ただし
`generate_image` 経由で見えないことは変わらない**（上の 16 分リトライは #537 でも同じ）ので、
呼び出し側の応答本文が要るならやはり生の HTTP を採ることになる。上の測定は #537 の前の
配備（2e765534）のもので、そのときログが無かったことは事実である。
...
**窓は約 2 分 02 秒**（05:07:00〜05:09:01.9）。前節の 2 分 39 秒より短いのはインスタンスの取得が速かった
ためで（`mode: on` から `running` まで前節は 10 分 50 秒、今回は 6 分 46 秒）、**窓の長さは
「start 以外の合計バイト数 − インスタンスの取得時間」**という前節の式のとおりに動いている。`state` は
...
（`g6.xlarge` は 1 台 4）で、image と llm の 2 台でちょうど埋まるためである。**インスタンスを作り直す
`off` → `on` の間は退場中の 4 vCPU が残るので、両レーンが同時に作り直すと後から要求した側が
`VcpuLimitExceeded` で静かに置かれなくなる**——今回は作り直しを 1 回に抑え、前後を相手に
通告して避けた。GPU は 1 台を約 9 分（`mode: on` 05:00:14〜`mode: off` 05:11:55、うちインスタンスが
動いていたのは約 9 分）。
...
`base_model` も正しく、ファイルもディスクにあり、インスタンスは 1〜2.5 分のチェックポイント切り替えを
払い、そこでグラフが死ぬ。利用者の側にも手立てはない。`generate_image` に VAE を指す引数が
無いからである（ADR 0069 の語彙は意図的にプロバイダ非依存）。

### 変えたこと

`--vae` は元からファイル語彙にあり、分割 3 ファミリーでは必須だった。これを単一チェックポイントの
2 ファミリー（sdxl・sd35）でも読むようにした。ここでは**任意かつ上書き**で、宣言すれば 1 つの
...
- **既知の名前の VAE への自動フォールバック**。`VAELoader` の `vae_name` は `models/vae` の
  列挙で、SD3.5 の `clip_name1` を拒んだのと同じ形（P2 残作業 5）。インスタンスが持っていない
  `sdxl_vae` を名指しすれば、今動いているファミリーが全行 `Value not in list` になる。
...
## 追記 — パネルを 2 画面に割り、上流の制限・ファミリー・推奨パラメータを取り込んだ（2026-09-12）

決定 2・4・10・11 の**広げ方**であって、決定そのものの差し替えではない。5 件を入れた。
実測はすべて 2026-09-12 の匿名アクセスによる。

### 1. パネルを「運用」と「モデル」に割る

1 つのレール項目に 2,991 行が入っていて、GPU のモード・インスタンス・段・稼働履歴と、カタログ・
取り込み・上流検索・HF トークンが 1 ページに縦に積まれていた。問いが 2 つ混ざっている
（「いま GPU に金を払っているか」と「このエンジンに何を積むか」）うえ、**権限の境界も
まさにそこにある**——未解決 11 の granted な tenant_admin はカタログを埋められるが GPU は
買えない。レールを「推論エンジン」「推論エンジンのモデル」の 2 項目にし、テナント設定の
`tenant.tab_engines`（ラベルは元から「推論エンジンのモデル」）は後者を描く。

モデル側の中は 役割（文章 / 画像）→ モデル / LoRA の 2 段タブ。役割タブはエンジンが 2 本
以上あるときだけ出す（1 本のタブは操作できないコントロールである）。

🔴 **種類の select を外し、タブに一本化した。** 取り込みフォームと登録フォームは「この行の
種類（モデル / LoRA）」を持っていたが、その上の検索は常に `types=Checkpoint` を投げていた
——LoRA を登録するつもりのフォームの上に、チェックポイントが 20 件並んでいた。同じ画面の
2 つが食い違えない形にした。

### 2. 上流が「できない」と言っていることを検索結果に出す

**Civitai の「要ログイン」はメタデータからは判別できない。** 月間 DL 上位 20 件の
Checkpoint の `downloadUrl` を匿名 HEAD した結果:

| 結果 | 件数 |
|---|---|
| 401（ログイン必須） | **13 / 20** |
| 307（CDN へリダイレクト = 匿名可） | 7 / 20 |

20 件すべてが `availability: "Public"` / `flags: 0` / `status: "Published"` /
`usageControl: "Download"` で、**401 を予測できるフィールドは一つも無い**（`paidAccess`
が非 null の 3 件は必ず 401 だったが、残り 10 件の 401 には印が無い）。よって検索の 20 件
に対しても HEAD を撃つ。並列 8・全体予算 8 秒。**401/403 のみ yes、2xx/3xx のみ no、
それ以外は空**（「誰にも分からなかった」）とし、resolve 側の `engineCivitaiAnonymous` より
厳しくした——あちらは取り込みを止めないための fail open で「no」が既定、こちらは 20 行の
隣に事実として描かれるので 429 を「誰でも DL 可」と読ませてはならない。

同じ 1 回の読みから取れる制限も出す（コードの閉じた集合。語は Console の catalogue に置く）:
licence matrix（商用不可・要クレジット表記・派生不可・同一ライセンスのみ）・有料 / 早期
アクセス・非公開・サイト内生成専用（`usageControl` は**版の文書にしか無い**ので検索一覧では
空で、resolve で初めて出る）・NSFW / 実在人物 / 未成年・スキャン結果。

🔴 **Civitai の許可フラグはすべて否定形である。** `allowNoCredit: false` が「要クレジット」
なので、model 文書を読めなかったとき（resolve の best-effort な 1 回）に**誰も公表していない
制限が 4 つ出る**。`known` の門を作り、試験の陽性対照で固定した。

HF は `gated` が `false` / `"auto"` / `"manual"` の 3 値（実測: `google/gemma-3-1b-it` が
`"manual"`）なのを bool に畳んでいた。運用者の手間が「規約に同意する」と「作者の承認を
待つ」で全く違うので、2 種類に割った。

### 3. ファミリーは推定して入れる（決定 2 は維持）

決定 2（宣言するのは運用者・上流の表示名は保存しない）は維持したまま、CP に正規化表を置き、
resolve と検索の答えに `base_model_suggest` を足す。Console は**未操作のセレクトにだけ**
初期値として入れ、「『SDXL 1.0』から推定しました」と出所を言う。Pony / Illustrious /
NoobAI は SDXL 系なので `sdxl` に写す。

🔴 **自信が無いものは空のままにする。** 同じ 20 件のうち `Anima` / `Krea 2` / `LTXV 2.5` /
`SD 1.5` はこの配備にテンプレートが無い。誤ったファミリーは `base_model_missing`——その行が
生成できない唯一の印——を黙らせるので、無印より悪い。推定は provider の語彙の要素だけを返す。

### 4. 推奨パラメータを行に持たせ、provider に効かせる

**構造化された生成条件は匿名では取れない。** `/api/v1/models` の `images[]` に `meta` は
無く（`hasMeta` だけ）、`/api/v1/images?modelVersionId=…` を叩いても 5 件すべて `meta: null`
だった。取れるのは作者の説明文（HTML・実測 11,906 文字）と、LoRA の `trainedWords`。

だから **散文から拾う**。Steps / CFG / Sampler / Scheduler / Clip skip / strength を正規表現で
拾い、範囲外は**握り潰さず捨てる**（「25000 steps で学習」を sampler の steps にしない）。
拾ったものは**引用した一文と一緒に**取り込みフォームへ出し、書き込むのは人間が押したときだけ
——他人の段落に対する正規表現の推測なので、判断できる形で人の前に置く。

カタログ行に `params` 列（JSON・1 列）を足し、comfy の 5 テンプレートがファミリーの recipe に
**フィールド単位で**被せる。`steps` だけ宣言した行はファミリーのサンプラーを保つ。sampler /
scheduler は Agent が知らない名前なら**無視してファミリーの値を使う**——ComfyUI の enum に無い値は
コールドスタートの後に `Value not in list` で全滅するので、「ファミリーのサンプラーで絵が出る」と
「絵が出ない」の二択なら前者が正しい。

🔴 **flux1 と klein は cfg を受けない。** あの 2 ファミリーの「CFG」は FluxGuidance / 蒸留パスの
...
（無ければ「ファミリー未宣言」）を頭に出す——対にできるかを決める唯一の事実が、メタ行の
「·」区切りの 1 項目だった。検索も `types=LORA` に連動する。

🔴 **HF は `filter=lora` を単独で投げると接続を切る**（実測・curl 56・`filter=gguf&filter=lora`
でも同じ）。`pipeline_tag` と対で送ると 200。この pipeline_tag は精度ではなく必須である。

### 実描画で直した 4 件

実バンドル + スタブ CP + headless Chromium（`~/af-shots-engines/`・リポジトリ外）で dark /
light の 7 場面を描いた。DOM 試験 112 件が全部緑のまま、次の 4 件が出ていた:

- 宣言なしの行に**フォームの注意文**（「空欄はファミリーの既定のまま…」）が出ていた
- 「パラメータ」ボタンが枠なしで、要約文の続きの単語に見えていた
- LoRA タブに「選び直しは次の起動から効きます」が残っていた（LoRA では出ない操作の説明）
- パラメータ欄のラベルが `7ch` 固定で「ステップ数」「サンプラー」が**切れていた**

加えて、Civitai の検索が合成していた `license_name: "non-commercial"` と restrictions の
`noncommercial` が**同じカードに 2 つ並んで**いたので、合成をやめた。

### 実機に残したこと

この回は実機（GPU）に当てていない。**`params` が生成に効いていることは実機でしか言えない**
——完了の定義は「同じ prompt と seed で、`steps` を宣言した行と宣言しない行が違う絵を返す」。
golden 試験が pin しているのはグラフの形であって、それは SD3.5 が 1 枚も出せなかったときに
通っていたのと同じ主張である。
## 追記 — ネガティブプロンプト: 宣言するのは 3 か所、合成するのは 1 か所（2026-09-12）

これまで配備に存在したネガティブプロンプトは **Agent にコンパイルされた 1 行**だけで、
どのエンジンのどのチェックポイントでも同じだった——`blurry, lowres, deformed, watermark, text`、
`bench-image-engine.py` が測ったときの文字列である。誰も変えられず、変えたい理由を持つ人が
3 種類いた。

- **利用者**、要求ごと——語彙としての半分は ADR 0069 の追記が扱う。
- **カタログの行**。チェックポイントの配布元は「入れるもの」と同じ頻度で「避けるもの」を
  書いており、SDXL 系の作例はその列挙つきで作られている。固定の 1 行は、全モデルの代役として
  出来が悪かった。
- **配備の管理者**。この配備では描かせたくない語のために。

3 つは**足し合わされる**。順序は上のとおりで、どれも他を消せない（`comfyNegativeFor`）。
利用者が 1 つ除外を名指したことは「配布元の推奨も止めろ」ではないし、管理者の列挙が最後なのは
どの要求もそれを外せないからである。3 つとも黙っているときは Agent 自身の実測 1 行が使われる
——これは**フォールバックであって下限ではない**: 自前のものを宣言した行は、配布元が頼んでも
いない定型文に足されるのではなく、置き換える。

### どこに住むか

| 誰 | どこ | 経路 |
|---|---|---|
| 利用者 | `generate_image` の `negative_prompt` | ツール・要求ごと |
| カタログの行 | `engine_models.negative_prompt`（migration 0065 / pg 0050） | `PUT …/models/{id}` |
| 管理者 | 設定 `engine_<key>_negative` | `PUT …/engines/{key}/negative` |

管理者の列挙は配備単位でもテナント単位でもなく**エンジン単位**にした。実際に適用できるのが
そこだからである——ネガティブプロンプトがサンプラーに届くのは、このリポジトリがグラフを組む
エンジンだけ——つまり設定する場所と効く場所が一致する。Agent が既に取りに来ているカタログの
答えに相乗りするので、反映はそのキャッシュの TTL（10 分）か次の push。文字数に上限（500）が
あるのは、その文字列を全ワークスペースが更新のたびに払うからである。

### 🔴 これはコンテンツフィルタではなく、パネルもそう言う

ネガティブプロンプトは**サンプラーへの傾きであって門ではない**。そのうえ、5 ファミリーのうち
それを持つのは 2 ファミリーだけである。

- **sdxl と sd35** は cfg 7・4.5 でサンプリングし、ネガティブの枝こそが guidance の計算相手;
- **zimage と flux2-klein** は **cfg 1**。`uncond + 1*(cond - uncond)` は厳密に `cond` で、
  語はエンコードされるだけで何も変えない;
- **flux1** は guidance を条件側に畳み込み（`BasicGuider`）、ネガティブの入力が無い。

したがって「管理者の列挙が黙って効かない」状態は実在し、それは報告する: この 3 ファミリーが答えた
要求には、モデル自身のネガティブと配備の除外語が適用されなかったこと、そしてファミリーの名前を
警告に載せる。誰も悪くなく何も失敗していなくても言うのは、その沈黙こそがこの経路の存在理由
だからである。利用者自身の `negative_prompt` は中核が `Caps.Negative` に照らして別に報告
するので、同じことを 2 度言うことはない。

蒸留系のグラフにも語を配線してしまう案は検討して却下した。`Caps.Negative` がどこでも true に
なり、利用者にはそれが「効いた」と読める——絵は正しく見え、除外したかったものが写っている
という、ここで最も高くつく間違いになる。

### あえてやらなかったこと

- **steps / cfg / sampler は入れない。** ADR 0069 が外に置いている「見え方」の摘みであり、
  5 経路のうち 4 経路で何も動かない摘みはシステムについて誤ったことを学ばせる。ネガティブ
  プロンプトが入るのは、利用者が他の言い方では表現できないからである（0069 の追記）。
- **モデル既定の住所を 2 つ作らない。** 推奨パラメータの JSON 1 列の鍵ではなく専用の列にした:
  これは人がテキストボックスに書く散文で、パネルは行の上に出し、検証すべき範囲を持たない
  唯一の推奨値である。
- **テナント軸は作らない。** 適用できる単位はエンジンである。テナント単位の列挙は、グラフを
  組まない層に 2 つ目の適用点を要求する。

実機未検証。固定できているのは、合成した文字列がグラフのネガティブ側 `CLIPTextEncode` に
届くこと、合成が空ならグラフがゴールデンとバイト単位で同じままであること、枝を持たないファミリーが
落としたことを報告すること。GPU がまだ返していないのは、同じ prompt・同じ seed で、宣言した
ネガティブプロンプトだけが違う 2 枚である。

## 追記 — カタログの行を訂正できなかった（2026-09-13）

ADR 0079 の実機で、別配備の llm エンジンを借りて冷間起動を測った。エンジンはモデルの
読み込みで落ちた:
...
行の `context_tokens` はモデル自身のヘッダが言う 262144 のままで、L4 は重み 17 GB も、その窓が
要求する KV キャッシュ 16 GB も載せられない。**Console にその欄が無い。** インスタンスを買い、重みを同期
し、失敗はその 4 分後に来る——訂正できない行の代価は GPU 時間で払われる。

その周辺で 3 つの穴が見つかり、運用者がさらに 2 つ挙げた:

1. 行の数値をそもそも編集できない——`putModel` が受けるのは `enabled` / `selected` /
   `default` / `base_model` / `negative_prompt` / `params` で、それ以外は `encoding/json` が
   黙って捨てる。
2. 取り込み元は保存されていたのにリンクになっていなかった（後述）。
3. 分割モデルは部品の出所を言えなかった（後述）。
4. ファイル選択は名前とサイズを言うだけで、そのカードに載るかどうかは何も言わない。
5. 行のファイルを別のものに交換できない——`engineAttachAllowed` は
   *「行を忘れるか、独立した行として取り込め」*と答え、それはライセンス受諾を捨てることである。
6. 取り込み履歴に削除が 1 つも無い——ルートも store のメソッドも TTL も無く、
   `engine_ingest_jobs` は配備の寿命のあいだ育ち続ける。
7. 忘れた行がバケットに残したファイルを、選ぶだけで登録し直せない。

### 決定

- **`putModel` はポインタ欄で広げる。再登録にはしない。** この経路は既に 3 つの欄をその形で
  編集していて、理由もその隣に書いてある——行は 1 つの値だけが間違っていて他は正しいことが
  あり、1 つの数を直すために行ごと POST し直すのは、ライセンス受諾と S3 キーを往復させること
  である。受けるのは `context_tokens` ＋ `max_output_tokens`（**対で**。行は前者があるときしか
  後者を答えないので、片方だけはこの画面が説明できない行になる）と `vram_mib`。
- **`kind` とライセンス 3 種は入れない。** `kind` は行をモデル一覧と LoRA 一覧のあいだで動かし、
  付く操作子まで変える——編集の服を着た再登録である。ライセンス 3 種は**人間の行為の証拠**で、
  `commercial_use` はそこから導出される（決定 10）。打ち直せるライセンスは、監査が読む記録の
  中の偽造されたスナップショットである。
- **VRAM の門は「これから書く値」に対して訊く。** `engineVramGuard` はモデルを有効にするときに
  しか走らず、しかも保存済みの行を読んでいた——つまり**すでに有効な行の窓を上げる操作は誰も
  検査していなかった**。冒頭の失敗そのものの形である。
- **出所を URL に変えるのは control plane であって画面ではない。** `civitai:<id>` はモデルの
  **バージョン** id である: `civitai.com/models/<id>` は**別のモデル**を開き、解決する形は
  このファイルが既に `LicenseURL` に渡しているものである。画面は URL が来たかどうかで分岐し、
  文字列を 2 度目に解析しない——検索結果の `url` が決定 11 以来守っている規則と同じ。
- **素の URL はリンクにしない。** それは重みの直接ダウンロードで（この ADR の表の 1 行目は
  22 GB）、「これがどこから来たか」と言う行が、それを始める click であってはならない。読める
  テキストのまま残る。この配備の control plane が知らない綴りも同じ——運用者のコンソールでは、
  壊れたリンクは文字列より悪い。
- **ファイルは 1 つずつ自分の出所を記録する。** 行の `source` を書くのは行を**作った**取り込み
  だけで、attach はファイルしか書かない。だから 4 ファイルの FLUX.1 の行は
  `hf:black-forest-labs/FLUX.1-dev/…` 1 行を持ち、それが 4 つ全部の出所として読まれていた。
  部品はふつうに別のリポジトリから来る——この ADR 自身の表の text encoder は、diffusion model
  を出した者の発行物ではない——そして `engine_ingest_jobs.source` はジョブ行が生きているあいだ
  しかそれを持たない。**移行は不要だった:** `files` は移行 0057 から JSON 列なので、古い行は
  単に欄を持たず、それは「誰も記録しなかった」のまま残り、「不明」とは描かれない。
- **バケットからの再登録は S3 ではなく取り込み履歴を読む。** レビュー R3 の「CP はバケットを
  見られない」はいまも真で、2026-09-13 に測り直した: CP のタスクロールに S3 のアクションは
  1 つも無く、`aws-sdk-go-v2/service/s3` は control plane の依存に入っておらず、CP はバケットの
  **名前すら**知らない——`BUCKET` は取り込みタスクとエンジンタスクの定義に届いており、バケットは
  engines スタックの宣言で、CP のタスク定義は platform スタック、しかも engines は platform
  から import する側である。一覧を取るには IAM の 1 文・新しい依存・スタックをまたぐ配線・配備が
  要る。一方で `engine_ingest_jobs` は一度も間引かれないので、この配備がそのバケットに入れた
  全ファイルを既に持っている。選択一覧が読むのはそれである。
- **履歴の削除とその選択一覧は 1 つの仕事である。** ジョブを消すことが、そのファイルの存在を
  知る手段を消すことだからである。

### 未解決

- **ファイルの差し替えは VRAM の門を通らない。** 門はいま「編集がこれから書く値」と「有効化
  されようとしているモデル」には訊くが、**すでに有効な行の本体を大きい量子化に差し替える**と
  同じ数字が黙って変わる。意図して外してある: 拒むなら取り込みの**開始時**しかなく（書き込みは
  数分後のジョブ照合の中で、そこには答えを待っている相手がいない）、そのルートに `confirm_vram`
  と、取り込みフォーム側の確認が要るからである。和らげているのは、フォームが押す前に重み・KV
  キャッシュ・カードの数字を出すようになったこと——門が訊くはずだった問いそのものである。
- **本物の `ListObjects` は履歴に見えないものを見る**: 手で staging されたファイルと、そもそも
  バイトがまだ在るかどうか（purge はオブジェクトを消し、ジョブ行は `done` のまま残る）。だから
  画面は読んだこと——完了したダウンロード——だけを言い、ファイルが在るとは決して主張しない。
  `postModel` が打ち込まれた S3 キーについて昔から取ってきた姿勢と同じである。上の IAM 文で
  一覧を買うかどうかは運用者の判断であり、実装の細目ではなくレビュー R3 の反転である。

### あえてやらなかったこと

- **新しいエラーコードは作らない。** カードに載らなくなる編集は、既存の `engine_vram_confirm`
  に既存の `confirm_vram` の逃げ道つきで答える（ADR 0074 決定 6）。同じカードについての同じ
  問いだからである。
- **見積もりを Console で計算し直さない。** KV の幾何はワイヤに乗っておらず、乗せるつもりも
  ない。`engineKVCacheMiB` の 2 つ目の写しを TypeScript に持てば、門が使う式とずれる——そして
  運用者が読む数は、その運用者を拒む数と同じでなければならない。
- **ファイルが出所を持つようになっても、行自身の `source` は消さない。** ライセンス受諾が
  帰属する先はそれである。行と同じ出所のファイルを隣で繰り返さない、というだけにした。

### 作って分かったこと（2026-09-13）

実機の失敗を構成する数字を、信じるのでなく検算できる形で書いておく: 重み 17 GiB は 17408 MiB、
KV キャッシュは `layers × heads_kv × (key_len + value_len) × tokens × 2` で、このモデルなら
16384 トークンで 1024 MiB（合計 18432・L4 段の 21000 に載る）、262144 で 16384 MiB（合計 33792・
載らない）。画面へは 1024 トークンあたりの KV（30B で 96 MiB）を渡して掛け算させる——キャッシュ
はトークン数に線形で、式は 1 か所に置いておきたいからである。

- 🔴 **`vram_need_source: "weights_kv"` は既にワイヤに乗っていて、Console の union に無かった。**
  結果は 2 つ、どちらも無音: 確認ダイアログが存在しないメッセージキーを描き、meta 行がその値を
  **実測の言い回し**で描いた——導出値が誰かの実測として読める、というのは ADR 0074 決定 6 が
  防ぐために書かれたまさにそれである。知っている値を列挙する union は、サーバが新しい値を
  覚えても落ちない。
- **「載るか」は選択の時点では片側しか答えられない。** 重み > カードは確定の否。重み ≤ カードは
  **まだ是ではない**——KV キャッシュはファイルを解決するまで不明だからである。一覧の印はこの
  非対称を保ち、支えられない緑の印は出さない。
- **差し替えは KV 幾何を書き直す。ただし幾何を持つスロットだけ。** 行は attention の数値を 1 組
  しか持たず、それを書いていたのは行を作る経路だけだった。だから gguf を別の量子化に差し替えると
  **前のファイルの幾何が残り**、もう存在しないファイルから VRAM を見積もり続けた。text encoder の
  差し替えは checkpoint の attention について何も言わないので据え置く——そしてヘッダが読めなければ
  0 を書き、行は floor に戻る。前のファイルの数字こそが確実に間違っている唯一の答えだからである。
- **差し替えは古いオブジェクトを purge しない。これは後で直す制限ではない。** 書き込みは押下から
  数分後のジョブ照合の中で起き、そこでは断られた削除を報告する相手がいない——そして鍵は共有
  されている: `text_encoders/` は複数の行から指されている（af-sandbox で実測: `clip_l.safetensors`
  が 2 行から）。そこで消すのは、誰も触っていないモデルが読み込めなくなる道である。画面は
  「バイトは残る」と言う。
- **取り込みの完了に、目録の push が無かった。** `publishActiveSet` は走っていたが
  `notifyEngineCatalogChanged` は走っておらず、各ワークスペースは自分の 10 分 TTL まで古い目録を
  持っていた。部品の attach にも同じ穴があった。

そして**試験の側**の罠が 3 つ。この画面が過去に噛まれてきた場所である:

- **共有されたクラス名は他人の選択子である。** 数値欄に `className="mono"` を足したら、既存の
  試験の `ul.engines-model-list .mono` の数が 1 から 4 になった。装飾は stylesheet 側へ移した。
- 🔥 **「ここには出さない」を間違ったタブで assert すると永久に緑になる。** LoRA 行にネガティブ
  プロンプト欄が無いことの確認は **model タブ**で行われていて、そこには LoRA 行がそもそも描かれ
  ない——だから門を消しても緑だった。いまはタブを切り替え、行が画面にあることを先に確かめる。
  窓の編集欄の同種の試験は最初からその形で書かれている。同じ「対の取り方」の規則は借用行にも
  効く: 比較の相手は **external の行**であり、これは理屈ではない——`refuseBorrowedWrite` を
  素の「ここの管理下ではない」に置き換えると external が落ちる＝ADR 0076 の LAN ComfyUI である。
- **陽性対照そのものが無効になることがある。** リクエスト body を「オブジェクトリテラルに同じ
  キーを 2 度書く」ことで壊しても何も起きない——JavaScript は後勝ちだからである。この種の body は
  キーを**消す**ことで壊すこと。

そして履歴そのものが何でできていたか:

- 🔴 **`engine_ingest_jobs.spec` は取り込み要求を素の `json.Marshal` したもので、キーは Go の
  フィールド名**（`Kind` / `FileFlag`）である——その構造体に json タグが無いからである。パネルは
  いまそれを読んで登録を差し出すので、フィールド名を変えるとその差し出しが**黙って空になる**。
  だからジョブ行は答えを明示する（`kind` / `file_flag`）ことにし、spec の形に委ねていない。
- **共有されたジョブ行の変更は、wire の golden では `listIngest` ではなく `postIngest` の行に
  現れる**——`engineIngestJobRow` は両方の背後にある 1 つの関数で、golden は最初の出現地点の名を
  書くからである。「関係ないルートが変わった」と読み違えやすい。
- **履歴の行を消すことは 2 つの別々の問いであり、パネルは真である側を訊く。** その鍵を指す
  カタログ行があれば、削除は片付けである——平文で、1 押し。指す行が無ければ、その行はこの配備が
  「ファイルがバケットに在る」ことを記録している唯一の場所なので、明示的な承認を要する。🔴
  どちらの文面も**ファイルが在るとは言わない**: purge はオブジェクトを消してジョブを `done` の
  まま残し、CP は見に行けない。カタログの読みが失敗したときは、全行が強い側の警告に倒れる。
- 🔥 **背景で回した試験のラッパが exit 0 を返し、出力ファイルには golden の FAIL があった。**
  このリポジトリの歴史で 2 度起きていて、今回また起きた。exit code でなく出力を読むこと。

## 追記 — VAE を同梱しないチェックポイントと、それを直せなかった画面（2026-09-13）

配備の持ち主が一晩で踏んだ 2 つの欠陥。根は 1 つ。

**1 つ目は、何も言えなかった事実。** `waimatureillustrious_v30` は VAE テンソルを持たない
Illustrious/SDXL のチェックポイントで、ファミリーも宣言され、そのファミリーが要求するファイルも
持ち、`engineMissingFileFlags` を通り、有効にでき、`generate_image` の `model` にも現れる
——そしてインスタンスがチェックポイント切り替え（1〜2.5 分）を払ったあと、**すべての**要求が
ComfyUI の中で `ERROR: VAE is invalid: None` で落ちる。生成は `VAEDecode`、編集は
`VAEEncode` なので「ワークフローではなくチェックポイントが原因」とは切り分けられるが、
分かるのは全部払ったあとになる。呼ぶ側に手立ては無い（`generate_image` に VAE の引数が無い）。
2026-09-12 の修正で Agent はカタログの `--vae` を読むので**宣言はできた**——宣言が要る行を
指すものが無く、そういう行の有効化を拒むものも無かった。

**2 つ目は画面。** その `--vae` を手で足すには、正しいタブにいて（モデル/LoRA のタブが
フォームの意味を黙って変える）、既存行の id を正確に打ち直し、役割を選ぶ——その 3 つが
揃って初めてチェックが現れる。しかもそれまで、そのチェックは押せない理由をどこにも
書かずに無効のままだった。持ち主の言葉は「さっぱりわからない。使い勝手最悪」で、
そのとおりだった。行為に辿り着く唯一の道が「既に知っている id と衝突させること」だった。

### 事実はファイルに訊く

判定はチェックポイント自身の safetensors ヘッダ——先頭 8 バイトの長さ（LE u64）と
テンソル名の JSON——を**取り込み元**から HTTP Range で読む（`engine_safetensors.go`）。
🔴 S3 からは読まない: レビュー R3 はそのままで、CP はバケットの権限もバケット名も
持たないし、この機能でそれを覆さない。`first_stage_model.`（ここで使う SD/SDXL/SD3.5 の
単一ファイル配置）か `vae.`（diffusers の綴り）で**始まる**テンソルがあれば同梱。
部分一致ではなく前方一致であること: さもないと `model.diffusion_model.vae.proj.weight` が
壊れたチェックポイントを健全と判定する——静かに失敗する方向。

🔴 **判定は 3 値で、3 つ目が設計を支える。** 「読めなかった」——ヘッダの無い `.ckpt`、
消えた取り込み元、匿名の Range を拒む Civitai の資産、そもそも出所の記録が無い行——は
「VAE が無い」ではない。印を付け、有効化を拒み、ダウンロードを買うのは明示的な `no` だけ。
これ以前に取り込まれた行はすべて未読で、全部に印を付けたら印の意味が無くなる。

事実は**ファイル**に持つ（`files` の JSON 列なので移行不要——`EngineModelFile.Source` が
そうだったのと同じ理由で——差し替えのとき一緒に動き、行が持っていないファイルの話を
し続けることがない）。

### 「無い」と分かったら何をするか

- 行の印 `vae_missing`（`base_model_missing`・`files_missing` の隣）と、終わらせる
  ファイルを名指す `vae_fix`。
- `engineVaeGuard` が有効化を拒む（`engine_vae_missing`。`files_missing` と同じく、
  VRAM の門と違って `confirm` は無い）。正当な場合が無いため——届く引数も、
  別の入手経路を持つテンプレートも無い。
- 行の 1 ボタンがファミリーの VAE を取り込んで `--vae` として足す（`POST …/models/{id}/vae`）。
  その鍵をこの配備が既に持っていれば（別の行が宣言していれば）宣言するだけで、
  ダウンロードは起きない。
- 新規の取り込みでは、resolve がヘッダを読んで「無い」と言ったときだけ、同じものを
  **既定 ON** のチェックで先回りして出す（`with_family_vae`）。2 本目のライセンスは
  本体のものと並べて画面に出し、受諾は両方を覆う。2 本目の計画はフォームの前に人が
  いるあいだに決める——実行はジョブの reconciler で、そこでは resolve の失敗を
  伝える相手がいない。
- 未読の行はパネルの読み込み時に 1 回 `POST …/models/vae-scan` で答える。行ごとではなく
  カタログに 1 回: これは誰でも開ける画面で発火するので、扇形に展開すると上流への
  読み取り回数が「その画面を何回開いたか」になってしまう。

🔴 **sdxl には既定のファミリー VAE（`stabilityai/sdxl-vae`）があり、sd35 には意図的に無い。**
SD3.5 の標準オートエンコーダは gated のリポジトリにあり、自動の 2 本目は reconciler の
中で 403 になる。本当に VAE の無い sd35 の行は印と拒否を受け、直し方は運用ガイドの
手動宣言になる。

2026-09-12 の修正から変わらず不可能なものが 2 つ: 既知名の VAE への実行時フォールバック
（`VAELoader.vae_name` はインスタンスの `models/vae` の列挙なので、持たない名前を書くと今動いて
いるファミリーが壊れる）と、カタログ側で `--vae` を必須にすること（同梱の行が全部
「ファイル未設定」になる）。

### 画面: 4 つの問い

`EngineIngest` は「1 列に 12 項目、どれが効くかは画面に出ていない状態が決める」フォーム
だった。いまは「モデルを追加」で、**ペイン**（`PaneContent.kind = "engineAdd"`）である——
パネルの節でもダイアログでもない。

🔴 形は 2 回間違えてから決まったので、その理由を残す。パネルの中に描くとカタログと
スクロールバーを共有し、スマホでは「いま答えている問い」がページのスクロール位置次第に
なる。独立したダイアログに移すと読みはするが、器としてはまだ違った——最後の押下が
**始める**ものは数分走り、仕事はダウンロードの終わりでは終わらない。取り込みが作る行は
設計上無効で、「取り込めた」と「使える」は有効化 1 回ぶん離れており、その 1 回は誰かが
探しに行く必要がある。ダイアログは他の全部の前に立つので閉じられ、そこで糸が切れる。
ペインなら開けたままにでき、よそ見でき、別窓に出せて、**仕事が終わる場所で終われる**——
ダウンロードの進み具合になり、そのまま有効化を出し、門（ファミリー・ファイル・VAE・VRAM）は
ダウンロード代を払った当人の前で答える。開くとき管理ダイアログは閉じる。ダイアログは
レイアウトの上に描かれるので、その中から開いたペインは誰にも見えない画面になるため。

1. **何をする？** 新しいモデルを追加 / 既存の行に部品を足す / 既存の行のファイルを
   差し替える。id の衝突から**推測せず、訊く**。対象の行は一覧から選び、枠は
   **その行為が実際に使える枠だけ**を出す（attach は空き枠、replace は埋まった枠）。
   `engineAttachAllowed` と `engineReplaceAllowed` を、不可能な選択肢を出さないことで
   表現する——互いを無効化し合う 2 つのチェックボックスではなく。
2. **どこから。** 検索・取り込み元・リポジトリ欄を 1 画面に。この問を**出ることが**
   resolve そのもの: 「調べる」はリポジトリを指定した時点で決まっている決定への
   2 度目の確認だった。貼った URL は何を指していたかをその場で言う（それが埋める欄は
   次の問にあるため）。
3. **どれを。** 候補・ライセンス・カードに載るか・ファミリー（提案とその出所）・
   作者の推奨値、そして上記の VAE の申し出。
4. **確認。** 何が起きるかを平文で並べ、受諾のチェックの隣に置く。

🔴 **次へが押せないときは、理由をボタンの横に書く。** これが作り直しの目的そのもので、
古いフォームは 3 か所で破っていた。

枠の選択は select ではなくラジオ。答えの 1 つが空文字（行自身のチェックポイント）なので、
select だと placeholder が同じ値を持ってしまう。

### 試験にいくら払い、それが何を証明したか

パネルの dom 試験 79 本のうち 26 本が古いフォームを操作していたので、4 つの問いを歩く形に
書き直した。名前を挙げる価値があるのは 2 本。「公開された上限が、打ち込んだウィンドウを
上書きしない」は場面を作り直す必要があった（ウィザードでは最初の resolve の**後に**
打つので、いまは 2 つ目のファイルを選ぶ場面になる）。そのうえで変異で確かめた——
`!ctx.trim()` の番人を外すと赤くなる。印の試験も同じやり方で: `engineVaeMissing` を
「yes 以外」にすると未読の行に印と 409 が付き、2 本とも落ちる。

完了と呼ぶ前に、headless Chromium で dark / light と 390px 幅を実描画した——レールは
折り返し、行為は説明文とともに縦に積まれ、次へが灰色である理由は横に残る。

## 改訂 — モデルを探し、カードから操作する（2026-09-14）

状態: 実装方針を承認済み。この決定は上記の4段階ウィザード、日付順を意図的に除外する
判断、Control PlaneのS3読み取り禁止を置き換える。上記の検証は旧実装の記録であり、
この改訂の受け入れ検証ではない。

### モデル管理をペインにまとめる

ImageとLLMは別ビュー・別カードとする。いずれもペインの利用可能な幅を使い、
「探す」「登録済み」と、検索・ソート・モデル／LoRA切替を備えるヘッダを置く。
設定のモデル管理入口はこのペインを開き、機械の運用は運用画面に残す。保存済みの
`engineAdd` ペインはエンジン・LoRA選択を含めて引き続き復元できる。

ImageはCivitaiを初期表示し、Hugging Faceへ切り替えられる。Civitaiの初期順は上流の
`Newest` とし、最終更新順ではなく「新着順」と表示する。Hugging FaceはImage・LLMとも
`lastModified` 降順を初期値とする。LLMにCivitaiは出さない。自動量子化リポジトリを
日付順から除外しない。他の対応済みランキングも選択可能とし、追加取得は上流の順位を
継続する。人気順の取得済みページをローカルで並べ替えて更新順と呼ばない。
公開日と更新日は別の事実として扱い、欠けた更新日を作らない。

検索カードはHFのリポジトリまたはCivitaiのモデル単位とする。バージョンとファイルは
操作モーダルで選ぶ。Imageはファミリーとモデル種別、LLMは取得できたファイル・量子化・
コンテキスト情報を中心に表示する。作例があれば**カード右側に小さなサムネイル**を置き、
操作するとライトボックスを開く。画像がなければ画像枠を確保しない。狭いペインでは
一列にし、下部ボタンは折り返す。ライトボックスはキーボード操作・Escape・終了時の
フォーカス復帰に対応する。

### ボタンで操作を決める

「追加」「部品として追加」「差し替え」はカード最下部に置く。各ボタンはその操作だけの
単一モーダルを開き、元のバージョン・ファイル、対象、必須設定、任意の詳細設定を示す。
「次へ／戻る」は設けない。部品追加は互換性のある空き枠を、差し替えは埋まった枠と
現在のファイルを示す。登録・有効化／無効化・既定指定・編集・部品管理・削除は登録済み
カードから操作できる。URL指定、既存S3キーの登録、トークン管理、取り込み履歴も
補助操作として残す。

ジョブ開始後は一覧へ戻り、対象カードと参照可能なジョブ一覧に進捗を表示する。
ペインを再度開くとサーバーから復元する。モーダルを閉じてもジョブは中止しない。
完了だけでは自動有効化しない。ライセンス受諾、ファミリー・VAE・VRAMの検証、借用
エンジンの読み取り専用制約は引き続き適用する。差し替えは成功後に反映し、旧オブジェクトを
残す。ダウンロード失敗時は旧カタログ宣言を維持する。

### 保存状態は履歴から推測せず観測する

Control Planeは配備に依存しないポート経由でモデルオブジェクトのメタデータを読める。
AWS実装には存在確認に必要なモデル保管領域の読み取り権限だけを与え、不在と権限拒否の
区別に必要な場合は範囲を絞ったバケット一覧権限を含める。アップロードと削除はワーカーの
操作を維持し、この改訂でControl PlaneにS3書き込み権限は与えない。

エンジン単位の保存状態APIは、サーバーが把握するカタログ・部品・取り込みのキーだけを
列挙する。呼び出し元は任意のバケットやキーを指定できない。読み取りは同時実行数を制限し、
短時間キャッシュする。確認機構が未設定、権限拒否、確認失敗は「不明」であり「不在」ではない。
APIはファイル単位の存在・不在・不明と確認時刻を返す。UIは確認中、複数部品の一部保存、
登録状態、有効状態を区別する。

検索カードは保存済みファイル数を表示し、上流リポジトリ全体をDL済みと呼ばない。
再利用にはオブジェクトの存在と、不変な取得元ファイル識別情報の一致が必要である。
モデル名、変更可能なブランチ、古い完了ジョブだけの一致では不十分。新規取り込みには
判定に必要な識別情報を保存し、曖昧な既存記録は未確認とする。再利用にもダウンロードと
同じ登録・部品追加・差し替え検証を適用し、行を黙って上書きしない。

### 受け入れと配備

役割別の検索、上流の日付順とページング、単一操作モーダル、右側の小さな作例と
ライトボックス、保存済みペイン・ジョブの復元、正確なファイル再利用、保存状態の全判定、
既存のライセンス・VAE・VRAM・権限検証を受け入れ対象とする。Consoleのテストと
本番ビルド、Control Planeの関連テスト、headlessでの広い／狭いペイン確認を行う。
AWSの実在確認には対応するIAMの配備が必要であり、ローカルのモックを実機検証と呼ばない。
配備と共有ベースへのマージは実装とは別の工程とする。

### 実装結果（2026-09-14）

この改訂は `temp/silzntq` に実装した。Image/LLMのカタログペインは全体で共有 `Button`
部品を使う。カード下部の押下で追加・部品追加・差し替えのどれかを一つの平坦なモーダルの
間固定し、操作選択や次へ／戻るは無い。ジョブはPOST応答から即時表示し、サーバーから復元し、
完了後に行と保存状態を更新する。ページングは送信済み検索語へ束縛し、バージョン／ファイルの
古い応答を捨て、Civitai URLの版を保ち、LLM LoRAの基底には登録済み非LoRAモデルだけを認める。

保存状態はサーバーが把握するカタログとジョブのキーを存在・不在・不明として返す。再利用には
`reusable`、オブジェクトの存在、`artifact_identity` の完全一致が必要で、サーバーが変更前に
新たに確認する。人向け `source` は由来のままで識別子には流用せず、不変identityと旧形式記録が
同じキーに混在すれば曖昧として再利用しない。通常ダウンロードは、
カタログ行またはジョブが既に記録した宛先をアップロード開始前に拒否し、後段の追加／差し替え
競合で同名の別版を上書きしない。カタログ登録失敗はダウンロード済みというだけで完了扱いしない。

統合ツリーで、Control Plane全パッケージを制限付き並列で通し、対象カタログDOM 15件、Consoleの
型検査・i18n lint・本番buildを通した。そのbuildに対するローカルChromiumのPlaywright 7件も
成功した。ブラウザでは1400pxのdark/light、390pxのdark、登録済み編集、ジョブ即時表示、右側
サムネイル、ライトボックス終了時のフォーカス復帰を確認した。Console全体テストでは新モーダルが
共有modal bodyを通らない退行を追加で発見し、修正した。AWS配備、実bucket/GPU検証、`develop`
へのmergeは行っていない。

## 追記 — SD1.5 をファミリーの語彙に足した（2026-09-15）

### 決定 10 を覆してはいない

決定 10 は「**SD1.5 と SD3 Medium は出さない**。前者は SDXL に、後者は SD3.5 に
置き換えられている」と書いた。あれは**種として配る既定モデルの選定**であって、
決定 2 の「運用者が宣言してよいファミリーの語彙」ではない。既定は今も klein / Z-Image / SDXL で、
SD1.5 を種のカタログに足してはいない。

変わったのは、運用者が自分で SD1.5 のチェックポイントを取り込もうとしたときの答である。
それまでの答は `base_model_missing` で、行は登録できるが生成に届かなかった。Civitai に
残っている LoRA とチェックポイントの資産は今も SD1.5 に厚く、**利用者側に回避手段が無い**
（0069 で語彙を足すかどうかを決めた基準と同じ）。

### ファミリーを分けた理由は画質でも recipe でもない

SD1.5 のグラフは SDXL と同形である——`CheckpointLoaderSimple` が MODEL・CLIP・VAE を出し、
`CLIPTextEncode` 2 本、`EmptyLatentImage`、`KSampler`、`VAEDecode`。だから
「SD1.5 のチェックポイントを `base_model: sdxl` で登録する」でも**絵は出る**。
それでもファミリーを分けたのは **LoRA のファミリーゲート**（`comfyResolveLoras`）のためである。
背景の表にもあるとおり、`baseModel` が合わない LoRA は失敗せず**黙って崩れた絵**を出す。
SD1.5 を sdxl と名乗らせると、この門が SD1.5 と SDXL のあいだで開きっぱなしになる——
SD1.5 を足す価値の大半が LoRA 資産である以上、本末転倒になる。

テンプレートは `comfyGraphSD15` として書き下ろし、本体は `comfyGraphSingleCheckpoint` を
SDXL と共有する。**`case ComfyFamilySD15: return comfyGraphSDXL(...)` と使い回してはならない**:
`engine_catalog_test.go` は `comfyGraph*` の本体を正規表現で読み、`errComfyMissingFile` が
名乗るファミリーと `f.X == ""` の対から「どのファミリーがどのファイルを要るか」を組み立てている。
refusal で自分を名乗らないファミリーは、その検査が**黙って数えなくなる**ファミリーである。

### 寸法がファミリー別でなかった（こちらが本題）

`comfySizesFor` の既定は 1024 系固定で、寸法を名指さない要求も 1024×1024 に落ちていた。
5 ファミリーはどれもメガピクセル前後で訓練されているので、これで正しかった。SD1.5 の UNet は 512 で、
1024 を頼むと**失敗ではなく被写体が二重になった絵**が返る——この repo が繰り返し払ってきた、
エラーも警告も出ない「もっともらしい誤答」である。

そこで既定の一覧をファミリー別にした（`comfyDefaultSizes`）。SD1.5 は
`512x512 / 512x768 / 768x512 / 640x512 / 512x640` で、長辺を 768 で止めてある——
それを超えると二重化が始まる。他 5 ファミリーは `comfyMegapixelSizes` として今日の 5 つを
そのまま据え置き、golden も動いていない。カタログ行の `Sizes` 宣言は従来どおり
両方に勝つ。Console 側 `families.ts` にも同じ対を置いた（CP のドリフト検査の**対象外**の
コピーなので、ここは ADR が対で保つしかない）。

ファミリーを宣言していない行は**メガピクセルの一覧のまま**にした。その行はどのみち生成に届かないので
配る一覧は何も決めておらず、そこで 512 を返すのは決定 2 が禁じた「宣言されていない行について
の推測」になる。

### 推測表と VAE

`engineFamilyRules` の「SD1.5 は意図的に不在」は消して、規則を足した。SD 1.4 も同じファミリーに
寄せている（UNet・CLIP・VAE が同一で、同じグラフを通る）。🔴 **SD 2.0 / 2.1 は別物**で、
どの needle にも当たらない——OpenCLIP-H の 768 v-pred で、ここに落ちてはいけない。だから
needle は `sd1` へ短縮せず `sd15` / `sd14` と綴ってある。蒸留版（`SD 1.5 LCM` / `SD 1.5 Hyper`）
は sd15 に当たり、それで正しい。同じグラフを通るからで、共有しないのは step 数のほうであり、
それは行の `params` が宣言する。

VAE は `engineFamilyVaes` に SD1.5 の項を足した。🔴 **SDXL の VAE とは非互換**で、
取り違えると失敗せず色が崩れる。定石の `stabilityai/sd-vae-ft-mse-original` は**ゲートされて
いない**ので、sd35 をこの表から外した 403 の問題は起きず、sdxl と同じく自動で修復できる。

### 測っていないこと

🔴 **SD1.5 は GPU で一度も走らせていない。** recipe（20 steps・cfg 8・euler・normal）は
ComfyUI が v1-5-pruned-emaonly と一緒に出す既定グラフそのままで、**実測ではなく出典**である。
golden はグラフの形を固定するだけで、それが「正しい」という主張ではない——SD3.5 が
そうやって緑のまま実機で全く生成できなかった前科がそのまま当てはまる。寸法の既定も同じで、
512 が二重化を避けるという根拠は SD1.5 の訓練解像度であって、この配備で撮った絵ではない。

**残**: 実機で SD1.5 の行を 1 つ取り込み、寸法を名指さずに 1 枚生成して、被写体が
二重になっていないことを見る。それまでこのファミリーは「形は固定した」以上の主張を持たない。

## 追記 — Anima をファミリーの語彙に足した（2026-09-15）

### なぜこのファミリーで、残り 3 つではないのか

`engineFamilyRules` の裏にある計測では、Civitai の月間上位 20 のうちファミリーを持たない上流の
...
**guided** な最初のファミリーであり、だから `comfyFamilyTakesNegative` と `comfyFamilyKnobs` には
...
このファミリーは「形は固定した」以上の主張を持たない。

## 追記 — Krea 2 をファミリーの語彙に足した（2026-09-15）
...
anima の追記と正反対の向きだが（あちらはファミリーの既定が非蒸留で Turbo が宣言側）、どちらも規則は
...
### 能力の申告が「ファミリー」から「行」になった

`comfyModelTakesNegative` はファミリーだけで答えていた。Krea 2 ではそれは、このファミリーの**大半の**利用者に
...
そこで 3 つの面がファミリーではなく**行の実効 cfg** を読むようになった：
...
- 運用者の除外語が適用されなかったという警告（`comfyNegativeIgnoredWarning`。**理由**も
  ファミリーでなく行の cfg で言うようになった）

`comfyFamilyTakesNegative` は残り、意味は「このファミリーのテンプレートが negative を配線しているか」
だけになった。同じ訂正は Anima-Turbo と、cfg 1 を宣言した蒸留版 SD1.5 にも効く——この変更前は
どちらも誤って申告されていた。

### 測っていないこと

🔴 **Krea 2 はここで一度も GPU で走らせていない。** しかも語彙の中で最大のファミリーだ：
...
沿っていることを見る——汎用 `qwen3vl` での誤読は、形では見えないこのファミリー固有の壊れ方だ。

## 追記 — 追加した 2 ファミリーがピッカーから見つからなかった（2026-09-15）

2 ファミリーとも入れたのに、Console の検索からはどちらにも辿り着けなかった。原因は決定 11 の
...
## 追記 — ファミリーで絞る／表を逆から読む（2026-09-15）

ピッカーは名前でリポジトリを見つけられた。だが運用者が実際に持っている問い——**「このエンジンが
読み込める物のうち、この系統のものを見せてくれ」**——には、先にリポジトリ名を知っていないと
答えられなかった。

そこで検索が `family` を受け取るようにした。provider がディスパッチする語彙の 1 つで、取り込み
フォームのセレクタが出しているのと同じ一覧だ。🔴 これは上流ごとに翻訳され、**素通しはしない**。
どちらの API も知らない名前に**空のリスト**を返しエラーは返さない——2026-09-15 実測で
`baseModels=SD 3.5` も `baseModels=Z-Image` も 0 件——ので、素通しは打ち間違い 1 つを
「このファミリーにはモデルが無い」に変えてしまう。それは答えとして読まれる。

`engineFamilyUpstreams` がその翻訳で、値は全部 2026-09-15 に実 API で測った:

- **Civitai** はファミリーを `baseModel` の文字列で公開し、`baseModels=` は繰り返せる（実測：2 つ渡すと
  和集合が返る）。1 つのファミリーはたいてい複数の名前になる——SDXL の fine-tune はそれぞれ自分の名前を
  公開し、Klein はサイズごとに 1 つ公開する。
- **Hugging Face** にその欄は無い。あるのは他のリポジトリが付ける `base_model:<repo>` タグで、
  ファミリーごとに 1 つだけ持つ。HF は filter を AND で結ぶので 2 つ入れると「両方から派生したモデル」
  ＝何も返らないからだ。⚠️ これで見つかるのは**派生物**。本家は自分自身にタグを付けないので、
  `base_model:circlestone-labs/Anima` は `circlestone-labs/Anima` を返さない。そこへ届くのは
  ブラウズの順位と名前での検索で、それがそれらの役目だ。

**そのファミリーで絞れない提供元は拒否する**（400・ファミリーの名前と代わりの手段を書く）。sd35 と zimage は
Civitai に `baseModel` 文字列が存在しないので、これは今日パネルから踏める。そしてどちらの
代替も嘘になる——絞らないリストはそのファミリーではないし、空のリストは「存在しない」と読まれる。

### これで見つかったバグ: Pony V7 は SDXL ではない

表を逆から書いたことで、全部の名前を「実際には何なのか」と突き合わせることになり、1 つが
生き残らなかった。`engineFamilyRules` は `pony` を部分一致で拾っていて、Pony V6——SDXL の
fine-tune——には正しく、**AuraFlow で作り直された V7 には誤り**だった。V7 は UMT5 エンコーダを
持つ flow-matching DiT で、SDXL のグラフでは読み込めない。Civitai はこれを `Pony V7` という
独自の baseModel 文字列で公開しているので、推測は sdxl を先に選び、`base_model_missing` を消し、
生成時にしか落ちない行を残していた。

`engineFamilyRule` に `not`（needle より先に判定する拒否リスト）を足した。用途はまさにこれ
——**名前が構造より長生きした製品**だ。

## 追記 — Civitai のログイン制限は、拒否のほうが先に古くなっていた（2026-09-15）

P2 欠落 5 で、投稿者がログイン済みアカウントを要求する Civitai の資産に拒否を入れた。resolve が
印を付け、`can_ingest` が false になり、取り込み経路はタスクを起こす前に
`civitai_login_required` を返す。書いた時点では正しかった——コードの中の一文が
「この配備にはそれを満たせるものが無い」と言っていた。

それは **Civitai トークン**を登録できるようになった時点で真実でなくなった。アカウントは封印され、
専用のシークレットに運ばれ、fetch コンテナはそれ以来ずっと送っている
（`Authorization: Bearer $CIVITAI_TOKEN`・deploy/aws/ecs/engine-tools/ingest-fetch.sh。CFN は
`CivitaiTokenSecret` を `HF_TOKEN` と同じコンテナに配線している）。知らされていなかったのは
手前の門 2 つだけだった：`engineResolvedRow` は **Hugging Face** のトークンだけを渡されていたので、
Civitai アカウントを登録済みの配備でも、ログイン制限のある資産は必ず `can_ingest: false` に
なっていた——動いたはずのダウンロードを、パネルが起こすことを拒んでいた。

見つかったのはパネルからで、Anima のチェックポイント WAI-ANIMA。ボタンは灰色で、しかも
メッセージは「登録済みのトークン」の話を既にしていた——コードがそれを見ていないのに。

そこで、**制限ごとに、それを満たすアカウントで答える**ようにした：

- `can_ingest` は `(!gated || hf) && (!login_required || civitai)`。行は `deployment_token` の
  隣に `deployment_civitai_token` を持つ。
- 取り込み経路が拒否するのは Civitai トークンが未登録のときだけで、そう書く。
- トークンがあるときは `civitai_needs_account` が警告になる。`gated_needs_acceptance` と
  まったく同じ形で、理由も同じ：CP は**匿名で**解決する（決定 6）ので、そのアカウントが
  **この**投稿者の条件を満たすかは訊けない。早期アクセスは作者ごとに買うものだ。始めさせて、
  401 が答えならそれを答えにする。
- Console はそれを赤い行き止まりでなく muted の但し書きとして描き、何も登録していない配備には
  従来どおり強い拒否を出す。

🔴 教訓はコードではなくコメントのほうだ。**「これを満たせるものは無い」は日付付きの主張**である。
無い能力を根拠にした拒否は、その能力を名指しして書いておかないと、能力を足す作業と拒否を
見直す作業が別々になってしまう。

## 追記 — 分割ファミリーを「1 回の操作」で取り込む（2026-09-15）

🔴 **[ADR 0085](0085-model-ledger-and-one-press-ingest.ja.md) で置き換え（2026-09-15、同日に実装）。** `with_family_parts` と `POST …/models/{id}/parts` はワイヤから消えた。resolve がファミリーの読む全ファイルを載せた**計画**を返し、行の救済は `…/complete`（揃える）1 つ。部品表が何のために書かれたかの記録として残す。

Anima を入れるのに**取り込みを 3 回**——しかも誰も書いていない順番で——必要で、3 つ目が着地する
まで行は印が付いたままだった。部品は別のリポジトリにあり、Civitai のマージなら**別のソース**に
すらあるので、運用者が見ている画面からそこへ辿る道が無い。Sandbox から
「いろいろ取り込んだが、どうしていいか分からない」として報告された。

この仕掛け自体は既にあり、問題の半分だけを向いていた：`engine_vae.go` は VAE を持たない
チェックポイントの隣にファミリーの VAE を既定でチェック済みで出し、既存の行にはワンプッシュの修復まで
用意している。そこで止めた理由も書いてある——分割ファミリーは「もともと `--vae` が必須なので、無い行は
既に印が付いて拒否されている」。正しい。ただし**印が前進の手段である**と仮定していた。部品が
リポジトリ 2 つ先にあるファミリーでは、そうではない。

そこで同じ形を分割ファミリーの部品にも広げた：

- **`engineFamilyParts`** がファミリーごとに宣言する（flag・リポジトリ・パス・着地する S3 キー）。
  2026-09-15 実測、gated でないことと両方のパスが取れることを確認済み。
- 取り込みフォームがセットを**既定でチェック済み**で出し（`with_family_parts`）、各部品の
  サイズとライセンスを隣に置いて、同じ押下の追従取り込みとして入れる。
- **`POST …/models/{id}/parts`** が既存の行の修復になる——「4 GB の拡散モデルを、今度はチェックを
  入れてもう一度取り込む」は修復ではないからだ。
- 登録済みカードが `files_missing` を描くようにした。ワイヤは P2 から運んでいたのに、この画面が
  一度も出していなかった——使えない行が他の行と同じ見た目だった。

### 配備が既に持っているバイトは、宣言するだけで二度と取りに行かない

安い順に 3 段：

1. **どれかの行がそのキーを宣言している** — 配備自身の宣言として信用する。S3 も叩かない。
2. **行は無いが、バイトはある** — 消された行のために完了したジョブ、別エンジンで入れた同じ
   ファイル。`reuse_s3_key` と同じ**2 つの証明**を要求する：アップロード前に記録した identity が
   いま resolve した identity と一致すること、そして HeadObject でいま実体があること。
   記録だけなら消されたキーを指し、実体だけなら出所を言えない。
3. それ以外はダウンロード。

🔴 **2 ファミリーは Qwen-Image VAE を 1 つのリポジトリから宣言している。** 同じファイルが
`circlestone-labs/Anima`・`Comfy-Org/Krea-2`・`Comfy-Org/Qwen-Image_ComfyUI` にある（sha256
a70580f0…・実測）が、再利用の判定はハッシュではなく `hf:<repo>@<rev>/<path>#sha256:…` の
identity なので、各々の出所から宣言すると**同じバイトを 2 回落として 2 キーにする**。Anima →
Krea 2 の順に入れると、エンコーダ 1 本のダウンロードだけで VAE は 0 になる。

### 部品はモデルではない

取り込みは `新規` + `file_flag` を受け付けていて、テキストエンコーダだけの行が作れた。登録済み
一覧にモデルとして並び、有効化は永久にできず、運用者が本当にやりたかったこと——チェックポイントの
行に**エンコーダを付ける**——は起きていない。これは拒否するようにし、本来の操作の名前を返す。
⚠️ 既にそうやってできた行は移行しない：`忘れる` で消し、チェックポイント側の行を上のボタンで
揃える。

### 対象外

⚠️ `flux1` / `flux2-klein` / `sd35` / `zimage` も分割ファミリーだが**部品表はまだ無い**。この 2 ファミリーのように
実測していないからで、記憶で書いた項目は押下の数分後の 404 になる。従来どおり、印が付き、手で
付ける。

🔴 どちらのファミリーも GPU では未走なので、「行が揃った」は今も**宣言が揃った**という意味にとどまる。

## 追記 — 分割ファミリーの「既定の選択肢」が唯一動かない選択肢だった（2026-09-15）

🔴 **[ADR 0085](0085-model-ledger-and-one-press-ingest.ja.md) で置き換え（2026-09-15、同日に実装）。** フォームはもう役割を訊かない。`main_flag` は計画の中の CP の答えで、`file_flag` は要求から消えた。

上の追記を入れた直後、af-sandbox の実機で 3 つ出た。どれも部品表の話ではなく、**その手前**の話だった。

1. 🔴 **本体すら役割が付いていなかった。** `anima-aesthetic-v1.1` は「チェックポイント（単一
   ファイル）」として登録されていた——フォームが最初に出す選択肢がそれだからだ。分割ファミリーは
   `engineComfyRequiredFlags` に `""` を持たない＝**どのテンプレートも読まない役割**なので、
   4.2 GB のファイルを抱えた行が「`--diffusion-model` `--clip_l` `--vae` が不足」と言う。
   自分が持っているものまで不足に数える。
   → 新規の行で、ファミリーが whole checkpoint を読まないなら**拒否**し、**その役割を名指しする**。
   resolve は `family_main_flag` を返し、フォームの役割セレクタはそこから始まる。
2. 🔴 **部品が単独の行として並んでいた**（エンコーダ・VAE が「モデル」として 3 行）。これも
   同じ原因で、役割を付けずに新規で取り込めば行になる。
3. 🔴 **「S3 キーは既に記録されています」で行き止まりになった。** 部品の計画が、キーを
   押さえているものと identity を突き合わせられないと**ダウンロードに落ちていた**——その
   ダウンロードは `engineIngestDestinationUnused` に必ず拒否される。計画がそこへ進むこと自体が
   誤りで、いまは**何がキーを押さえているか**（行の id／実行中のジョブ／過去のジョブ）を返す。

教訓は 1 つで、3 つに共通する：**既定の選択肢が、そのファミリーでは唯一動かない選択肢だった。** 分割ファミリーの
知識（どの役割を読むか）は CP にあり、画面はそれを訊いていなかった。

## 追記 — ファイルは「不足」していない。ローダーが一覧しないディレクトリに居ただけだった（2026-09-15）

🔴 **[ADR 0085](0085-model-ledger-and-one-press-ingest.ja.md) で置き換え（2026-09-15、同日に実装）。** `s3Key` は要求から消え、鍵は CP が計算する（0085 決定 1）。この追記が足した宛先検査は到達不能になり、`main_file_fix` は行の属性でなく、S3 Bucket の台帳（`GET …/objects`）を読む `…/complete` の 1 ケースになった。

上の追記は新規の行の扉を閉じた。**既に配備にある行はそのまま**で、しかも目の前の印と remedy が
矛盾していた——カードは `不足: --diffusion-model` と出しながら、その 4.2 GB を行自身が持っており、
「不足ファイルを揃える」は **`none`**（不足はありません）と答えた。ファミリーの部品表（エンコーダと VAE）は
本当に揃っていたからだ。

**取り込みフォームの 1 行の、両側**がそうさせていた。直っていたのは片側だけだった。

```ts
s3Key = engineIngestPrefix(image, fileFlag, isLora) + file
```

1. 前置きは**役割**から引かれる。既定は「チェックポイント（単一ファイル）」＝ `image/checkpoints/`。
2. `file` は**上流リポジトリ内のパス**で、これらのファミリーは `split_files/…` 以下に公開されている＝
   そのディレクトリごとキーに入る。

🔴 **どちらか一方だけでも、どの ComfyUI ローダーも出せないファイルになる。** インスタンスは S3 Bucket をそのまま
写し（`/ComfyUI/models` → `/models/image`）、各ローダーは**1 つのディレクトリ**から一覧を作り、Agent は
ファイルを**ベース名**で名指す。つまり `image/diffusion_models/split_files/…` は `image/checkpoints/…`
と同じくらい読めない——**役割を正しく選んでいても踏む**方の罠だ。どこにも報告されない：行は通り、
インスタンスは起動し、コールドスタートの後に「一覧に無い名前」でノードが落ちる。

### 直し方は「移設」であって 2 度目のダウンロードではない

取り込みタスクの `MODE=move`（contract 3）。**同一 S3 Bucket 内の `aws s3 mv` 1 回**で、S3 がサーバ側で
（大きければ分割して）コピーする。取得は無し、タスクのディスクも使わず、ライセンスも再度は訊かない
——このバイト列は取り込み時に受諾済みだ。カタログ側の変更（`MoveEngineModelFile`：1 ファイルの
**役割とキー**だけを書き換える）は、他の取り込みと同じくタスク着地時に入る。13.1 GB を 1 階層
上げるために取り直すのは、部品表を書いたときに既に拒んだ答えそのものだ。

タスクを起こす前に拒む条件は、どれも「バイト列を失う経路」だから：他の行が宣言しているキー
（その行の宣言が空のキーを指し続ける）、取り込みが書いている最中のキー、既に記録済みの宛先、
そして**実体が無い**キー（その行は取り込み直すしかなく、拒否文がそう言う）。

### 併せて直した 2 つ

- **宛先を入口で検査する。** 取り込みは、その役割に対して
  `<role>/<役割のディレクトリ>/<ベース名>` でないキーを拒否する。`POST …/models` は**あえて対象外**：
  既にどこかにあるバイト列を登録する経路は復旧の道であり、そこに規則を置くと後ろの扉を閉める。
- 🔴 **分割ファミリーは、そもそも新規に取り込めなくなっていた。** 前の追記が「役割を宣言せよ——ほぼ確実に
  `--diffusion-model` だ」と無印を拒否し、その隣の規則が「フラグ付きはモデルではなく**部品**だ」と
  拒否する。2 つが噛み合い、助言の行き先が別の拒否だった。いまは**ファミリーの本体フラグだけ**が部品規則の
  例外で、エンコーダを単独の行にするのは従来どおり拒否される（それがこの規則の目的）。

### 追記の続き — テーブルはリビジョンを載せ、CP はそれを 1 度しか読まなかった（2026-09-15）

上の移設を af-sandbox に反映したところ、取り込みが軒並みこうなった。

```
InvalidParameterException: TaskDefinition is inactive
```

エンジンテーブルの**両側**に 1 つずつ、どちらも取り込みの話ではない。

1. 🔴 **テーブルが `!Ref IngestTaskDef`＝リビジョン付き ARN を載せていた。** タスク定義に触る
   CFN の更新は**新しいリビジョンを登録し、前のリビジョンを登録解除する**ので、固定された値は
   `RunTask` が拒む値になる。いまは**ファミリー名**を載せる（呼び出し時に最新の ACTIVE に解決
   される）。文字列にすると順序が消えるので `EnginesParam` に `DependsOn: IngestTaskDef` を足した。
2. 🔴 **CP は ingest ブロックを起動時に 1 度読んだきりだった。** `startIngest` は最初に宣言が
   来たときにランナーを作り、以後は storage チェッカーを差し替えるだけ＝タスク定義・サブネット・
   SG・ロググループ・シークレットは**起動時の値のまま**。ランナー自体は今も作り直さない
   （走っている reconcile ループがジョブの持ち主だから）が、リロードのたびにブロックを**採り直す**。

どちらか一方でもエラーは消えるが、壊れ方が別方向なので両方直した。1 は「そもそも固定してはいけない
値だった」、2 は「変わる値を 1 度しか読んでいなかった」——後者は S3 Bucket 名にもシークレットにも
等しく当てはまっていた。

### 追記の続き 2 — 失敗した試行の残骸が、その再試行を締め出していた（2026-09-15）

🔴 **[ADR 0085](0085-model-ledger-and-one-press-ingest.ja.md) で置き換え（2026-09-15、同日に実装）。** ここで書いた押さえ手の規則（#691）は 0085 決定 6 の「押さえ手＝オブジェクトか、オブジェクトを書き得たタスク」の定義になり、この領域の拒否はすべて `holder` と `next` を文でなくフィールドで運ぶ。
