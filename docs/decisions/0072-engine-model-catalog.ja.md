...
### P2 の残り（族ごとの image-to-image）も実機で通った——10/10
...
1 つのレール項目に 2,991 行が入っていて、GPU のモード・箱・段・稼働履歴と、カタログ・
...
カタログ行に `params` 列（JSON・1 列）を足し、comfy の 5 テンプレートが族の recipe に
**フィールド単位で**被せる。`steps` だけ宣言した行は族のサンプラーを保つ。sampler /
scheduler は Agent が知らない名前なら**無視して族の値を使う**——ComfyUI の enum に無い値は
コールドスタートの後に `Value not in list` で全滅するので、「族のサンプラーで絵が出る」と
「絵が出ない」の二択なら前者が正しい。

🔴 **flux1 と klein は cfg を受けない。** あの 2 族の「CFG」は FluxGuidance / 蒸留パスの
...
ネガティブプロンプトは**サンプラーへの傾きであって門ではない**。そのうえ、5 族のうち
それを持つのは 2 族だけである。
...
したがって「管理者の列挙が黙って効かない」状態は実在し、それは報告する: この 3 族が答えた
要求には、モデル自身のネガティブと配備の除外語が適用されなかったこと、そして族の名前を
...
届くこと、合成が空ならグラフがゴールデンとバイト単位で同じままであること、枝を持たない族が
...
要求する KV キャッシュ 16 GB も載せられない。**Console にその欄が無い。** 箱を買い、重みを同期
...
Illustrious/SDXL のチェックポイントで、ファミリーも宣言され、その族が要求するファイルも
持ち、`engineMissingFileFlags` を通り、有効にでき、`generate_image` の `model` にも現れる
——そして箱がチェックポイント切り替え（1〜2.5 分）を払ったあと、**すべての**要求が
...
- `engineVaeGuard` が有効化を拒む（`engine_vae_missing`。`files_missing` と同じく、
  ...
  別の入手経路を持つテンプレートも無い。
- 行の 1 ボタンが族の VAE を取り込んで `--vae` として足す（`POST …/models/{id}/vae`）。
...
🔴 **sdxl には既定の族 VAE（`stabilityai/sdxl-vae`）があり、sd35 には意図的に無い。**
...
（`VAELoader.vae_name` は箱の `models/vae` の列挙なので、持たない名前を書くと今動いて
いる族が壊れる）と、カタログ側で `--vae` を必須にすること（同梱の行が全部
「ファイル未設定」になる）。
...
それでも族を分けたのは **LoRA の族ゲート**（`comfyResolveLoras`）のためである。
...
名乗る族と `f.X == ""` の対から「どの族がどのファイルを要るか」を組み立てている。
refusal で自分を名乗らない族は、その検査が**黙って数えなくなる**族である。

### 寸法が族別でなかった（こちらが本題）

`comfySizesFor` の既定は 1024 系固定で、寸法を名指さない要求も 1024×1024 に落ちていた。
5 族はどれもメガピクセル前後で訓練されているので、これで正しかった。SD1.5 の UNet は 512 で、
1024 を頼むと**失敗ではなく被写体が二重になった絵**が返る——この repo が繰り返し払ってきた、
エラーも警告も出ない「もっともらしい誤答」である。

そこで既定の一覧を族別にした（`comfyDefaultSizes`）。SD1.5 は
`512x512 / 512x768 / 768x512 / 640x512 / 512x640` で、長辺を 768 で止めてある——
それを超えると二重化が始まる。他 5 族は `comfyMegapixelSizes` として今日の 5 つを
...
族を宣言していない行は**メガピクセルの一覧のまま**にした。その行はどのみち生成に届かないので
配る一覧は何も決めておらず、そこで 512 を返すのは決定 2 が禁じた「宣言されていない行について
の推測」になる。

### 推測表と VAE

`engineFamilyRules` の「SD1.5 は意図的に不在」は消して、規則を足した。SD 1.4 も同じ族に
...
「この族にはモデルが無い」に変えてしまう。それは答えとして読まれる。

`engineFamilyUpstreams` がその翻訳で、値は全部 2026-09-15 に実 API で測った:

- **Civitai** は族を `baseModel` の文字列で公開し、`baseModels=` は繰り返せる（実測：2 つ渡すと
  和集合が返る）。1 つの族はたいてい複数の名前になる——SDXL の fine-tune はそれぞれ自分の名前を
  公開し、Klein はサイズごとに 1 つ公開する。
- **Hugging Face** にその欄は無い。あるのは他のリポジトリが付ける `base_model:<repo>` タグで、
  族ごとに 1 つだけ持つ。HF は filter を AND で結ぶので 2 つ入れると「両方から派生したモデル」
...
**その族で絞れない提供元は拒否する**（400・族の名前と代わりの手段を書く）。sd35 と zimage は
Civitai に `baseModel` 文字列が存在しないので、これは今日パネルから踏める。そしてどちらの
代替も嘘になる——絞らないリストはその族ではないし、空のリストは「存在しない」と読まれる。
...
## 追記 — 分割族を「1 回の操作」で取り込む（2026-09-15）

🔴 **[ADR 0085](0085-model-ledger-and-one-press-ingest.ja.md) で置き換え（2026-09-15、同日に実装）。** `with_family_parts` と `POST …/models/{id}/parts` はワイヤから消えた。resolve が族の読む全ファイルを載せた**計画**を返し、行の救済は `…/complete`（揃える）1 つ。部品表が何のために書かれたかの記録として残す。
...
チェックポイントの隣に族の VAE を既定でチェック済みで出し、既存の行にはワンプッシュの修復まで
用意している。そこで止めた理由も書いてある——分割族は「もともと `--vae` が必須なので、無い行は
既に印が付いて拒否されている」。正しい。ただし**印が前進の手段である**と仮定していた。部品が
リポジトリ 2 つ先にある族では、そうではない。

そこで同じ形を分割族の部品にも広げた：

- **`engineFamilyParts`** が族ごとに宣言する（flag・リポジトリ・パス・着地する S3 キー）。
...
🔴 **2 族は Qwen-Image VAE を 1 つのリポジトリから宣言している。** 同じファイルが
...
⚠️ `flux1` / `flux2-klein` / `sd35` / `zimage` も分割族だが**部品表はまだ無い**。この 2 族のように
実測していないからで、記憶で書いた項目は押下の数分後の 404 になる。従来どおり、印が付き、手で
付ける。

🔴 どちらの族も GPU では未走なので、「行が揃った」は今も**宣言が揃った**という意味にとどまる。

## 追記 — 分割族の「既定の選択肢」が唯一動かない選択肢だった（2026-09-15）
...
教訓は 1 つで、3 つに共通する：**既定の選択肢が、その族では唯一動かない選択肢だった。** 分割族の
知識（どの役割を読むか）は CP にあり、画面はそれを訊いていなかった。

## 追記 — ファイルは「不足」していない。ローダーが一覧しないディレクトリに居ただけだった（2026-09-15）

🔴 **[ADR 0085](0085-model-ledger-and-one-press-ingest.ja.md) で置き換え（2026-09-15、同日に実装）。** `s3Key` は要求から消え、鍵は CP が計算する（0085 決定 1）。この追記が足した宛先検査は到達不能になり、`main_file_fix` は行の属性でなく、バケツの台帳（`GET …/objects`）を読む `…/complete` の 1 ケースになった。

上の追記は新規の行の扉を閉じた。**既に配備にある行はそのまま**で、しかも目の前の印と remedy が
矛盾していた——カードは `不足: --diffusion-model` と出しながら、その 4.2 GB を行自身が持っており、
「不足ファイルを揃える」は **`none`**（不足はありません）と答えた。族の部品表（エンコーダと VAE）は
本当に揃っていたからだ。
...
2. `file` は**上流リポジトリ内のパス**で、これらの族は `split_files/…` 以下に公開されている＝
   そのディレクトリごとキーに入る。

🔴 **どちらか一方だけでも、どの ComfyUI ローダーも出せないファイルになる。** 箱はバケツをそのまま
...
箱は起動し、コールドスタートの後に「一覧に無い名前」でノードが落ちる。

### 直し方は「移設」であって 2 度目のダウンロードではない

取り込みタスクの `MODE=move`（contract 3）。**同一バケツ内の `aws s3 mv` 1 回**で、S3 がサーバ側で
...
- 🔴 **分割族は、そもそも新規に取り込めなくなっていた。** 前の追記が「役割を宣言せよ——ほぼ確実に
  `--diffusion-model` だ」と無印を拒否し、その隣の規則が「フラグ付きはモデルではなく**部品**だ」と
  拒否する。2 つが噛み合い、助言の行き先が別の拒否だった。いまは**族の本体フラグだけ**が部品規則の
