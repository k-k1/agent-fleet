...
> ✅ **欠落（実機・2026-09-10・別レーンの観測）: gated の拒否は 1 種類ではない。** 同じ
> トークンで FLUX.1-dev は通り、SD3.5 Medium は取り込みタスクが
> `curl: (22) The requested URL returned error: 403` の 1 行で落ちた。利用者が HF で条項に
> 同意してから再試行すると通っている。**401 と 403 は直す場所が正反対**——401 はトークンが
> タスクに届いていない、403 は届いたうえでそのアカウントが**そのリポジトリの**条項に未同意
> ——なのに、パネルはどちらも curl の 1 行しか出していなかった。ジョブ行が
> `gated_no_token` / `gated_not_accepted` を名乗るようにし（`engineIngestFailureCode`。
> Civitai 由来のものは `civitai_login_required`）、Console が ja / en で行き先の違う文を
> 出す。`resolve` 側は**コードを出せない**——CP は匿名で調べるので、登録済みトークンの
> アカウントがそのリポジトリに同意しているかを知る術が無い——ので、gated ＋トークン有りの
> ときに `gated_needs_acceptance` の**注意**を先に出すところまでにした。試験は 401 と 403 の
> 両方（片方だけだと分岐の無い実装でも緑になる）。
