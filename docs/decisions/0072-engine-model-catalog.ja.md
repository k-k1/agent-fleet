...
12. ~~**HF トークンを Console から登録できるようにする**~~ **決まった（レビュー 2026-09-09 の
    ...
    （`PutSecretValue` だけ・`GetSecretValue` は与えない）で縛る。** 実装は P5 で、着手条件は
    **`60-engines.yaml` の余白を資源 1 つ分空けること**。以下は当時の記述:
...
**次の一手は条件 3**——`--no-mmap` の有無で llama.cpp にマウント上の GGUF を読ませ、91 秒と
比べる。それが済むまでは**有望な候補であって結論ではない**。
