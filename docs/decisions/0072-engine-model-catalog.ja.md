...
- 状態: **P0・P1・P4 実装済み・実機検証済み（2026-09-08〜09）。P2・P3・P5 は未着手。** 起草・レビュー・改訂・
...
## 未解決の点——P2 の前に 8・9 を測る（10 は別セッション。2 と 6 は sd-server の配備が要るときだけ）
...
10. **モデルの置き場そのものを問い直す（P4 の実機検証中に利用者から出た。別セッションで
    進める）。** 出発点は「同期が毎回のコールドスタートを支配している」という実測で、
...
11. **テナント軸をどこに入れるか（同上・別セッション）。** カタログの主キーは `(role, id)` で
...
12. **HF トークンを Console から登録できるようにする（同上・別セッション）。** いまは
