...
- 同日、**P1（llm 役のルーターモード）を実装し、CPU と実機で測った**（「P1 の実測」節）。
  ...
  **1 つ目と 3 つ目が実機で通り**（同じ箱で 2 つの GGUF が使え、交替のリロードが 1 回の試行で
  答えを返す）、2 つ目（起動メニューに 2 つ）は単体テスト止まり——行を作るのが super_admin の
  画面だからで、P0 と同じ壁である。🔴 本文を 3 か所直した——**`--models-dir` は使わない**
...
3. **llm 役はルーターモードで動かし、モデルは要求の `model` で選ぶ。窓はモデル毎になる。**
   ...
   - **未解決 1 の 4 点はレビューが CPU で解いた（R1）**: 空の `--models-dir` で起動して
     ...
     **生成中**のモデルをどう扱うか。それは P1 の完了の定義（交替のリロード）と同じ観測である。
     ...
     待ち）。だから `--models-max 1` の代償は「答えが壊れる」ではなく「待つ」だが、
     **非ストリーミングの要求はその待ちで ALB に切られる**（0071 P1 実測 9 の 60 秒）。
...
**完了の定義は 1 つ目と 3 つ目が実機で通った**——`llamacpp/` のモデルが 2 つ載り、片方ずつ
使え、交替のリロードが 1 回の試行で答えを返した（下の 7）。**2 つ目（起動メニューに 2 つ出る）
はカタログ→Agent→opencode の連鎖を単体テストで固定するに留めた**——行を作るのは super_admin の
画面で、AWS の資格情報では駆動できない（P0 の実測 5 と同じ壁。利用者の判断で、実機は
エンジン層までとした）。
...
- **P1 — llm のルーターモード。実装済み・実機検証済み（「P1 の実測」）。** preset 生成
  ...
  完了の定義と同じ観測を交替で行う）。**1 つ目と 3 つ目は実機で通り**（同じ箱で 2 つの GGUF が
  0.4〜0.7 秒／10 秒／276〜282 秒で答えた）、**起動メニューの 2 つは単体テスト止まり**——行を
  作るのが super_admin の画面で、AWS の資格情報では駆動できない。
