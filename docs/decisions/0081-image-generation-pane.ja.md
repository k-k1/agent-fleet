...
- **Agent のルート**と、CP のリストに載せる 6 行（グループとキューの操作は決定 12）:
...
- **wire。** `GET /imagegen/jobs` に `groups[]`（`id`・`label`・`state` ∈ `running | paused | done |
  ...
  `POST /imagegen/queue` が決定 2 のリストに足す 2 ルート——中継は 4 行でなく 6 行。
...
- **Agent**（`workspace/agent`）: `internal/imagegen/` に `jobs.go`（キュー・ワーカー・グループ・EMA・
  ...
  pause / skip。`routes.go` に 6 ルート
  （`testdata/routes.golden` が動く——ADR 0080 と違いここでは想定内）。`usage_series.go` が
  `Images` / `Pixels` を畳む。
- **CP**: `routes.go` の中継 6 行。`engine_models.trained_words`（両方言の移行）を取り込みが書き、
