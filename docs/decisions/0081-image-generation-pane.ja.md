...
- 状態: **受理**（2026-09-13 起草、同日のレビューで訂正を反映して受理。レビューは以下の「ある」「無い」の
  ...
  PNG チャンクが実際に与えるファミリーと seed、サムネイルの小ファイル例外、ライトボックスの `path`、行番号 5 件）。
  **P0 は同日に #625 / #626 / #627 で実装**（GPU 無し）。実装後の wire は末尾の「P0 の実装」、実機 1 回はまだ負っている。
  状態の更新（2026-09-24）: P1 は列挙どおりには作らない。ADR 0100 が向きを変えた——スイープは ADR 0100 の P2 へ移り、`POST /imagegen/suggest` はセッションのエージェントが直す下書きに置き換わる。ギャラリーの「画像生成で開く」は作った。残り（プリセット、キューの記録、ステップごとの進捗とプレビュー、ほか）は求められるまで作らない。
- Follow-ups: #956
- 関連: [0069](0069-image-generation-providers.ja.md)（このペインが駆動する provider 抽象。
  未解決 1 でジョブ形を先送りにした——本 ADR がそれを引き取る） /
  [0072](0072-engine-model-catalog.ja.md)（カタログ行・`params`・`negative_prompt`・comfy の 5 ファミリー） /
  ...
  [0071](0071-self-hosted-inference-engines.ja.md)（エンジンのインスタンス・コールドスタート・誰が払うか）
...
- **絵を作るものは全部 `workspace/agent/internal/imagegen/` にある**: `Provider` インタフェースとコア
  ...
  `flux1` / `flux2-klein` / `zimage`）、ファミリーの検査と重み付きの LoRA 解決（`comfy.go:303-350`）、
...
- **要求語彙**（`imagegen.go:56-125`）: `op`・`prompt`・`negative_prompt`・`size`・`aspect_ratio` と
  ...
  `params`（ADR 0072・`EngineParams`）がファミリーの recipe にフィールド単位で被さる（`comfy_workflows.go:174-195`）。
  ADR 0069 はこれを意図して MCP ツールの外に置いた（「provider 差が大きすぎる／摘んでも動かない経路が
  ある」）。その理由は**エージェントが見るツール**の話で、フリート自身の provider しか持たないペインを
  縛らない。
- **ファミリーが読むもの**（`comfy_workflows.go`・括弧は recipe の既定）:

  | ファミリー | steps | cfg | sampler | scheduler | ネガティブ |
  |---|---|---|---|---|---|
  ...
  リストに無ければ `comfyRecipe.with` は recipe の名前を保つ）。インスタンスが知らない
  名前はコールドスタートの後に `Value not in list` で落ちるので、Agent の許可リストが契約である。
  蒸留ファミリーへのネガティブは警告付きで落とされる（`comfy.go:187-204`）。Console はこの表を知らない。
...
- **ディスクに由来が無い。** Agent は PNG のバイトをそのまま書き、サイドカーは書かない（`store.go`）。
  ComfyUI の `SaveImage` はインスタンスが `--disable-metadata` で無い限り API グラフを `prompt` テキストチャンクに
...
- **メンバー向けカタログが無い。** 管理画面（`console/src/features/settings/admin/adminEngines.tsx`・
  ...
  ——ファミリーも sizes も `params` も行のネガティブも無い（`http.go:61-99`）。
...
- **上流（ComfyUI の `server.py`・2026-09-13 に確認）:** `POST /interrupt` は `{"prompt_id"}` 付きなら
  その 1 件だけ、無しならインスタンス全体を止める。`POST /queue {"delete":[id]}` で待機中を外せる。`GET /queue`
...
- グラフ組み立てを CP に持ち上げる案は、存在しないブラウザ側のエンジン資格情報、`engine_catalog_test.go`
  が Agent のソースを読んで一致させているファミリーディスパッチの 2 つ目の写し、ギャラリーが見られない 2 つ目の
...
- **Agent のルート**と、CP のリストに載せる 7 行（グループとキューの操作は決定 12・`props` は決定 3）:
  - `POST /imagegen/jobs` — 本文は既存の `generateRequest` から **`session` を除き**（ペインにセッションは
    ...
    ——共有のインスタンスに 1 人が 1 日分の GPU をうっかり並べられないように。
  - `GET /imagegen/jobs` — Agent がまだ覚えているジョブ全部（待機・実行中・完了の直近 500）を新しい順に。
    ...
    `position`、`started_at`・`finished_at`・`elapsed_ms`、解決済みの要求（model・ファミリー・seed・実効
  ...
  - `DELETE /imagegen/jobs/{id}` — 取消。待機中なら外す。実行中なら provider の任意インタフェース
    ...
    インスタンスはワークスペース間で共有で、素の interrupt は他人の絵を殺す。`sdcpp` に取消は無い。走り切って
    結果を捨てる。
  - `GET /imagegen/status` — 拡張する（決定 5）。
- **provider ごとにワーカー 1 本、ジョブは 1 つずつ。** インスタンスは既にサンプリングを直列化しており、先に投げても
...
- **サイドカー:** 各画像の隣に Agent が `<name>.json` を書く——解決済みの要求（プロンプト・合成後の
  ネガティブ・model id・ファミリー・実際に使った seed・実効 `params`・重み付き loras・size・op・strength・
...
- **絵のプロパティの表示とコピーは本 ADR の物で、0080 の物ではない**（2026-09-13 にギャラリーの
  ...
  1. **comfy 経路の PNG には既に `prompt` キーの `tEXt` チャンクがある**（1 枚 1,550〜1,716 バイト。この
     ...
     **唯一**の経路。走査で分かり、初稿が推測で書いていた 2 点 *（レビュー）*: グラフのノード id は
     ...
     なのでファミリーは推定でなく**読める**。バッチの PNG が持つのは**基底** seed と `batch_size` で、その絵自身の
  ...
  プロンプトはサンプラーの `positive` 入力に繋がる `CLIPTextEncode`、ファミリーは `filename_prefix` から `af-` を
  ...
  *（レビュー）*、任意の `path` prop——ギャラリー・ミラーのファイルカード・このペインが皆持つ browse-root
  ...
  出会う唯一の場所: 「プロパティ」の切替で解決済みの欄（model・ファミリー・seed・size・steps・cfg・sampler・
...
### 決定 4 — 要求にカタログと同じ形の `params` を被せる。何を読むかはファミリーが決め、言葉で言う
...
- `Request.Params *EngineParams`（`steps`・`cfg`・`sampler`・`scheduler`。`clip_skip` と `weight` は要求
  ...
  合成順は **ファミリーの recipe ← カタログ行 ← 要求**、フィールド単位、既存の `comfyRecipe.with` で。
- **MCP ツールに `params` は付けない。** ADR 0069 の理由はエージェントに対して立つ。ペインの provider は
  ...
  - 🔴 **2026-09-15 に撤回した**（ADR 0069 の同日の追記）。この行が引いている「ADR 0069 の理由」の半分
    ——「摘んでも何も動かない」——は、下の `comfyFamilyKnobs` を数えれば偽である: **7 ファミリーすべてが
    `steps` と `sampler` を読む**。ツールは入れ子の `params` を受け、読まない 2 つはこの決定が作った
    警告がそのまま名指しする。**この決定の残り（検証・警告・ファミリーごとの表）は全部そのまま効いている**
    ——ブロッキングルートにも `validateRequestParams` を足したので、2 つの経路の厳しさが揃った。
- **Agent が検証し、コールドスタートの後にインスタンスが不正値を見ることは無い。** sampler / scheduler は
  ...
  ピクセル上限（`imagegenMaxPixels`・4 M——`l4` での SDXL 2048² は 5 分待った後の OOM で、インスタンスの 400 は
  `engine_waking` ですらない裸の 400 で戻る）。
- **ファミリーが無視する物は報告し、飲まない。** `flux1`・`flux2-klein` への `cfg`、`flux2-klein` への
  `scheduler`、蒸留 3 ファミリーへのネガティブは、ネガティブが既にそうしている通りジョブに警告を出す。
...
- モデルごとに `family`（行の `base_model`）、`sizes`（行か既定 5 つ）、`params`（recipe ← 行の
  **実効**既定。フォームのプレースホルダが「実際に走る値」になる）、`negative`（行の物。利用者が外せない
  固定チップとして出す——管理者の物であり、`negative_always` も同様に出す）、`knobs`（ファミリーが読む
...
- **ファミリーのカード。** 選んだモデルのファミリーについて折り畳み 1 行で: 方言（`sdxl` とその Pony / Illustrious /
  ...
  ネガティブがこのファミリーに届くか、推奨 steps / cfg の範囲、サイズのプリセット。ファミリー 5 つ・Console の i18n
...
- **管理者のネガティブと行の `params`** はフォームの固定部として出所付き（「管理者がこのモデルに
  宣言」）で出し、見えないところで混ぜない。
- **sampler と scheduler** は Agent の一覧からの select。ファミリーが読まない物は理由付きで無効表示。

**層 B——モデルを 1 回、ボタンで、着地前にプレビュー。**

- 「プロンプトを書いて」は Console が組んだメッセージ 1 通を `askAssistant()` に送る: ファミリーのカード、
...
- **なぜテナントの `llm` エンジンでなく `api/chat/ask` か。** ブラウザは `/engine/llm/*` に届かず
  （資格情報が無い）、その役が無い配備もあり、冷えた llama.cpp のインスタンスは一文のために数分の GPU を使う。
...
- `edit` と `inpaint` は既に `inputs[]` を browse-root パスで取る。ペインは: ギャラリーから選ぶ
  ...
  🟢 **「初日から動く」ほうを 2026-09-21 に入れた**（`op=inpaint` のときだけ出る 1 本のパス欄。
  参照画像と同じ `InputPicker` を `max=1` で使い回す）。**塗るキャンバスは P2 のまま。**
  🔴 それまで穴だった: `model.ops` をそのまま選択肢に出す一方で `mask` を組み立てる場所が
  Console のどこにも無く、`inpaint` を選ぶと Agent が「マスクが要る」で断った——キュー経路では
  それが**失敗したジョブ**として届くので、直すべき欄の横ではなくジョブ一覧で読むことになる。
  だからマスクが無いうちは投入も試しも**押せない**ようにしてある。
...
- **steps はファミリーの試走値**でフォームの値ではない——`sdxl` 10・`sd35` 12・`flux1` 8・`zimage` 4・
...
40 枚は温かいインスタンスでも 15 分かかる。見ている人は、どこまで来たかを知り、作った物を失わずに止められる
...
- **進捗は P0、WebSocket 無しで。** Agent はグループごとに `done`・`failed`・`total`・`running`
  ...
  `typical_ms`、インスタンスが冷えていれば観測済みの起床時間を足す）を返す。ペインはグループごとにバー 1 本
...
- **中断**（`{"op":"cancel"}`）は実行中を同じく中断し、そのグループの待機中を全部外す。できた絵は
  ...
  `DELETE` が残る。
- **止めたバッチはインスタンスを冷やす。** エンジンのアイドル窓は管理者の物でこのペインの物ではない。窓を
...
- **CP でグラフを組み、ブラウザにエンジン資格情報を渡す。** 存在しない資格情報、ファミリーディスパッチの
...
- **Console から生の ComfyUI グラフを投げさせる**（「カスタムワークフロー」）。中継は運ぶ——CP は設計上
  ...
  ファミリー検査に意味を与える契約である。上級者向けのワークフロー機能は方針の問いを伴う別の ADR。
...
- **素の `POST /interrupt`。** 共有のインスタンスで他のワークスペースの絵を殺す（決定 2）。
...
- **CP**: `routes.go` の中継 7 行。`engine_models.trained_words`（両方言の移行）を取り込みが書き、
  ...
  素通しのパスが 2 つ増えるだけ。
- **Console**: `features/imagegen/`（ビュー・フォーム・ジョブ一覧・ファミリーのカード・プロンプト支援モーダル・
...
- **新しい依存は無い。** スライダー・select・ライトボックス・サムネイル・一発実行のアシスタント呼び出し・
  アップロード経路は全部ある。
- 試験: 純関数（下書きの往復・グループの畳み込み・seed 方針・ファミリーカードの選択・プロンプト支援が
  ...
  ETag が効くバイトの安定・EMA）、そして P0 を完了と呼ぶ前に GPU のインスタンスへの実機 1 回——golden が pin
...
- **P0**（決定 1〜12 から、各決定が P1 と名指した項目を除く）: ペイン、取消・グループ・試走・
  ...
  `params`、拡張 status、サイドカーと seed、ファミリーのカード、トリガー語のチップ、`api/chat/ask` 経由の
...
- **P1**: スイープとプロンプト行列。ギャラリーの繋ぎ（「画像生成で開く」「参照にする」「ここに生成」）。
  ...
  通知。ファミリーが粗すぎたらチェックポイントごとのプロンプト注記。**ステップ単位の進捗とプレビュー画**
  （中継した WebSocket・決定 12）。
- **P2**: マスクの塗り。
  「この画像を記述」（vision）。77 トークンのファミリー向け CLIP トークン計数（ブラウザ内トークナイザ——
...
1. **時間で満ちる区分は P1 まで正直でいられるか。** 実機の後に決める。`flux1` のコールドスタートの
   ...
   だけでなく最初のジョブの区分にも足す。
2. ~~ファミリーのカードか、チェックポイントごとのカードか。~~ **決着（2026-09-13）: ファミリーのカード。**
   Pony・Illustrious・素の SDXL は 1 ファミリーに 3 方言。最初の本物の Pony 行でファミリーカードが誤導するなら、
...
2. **LoRA のファミリーは `baseModel` のまま**（camelCase・status が既に持っていた欄）。隣に `base_model` を足せば同じ事実の
...
8. **「出さない行」**は Agent では「グラフが組めない行」（ファミリーの宣言が無い、またはファミリーが要る宣言済みファイルが
...
実応答に対する `knobs` とファミリーカード。走行中バッチでの 304 の連鎖。サイドカーが無い過去の絵での `source: "png"`。
