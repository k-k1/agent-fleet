...
- **P0**（決定 1〜11 から、各決定が P1 と名指した項目を除く）: ペイン、取消・グループ・試走付きのキュー、
  `params`、拡張 status、サイドカーと seed、族のカード、トリガー語のチップ、`api/chat/ask` 経由の
  「プロンプトを書いて」、パスかドロップによる edit、`out_dir`。ファイルを共有しない 3 レーン:
  - **レーン A（Agent）**: `jobs.go`（先頭差し込みと試走の上限込み）・`Request.Params`・検証・seed とサイドカー・段階と取消・
    status 拡張・ルートと golden・使用量の畳み込み。
  - **レーン B（CP）**: 中継の行・`trained_words` 列を端から端まで。
  - **レーン C（Console）**: ペイン種別と `features/imagegen/`（フォーム・試走枠・結果カード・ジョブ一覧）・i18n・導線・使用量ラベル。
...
7. **試走の steps（10 / 12 / 8 / 4 / 4）と `trial/` の 7 日掃除は当て推量。** steps は実機で構図が
   分かる最小の数にする。`trial/` の掃除が望ましくなければ決定 3 の「掃除しない」をそこまで広げ、
   フォルダはただ育つ。
