...
- 状態: **採用・実装済み**（2026-09-09。実装の記録は
  ...
  補遺: §3-b に `list_child_sessions` と `rename_child_session`、決定 4 に題名の書き手の順序、
  決定 6 のアーカイブ、決定 9 の完了時刻、用語表と決定 1・5 に引き継ぎ提案の系譜）。設計は別セッションのレビューを 2 巡、
...
### 3-b. 段階 2 で広告する ~~9 本~~ 10 本（フラグ別の全量）
...
| `rename_child_session` | `--fleet-spawn` | 決定 4（子限定）＋ `title_set_by`（利用者が付けた題名には届かない） |
...
**補遺（2026-09-11・docs/log/89 §89.11）: `rename_child_session` が 10 本目である。** 理由と
「利用者が勝つ」の順序は決定 4 の補遺にある。**オペレーター面にも無いツールの 2 本目**なので、
呼び出し側のゲートは `list_child_sessions` と同じく**フラグ単独**である——`stop_session` /
`resume_session` と同じ `writeEnabled() || フラグ` にすると、**広告していない経路へ `--write` の
アシスタントが到達し、`sessionDriveAllowed` がオペレーターを素通しする設計なので
ワークスペースの任意のセッションを改名できる**（実測。docs/log/89 §89.11.3）。名前を取るツールな
ので、その先で `sessionDriveAllowed` も通す 2 段である。
**試験名も `TestFleetSpawnAddsExactlyItsTenTools` へ変わった**（本数が名前に入っているため、
以降も増えれば変わる）。
...
#### 補遺（2026-09-11・docs/log/89 §89.11）: 改名を足した。書き手が 2 人になるので「利用者が勝つ」を明文化する

**`rename_child_session`（10 本目）を同じ述語の下に足した。** 子を再利用するたびに題名が古くなり、
`list_child_sessions` が返す題名も Console の左ペインも中身と食い違っていた。列挙・出力・停止・再開と
同じ種類——破壊的でも不可逆でもなく、対象は自分が起こした子だけ——なので、決定の**述語は 1 文字も
変えていない**。ゲートは `stop_session` / `resume_session` と同じ `sessionDriveAllowed` である。

**ただし題名だけは、親のほかに利用者という書き手がいる。** 他の 4 本が触るもの（プロセスの生死、
端末の出力）に人の書き手はいない。そこで Meta に **`title_set_by`（`user` / `parent` / 空＝作成時）**
を持たせ、**調停ではなく順序**を置いた:

- **利用者の改名は常に通り、題名を親の手から外す。** Console のリネーム欄と、提案バナーの
  「採用」の両方が `user` を書く（採用は利用者がその名前を選ぶ操作である）。
- **親の改名は、まだ誰も付けていないか、親自身が付けた題名にだけ届く。** `user` が立っていれば
  409 `title_set_by_user` で拒否し、**理由を名指しする**（呼び手はモデルなので、黙って失敗させると
  再試行する）。
- **題名を空にすると `title_set_by` も空に戻る。** 自動題名の再提案が再び開くのと同じ理由で、
  セッションは新規と同じ状態に戻る。逆に**親は空を送れない** — 自動ラベルへ戻すのは利用者の操作で
  あり、親の空文字は「名前を選んだ」ではなく「名前を失った」である。

**拒否は Agent 側（`SpawnRenameRefusal`）に置く。** 本 ADR の他の拒否と同じ理由——MCP 層だけに
ある不変条件は、その層を差し替えれば回避できる——に加えて、**Console が同じエンドポイントを叩く**
からである。所有権の判定（呼び手が誰か）だけは MCP 側に残る。これは stop / resume と同じ分担で
ある。

`title_set_by` は `GET /sessions` のワイヤにも載せる（Console 側の綴りは `titleSetBy` — この一族の
キーは全部 camelCase で、ここだけ snake_case にすると契約試験の表と Console の型に唯一の例外が
できる）。**Console の表示は変えていない**: 運ぶのは、人が付けた名前と親が付けた名前を区別できる
readers がいずれ要るからで、CP の `sessionWire` に無い欄は黙って落ちる。
...
#### 補遺（2026-09-10・docs/log/87 §87.16）: 本数は設定になった（1〜6・既定 3。2026-09-24 から 1〜10）
...
- **上限は 6（`session.SpawnChildLimitMax`）。無制限にはしない。** 生きた claude セッション
  ...
  **補遺（2026-09-24）: 上限を 6 から 10 に上げた（既定 3 は据え置き）。** 利用者の要望で、
  1 つの親に並列レーンを 6 本より多く持たせたいという用途が出た。上の見積りでは子 10 本＋親で
  11 本＝3.7〜4.7 GiB（RSS 単純和・過大側）、ホストの半分弱に収まり、利用者が開いた
  セッションの分は残る。⚠️ **10 は 6 よりさらに遠い外挿であり、実測ではない。** また親 2 つが
  各 10 本なら 22 本で、上で見積もったホストの容量（23 本程度）にほぼ並ぶ——次項の「ワーク
  スペース合計はこの設定では有界にならない」がそのまま当てはまる。上限を選ぶのは利用者であり、
  既定 3 のワークスペースの挙動は変わらない。
...
- 正の経路は**親が `get_session_status` をポーリングする**こと。~~決定 3 の前提条件はこのためにある。~~
  → 前提条件は 2026-09-09 に消滅した（決定 3 の補遺）。全セッションが `get_session_status` を持つので、
  この経路は設定に依らず常に開いている。
...
`agentResumeAndSend`（`:3246`。再開待ち 30 秒＋配達確認 45 秒）で、**これは段階 2 で開ける ~~8 本~~ 9 本には
含まれない**。
...
- **Meta による系譜追跡は Meta が在るあいだだけ有効。** `RemoveMeta`（削除導線
  ...
  **補遺（2026-09-10・docs/log/94）: 判断は変えないが、対価が目に見えるようになった。** 左ペインが
  系譜で worktree を入れ子にし、同じ家系に同じ色の縦線を引くようになったので、**セッションを 1 本
  削除すると、その子の入れ子と色がその場で失われる**（子は残るが、親が居ないので group の root へ
  落ちる）。この節を書いた時点では「台帳の話」だったものが、いまは**利用者に見える帰結**である。
  台帳を作らない判断はそのまま——保持期間の中で描ける俯瞰に賭ける、が本 ADR の立場である——が、
  **掃除の順番が表示に効く**ことは記録しておく。
...
- ~~段階 1 と同じく、**実機での発火確認は未実施**。説明文は「いつ呼ぶか」を規定して発火率を
  ...
  → **2026-09-09 に実施**（docs/log/87 §87.15）。`report_back` の遵守率は **3/3（100%、n=3）**。
  ⚠️ **n=3 であって「発火率を測った」とは言えない**——確かめたのは「実セッションで呼ばれる」
  ことであって、どれくらいの確率で呼ばれるかではない。
  → **2026-09-11 に n を増やした**（docs/log/87 §87.19.4）。起動時の指示に対しては**累計 8/8**
  （claude 5・codex 2・agy 1、課題はいずれも 30 秒級の読み取り専用）。⚠️ 同じ 1 行を後から peer の
  `request` の本文に付けて再依頼した場合は **6/10**——封筒の `reply=only-if-blocked` が本文に勝つ
  （設計どおり）。`report_back` は**起動時の指示にだけ効く数字**として読むこと。
- 子が親より長生きしたときの掃除は利用者の手に残る（決定 13）。~~決定 6 の枠は呼び手ごとの
  数え上げであって予約ではないので~~ 決定 6 の枠は予約だが、予約の主体は親であって子ではないので
  （`reserveSpawnSlot` は親の名前で数え、子の Meta に枠は無い）、**親が消えれば上限も消える**。
  結論は変わらず、理由の綴りだけがラウンド 2（予約方式）より前のものだった。
