...
- 状態: 採用・未実装（段階 2 の設計。実装は本 ADR の合意後）。**別セッションのレビューを 2 巡
  反映済み**（2026-09-09。1 巡目: 決定 1・4・5・6・7・10・11 の訂正、決定 14 と §3-b の追加。
...
呼び手を `origin_session` に持つ**Meta が存在する子**を数え、3 本で拒否する。
...
`dir` の正規化した絶対パス**——つまり作業コピーそのもの——で、`subdir` は比較に含めない。同じ
作業コピーである以上、片方が `console/` で片方が直下でも事故は同じだからである。停止中のセッションは数えない
