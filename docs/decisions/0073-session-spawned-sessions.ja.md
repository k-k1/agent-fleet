...
撃つ」から分離して塞いだ。出どころは `TurnEnd` ビットの書き込み時刻から、そこから切り出した
`status.SessionStatus.TurnEndAt` へ移っている——**本物のターン終端でしか書かれない**という証拠
としての意味は同じで、説明の付かない idle では書かれず、次のターンが始まれば消える。
