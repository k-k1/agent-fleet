...
- 状態: **提案・レビュー済み**（2026-09-11）。実装はしていない。レビュー（末尾の節）は
  ...
  入ってよい。
  状態の更新（2026-09-24）: P0 は実装済み。4 レーンは #556（Agent）・#557（文書）・#558（Console）・#560（CP。`AF_COMFY_URL` から合成する外部の行、`control-plane/engines.go`）で入った。P0 の完了条件である実機 1 回（LAN の ComfyUI から画像 1 枚）は記録が無い。
