...
- **(c) opencode 経路より事故が減る見込みがあるか → 判定は合格だが、実機だけが見つけた欠陥が段 1 全体で
  8 件に達した**（利用者からの引き継ぎによる集計値）。**本 ADR が具体的に記述するのは 4 件のみ**——下記
  「実機だけが見つけたもの」節の既存 3 件（段 1 前半・上記「段 0・段 1 の実装記録」参照）と、今回追記する
  #811 の 1 件。🔴 **残り 4 件がどの欠陥を指すかは、本 ADR・`docs/log/99` のどこにも個別に特定されておらず、
  この文書の範囲では列挙できない**（前回「§12.1 の繰り返し検知の空振り」「§13.4/§13.8 の KV geometry 409」
  に分散記録済みと書いたのは誤りで、そのような記述はどちらの文書にも存在しない——訂正する）。**判明している
  最後の 1 件（#811・下記に追記）は「偽クライアントのテストは全部緑・実機でも当初の症状は直っている・
  それでも実用上 6 倍遅い」という、テストでは原理的に見えない種類**だった。決定 5 の帰結（実行者がうちに
  なる＝この種の問題をうちが抱える）の実例として、下記の既存節に続けて記録する。
...
3. 🔴 **`qwen3-coder-30b-a3b` は #811 最終版のハーネスでは 1 度も測っていない。** 3 回の測定はそれぞれ別の
   ハーネス版に当たっている（走行順とハーネス版の確定は `docs/log/99` §14 の 2026-09-20 追加訂正参照）:
   **§14 #3＝段 1 版で 72 ターン連続ループ**／**§14 #9＝段 1 版で実窓超過**（原因の「`Run` に窓が渡らない」
   欠陥は 52bbda2d と 570d5042 で直っている）／**§14 #14＝#811 初版で非収束**（この版自体が §14 #10 と同じ
   圧縮スラッシング欠陥を持つため、coder の性質の証拠にならない＝交絡）。したがって「3 回とも別々の理由で
   モデルがダメ」ではなく、**「3 回とも別々のハーネス版で測っており、直った版（#811 最終）での評価がまだ
   無い」**が正しい要約。合格チェックポイントには入っていない。**段 2 の残作業として、#811 最終版で
