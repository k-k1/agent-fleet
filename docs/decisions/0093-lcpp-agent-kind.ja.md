...
- 状態: **採用**（2026-09-21）。段 2 は PR #816・#818・#821・#823・#824・#825・#826・**#829**
  ...
  状態の更新（2026-09-24）: 「段 2 で新しく生まれた残件」の「通し経路が未検証」は閉じた。2026-09-21 に駆動役のセッションが配備済みの Agent から `lcpp` セッションを起こし、温かいインスタンスで 1 往復を通した（転写 5 レコード、`window: 262144`）。その通しで見つかった欠陥 3 件——冷えたインスタンスでの 45 秒の中断、待ち時間の表示、peer 送信——は PR #840 で直した。冷えた場合の修正はテストで押さえてあり、冷えた経路を実機で通し直した記録は無い。残件のうち 2 つは、実測で費用が見えない限り予定しない: 「エンジン起動待ち」の表示を経過秒数だけで判断していること（決定 4 が受け入れた v1 の簡略化）と、負債 8 の `InputTokens` を毎反復で呼ぶこと（#840 で起動待ちの再試行の内側に入った。費用は測っていない）。
  状態の更新（2026-09-25）: 負債 7（llama.cpp エンジンイメージの版ピン）にこれ以上のコードは要らない。機構は PR #803 で入った（`standup.sh --llm-digest`、手順は `deploy/aws/ecs/cfn/PARAMETERS-60-engines.md`）。ピン留めは運用者の作業で、ECR の `af-llamacpp` を次に入れ替えるとき——撤去後の立ち上げ、`LlmImageTag` の変更、意図した削除／付け替え——に行う（`update.sh` と `release-ecr.sh` はこのイメージをコピーしない）。
...
### 訂正（2026-09-23）: 表が追いつかないままだった能力 2 行

`guide/ref/agents.md` は lcpp の**「起動時のモデル選択」**と**「コンテキスト使用量ゲージ」**を
`—` としていた。どちらも誤りで、しかも機能が入った瞬間からずっと誤りだった。

理由は順番にある。lcpp の列は PR #816 で足され、モデル選択は #824 で開放、ゲージは #829 で
実装された——すぐ上の段落自身が picker の配線を記録している（「これが無ければ起動時にモデルを
選べない」）。誰も表に戻らず、誰も検査していなかった: `scripts/docs-check.py` が Go の `Caps()`
と突き合わせるのは 3 行（`CanFork` / `CanForkAt` / `PermissionChoice`）だけで、この 2 行はそこに
入っていない。

🔴 モデルの行は**誤っている方向が悪い**。lcpp は「ツールに任せる」項目を持たない唯一の種別で
（`console/src/lib/agentModels.ts` の `requiresConcreteModel`）、`StartModal` はモデルを提示する
どころか**選ぶまで起動を拒む**。最初のセッションの前に表を読んだ利用者は、必須要件の正反対を
教えられていたことになる。ガイド自身の lcpp 解説とも矛盾していた（実機 16 本にわたるモデル
ファミリーとモデル差し替えの話がそこにある）。セルは `✓¹⁹` にし、脚注 19 に「一覧は利用者自身の
サーバーのもの」「この種別ではどれを選ぶかが費用の判断」と書いた。

ゲージの行は `✓`。`WireLive` は `Store.LastUsage()` から `li.Context` を組み立て、窓が解決した
ときだけ `WindowSource="recorded"` と名乗る（決定 8）。`TestWireLiveContextRecordedWindow` と
未解決側の双子が固定しており、2026-09-21 の通し確認では転写に `window: 262144` が記録された
——`WindowGuess` を通らないという、この ADR の動機そのものの証拠である。

再発を止めるのは `console/src/agents/guideTable.test.ts`（ADR 0095 P2-22）である。いまは表の
9 行を全種別で Console の caps に突き合わせるので、セルと cap が食い違えばビルドが赤くなる。
その過程で 1 つ分かったことも書いておく: **`caps.contextBar` は Console のどこからも読まれて
いない**——ゲージは種別によらずセッションの文脈ペイロードから描かれる——ので、あのフラグは
文書であり、意味を与えているのはこの試験である。
