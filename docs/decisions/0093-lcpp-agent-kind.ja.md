...
- 状態: **採用**（2026-09-21）。段 2 は PR #816・#818・#821・#823・#824・#825・#826 で develop に入った。
  ...
  実装記録（2026-09-21）」節。🔴 **決定 8（使用量 exact）と、Console での起動可能化は段 2 の 7 PR に
  含まれず、この文書を書いている時点で別セッションが並行して作業中**——「段 2 の実装記録」節に、進行中の
  事実として記録する。
...
| 2 | P2: kind の配線・driver・転写の書き手・契約テスト・guide・本 ADR を *採用* に。**完了**——PR #816
  ...
  ⚠️ **使用量 exact（決定 8）と Console での起動可能化は上記 7 PR に入っておらず、段 2 の最後の 1 歩として
  別セッションが進行中**（詳細は「段 2 の実装記録」節） | — |
...
段 2 は develop に入った（#816・#818・#821・#823・#824・#825・#826）。**決定文は変えていない。**以下は
実装と実測が決定文に対して何を足し、何を変えたかの記録である——段 0・段 1 の実装記録と同じ体裁。
...
🔴 **決定 8（使用量 exact）と、Console での起動可能化はこの文書を書いている時点で develop に入っていない。**
`workspace/agent/usage_fold.go` の exact 集合に `"lcpp"` は無く、`console/src/agents/registry.ts` の
`lcpp.available` は `() => false` のまま（コメント: 「Registered (ADR 0093) but not launchable yet」）。
別セッションがこの 2 点を並行して作業中と聞いている——本 ADR を「採用」に移す判断は、この 2 点を含めずに
下した。マージされ次第、この節を更新すること。
