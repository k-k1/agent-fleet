...
- 状態: **P0〜P3 すべて実装済み・実機受け入れ済み**（2026-09-20〜21）。各フェーズの完了条件と
  ...
  🔴 **今の配線でも 3:2 などの写真がぼけている**（改訂節の 3。実装で一緒に直す）。
...
- **`FluxKontextImageScale` を外して `size` を尊重する。** 出力寸法は自由になるが、上流が「この比率で
  ...
  写真がぼけた原因はエンコーダの格子のずれと分かった（末尾「改訂」節）。`size` は今も尊重しない。
...
この節は同日に 2 度書き直している（「表の最近傍へ縮める」→ レビューで条件付き → 切り分けの 4 本で今の形）。
...
3. **写真がぼけるのは、参照 latent と生成 latent の格子がずれるときである。** `TextEncodeQwenImageEditPlus` は
   image1 を**自分で**約 1MP・8 の倍数へ縮め直してから参照 latent を作る（ComfyUI v0.37.0 `nodes_qwen.py`:
   `s=sqrt(1024²/(w·h))`, `round(w·s/8)*8`, `round(h·s/8)*8`）。渡した寸法がこの丸めの**不動点**でないと、
   `enc.pixels` から作る生成側の latent と参照側の latent のパッチ格子（latent/2）が 1 行・1 列ずれる。
   写真の 8 点はすべてこれに従い、「候補表の中か外か」「高さ」はどちらも反証された（log 112 §12・§13）:
   ...
   （較正: 1496×800 の写真では σ1.0 のぼかしが 0.874、1536×1024 の写真では σ0.7 が 0.547。S3 と S4 は同じ写真・
   同じ seed で、どちらも切られていない——違いは寸法だけ。）イラスト（A〜D・T1）が表の外でも落ちなかったのは、
   1376×768 が格子の一致する寸法だったからで、「質感の無い絵には出ない」だけではなかった。

🔴 **今の配線も、この欠陥を持っている。** 表の 17 件のうち **1248×832・1504×688・832×1248・688×1504** の 4 件は
不動点でなく、パッチの数まで食い違う（測った非不動点の 4 点と同じ形）。厳密には 1328×800 とその縦も不動点でないが、
latent が 1 行・1 列違うだけでパッチの数は揃う——この形は測っていないので、ここでは数えない（新しい規則は
厳密な不動点を選ぶので、どちらにしても外れる）。比率 1.42〜1.58（一眼レフの 3:2 を含む）・2.10〜2.26 と、その縦長の入力の写真は、**今日の時点で
...
| 縮める先の寸法（決定 4） | **入力の寸法に、`TextEncodeQwenImageEditPlus` 自身の丸めをかけて動かなくなった寸法**（不動点）。Agent が image1 の寸法（`comfy.go:1050` で既に読んでいる `params.Width` / `params.Height`）から計算して `ImageScale` の width・height に書く。比率 0.25〜4.0・入力幅 4 通りを掃引して**必ず収束し（最大 3 回）**、**比率の歪みは最大 1.48%**。この寸法はエンコーダが参照のために作る寸法と**定義上一致する** |
...
| **比率 0.25〜4.0 の最悪** | **−41.7%** | **1.48%** |

このため、前回の判断で残っていた 2 件は**対象がほぼ消える**: 「極端な比率も表へ縮める」は歪みが 1.48% 以下に
...
- 不動点を計算する関数を新設し、単体試験で「比率 0.25〜4.0 で収束し、結果が不動点で、歪みが 1.5% 以下」を
  主張する。式の出所（`nodes_qwen.py`・v0.37.0）を注釈に書く。
...
(d) **3:2 の写真（S0）で S4 の水準以上**——今の配線の S3（0.4053）が直ったことの確認、(e) 16:9 と 3:1 の写真で
(b) の水準。

### 未解決（この改訂の）

1. **仕組みそのものは証明していない。** 写真 8 点・2 場面・seed 3 本が格子説と矛盾しない、というところまで。
   格子説が予測した空間の偏り（左上で合い右下ほど落ちる）は S2 で弱く、S3 では出なかった（全面が均等に落ちた）。
   受け入れの (d)・(e) が、規則そのものを実機で確かめる機会になる。
