...
- 状態: **P0〜P3 すべて実装済み・実機受け入れ済み**（2026-09-20〜21）。各フェーズの完了条件と
  ...
  🔄 **改訂（2026-09-23）: 中央クロップをやめ、候補表の寸法へ全体を縮める**——末尾「改訂」節。
...
🔄 **改訂（2026-09-23）: 枠は今も表の最近傍だが、切らずに全体を縮めて届ける。** `size` を出さないのは
変わらない（末尾「改訂」節）。
...
- **`FluxKontextImageScale` を外して `size` を尊重する。** 出力寸法は自由になるが、上流が「この比率で
  ...
  🔄 **2026-09-23 に測った**: 表から外れた寸法では写真がぼける——懸念は当たっていた。`size` は今も
  尊重しない。改訂で変えたのは切ることだけ（末尾「改訂」節）。
...
2. **inpaint を名乗れるか**（決定 3 で見送り）。`SetLatentNoiseMask` を denoise 1 の指示編集に足した
   ...
   **マスクと絵が別の写像に載っていた**ことを見つけて塞いだ。以下は両方の記録。
   ...
   - **直し**: マスクを絵と同じ `FluxKontextImageScale` に通す（`LoadImage` →
     ...
     なる（log 112 §11 の T3: 15.9 px → 3.0 px）。
...
## 改訂 — 中央クロップをやめ、候補表の寸法へ全体を縮める（2026-09-23）

**利用者の判断で決まり、まだ実装していない。** 根拠の実測は [docs/log/112](../log/112-kontext-crop-necessity.md)
（§3〜5 が 4 組 8 本、§11・§12 が着工前の追加実測）。決定 3・4・「却下した案」・未解決 2 の本文は
書き換えず、その場に 🔄 の指し示しを入れた——当時それが何を根拠にそう決めたかを読めるようにしておくため。
...
2. **クロップはモデルの要求ではない。** 同じ表の寸法 1392×752 に、**切って**届けても（今の配線）**全体を
   縮めて**届けても、写真の質感は変わらない（勾配エネルギー比——平行移動に鈍く、ぼかすと下がる指標——
   seed 602 で **1.0297 対 1.0300**、seed 607 で **1.0172 対 1.0134**。log 112 §12 の R3・R4）。
3. **効いているのは「表の寸法に乗ること」である。** 表から外れた寸法では写真がぼける——1408×736 で
   **0.8974**、1376×736 で **0.8823**（σ≒1.0 のガウスぼかしが 0.874）。比率の伸びを +2.30% から −0.02% に
   動かしても変わらない（R1）ので、原因は伸びではなく**表の外**にある。イラスト題材（A〜D・T1）では
   表の外でも落ちなかった——**質感の無い絵には出ない劣化**で、112 §1〜8 が最初に見落としたのはここである。

→ 「却下した案」が挙げた懸念（表から外れると品質を賭ける）は、**写真では当たっていた**。この改訂は
その懸念を守ったまま、**切ることだけ**をやめる。

### 決めたこと

| 変えるもの | 中身 |
|---|---|
| 枠の決め方（決定 4） | `FluxKontextImageScale` は**寸法を選ぶためだけに**残す。`img → FluxKontextImageScale → GetImageSize` の width・height を `ImageScale(image=img, upscale_method="lanczos", crop="disabled")` に繋ぎ、その出力を `enc.pixels` / `pos.image1` / `neg.image1` へ渡す。入力は**切られず**、表の最近傍の寸法へ**全体が縮む** |
| 比率表の置き場 | **置かない。** 表はピン留めした ComfyUI のノードの中にだけ在る——決定 9 の「ピン留めが無意味になる」を踏まない。`GetImageSize` は v0.37.0 の `comfy_extras/nodes_images.py` に在る（`RETURN_NAMES=("width","height","batch_size")`・INT） |
| マスクの配線（決定 3） | `maskscale` を外す。絵もマスクも一様な引き伸ばしになるので、2 つの写像は定義上一致する（実測: 境界のずれ **15.9 px → 3.0 px**・log 112 §11 の T3） |
| マスクの寸法制限（決定 3） | **外す。** 理由（クロップで写像が分かれる）が消えるので、他のファミリーと同じ「同じ相対領域」に戻る |
| `size`（決定 4） | **変えない。** 出力寸法は表の最近傍のまま、呼び出し側は選べない。`sizes` を出さないのもそのまま |
| 2 枚目以降の参照（決定 5） | **変えない。** もともと `FluxKontextImageScale` を通していない |

🔴 **極端な比率も常に表へ縮める**（利用者の判断）。代償は幾何の歪みで、表の範囲の外では大きい:

| 入力 | 最近傍の表 | 横の伸縮 |
|---|---|---|
| 1:1・3:2・21:9 | 同じ比率が表に在る | 0% |
| 4:3 | 1184×880 | +0.91% |
| **16:9**（1820×1024） | 1392×752 | **+4.15%** |
| 1496×800（R3 の写真） | 1392×752 | −1.01% |
| 9:16（縦） | 752×1392 | −3.96% |
| **表の範囲内の最悪**（入力比 1.085 付近） | 1104×944 | **+7.80%** |
| **3:1** | 1568×672 | **−22.2%** |

切る（端を捨てる）・表の外へ出す（写真がぼける）のどちらも採らなかった理由は、**例外の無い規則になる**
ことである——どの入力でも「アップロードした絵の全部が編集され」「塗った所は出力の同じ相対位置に
在る」。マスクの UI は帯を描く必要がなくなり、比率表も要らなくなる。

### 却下した案（この改訂で）

- **Qwen-Image 2.1 と同じ寸法規則にする**（比率を保って 1MP・32 の倍数）。log 112 §8 で「案 2」として
  最初に採った形で、ComfyUI v0.37.0 の `TextEncodeQwenImage21` の式（`round(sqrt(res²·ratio)/32)*32`）と
  一致する。**表の外に出るので写真がぼける**（上の 3）。
...
- **極端な比率だけ従来どおり切る。** 規則に例外が 1 つ入ると、マスクの UI は「この入力では帯が在る」を
  描く必要に戻る。
- **極端な比率だけ表の外の寸法にする。** 写真でぼける（上の 3）。

### 影響（実装のとき触るもの）

- `comfy_workflows.go:1306`（`g["scale"]`）——`FluxKontextImageScale` を「寸法を選ぶノード」に格下げし、
  `GetImageSize` と `ImageScale(crop="disabled")` を足す。3 つの継ぎ目の行き先が変わる。
- `comfy_workflows.go:1386`（`comfyQwenEditNoiseMask`）——`maskscale` を外す。
- `comfy.go:1065`——マスクの寸法が絵と違うと断る分岐を外す。
...
(d) `GetImageSize` のリンクを渡したグラフがエンジンの検証を通る。

### 未解決（この改訂の）

1. 🔴 **`GetImageSize` の INT を `ImageScale` の width・height へリンクで渡せるか**——ComfyUI の API 形式では
   ウィジェットの値をリンクに差し替えられるはずだが、**この配備では確かめていない**。駄目なら代わりは
   「Agent が表を持ち、ピンを動かすたびに上流の表と突き合わせる試験を足す」になる（上の却下を覆すので、
   そのときは改めて判断する）。
...
3. **伸縮と質感。** 測ったのは −1.01%（R3・R4）だけで、+4.15%（16:9）・+7.80%・−22% の写真は測っていない。
   R1 は表の外どうしで ±2.3% の伸びが効かないことを示しただけで、表の中での大きな伸縮は別の問いである。
4. **写真の場面は 1 つ・seed は 2 本。** 2 枚目の写真（R2）は編集が画面の大半を置き換え、指標がそれに
   支配されて読めなかった（log 112 §12）。
5. **2509 は写真で測っていない**（T1 のイラストだけ）。
6. **ADR 0098 の Qwen-Image 2.1 には持ち込まない。** 2.1 は自分のテンプレートで上の丸めを使うが、
   「表の外で写真がぼける」は 2511 で測った事実であって、2.1 に訓練済みの表が在るかは分からない。
