...
- 状態: **P0〜P3 すべて実装済み・実機受け入れ済み**（2026-09-20〜21）。各フェーズの完了条件と
  ...
  🔄 **改訂（2026-09-23）: 中央クロップをやめ、全体を縮めて届ける**——末尾「改訂」節。
  決定 3・4・却下した案・未解決 2 にはその場に指し示しを入れた。**条件付き・未実装**（写真がぼける原因の
  切り分けが済むまで、縮める先の寸法は仮置き）。
...
🔄 **改訂（2026-09-23）: 切らずに全体を縮めて届ける。** 縮める先は仮置きで表の最近傍。`size` を出さない
のは変わらない（末尾「改訂」節）。
...
- **`FluxKontextImageScale` を外して `size` を尊重する。** 出力寸法は自由になるが、上流が「この比率で
  ...
  写真で 2 本ぼけたが原因は未切り分け（末尾「改訂」節）。`size` は今も尊重しない。
...
## 改訂 — 中央クロップをやめ、候補表の寸法へ全体を縮める（2026-09-23・条件付き）

**利用者の判断で方向は決まったが、原因の切り分けが済むまでの条件付きで、実装していない。** 根拠の
実測は [docs/log/112](../log/112-kontext-crop-necessity.md)（§3〜5 が 4 組 8 本、§11・§12 が着工前の
追加実測）。決定 3・4・「却下した案」・未解決 2 の本文は書き換えず、その場に 🔄 の指し示しを入れた——
当時それが何を根拠にそう決めたかを読めるようにしておくため。
...
2. **クロップはモデルの要求ではない。** 同じ 1392×752 に、**切って**届けても（今の配線）**全体を縮めて**
   ...
   **この主張だけは強い。**
3. 🔴 **写真がぼける寸法は在るが、原因はまだ切り分けていない。** 1408×736 で **0.8974**、1376×736 で
   **0.8823**（σ≒1.0 のガウスぼかしが 0.874）。比率の伸びを +2.30% から −0.02% に動かしても変わらない
   （R1）ので伸びは原因ではない。残る説明は少なくとも 3 つあり、手元の写真 4 点はどれとも矛盾しない:
   - **表説**: `PREFERRED_KONTEXT_RESOLUTIONS` の外だから。
   - **格子説**（別セッションのレビューで判明）: `TextEncodeQwenImageEditPlus` は image1 を**自分で**約 1MP・
     8 の倍数へ縮め直してから参照 latent を作る（ComfyUI v0.37.0 `nodes_qwen.py`）。渡した寸法がその丸めの
     **不動点**でないと、生成側の latent と参照 latent のパッチ格子（latent/2）が 1 行・1 列ずれる。
     1392×752 は不動点（87×47 同士）、1408×736 → 参照 1416×744（88×46 対 89×47）、1376×736 → 1400×752
     （86×46 対 88×47）。参照の位置 ID は中央寄せなので、ずれは左上で 0・右下で最大 1 パッチ（16 px）——
     タイル別の比も表外の 2 腕だけが上段 0.91〜0.99・下段 0.74〜0.94 と偏る。イラストの 1376×768 は
     参照 1368×768 だが格子は 86×48 で一致しており、「イラストは落ちなかった」もこれで説明できる。
   - **高さ説**: 表外の 2 腕はどちらも高さ 736、表内は 752。
   🔴 **格子説が正しければ、表の 1248×832（3:2）・1504×688 と縦の 2 件は不動点でなく、今の配線のままでも
   写真がぼけている。** 切り分けの 4 本（1400×752＝表外・不動点、1424×752＝表外・非不動点・高さ 752、
   3:2 の写真を今の配線で、同じ写真を 1264×832＝表外・不動点で）を回している。**結果が出るまで、
   下の「決めたこと」の寸法の規則は仮置きである。**

→ 「却下した案」は「`size` を尊重する（画素予算も自由）」案で、ここで測ったのは 1MP 固定のまま比率だけが
自由な寸法である。**別の案を測った**ので、あの却下理由を裏付けたとも覆したとも言わない。

### 決めたこと

| 変えるもの | 中身 |
|---|---|
| 切るのをやめる（決定 4） | 入力は**切らず**、全体を縮めて `enc.pixels` / `pos.image1` / `neg.image1` へ渡す（`ImageScale(crop="disabled")`）。上の 2 が根拠で、**これは切り分けの結果に依らない** |
| 縮める先の寸法 | **仮置き: 表の最近傍**。`img → FluxKontextImageScale → GetImageSize` の width・height を `ImageScale` に繋ぐ。格子説が勝ったら「`TextEncodeQwenImageEditPlus` の丸めの不動点」に置き換える（その場合は表を通す必要も無くなる） |
| 比率表の置き場 | **置かない。** 表はピン留めした ComfyUI のノードの中にだけ在る——決定 9 の「ピン留めが無意味になる」を踏まない。`GetImageSize` は v0.37.0 の `comfy_extras/nodes_images.py` に在る（V3 のノードで、出力は `IO.Int.Output` の width・height・batch_size） |
| マスクの配線（決定 3） | `maskscale` を外す。絵もマスクも一様な引き伸ばしになるので 2 つの写像は一致する（実測: 境界のずれ **6.6 px → 3.0 px**・log 112 §11 の T3。3 px は塗り替えの遷移帯の幅の内。§11 の表にある 15.9 px は、対照を**クロップ無しの**写像の予測と比べた値で、対照が設計上従わない写像なので比較にならない） |
| マスクの寸法制限（決定 3） | **外す。** 理由（クロップで写像が分かれる）が消えるので、他のファミリーと同じ「同じ相対領域」に戻る |
| `size`（決定 4） | **変えない。** 呼び出し側は寸法を選べない。`sizes` を出さないのもそのまま |
| 2 枚目以降の参照（決定 5） | **変えない。** もともと `FluxKontextImageScale` を通していない |

🔴 **極端な比率も常に縮める先の寸法へ縮める**（利用者の判断）。代償は幾何の歪みで、表の範囲の外では
大きい（下は「表の最近傍」を仮置きしたときの値）:

| 入力 | 最近傍の表 | 横の伸縮 |
|---|---|---|
| 1:1・21:9 | 同じ比率が表に在る | 0% |
| 3:2 | 1248×832（同じ比率。ただし格子説では**不動点でない**） | 0% |
| 4:3 | 1184×880 | +0.91% |
| **16:9**（1820×1024） | 1392×752 | **+4.15%** |
| 1496×800（R3 の写真） | 1392×752 | −1.01% |
| 9:16（縦） | 752×1392 | −3.96% |
| **表の範囲内の最悪**（入力比 0.927 付近。鏡像の 1.085 付近でも同じ） | 944×1104 | **7.81%** |
| **3:1** | 1568×672 | **−22.2%** |

切る（端を捨てる）ことを採らなかった理由は、**例外の無い規則になる**ことである——どの入力でも
「アップロードした絵の全部が編集され」「塗った所は出力の同じ相対位置に在る」。マスクの UI は帯を描く
必要がなくなり、比率表も要らなくなる。

### 却下した案（この改訂で）

- **極端な比率だけ従来どおり切る。** 規則に例外が 1 つ入ると、マスクの UI は「この入力では帯が在る」を
  描く必要に戻る。
- **Agent が比率表を持って寸法を計算する。** 決定 9 の 2 つ目の危険をそのまま踏む——表は ComfyUI の版に
  属し、ピンは実際に動いた（ADR 0098 で 0.35.2 → 0.37.0）。`GetImageSize` 経由なら持たずに済む。
- ⏸ **Qwen-Image 2.1 と同じ寸法規則**（比率を保って 1MP・32 の倍数。ComfyUI v0.37.0 の
  `TextEncodeQwenImage21` の式・diffusers の `QwenImageEditPlusPipeline` の `calculate_dimensions` も同じ形）と、
  ⏸ **極端な比率だけ表の外の寸法にする**案は、**保留**。写真で 2 本ぼけたが、原因（表・格子・高さ）が
  切り分けられるまで却下の理由を書けない。格子説が勝てば、32 の倍数の寸法の多くは不動点なので
  （例: 1376×768）、却下どころか第一候補に戻りうる。

### 影響（実装のとき触るもの）

- `comfy_workflows.go:1306`（`g["scale"]`）——`FluxKontextImageScale` を「寸法を選ぶノード」に格下げし
  （または外し）、`ImageScale(crop="disabled")` を足す。3 つの継ぎ目の行き先が変わる。直前の注釈
  （`:1307-1311`「that node exists to fix the FRAME」）も書き直す。
...
残る、(b) R3 の写真で勾配エネルギー比が今の配線と同じ水準、(c) マスク入りで境界のずれが T3 並み、
(d) `GetImageSize` のリンクを渡したグラフがエンジンの検証を通る、(e) **16:9（+4.15%）と 3:1（−22%）の写真**で
勾配エネルギー比が (b) の水準。

### 未解決（この改訂の）

1. 🔴 **写真がぼける原因の切り分け**（上の 3）。表・格子・高さのどれか。結果で「縮める先の寸法」を決め直し、
   保留の却下 2 件に理由を書く。格子説なら、**今の配線の 3:2 も直す対象**になる。
2. 🔴 **出力を入力の比率へ戻すか**（利用者の確認待ち）。今の案のままだと、16:9 の写真は横に 4.15% 伸びた
   絵で返り、3:1 は 22% 縮んだ絵で返る。デコードの後に `ImageScale` をもう 1 つ置いて入力の比率へ戻せば、
   **返る絵の比率はアップロードした絵と同じ**になり、マスクの「同じ相対位置」も保たれる。推奨は戻すこと。
   ⚠️ 戻す場合、3:1 は −22% に縮めた後で +28% 伸ばし直す**二重の lanczos** になる——その質感も測る。
3. **`GetImageSize` の INT を `ImageScale` の width・height へリンクで渡せるか**——**ソース上は通る・実機は
   未確認**。ComfyUI v0.37.0 の `execution.py` の入力検証は、値がリストならウィジェットかどうかに関係なく
   リンクとして扱い、出力型（`INT`）と入力型（`INT`）を照合するだけで、min/max の検査はリテラル値の枝に
   しか無い。V3 ノードの出力をリンクで渡す経路は、今のゴールデンの `FluxKontextImageScale` → `VAEEncode` が
   既に本番で通っている。受け入れの (d) で確かめる。
4. **伸縮と質感。** 測ったのは −1.01%（R3・R4）だけ。受け入れの (e) で測る。
5. **写真の場面は 1 つ・seed は 2 本**（切り分けの 4 本で 2 場面目が加わる）。2 枚目の写真（R2）は編集が画面の
   大半を置き換え、指標がそれに支配されて読めなかった（log 112 §12）。
6. **2509 は写真で測っていない**（T1 のイラストだけ）。
7. **ADR 0098 の Qwen-Image 2.1 には持ち込まない。** 2.1 は `TextEncodeQwenImage21` で参照を自分で縮める
   別の構成で、上の 3 は 2511 で測った事実である。
