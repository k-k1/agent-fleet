...
- 状態: **採用・P0〜P2 実装済み**（2026-08-26。P2 のうち作業グループ自動作成は**不採用**＝決定 11、PR 起票は**取り下げ**＝決定 6.1）。
  **2026-08-27: 実データ（Jira 41 件）を見て決定 14〜16 を追記** —— 決定 8 の「保存クエリが唯一の絞り込み」を一部撤回した（[docs/80](../80-work-item-inbox.md) §80.18）。設計と経緯は [docs/80](../80-work-item-inbox.md)。
...
**16（P2.5）. 行に出すのは「行ごとに違う情報」だけ。** 実機では全 41 行の 2 行目が同じ
`@Keiichi Kamikawa` で、そのためにタイトルが強く省略されていた。規則はこう:
