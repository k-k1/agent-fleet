...
- Status: **adopted**（2026-09-21）。段 1 の 3 門は 2026-09-20 に回答済み、段 2 は 2026-09-21 に
  ...
  P2-20 でアシスタントチャットを実装し（`muse exec --json`・1 ターン 1 プロセス）、P2-21
  （2026-09-22）で事前に申告した実ターン 2 本により実機確認した: 実際の返答が返り、2 本目は
  `--session-id` で 1 本目の文脈から答えた。脚注 11 の最後の行と `caps.headlessChat` は ✓。
  この 2 本の陰性対照は、チャット経路がカタログの `-contributor` 既定で走っていたこと——決定 6
  clamp 8 が覆えていなかった唯一の経路——も見つけた。いまは `muse.SafeDefaultExecModel` が門に
  なり、読めないときは既定に落とさずターンを断る。続く P2-22 でアシスタントのターンが触れる範囲を
  実測し（ツール呼び出し 0・ファイル無し・2 回）、能力表に自分を読む試験を付けた。P2-23 で最後の
  未実装の行——`skill/list` によるスキルピッカーのネイティブ側——を作り、実ターン 1 本でホストが
  `skill` 入力パートを展開することを確かめた。
...
### P2-21: アシスタントチャット——実機確認と、陰性対照が暴いたモデルの門（2026-09-22）

**実ターンは 2 本。打つ前に本数を申告し、実際に 2 本で足りた。** 2 本とも `TestMuseChatLive`
1 回（実測 8.90 秒）の中で走る。`HOME` は投げ捨て、`~/.config/muse` は symlink（複製はしない）、
`auth.json` の sha256 は前後で一致。

| | 1 本目＝陰性対照 | 2 本目＝製品の経路 |
|---|---|---|
| argv | P2-20 のまま・**`--model` なし** | `museChat.Send`・`--session-id`＋`--model` |
| 返答 | `"PONG"` | `"PONG"`（1 本目から想起） |
| セッション | `stream.id` から捕捉 | 同一 ID のまま |
| ストアの記録 | **`muse-spark-1.3-contributor`** | **`muse-spark-1.3`** |

ガイドが待っていた行はこれで埋まった。実際の返答が返り、2 本目は `--session-id` で**1 本目の
文脈から**答えている——「さっき何と言えと頼んだか」を尋ねたのであって、復唱を頼んだのではない。

**🔥 神託は返答ではなくセッションストア。** `muse exec` は
`museChatDataHome()/muse/sessions/.msp-view-v1/<sid>/*.json` に耐久投影を書き、そこの
`model` / `modelId` がホスト自身の記録である。モデルに「あなたは何のモデルか」と訊けば学習
データから答えてしまう。live テストは 1 つのパスを読むのではなく、2 本目の前後で**そこに現れる
id の集合**を比べる——投影の形はベンダのもので、AF が固定できる契約ではないため。

**陰性対照が見つけたもの。** P2-20 のチャット経路にはモデル解決が一切無かった。muse の会話では
`chatModelFor` が `c.Model` をそのまま返し（`chat.go:478`）、`recommendedAssistantModel` に muse
の case が無い（`chat.go:426`）。つまり利用者がモデルを選んでいない会話は `""` を持ち、exec は
`--model` 無しで出ていく。枠 0 の `model/list` で実測したカタログは 4 行で、`isDefault: true` は
`muse-spark-1.3-contributor`——説明文が例の「製品改善に使われうる」一文の行——に付いており、
1 本目はホストが本当にそれを取ることを確かめた。決定 6 の clamp 8 が、managed ドライバの覆わない
唯一の経路で破れていたことになる。しかもガイドの既出の一文（脚注 14「Agent Fleet は代わりに
選んだりはしない」）と矛盾していた。

**修正と、なぜ既定に落とさず断るのか。** `muse.SafeDefaultExecModel()` は MSP クライアントを
持たない呼び出し側のために、カタログの「学習に使うと言われていない最初の行」を解決する（managed
の `SafeDefaultModel` はクライアントを要るが、`muse exec` は持たない）。`museChatModel` は利用者が
何も選んでいないときにそれを使い、カタログがまったく読めないときは**ターンを断る**——そこで `""`
を返すのが欠陥そのものだからだ（`--model` 無しの exec ＝ contributor 既定）。contributor を使い
たい利用者は選べる。双子はピッカーに残っている。

**Console。** `caps.headlessChat` は `true`。`recommendedModelId` に muse の行を足したので、
設定 › AI の行は「Default」ではなく「推奨（現在: muse-spark-1.3）」＝Agent が実際に走らせるものを
表示する。初版は間違っていて、新しい dom テストが捕まえた: 選択肢の先頭は `""`（既定）の行で、
`""` には `-contributor` の接尾辞が無いので探索がそれを返していた。

**ガイド。** 能力表「アシスタントチャットとして利用」の muse 欄は英日とも `✓¹¹`。脚注 11 は前半
——オンデマンド導入とサインイン。いまは起動メニューだけでなくアシスタントの相手選びについても
——を残し、「まだ実測していない」段落を落とした。番号を残すことで脚注 12〜18 は動かさずに済む。
脚注 14 には contributor 規則のアシスタントチャット側を書き足した。

**テスト。** `chat_providers_muse_test.go` は新規: JSONL パーサ（terminal のテキストが delta に
優先する・失敗 terminal が表に出る・run ストリームの id をセッション id と取り違えない）と
モデルの門。門のテストは変異試験で確認した——断りを `return "", nil` に置き換えるとこのテストが
落ち、戻したあとファイルの sha256 を照合した。`AiModelRow.dom.test.tsx` には muse の 2 件を追加し、
モック目録は**わざと contributor を先頭**に並べてある（先頭行を取るだけの推奨では通らないように）。

**測れなかったこと。**

- P2-20 のスケルトンの 3 本目（`--disable-write` がチャットターンの書き込みを止めること）は落とした。
  忘れたのではなく: 1 ターン掛かるうえ、P2-6 で「サンドボックス放棄の下では `serve` ではこれらの
  フラグは何も変えない」ことが既に実測されている。`exec` で効くかは未測であり、アシスタント
  チャットが書き込み不能だと仮定してはならない。
- `muse exec` はトークン数を報告しない（`serve` の `session/tokenUsage` だけが持つ）ので、
  アシスタントチャットのターンは要求モデルだけを記録し合計は残さない＝使用量チップは動かない。
- 能力表とコードの突き合わせは依然として fork 2 行と権限スキップ 3 行だけ。`registry.ts` の cap を
  戻す変異は Console のテストを全部緑のまま通る。

### P2-22: アシスタントチャットが触れる範囲と、自分を検査するようになった能力表（2026-09-22）

P2-21 は行を埋めたが、2 つを残していた。ここで両方を閉じる——前者は申告した実ターン 2 本で、
後者は 1 本も使わずに。

**`--disable-shell` と `--disable-write` は `muse exec` で効くのか。**

まず枠 0 の側。そしてこれは**罠として名指しておく価値がある**。チャットプロバイダ自身の argv を
`--provider echo` で走らせ、セッションログの `model_request_configured` を読むとこうなる:

| 腕 | `web_search` | `bash` / `bash_input` | `write_file` / `edit_file` |
|---|---|---|---|
| 製品の argv（`--disable-shell --disable-write --disable-web-tools`） | **消える** | 残る | 残る |
| フラグなし（対照） | 残る | 残る | 残る |

`toolset.mode` は両腕とも `all`、`toolset.source` は `default`。つまり `--disable-web-tools` は
この一覧を絞るが、他の 2 つは絞らない——「clamp は飾りだ」と読めるが、🔥 **その読みは誤りだった**。
`museChat.Send` 経由の実ターン 2 本、2 本目は押す形（「自分のツールが効くかを事前に決めるな。実際に
write_file を呼び、返ったエラーを逐語で報告せよ」）で、どちらも同じ結末だった:

- 投げ捨てディレクトリの目標パスにファイルは無い;
- **ツール呼び出しが 1 度も無い**——セッションログの `tool_name` も `tool_call_id` も空で、
  task kind は `model.meta.response` と `session_name.allocate` だけ;
- モデル自身が「このセッションでは書き込みとシェルは無効」と述べた。

よって上の記録は**モデルに何が渡るかの読み出しではない**。`active_tools` をそう使ってはならない。
測れたのは「clamp が強制されている」より狭く、いまガイドに書いたとおり——アシスタントチャットの
ターンはツールを 1 度も呼ばず、何も書かなかった、2 回とも。**未測**なのは実行時の拒否である。
何一つランタイムに届かなかったので、「強制」と「指示」はここからは区別できない。将来の版が
「無効だ」とモデルに伝えるのをやめた場合に気づけるのは `TestMuseChatLiveWriteClamp` /
`TestMuseChatLiveWriteClampForced` だけであり、だから両方をログではなくリポジトリに残した。

**能力表が自分を検査するようになった**（`console/src/agents/guideTable.test.ts`）。

P2-21 は「muse の `headlessChat` を戻しても Console のテスト 318 ファイルが全部緑」と記録した。
もう緑にはならない。新しいテストは `guide/ref/agents.md` から表を読み、7 行を全 11 種別について
`registry.ts` の caps と突き合わせ、英日の表が**セル単位で同一の印**を持つことを確かめ、表のすべての
行が「対応付け済み」か「理由付きで `UNMAPPED_ROWS` に載っている」かのどちらかであることを要求する
——行名を変えただけで検査から外れることが無いように。変異試験済み: `headlessChat: false` で赤になる。

🔴 **初回の実行で muse 以外のズレを見つけた。** `lcpp` の「起動時のモデル選択」と
「コンテキスト使用量ゲージ」のセルは `—` だが、`registry.ts` の `caps.model` と `caps.contextBar` は
どちらも `true` で、根拠もその場に書いてある（`DynamicModel`／ADR 0093 決定 8「WireLive が報告する・
WindowSource=recorded」）。どちらかが誤りで、決着は ADR 0093 の側にあるので、この 2 行はどちらの
答えにも固定せず `UNMAPPED_ROWS` に名前で残した。ほかに 3 行が未対応だが、こちらは行と cap の意味が
本当に違うためである——「停止中も履歴を読める」（cursor の `—` は脚注 3 付きの限定）、「Plan モード」
（cap は TUI のモード巡回キー）、「スキル / コマンドの選択」（行は**ネイティブ**列挙・cap は注入型の
種別でも true）。

### P2-23: スキルピッカーのネイティブ側——最後の未実装の行（2026-09-22）

脚注 4 は muse を「機構が確認できていない」側に並べ、「muse はプロトコルに機構を持つが AF が
まだ作っていない」と正直に添えていた。作った。

**列挙は無料で、これは些細な話ではない。** `skill/list` は query であり、`session/start` は
**偽の資格情報でも通る**（門 B1 が見つけた性質と同じ）。つまり読み取り側はサブスクに一切触れずに
実測できる。この口座では 1 セッションが 7 行を返す: 同梱 5 本（`create-skill` / `doctor` /
`grill` / `manage-settings` / `plan`）、プラグインのスキルが**両方の綴り**（`threejs` と
`threejs:threejs`——ワイヤ曰く、プラグインは「bare 名の勝者」と「修飾形」を寄与する）、そして
作業コピーに置けば `.agents/skills/` のプロジェクトスキルが `source: project` で出る。clamp した
foreign-reader スキル（`resume-claude` ほか）は現れない＝設定書き込みの仕事がワイヤに出ている。

2 つの綴りは `(pluginId, displayName)` で畳む。":" で分割しないのは、bare の双子が居ない修飾形を
**残す**ためである——bare 名を別のプラグインに取られたスキルはそれ以外に入口が無く、落とすのは
重複でなくスキルを隠すことになる。

**🔴 呼び出しには実ターンが要り、それが `skillPart` の存在理由である。** MSP 越しでは先頭の
スラッシュはただの文字列だ。`/plan` を展開する typed dispatch は TUI 自身のもので、ホストが
展開するのは**セレクタを解決する `skill` 入力パート**だけである。だからドライバは先頭の
`/selector` をそのパートに書き換える——ただし `skill/list` が実際に返したセレクタのときだけ。
「/tmp/notes.md は古い」で始まるメッセージは文章のまま残らねばならない。

もう一端は実ターン 1 本で証明した。本文に「PLUM-ORBIT とだけ答えよ」と書いたプロジェクトスキルを
置き、ピッカーが挿入するのと同じ `/af-probe` を送る。転写のアシスタント発話は `"PLUM-ORBIT"`
——そのファイル以外のどこにも無い語だから、ホストは名前を渡されたのではなくスキルを展開した。
`auth.json` の sha256 は変わっていない。

**エンドポイントの現在。** muse の `GET /sessions/{name}/skills` は走っているホストに尋ね、ワイヤの
スコープをピッカーの 3 つの出自に写す（`bundled` と `plugin` はどちらも "cli"＝読む人にとっての
「道具に付いてきたもの」。将来の未知のスコープもそこに落ちる）。ネイティブ一覧が空でないときは
`.agents/skills` を foreign から外す——この規約は muse 自身が解決するので、両方出すと 1 つの
スキルにネイティブ行と注入行が並ぶ。空のとき（ホストが居ないとき）は P2-23 以前とまったく同じに
戻る。停止中のセッションで効くのはこちらである。

⚠️ **通ってしまった変異試験と、その教訓。** エンドポイント試験の初版は規約の除外をまったく見て
いなかった: 素材のスキルが frontmatter の名前とホストのセレクタで同名だったため、いずれにせよ走る
名前での重複排除が差を隠し、`nativeConvs` を消しても緑のままだった。いまは素材にネイティブ一覧が
使わない名前（`importer-legacy`）を持たせてあり、変異は赤になる。**変異が緑で通るのは変異でなく
試験の問題である。**

**意図的に作っていないもの。** `skill/changed` は購読していない。無効化すべきキャッシュが無いから
である——ピッカーは開くたびに走っているホストへ問い合わせ、それは既に在るプロセスへの往復に過ぎ
ない。あの通知が要るようになるのは、キャッシュを置いたときである。
