...
- Status: **adopted**（2026-09-21）。段 1 の 3 門は 2026-09-20 に回答済み、段 2 は 2026-09-21 に
  13 個の作業パッケージで着地した（末尾の実装記録 P2-1〜P2-13＝下の作業パッケージ表の全行に加え、
  どの行も持っていなかった MCP のワイヤ経路と fork）。種別は 2 つの前提（プロプライエタリのバイナリが
  導入済み・資格情報がある）の先で起動メニューに出る。**作られていないもの**はガイドの能力表と
  P2-13 の末尾節に名指してあり、あそこの空欄は「まだ検討中」ではなく「まだ作っていない」を意味する。
...
**まだ `—` の能力 8 行は Muse Code 側で塞がれているのではない。ガイドも「構築中」ではなくそう
書くようにした。** どれも「この kind 向けに誰も配線していない Agent Fleet の機能」であり、形は
2 つに分かれる:

- **プロトコルは運ぶが、パッケージが意図的に手前で止めた 2 つ。** コンテキスト量ゲージ
  （`session/contextUsage` は在るが発火はターンの周りだけなので、`contextBar` を宣言すると
  「端から端まで実測した経路」ではなくスキーマから読んだ能力になる）と画像貼り付け（添付は今は
  パスを書いたテキスト part で渡る。MSP の画像 part は base64 を取るので、フラグではなく本当の
  作業である）。
- **そもそもエージェントごとに書かれていない 6 つ**——スキル/コマンドピッカー、引き継ぎ、
  worktree 起動、予約実行、チャットブリッジ、アシスタントチャットとしての利用。ADR 0093 自身の
  棚卸しが、引き継ぎ・spawn・共有ビュー・予約起動の経路に kind 別の分岐は無いと報告している。
  つまり大半は既に動いている**だろう**。その「だろう」こそが空欄のままにする理由である——この表は
  端から端まで動くのを見た行しか ✓ にせず、それを見るには利用者のサブスクの実ターンが要る。

⚠️ **muse のものではない発見が 1 つ**。上を確認する過程で出たが、意図的に手を付けていない:
`ScheduleDetailModal.tsx` の `AGENT_KINDS` は手で維持された 6 個の一覧
（`claude, codex, opencode, copilot, cursor, kiro`）で、**agy** が抜けている——このガイドの同じ表が
予約実行を ✓ としている kind である（lcpp と muse も無い）。つまり今日の Console では、予約の
エージェントを agy に編集できない。直さないのは、この kind の欠陥ではないことと、「muse は予約実行
できる」は正直に言うには実ターンが要る主張だからである。ただし一度ずれた手書きの kind 一覧は
また必ずずれるので、直し方は `mcpreg.ServedKinds` と同じ形になる: members を並べるのではなく軸に
名前を付けること。
