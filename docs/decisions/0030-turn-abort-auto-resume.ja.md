...
[English](0030-turn-abort-auto-resume.md) | 日本語

- 状態: **採用・実装済み**。設計は [docs/47](../log/47-turn-abort-auto-resume.md)。
- 関連: [0015](0015-agent-managed-driver.ja.md)（managed driver の turn 状態機械）/
...
## 追補（2026-09-14）— 上限の自動再開を codex managed へ広げる

上の追補（2026-07-31）は claude の止まり方を出発点にしていたので、実装も claude だけを見て
いた。利用者報告で、**codex のセッションが「制限解除待ち」を名乗りながら予約を 1 件も持たない**
ことが分かった（docs/log/47 §4-12）。表示だけ先に揃えて中身が無かった状態。

決定: **上限の自動再開は種別に依らない**。種別ごとに違うのは「いま上限で止まっているか」と
「いつ解けるか」の 2 つだけで、エピソード・通知・リトライ・畳み方は共有する。

- codex（managed）は前者を app-server の `usageLimitExceeded`、後者を codex 自身が記録して
  いる利用枠の窓から得る。**メニューの自動解除（①）は claude 固有**で、codex には存在しない。
- **満杯の窓が 2 つあるときは早い方を採る。** 早すぎた再開は一度上限に当たってやり直せば
  正しい窓へ収束するが、遅すぎた再開は数日の待ちを誰も直せない。
  （2026-09-14 追記: この「撃ち直して収束する」は実装では成立していなかった — 発火済みの
  エピソードが次の上限を飲み、窓の境界では読みが潰れて時刻を決められなかった。実機 1 回目で
  露見し docs/log/47 §4-13 で塞いだ。**決定は変えない**。）
- **対象は managed だけ。** ターミナル（CLI）の codex に残るのはアカウント全体の率で、
  「このセッションが上限で止まった」とは別のこと。材料が無いので広げない。
- **設定キーは増やさない。** `rateLimitAutoResume` 1 つが対象の全種別を左右する
  （`claudeAbortAutoResume` と同じ立て方）。スイッチ自体は Codex カードにも出す。

## 積み残し

- 中断（§3 / 2026-08-05 の追補）の対象は claude TUI と managed（codex / opencode）。他 TUI
  種別（cursor / copilot / kiro）は判別材料が claude の jsonl 形式に固有（`isApiErrorMessage`）
  なので別のシグナルが要る。**利用上限**（2026-07-31 / 2026-09-14 の追補）は claude と
  codex managed のみで、opencode は型付きの上限エラーと窓のリセット時刻の両方が要る。
