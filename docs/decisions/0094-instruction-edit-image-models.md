...
- Status: **P0 through P3 all built and accepted on real hardware** (2026-09-20 to 21). Each
  ...
  carry a pointer where they stand. **Decided, not built.** 🔴 **Today's wiring already makes 3:2
  photos, among others, soft** (the revision's point 3; fixed as part of building it).
...
- **Removing `FluxKontextImageScale` to honour `size`.** The output size becomes free, but leaves
  ...
  frees only the ratio; the photos that went soft were traced to the encoder's grid misalignment
  ("Revision" at the end). `size` is still not honoured.
...
made conditional by a review → its present form after four isolating runs).
...
3. **Photos go soft when the reference latent and the generated latent sit on different grids.**
   `TextEncodeQwenImageEditPlus` re-scales image1 **itself** to ~1 MP in multiples of 8 before
   encoding the reference latent (ComfyUI v0.37.0 `nodes_qwen.py`: `s=sqrt(1024²/(w·h))`,
   `round(w·s/8)*8`, `round(h·s/8)*8`). Unless the size handed in is a **fixed point** of that
   rounding, the generated latent made from `enc.pixels` and the reference latent land on patch
   grids (latent/2) one row or column apart. All eight photo points follow this, and both "inside or
   outside the ratio table" and "height" were refuted (log 112 §12–§13):
   ...
   (Calibration: on the 1496x800 photo a σ1.0 blur scores 0.874; on the 1536x1024 photo σ0.7 scores
   0.547. S3 and S4 are the same photo and seed, neither is cropped — the size is the only
   difference.) The illustrations (A–D, T1) held up off the table because 1376x768 happens to align
   its grids, not only because a picture without texture has none to lose.

🔴 **Today's wiring has this defect too.** Four of the table's seventeen entries — **1248x832,
1504x688, 832x1248, 688x1504** — are not fixed points, and their patch counts differ too (the same
shape as the four non-fixed points measured). Strictly, 1328x800 and its mirror are not fixed points
either, but their latent is one row and column off with the patch counts still matching — a shape
not measured, so it is not counted here (the new rule picks strict fixed points, which avoids both). Photos at input ratios 1.42–1.58 (which
...
| The size it shrinks to (decision 4) | **The input's size with `TextEncodeQwenImageEditPlus`'s own rounding applied until it stops moving** (a fixed point). The Agent computes it from image1's size (`params.Width` / `params.Height`, already read at `comfy.go:1050`) and writes it into `ImageScale`'s width and height. Swept over ratios 0.25–4.0 at four input widths it **always converges (in three steps at most)**, with **at most 1.48% aspect distortion**, and it is **by construction** the size the encoder makes for its reference |
...
| **Worst over ratios 0.25–4.0** | **−41.7%** | **1.48%** |

So the two questions the earlier decision left open **mostly lose their subject**: "shrink extreme
ratios to the table too" now costs at most 1.48%, and "put the output back to the input's ratio" is
...
- **Cropping extreme ratios as before.** One exception, and the mask UI is back to drawing "this
  input has a band" — and with distortion at 1.48% at most there is nothing to crop for.
...
- A new function computes the fixed point, with a unit test asserting "converges over ratios
  0.25–4.0, the result is a fixed point, distortion at most 1.5%". Its comment names where the formula
  came from (`nodes_qwen.py`, v0.37.0).
...
mask, the edge error is at T3's level; (d) **the 3:2 photo (S0) scores at S4's level or better** —
the check that today's S3 (0.4053) is fixed; (e) 16:9 and 3:1 photos score at (b)'s level.

### Open (for this revision)

1. **The mechanism itself is not proven.** Eight photo points, two scenes and three seeds agree with
   the grid explanation, and no more. The spatial skew it predicted (aligned at the top left, worse
   towards the bottom right) was weak in S2 and absent in S3 (which dropped evenly). Acceptance (d)
   and (e) are the chance to check the rule itself on hardware.
...
4. **Extra references were not measured.** They are re-scaled by the same encoder into reference
   latents, but only image1 has to share a grid with the generated latent (an inference).
