...
acrt で `Ec2MaxSlots` を 6 → 30 に上げたところ、それは同時に「30 本の root を無期限に買う」
...
実機で起きた（docs/64 §64.40）: ワークスペースのコンテナの中で出口の無い読み出しループが
走り、**root ボリューム（`/var/lib/docker`）と home の両方を gp3 のスループット上限に
3 時間張り付かせた**（root は 39.34 GB/5 分＝131 MB/s が有効数字 4 桁まで不変）。
箱の管理系（efs-utils の stunnel → ECS エージェント → SSM エージェント）は
**メモリではなく I/O で飢えて**順に落ちた。EC2 のステータスチェックは 3 つとも `passed` のまま、
つまり**外形上その箱は健全**である。

⚠️ **当初は「メモリ枯渇」と診断したが、隔離した箱の永続ジャーナルを読んで覆った**
（OOM killer 0 件・`hung_task` 0 件・swap 無し・`OOMKilled=false`）。
**この訂正は対策の中身を変える**——コンテナにメモリのハードリミットを付けても防げないし
（むしろ cgroup のページキャッシュを縛るぶん悪化しうる）、ホストのメモリ計測を足しても
見つけられない。異常を叫んでいたのは `VolumeReadBytes` / `VolumeQueueLength` /
`EBSByteBalance%` で、**これらは最初から CloudWatch に在った。足りないのは計測ではなく警報である。**そして 3 つの層がそれぞれ正しく振る舞った結果、
