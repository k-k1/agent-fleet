...
> ⚠️ **既知の欠陥（未修正・docs/64 §64.39・2026-08-26）**: そのうち **40 秒**は ECS ではなく
...
> 直しは①td だけ先に更新 →（`ACTIVE` な旧デプロイが消えるまで実測 10〜23 秒・スロットの
> ECS 登録待ち 18s と重なる）→ ②`desiredCount=1`。A/B は実測済み（1 回＝2 本 / 分割＝1 本）。
