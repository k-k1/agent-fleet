...
## 未解決 — AZ の選び方（実測で見つかった。まだ決めていない）

新規 home の AZ は `anyAZ()`＝**設定されたサブネットのうち ID の昇順で最初のもの**に
決め打ちで、分散はしない。既存 home の AZ は正しく尊重されるので、動きとしては正しい。
問題はその先で、**その AZ に容量が無いと `runSlot` は他の AZ で撮り直さない**——
既存ユーザーは動いたまま、**新規ユーザーだけが起こせなくなる**。

- `AF_ECS_SUBNETS` に書いた順ではなく **ID の昇順**である点も、運用者の期待と食い違う
  （1a を先に書いても ID の小さい 1c が選ばれた・docs/64 §64.20.4 で実測）。
- sandbox では容量不足を作れないので**未実測の設計上の穴**として残す。直すなら
  「新規 home（＝ AZ の制約がまだ無い人）に限り、`InsufficientInstanceCapacity` の
  ときだけ他の AZ を試す」が最小で、既存 home の AZ 固定には触らない。
