...
実機で起きた（docs/64 §64.40）: ハードリミットの無いワークスペースのコンテナが
スロットのメモリを食い尽くし、**同居している管理系（efs-utils の stunnel → ECS エージェント
→ SSM エージェント）を道連れにした**。EC2 のステータスチェックは 3 つとも `passed` のまま、
つまり**外形上その箱は健全**である。そして 3 つの層がそれぞれ正しく振る舞った結果、
