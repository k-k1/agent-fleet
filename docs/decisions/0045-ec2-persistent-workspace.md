...
> ⚠️ **決定 19 で撤回した。** 焼くと pull は本当に消える（実測 31.8s → **0.185s**）が、
> **自前 AMI の初回起動がそれ以上に高くつく**（新規ユーザーの Start が 144.0s → 179〜192s）。
> 以下は判断の記録として残す——設計としては正しく、**外れたのは前提の方**だった。
...
- **スクリプトは残す**（`deploy/aws/ecs/bake-slot-ami.sh`）。pull を消すこと自体は本当に効くので、
  レジストリ障害から切り離したいなど**別の理由**がある人には使える道具である。既定の助言ではない。
- Console の「スロットのイメージ」は、**焼いていない状態を推奨と書く**。焼くよう促す文言は
  そのまま残すと**誤った助言**になる（既定の状態が「直すべき状態」に見えてしまう）。
