...
- 1 台に複数ユーザーの EBS を同時に載せてタスクを並べることも成立した
  （`MaximumEbsAttachments` は m7i 系で **32**・ENI とは別枠）。
- root ボリューム（＝イメージキャッシュ）が**スロット数分**で済むので、ユーザー数分の root を持つ
  固定型よりストレージも安い。

**この形の代償**（採るなら設計に含める）:

1. **root をユーザー間で共有する** —— 前のユーザーのコンテナ書き込み層と `/tmp` が残る。
   ECS の掃除は `ECS_ENGINE_TASK_CLEANUP_WAIT_DURATION`（**既定 3 時間**）を詰めること。
2. **同居させるとカーネル分離が Fargate より弱くなる** —— **1 スロット 1 ユーザー（排他）**にすれば
   元の水準に戻る。そのときホットスロット数 ≒ 同時稼働ユーザー数で、Fargate で同時に走らせるのと
   同じ量の compute しか払わない（余るのは待機スロット分だけ）。
