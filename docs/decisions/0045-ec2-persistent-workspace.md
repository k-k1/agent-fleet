...
2. **保持期間スイープ**——退避のみ。既定オフ。
   > **実装で 1 点動いた。** 有効化は**テナント設定ではなくデプロイ単位**
   > （`AF_ECS_EC2_HIBERNATE_AFTER_SEC`）になった。掃除ループは EC2 のタグから世界を導出しており、
   > テナントも CP の DB も見えない（ADR 0012）。テナント単位にするには引き金を reaper へ
   > 上げる必要があり、機能そのものより大きな変更になる。docs/64 §64.18.2。
