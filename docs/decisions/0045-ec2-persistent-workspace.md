...
- ⚠️ **warm 床（最小 warm 台数）は置かない。** home が付いた箱は `occupiedInstances` が
  ...
  逆に home を外して汎用にすると次の人は wake ＋ attach ＋ mount で 123〜143s ＝ 新規 135s と
  ほぼ同じで、**持ち主から 25 秒を取り上げて他人には 0 秒しか渡らない**。しかも休止期間にも
...
**やらなかったこと**: 上限に張り付いた状態で**休眠中の型違いの箱を terminate して正しい型を
建て直す**こと。運用方針から言えばそちらが正しいが、決定 23 で定常の箱数が上限から離れた今、
この窓は「上限にほぼ張り付いた配備」でしか開かない。現状は**黙って詰まる**のを
**明示的に失敗する**に変えたところまでである。
