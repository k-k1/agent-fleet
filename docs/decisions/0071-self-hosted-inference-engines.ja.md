...
- 翌日（2026-09-07）に実測で改訂: 未解決の点 1〜3 を測って「実測で解けた点」へ移し、決定 5・7・8
  にその帰結を足した。opencode の切断は 300 秒、Managed Instances は CPU インスタンスで起動 109 秒・
...
- 同日、**P0 着手前のレビュー**を末尾の「レビュー（2026-09-07・P0 着手前）」に追記した。
  ...
  のはドレイン中のインスタンスがクォータを握っていたから**（決定 2）。決定 3 の訂正後の版「遅いのは HF が
...
- 同日、**P0 を実装・実機検証**（「P0 の実測」節）に続けて **P1（`image` 役と provider
  ...
  `describe-instances` は、**MI のインスタンスを一覧に出さない**——P1 の実測 2 で訂正した。
...
3. **ComfyUI** を、同じインスタンスで。画面も触れること。
...
1. **GPU は ECS Managed Instances で買う。自前の EC2 は建てない。スロットプールには置かない。**
   ...
   決して設定しない**——設定した瞬間、`LaunchType` を書き忘れたサービスが GPU インスタンスに乗る
   ...
   クラスタのインスタンスを条件なしに全部歩くので MI のインスタンスも見る。P0 は `DescribeContainerInstances` の
   `capacityProviderName` で MI のインスタンスを除外し、テストでそれを固定する（レビュー R7）。

2. **エンジン役は 2 つ（`llm`・`image`）。役ごとに capacity provider を分け、同じインスタンスに載せない。**
   ...
   **g6.xlarge（L4 24 GB・$1.26/h 込み）**で、`image` を安くする下限は測ってから決める。**インスタンス要件は
   役ごとのスタックパラメータ**（宣言する。導出しない。ADR 0053 の形）。同じインスタンスに 2 役を
   ...
   **20.9 GB**、SDXL が **7.4 GB** で、L4 の 23 GB に両方は入らない。片方が起きているだけの
   ...
   7〜8 分は前のインスタンスが 4 vCPU を握る**ので、片役の建て直しがもう片役の起動と重なると
...
3. **モデルは HF → S3 に一度だけ写し、起動のたびに S3 → ローカルディスクへ引く。HF を起動経路に
   ...
   HF は同じ NAT の先で **4〜236 MB/s**、**同じファイルでも 6 倍違った**（SDXL: MI のインスタンスの curl
...
6. **エンジンは llama.cpp・stable-diffusion.cpp・ComfyUI の 3 つで、前 2 つは公式イメージを
   ...
   - ComfyUI: 公式イメージが無いので、**ComfyUI のタグを固定した Dockerfile** を
     ...
     コードをフリートのインスタンスで実行する入口になる。画面は CP のリバースプロキシ
...
7. **コントローラは `tts_control.go`（0070 P1: `ttsDemand` / `decideEngineAction` /
   ...
   クールダウンが倍々に伸びる。`draining` は CPU インスタンスで **93 秒**、**GPU インスタンスでは 427 秒と 463 秒**
   （実測で解けた点 4）——ただしこれは MI の `infrastructureOptimization.scaleInAfter`（null＝
   既定・−1＝片づけない・0〜3,600 秒）を既定のまま測った値で、MI 固有の固定費と決めるのは早い。
   P0 で 0 と −1 を 1 回ずつ測り、−1 は「インスタンスを残す」設計の材料にする: desired 0 のあとインスタンスが残って
   いる間に来た要求は、イメージもモデルファイルもインスタンスにあるので S3 取得なしで起きるはずである。
...
10. **由来は結果の一部（0069 決定 11）。** provider は `llamacpp` / `sdcpp` / `comfy`、モデルは
    **ファイル名と sha256**（HF の repo id とリビジョンも取り込みジョブが残す）。プロンプトは
    コンテナの外へ出るが、宛先はフリートのインスタンスである——それも書く。
...
13. **現況は「分からないことを書かない」形で出し、稼働実績はコントローラのティックを
    ...
    （P1.5）。トグルだけの画面では「切っていいのか」が判断できない——$1.26/時のインスタンスは普段
    寝ているので、**押す前に見えるのは全部この画面が言うことだけ**である。設計のほとんどは
    「出さない」側の規則になった:
    - **起動時刻はインスタンスの `registeredAt`**（ECS の `describe-container-instances`）を第一とし、
      無いときだけサービスの `lastStart` に落ちる。両者は**別の事実**で、`lastStart` は
      スタック更新やタスクの差し替えでも動く——インスタンスを買い直していなくても。
      🔴 `ec2 describe-instances` は使わない（P1 の実測 2: **MI のインスタンスは一覧に出ない**ので、
      EC2 側で実装したら「動いている GPU についてインスタンスは無い」と答える）。
...
2. **Managed Instances のコールドスタートは CPU インスタンスで 109 秒、ドレインは 93 秒。**
   ...
   作った数字である。GPU インスタンス（2.3 GB の CUDA イメージ、17 GB のモデル）は未実測——3 を参照。
...
4. **GPU（g6.xlarge・L4 24 GB）でのコールドスタートとドレイン。** AMI は
   ...
   2.47 GB に 171 秒）、+232 秒でタスク RUNNING**。ここまでは CPU インスタンスと同じ形である。その後が
   問題で、**モデルの取得に 1,846 秒、VRAM へのロードに 272 秒、合計 2,351 秒（39 分）**
   かかった——取得を S3 に替える理由が実測で解けた点 5 である。**ドレインは 427 秒と 463 秒**
   （CPU インスタンスの 93 秒に対して 4〜5 倍）。desired 0 のあとも 7〜8 分は g6.xlarge の料金と管理料が
   走るので、**アイドル窓を切り詰めても回収できるのはその分だけ**である（窓 1 回 $0.65 に対し
   ドレインは $0.15）。sd-server 側は同じインスタンスで **タスク作成から listen まで 195 秒**（pull 135
   秒＋6.9 GB の取得 28 秒）。
5. 🔴 **HF からの取得速度はファイルで 50 倍違い、Qwen3-Coder の GGUF は何で引いても遅い。**
   ...
   **起動が 39〜74 分になるか 4 分で済むかが、引くまで分からない**という話だった。S3 からの
   ...
   一般化だった**——同じ SDXL を Fargate の取り込み curl は **39.6 MB/s** で引いている（MI のインスタンスの
...
6. **VRAM と G 系クォータ。** Qwen3-Coder-30B-A3B Q4_K_M は **20,943 MiB**、SDXL fp16 は
   ...
   **408 秒後**に placement した。両役を同時に起こす配備は 16 vCPU を申請する。🔴 **レビュー R2 で
   ...
   （07:19〜07:20）は直前に MI が片づけたインスタンス（07:17:13 終了）がまだ 4 vCPU を数えられていた間で、
   llm のインスタンスが shutting-down のうちに image のインスタンスが 07:26:04 に起動している。**8 vCPU で 2 台は
...
- **エンジンを 1 インスタンスに同居させる。** 費用は最大で $1.26/h 減るが、VRAM の取り合いは
  クラッシュとして現れる（決定 2）。
...
volume はインスタンスを残しても効かず（下の 4）、ECR 複製の効果は placement のばらつきに埋もれた（1）。
実測にかかった費用は g6.xlarge 約 1.4 時間で $2 程度。

1. **本番相当のコールドスタートは 586 秒**（`desired 1` → listen）。イメージは ECR、モデルは
   S3、インスタンスは無し。内訳: **+88 秒で task 作成と placement**、pull **79 秒**（GHCR の 178 秒に対して
   ...
   8 秒だったところが今回は 88 秒で、支配的なのは pull ではなく**インスタンスが現れるまでの時間の
...
3. **`scaleInAfter: -1` は本当にインスタンスを残す。** `desired 0` から **583 秒後も `running`** のまま
   （既定なら 456 秒で terminated）。🔴 ただし **`scaleInAfter` を後から変えても、既に `-1` の
   下で idle になったインスタンスは回収されない**——`0` に変えて 497 秒待っても `running` のままだった。
   残したインスタンスを片づける手は `ecs update-container-instances-state --status DRAINING` で、これは
...
4. 🔴 **決定 7(c) の「温かいインスタンス」は P0 では成立させられなかった。未証明のまま残す。** 二段構えで
   外れた:
   ...
   - そこで `Host: { SourcePath: /var/lib/af-engine-models }` に直したところ、**新品の 120 GiB の
     インスタンスでも `No space left on device`** でサービスが一度も起動しなくなった。🔴 つまり
   ...
   温かいインスタンスを成立させるには MI のデータボリュームが実際にマウントされているパスが要る——
   文書化されておらず、GPU 1 時間を追加で払う価値は無いと判断した。P4 の
   `useLocalStorage`（インスタンスストア 250 GB がディスクそのもの）で拾い直す。
5. **プール走査が MI のインスタンスを見ること**は本番のクラスタで現物を確認した。
   ...
   無ければ `registeredSlots` はエンジンのインスタンスを「空きスロット」と数え、ドレインに入った時点で
...
10. ✅ **完了の定義の後半——「停止中のエンジンへの最初の要求が 1 回の試行で答えを返す」——を
    実物で通した。** af-sandbox に P0 のコードを配備し（`0.16.1-dev-5b62a9b4`）、GPU のインスタンスが
...
- **S3 → インスタンスは 115〜147 MB/s**（18.5 GB を 126・161 秒）。0070 のハーネスの 104〜147 MB/s と
...
1 つ: 🔴 **MI のインスタンスは `describe-instances` の一覧に出ない**ので、P0 が「GPU 0 台」の根拠に
...
1. **image 役のコールドスタートは 197 秒**（`execute-change-set` から
   `listening on: http://0.0.0.0:8080` まで。イメージは ECR、チェックポイントは S3、インスタンスは
   無し）。内訳: **+57 秒でタスク作成**、+61 秒でインスタンスが container instance として登録、
   ...
   **6.94 GB を 65 秒＝107 MB/s**、そして **+197 秒で listen**。llm 役の 586 秒の
   ...
   全体は 5 分 4 秒。
2. 🔴 **MI のインスタンスは `ec2 describe-instances` の一覧に現れない。id で名指しすれば返る。**
   ...
   **g6.xlarge / running / `af-role=engine-image`** を返す（`OwnerId` は自分のアカウント、
   ...
   - 費用配分タグは効いている——インスタンスに `af-role=engine-image` が付いていた（決定 9）。
3. **sd-server の面は本番の経路で通った。** CP の SG に置いた使い捨ての Fargate タスクから
   ...
   **512px 11.2 秒（初回）・1024px 17.5 秒**、`"n":2` の 512px が **10.7 秒で 2 枚**
   ...
   （何のヘッダも付けずに 200）。P0 の 512px 7.8 秒 / 1024px 20.8 秒とは別の日の別のインスタンスなので、
...
5. **VRAM は params 6,624 MB**（P0 と同じ値）。auto-fit は DiT 4,897 MiB・Conditioner
   ...
   20.9 GB と足すと L4 には収まらないという決定 2 の根拠が、別の日の別のインスタンスでもう一度出た。
...
11. ✅ **暖まっていれば経路は全部通る。** 同じセッションから、CP ゲートウェイ→provider→
    ...
    - **エンジンを起こさずに答える面も確認**: `/imagegen/status` はインスタンスが 0 台のまま
...
13. ✅ **直したうえで測り直したら、完了の定義の残り半分が実物で成立した（2026-09-08）。**
    `0.16.1-dev-2e501835`（決定 5 の分岐入り）を配備し、**インスタンス 0 台**の状態から
...
15. ✅ **P1.5 のトグルを実機で三モードとも通した（2026-09-08・利用者が Console で押した）。**
    `0.16.1-dev-c346ad66`。CP のログ（UTC）:
    02:41:05 `stop (idle)`（**コントローラが自分で止めた**＝アイドル停止も同時に確認できた）
    → 02:46:52 `image set to off` → 02:46:54 `set to on` → **02:49:53 `warmed up (ready)`**
    → 02:50:11〜02:53:33 に生成 6 枚（初回 11.361 秒・以降 5.4〜5.5 秒）
    → 02:58:31 `set to off`（インスタンスは停止）→ 03:03:30 `set to ondemand`。
    **`off` が止め、`on` が起こし、モードが永続する**——三つとも実物で確認できた。
    `on` からの起動は **179 秒**で、コールドスタートは **165 / 172 / 173 / 179 / 197 秒**の 5 点目。
    - 🔴 **`offGrace: 0`（undo 窓を作らない）の根拠は、これでは検証されていない。**
      2 秒差の OFF→ON が実際に起きたが、その時点でインスタンスは 02:41:05 に既にアイドル停止しており、
      **off で失うものが無かった**。「OFF→ON がコールドスタートを 1 回買う」は未実測のまま。
    - 🔴 **`off` は永続する設定であって一時停止ではない**、という運用上の落とし穴を踏んだ:
      試験のあと `off` のまま 5 分放置され、その間 `generate_image` は `503 engine_off` で
      断られ、カタログからも消えていた。**インスタンスが止まっているだけの状態と区別がつきにくい**
      （どちらも desired 0）。パネルの文言はそこを言っているが、押した本人が戻し忘れる形は
      残る。P4 で「一時的に止める」を別に用意するか検討する。
...
- **`describe-instances` を一覧で見るなの続き**（実測 2）: 今回もエンジンのインスタンスは ECS 側
...
## 在庫切れでインスタンスが建たない——候補を 1 機種にしていた（2026-09-08）
...
`VcpuLimitExceeded` になる。純粋に、その時刻のその AZ に L4 のインスタンスが無かった。間に
...
証拠でもある。だから**優先順位は価格順で表現し、狙いのインスタンスを集合の最安に保つ**——上の価格表で
...
予備は狙いのインスタンスより**高い側**に置く。8 vCPU 機種（g6.2xlarge）を既定に入れないのも同じ用心で、
...
- **P1.5 — 実機で見つかった 2 つの穴（2026-09-08）。** 決定 5 の分岐（実測 9）、
  ...
  **「無効」はその場でインスタンスを止める**こと: TTS の undo 窓は誤爆した OFF→ON が 2 GB の pull と
  ...
  持たせて直した。**三モードとも実機で押して確認済み**（実測 15）。
  **同じ画面に現況を足した（決定 13）**——トグルだけでは「切っていいのか」が分からないため。
  `GET /api/admin/engines` の行にインスタンスの起動時刻・自動停止の予定・直近の使用状況・warm を足し、
...
- **P3 — llm を codex と claude へ。** codex は `model_providers` に `base_url`＋
  ...
  変わる**（利用者のログインでなくフリートのインスタンスに課金が移る）ので、起動メニューの表示と
...
- **R1. opencode の 300 秒は「壁」ではなく「無音の上限」である。** このコンテナの opencode 1.18.29
  ...
  **6 回の試行**（再送 5 回。間隔は 302・310・316・330 秒と伸び、切断からの待ちは 2・4・8・16・
  ...
  である）。
- **R2. 8 vCPU で g6.xlarge は 2 台建つ。1 台しか建たなかったのは、ドレイン中のインスタンスがクォータを
...
- **R4. 単価は AWS 価格 API で一致した。** g6.xlarge $1.1672/h（東京・Linux・オンデマンド）、
  ...
  計算違いは無い。GPU 実測の請求は CloudTrail 上の 6 インスタンスの合計が約 1.5 時間（本文の「約 1 時間」
  より多い）で、NAT 込みおよそ $3.5。
- **R5. MI の API には ADR が触れていないつまみが 2 つある**（aws-cli 2.36.40 のサービス定義）:
  `infrastructureOptimization.scaleInAfter`（**アイドルなインスタンスを片づけるまでの秒数**。null＝既定、
...
- **R6. HF の速度は「ファイルで決まる」とも言えない。** 同じログ群（`/af/af-ecs-engprobe/engine`）
  ...
  175 秒＝39.6 MB/s で引いている**——MI のインスタンスのサイドカー（同じ curl・同じ NAT）が 236 MB/s
...
- **R7. コードの現物。** (a) スロットプールの `registeredSlots` と `sweepGhostInstances`
  （`runtime_ecs_ec2.go`）は **クラスタの container instance を条件なしに全部歩く**——MI のインスタンスも
  並ぶ（ハーネスの最初の 1 回がスロットのインスタンスを測ってしまったのはこの裏返し）。EC2 タグ起点の
  ...
  とも desired 0・G 系のインスタンス 0 台・クォータ 8 のまま（起こしていない）。
...
- **決定 1** — 末尾に足す: 「ADR 0045 決定 6 は Workspace について MI を退けた（ライフサイクルの
  ...
  `sweepGhostInstances`）は MI のインスタンスも見る。P0 は `DescribeContainerInstances` の
  `capacityProviderName` で MI のインスタンスを除外し、テストでそれを固定する。」
- **決定 2** — 「8 vCPU では 1 台しか建たず……408 秒待った」を R2 のとおりに書き換える:
  「8 vCPU で 2 台は建つ。ただし**落とした直後の 7〜8 分は前のインスタンスが 4 vCPU を握る**ので、
...
- **決定 7** — 3 点。(a) 状態は 0070 決定 7 の `stopping`（OFF の取り消し窓）と `tts_ecs.go` の
  ...
  −1 は「インスタンスを残す」設計の材料になる: desired 0 のあとインスタンスが残っている間に来た要求は、
  **イメージもモデルファイルもインスタンスにある**ので S3 取得なしで起きるはずである（VRAM ロードだけ）。
  「アイドル窓を切り詰めても回収できるのはドレインぶんだけ」は、ドレインを純粋な無駄と読んで
  いる——温かいインスタンスの窓として設計に組み込めるかを、P0 で「停止 → ドレイン内に再要求」の 1 回で
  確かめる。
...
| 決定 | 何が変わるか | 変わらないもの |
|---|---|---|
| 決定 1「GPU は ECS Managed Instances で買う」 | **CP が EC2 Fleet で買う**。却下していた自前 EC2 の 3 点は 0077 の背景の表のとおり解けた | 「タスクが無ければインスタンスが存在しない」（0077 決定 5 で CP が保証する）／スロットプールには置かない |
| 決定 2「役ごとに 1 つの capacity provider」 | **provider は無くなる**。役ごとに launch template 1 本 | 2 つの役を同じインスタンスに載せない |
| 決定 7「`draining`」 | 意味が「MI の管理外のドレイン」から「CP の terminate 発行〜`terminated`」へ | 状態集合・退場待ち |

決定 5 の起こして保つ窓と決定 8 のエンジン表は引き継がれる。表の
`capacityProvider` / `spotCapacityProvider` の対は `launchTemplate` 1 欄になり、そこが
テンプレートとコントロールプレーンの継ぎ目である。この決定が Managed Instances を選んだ 3 つの
理由は 2026-09-12 に読み直され、0077 の背景にある——AWS 管理の ECS 最適化 GPU AMI は SSM の公開
パラメータから引ける、エンジンは停止せず終了する（「停止中も EBS が課金される」状態が無い）、
ADR 0045 の掃除から外れることはタグと ECS 属性 1 つで足りる。
