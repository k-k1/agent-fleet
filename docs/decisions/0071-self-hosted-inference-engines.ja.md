...
3. **モデルは HF → S3 に一度だけ写し、起動のたびに S3 → ローカルディスクへ引く。HF を起動経路に
   ...
   HF は同じ NAT の先で、SDXL（stabilityai）を **236 MB/s** で配る一方、Qwen3-Coder の GGUF
   （unsloth）は `llama-server -hf` で **9.6 MB/s（31 分）**、Fargate の `curl` では **4.2 MB/s
   （74 分）**でしか配らなかった。**ファイルによって 50 倍違い、どちらになるかは引くまで
   分からない。** 起動経路に置ける速度ではない。S3 は同じ箱へ **147 MB/s**（実測で解けた点 8）で
   一定に配る。費用（17 GB あたり NAT $1.05）とゲート用トークンをノードに置く話は、その上に
...
7. **ComfyUI は同じ L4 で SDXL を 8 秒で描き、VRAM は sd-server と同じ 6.9 GB。** コミュニティ
   ...
   sd-server の利点は**小さいイメージ（2.3 GB 対 5.1 GB）と OpenAI 互換の面**、ComfyUI の利点は
   **1 枚あたりの速度とワークフローの自由度**で、費用は窓で決まる（決定 5）ので 1 枚 13 秒の差は
   請求書には出ない。順序は据え置くが、絵を大量に作る配備では ComfyUI が本命になりうる。
...
1. **VRAM へのロード 267 秒を縮められるか。** S3 化で取得は 179 秒になり、残る大物は EBS gp3
   から 18.5 GB を読む時間である。MI の `LocalStorageConfiguration`（g6 のローカル NVMe）か
   gp3 のスループット指定で何秒になるかを P0 で測る。
