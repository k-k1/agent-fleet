...
- 誘導つきの 2 族（SDXL・SD3.5）は cfg 7・4.5 でサンプリングし、ネガティブの枝はまさに
  guidance を計算する相手である;
- 蒸留系の 3 族は違う。zimage と klein は **cfg 1** で、`uncond + 1*(cond - uncond)` は
...
できない族が「利用者が何も頼まなくても」そう言う理由もこれである（0072 を参照）。
...
実機未検証。どの族にネガティブの枝があるかについての上の主張はすべて、このリポジトリが送る
...
VAE 往復のために GPU の箱を 1 台起こすことになる。ポインタは、それを「未指定」と読んで既定の 0.6
...
5 族すべてが編集を利用者の絵から始めるし、その量はこのパッケージが書くグラフの中の数字であって、
...
### 部分 denoise が見かけどおりでなかった 1 族

5 族のうち 4 族はサンプリングの前にスケジュールを**伸ばす**。`KSampler.set_steps` は
...
`Flux2Scheduler` へ `int(steps/strength)` を要求するようにした。他の 4 族がしていることの再現で
...
測ればもう偽である: `comfyFamilyKnobs` が宣言するとおり、**7 族すべてが `steps` と `sampler`
を読む**（5 族は `KSampler`、flux1 と klein は `KSamplerSelect`）。読まないものは 2 つだけ
...
`sampler` と `scheduler` にはこの危険が無い——**どの名前もどの族でも通る**ので、enum が真実の
全部である。
...
**実機未検証。** 7 族が `sampler_name` を受けることは `comfy_workflows_test.go` が固定して
