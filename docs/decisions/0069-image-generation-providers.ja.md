...
0072 の edit / inpaint の 5 ファミリーとまとめて次の配備の回（H3）に当てる。

## 追記 — 保存済みの順番に無い provider の入れ場所（2026-09-11）

`effectiveOrder` は、保存された `imageProviderOrder` に名前の無い provider を**末尾に足して**いた。
それが実害を出したのを 0072 の実機検証（2026-09-11）で測った: 開発配備の保存値は
`["sdcpp","agy","codex"]` で、`comfy` が増える前に書かれたものだった。規則どおり comfy は末尾に
足され、`auto` は sdcpp（その配備には存在しない）→ agy → codex → comfy を歩く。つまり
**provider を省いた呼び出しは、フリートが金を払っている GPU に届く前に利用者本人のプランを使う。**
誰も設定を触っていないのに、provider が増えた日に保存値の意味が変わった——正規化という工程が
まさに防ぐためにある事故である。

**規則を変える: 保存値に無い provider は、フリート自前なら先頭側、外部プランなら末尾側に入れる。**
保存値が明示的に並べたものの相対順は動かさない（利用者がわざわざ最後に置いた自前エンジンも、
そのまま最後のままである）。この段が決めるのは「一度も名前が挙がらなかったもの」の位置だけ。

「自前か外部か」は provider の属性として 1 か所（`providerRanks`）で宣言する。`Caps` に置かない
のは、Caps が (provider, model) 毎だからで——**誰の財布が払うかはチェックポイントで変わらない**
——そこに置くと provider 本体と違う答えを返すモデルを招く。`id == "comfy"` のような条件を散らす
のは、次の provider が忘れられる経路そのものなので取らない。Console 側（`settings.ts` の
`IMAGE_PROVIDERS_RANKED`）にも同じ宣言を置いた。あちらの `normalizeImageProviderOrder` は
「Agent と同じ規則」を名乗っており、片方だけ直すと**利用者が画面で並べた順と `auto` が歩く順が
食い違う**。設定画面の説明文（ja/en）も、既定が「Antigravity が先」だった古い記述から、
自前が先・外部が後という現在の規則に直した。

## 追記 — 広告されているツールが自分の呼び出しを拒んでいた（2026-09-11）

**機構。** `mcpx` の `mcpStdioCall` は、呼び出し側でも「その名前を広告したか」を
`mcpStdioToolAdvertised()` で確かめる。これは推測した名前で未広告のハンドラに届かせない
ための境界で、それ自体は要る。問題は**確かめ方**だった: `mcpStdioToolList()` を呼び直し、
その中の `mcpImageGenAdvertise()` が Agent の `/imagegen/status` への**生きた HTTP 往復
（タイムアウト 3 秒）**に落ちる。status は各 provider の `Ready()` を呼び、フリートの
エンジンではそれが `engineToken` → Control Plane への往復（タイムアウト 15 秒）になる。

**再現条件（実機で踏んだ・ADR 0072 H3、2026-09-11）。** CP を
`update-service --force-new-deployment` で入れ替えた直後に `generate_image` を呼ぶと、
status が 3 秒を超え、**クライアントのツール一覧に載っているツールが**
`この対話セッション用サーバーでは許可されていないツールです: generate_image`
**を返す**。文面が権限の問題に見えるので、原因の切り分けに 30 分かかった。

**修正。** 呼び出し側の再評価をネットワーク越しにしない。直前の `tools/list` が実際に返した
名前の集合をプロセス内に保持し（`mcpAdvertised`）、呼び出し側はそれを読む。**クライアントが
呼んでよいのは、こちらが呼んでよいと告げたものであり、それはこのプロセスが既に持っている
事実で、読み直しがタイムアウトすることはない。** `tools/list` より前に `tools/call` が来た
ときだけ、比べる相手が無いので従来どおり評価する（実クライアントは通らない経路だが、
推測した名前に対する境界はそこでも保たれる）。`generate_image` の有効/無効の門
（`mcpImageGenEnabled`）は同じ関数の数行下にそのまま残る——こちらは設定であって、
ネットワークを見ない。

拒否の文面も直した。理由は「権限が無い」ではなく「その名前は `tools/list` に無い」であり、
2 つは正反対の対応を要求する——古い文面は、実際には存在しないツール名を名乗っただけの
モデルを設定画面へ探しに行かせていた。

## 追記 — `negative_prompt` を語彙に入れる（2026-09-12）

この ADR が「入れない」と言っていた 2 つ目の語。理由は `seed` と同じ種類で、絵の見え方を
変えるからではなく、**利用者が他の言い方では表現できない**からである。

「見え方」の摘み——steps・cfg・sampler——は引き続き外に置く。理由も変わらない: プロバイダ間の
差が大きすぎて共有語にならず、摘んでも何も動かない摘みは、システムについて誤ったことを
学ばせる。ネガティブプロンプトはその種類ではない。`prompt` の言い換えで書き下せるものでは
ないからだ——その語は classifier-free guidance の**無条件側の枝**を通ってサンプラーに届く。
`prompt` に書くのはその逆で、「文字を入れないで」は文字を条件に入れる。利用者側に回避手段が
無い、というのがこの語彙の判定基準である。

**形。** `Request.NegativePrompt string` と `Caps.Negative bool`（`Request.Seed` /
`Caps.Seed` と並ぶ）。ツールがこの引数を出すのは、その経路のどれかのモデルにネガティブの枝が
あるときだけ。seed と違いポインタではなく素の文字列にした: ここでは空文字と「未指定」が
本当に同じ要求だからである。空のネガティブプロンプトが意味するもの——「カタログが宣言した
ものすら除外するな」——は誰も望んだことがなく、かつ管理者が言いくるめられてはいけない唯一の
ものだ（合成の場所は ADR 0072 の追記）。

**どの経路が受けるか。** comfy だけ。しかもその中でも**モデルによって違う**:

- 誘導つきの 2 ファミリー（SDXL・SD3.5）は cfg 7・4.5 でサンプリングし、ネガティブの枝はまさに
  guidance を計算する相手である;
- 蒸留系の 3 ファミリーは違う。zimage と klein は **cfg 1** で、`uncond + 1*(cond - uncond)` は
  厳密に `cond`——語はグラフに載り、テキストエンコード分を払い、1 画素も変えない。FLUX.1 は
  guidance を条件側に畳み込んでおり（`BasicGuider`）、ネガティブの入力自体が無い;
- agy と codex にはそもそもそのパラメータが無い。

つまり `Caps.Negative` は、このパッケージで初めて**1 台の稼働中エンジンの中でモデルごとに
答えが違う**能力である。決定 5 が Caps を (provider, model) 単位にしたのはこの場合のため
だった。プロバイダの答えを全モデルに当てはめれば、別のチェックポイントを名指せば使える
セッションから引数を隠すか、何もしない引数を約束するかのどちらかになる。

**黙って劣化させない**——この規則が最も効くのがここである。アスペクト比を落とせば形の違う絵が
出て、seed を落とせば 2 回目に気づく。ネガティブプロンプトを落とした絵は**成功に見え**、
利用者が入れたくなかったものがそこに写っている。能力を切り上げて報告しない理由も、除外
できないファミリーが「利用者が何も頼まなくても」そう言う理由もこれである（0072 を参照）。

上の `seed` の追記の 1 行も退役する: 「この ADR の語彙にはネガティブプロンプトも steps も
cfg も無い」は、いま steps と cfg についてだけ正しい。そこで説明したキャッシュ警告は影響を
受けない——ネガティブプロンプトはグラフの一部なので、変えれば絵が変わる（キャッシュは返らない）。

実機未検証。どのファミリーにネガティブの枝があるかについての上の主張はすべて、このリポジトリが送る
グラフ（`comfy_workflows.go`）と ComfyUI v0.34.0 のノード定義から読んだものであり、
SD3.5 のテンプレートが「1 枚も生成できない」と判明する前に成り立っていたのと同じ種類の主張である。

## 追記 — `strength` を語彙に入れる（2026-09-13）

3 つ目の語。入れる理由は前の 2 つと同じ判定基準、すなわち**利用者側に回避手段が無い**である。

`op=edit` の自由度はちょうど 1 つ――利用者自身の絵をどれだけ残すか――で、それが定数だった
（`comfyEditDenoise = 0.6`）。つまり「少しだけ直す」と「構図だけ借りて残りは描き直す」は同じ
要求であり、同じ絵が返る。`prompt` の言い換えでも seed でもネガティブプロンプトでも引き離せない。
これは既に足した 2 語より鋭い。seed は 2 回の呼び出しを比べるための語、ネガティブプロンプトは
条件づけの第 2 の枝だが、これは **op 自身のパラメータ**である。この数字を固定したまま `edit` を
提供するのは、`size` の候補を 1 つだけにして提供するのに等しい。

provider 私有の摘みでもない――steps・cfg・sampler を外に置き続ける理由はそこにある――img2img の
実装はどれもこの数字を持っており、綴りが 3 通りあるだけである: diffusers は `strength`、
A1111 は `denoising_strength`、ComfyUI は `denoise`。

**向きは引数の一部であって、推測させるものではない。** 0 が利用者の絵を保ち、1 が無視する
（diffusers の向き）。上流はここで一致していない: Stability の `image_strength` は逆向きなので、
名前から推測した呼び出し側は五分五分で逆の絵を頼むことになる。どちらの端が何かはツールの説明文に
書く。

**形。** `Request.Strength *float64` と `Caps.Strength bool`（seed・ネガティブプロンプトと並ぶ）。
ポインタにしたが、seed とは理由が違う。ここでは 0 は使える値ではなく**拒む**値である: denoise 0
で ComfyUI のサンプラーは latent をそのまま返すので、受け付ければ利用者が既に持っている絵の
VAE 往復のために GPU のインスタンスを 1 台起こすことになる。ポインタは、それを「未指定」と読んで既定の 0.6
で――頼まれたことの正反対で――黙って編集するのではなく、**値として理由つきで拒む**ためにある。
受ける範囲は `(0, 1]` で、外れた値は `HandleGenerate` が `bad_strength` で断る。丸めないのは、
丸めた要求はどの provider も正直に報告できないからである。

**能力が真でも edit だけ。** inpaint は denoise を 1 のまま保つ。inpaint のマスクの外を守って
いるのは `SetLatentNoiseMask` であって部分 denoise ではないので、そこで下げても何も守られず、
塗り直した領域が元の弱い反響になるだけである。`op=inpaint`（や `generate`）に付いてきた strength
は warnings で「無視した」と言う。ツールがこの引数を出すのを提供 op に `edit` があるときだけに
したのも同じ理由で、`inpaint` があることは、グラフが捨てる義務のある数字を呼び出し側に渡す理由に
ならない。

**どの経路が受けるか。** comfy だけ。しかも `negative_prompt` と違い、**どのモデルでも**受ける:
5 ファミリーすべてが編集を利用者の絵から始めるし、その量はこのパッケージが書くグラフの中の数字であって、
vendor の API が公開している欄ではない。sdcpp は受けない――`/v1/images/edits` に渡すのは
`prompt` / `n` / `size` / `image` / `mask` だけである。agy と codex は prompt で CLI を駆動する
だけで、そのパラメータ自体が無い。3 つとも warnings で言う。

### 部分 denoise が見かけどおりでなかった 1 ファミリー

5 ファミリーのうち 4 ファミリーはサンプリングの前にスケジュールを**伸ばす**。`KSampler.set_steps` は
`new_steps = int(steps/denoise)` を計算して最後の `steps+1` 個のシグマを取り
（`comfy/samplers.py`）、`BasicScheduler` も同じことをする。つまり**実際にサンプリングする
ステップ数は denoise で変わらず**、スケジュール上のどこから始めるかだけが動く。klein はそれが
できない: `Flux2Scheduler` に denoise 入力が無いので尾を `SplitSigmasDenoise` で切るが、この
ノードの尾は渡されたシグマの `round(len(sigmas)*denoise)` ステップである
（`comfy_extras/nodes_custom_sampler.py`。仮定ではなく読んで確かめた）。

伸ばさない 4 ステップのスケジュールを切っていたので、**編集が穏やかなほどサンプリングのステップが
減っていた**: 固定の 0.6 で 2 ステップ、0.25 なら 1 ステップ。0.6 に決め打ちしている限り誰にも
見えない――蒸留モデルの 4 ステップのうち 2 ステップでも絵は出る――が、摘みを回した利用者が最初に
踏む形そのものであり、「klein は小さな編集が苦手」と読めてしまう。klein は分割の前に
`Flux2Scheduler` へ `int(steps/strength)` を要求するようにした。他の 4 ファミリーがしていることの再現で
ある。これは既定の 0.6 でも klein の編集グラフを変える: 実効 2 ステップだったものが 4 になる。

**実機未検証。** グラフの形は `comfy_workflows_test.go` が固定している（klein の伸長を含む）し、
上に挙げた上流ノードの式は ComfyUI 自身のソースから読んだ。SD3.5 のテンプレートが「1 枚も生成
できない」と判明する前に成り立っていたのと同じ種類の主張である。実機で見るなら、モデル 1 つ・
入力画像 1 枚・seed 1 つで strength を大きく離した 2 点（0.2 と 0.8）＝画像 2 枚。入力からの
距離が目に見えて違うことが合格条件で、klein はステップ数が変わった分だけ別に 1 組要る。

## 追記 — `params`（steps・cfg・sampler・scheduler）を語彙に入れる（2026-09-15）

4 つ目で、この ADR が名指しで「入れない」と言い続けてきた 3 語そのものである。判定基準は前の
3 語と同じ——**利用者側に回避手段が無い**——で、`dpmpp_2m` は prompt の言い換えでも seed でも
ネガティブプロンプトでも頼めない。変わったのは、外に置く**理由**のほうが成り立たなくなった
ことである。

理由は 2 つ挙げられていた。「provider 差が大きすぎる」と「摘んでも何も動かない」。後者は
測ればもう偽である: `comfyFamilyKnobs` が宣言するとおり、**7 ファミリーすべてが `steps` と `sampler`
を読む**（5 ファミリーは `KSampler`、flux1 と klein は `KSamplerSelect`）。読まないものは 2 つだけ
——flux1 と klein の `cfg`、klein の `scheduler`——で、そのどちらも ADR 0081 決定 4 が作った
`comfyIgnoredParamWarnings` が**名指しで警告する**。前者は `strength` が既に踏んだ形（comfy
だけが受け、他は warning）で、そのときと同じに扱う。

**形は入れ子の `params` オブジェクト**で、キーは `EngineParams` 自身の綴りである。フラットな
4 引数にしなかったのは、1 つの事実が 2 つの綴りを持つと中継のどこかで静かに落ちるからで
（sessionWire の教訓）、この形ならツール → Agent の REST → キュー → カタログ行 → サイドカーが
全部同じ語で書かれる。

**ブロッキングルートにも `validateRequestParams` を付けた。** これが最初の落とし穴だった:
0081 はキューの側にだけ検証を置いており、MCP 経路に `params` を通すとそこだけキューより緩く
なる——`steps: 10000` が素通りし、共有 GPU の 1 時間をタイプミス 1 つで買える。0081 の非対称
（カタログ行には寛容、要求には厳格）はそのまま保たれる。

**`steps` と `cfg` は sampler と同じ危険度ではない。** 正しい値は**チェックポイントの属性**で、
スキーマはそれを運べない——enum は他の引数に依存できず、`loras` が BaseModel を説明文に書く
羽目になったのと同じ制約である。蒸留系の行（Krea 2 Turbo = 8 steps・cfg 1）に 30/7 を入れた
呼び出し側は焼けた絵を受け取る。そこで 4 語を 1 つのオブジェクトに入れたうえで、説明文に
**「利用者が言ったときだけ」「省略した欄はそのチェックポイントの公表値で走る」「steps と cfg
はチェックポイントごとで、この schema には出せない」**を書き、上限（150 / 30）で守る。
`sampler` と `scheduler` にはこの危険が無い——**どの名前もどのファミリーでも通る**ので、enum が真実の
全部である。

**enum は Agent の許可リストを中継する。** `mcpx` は `internal/imagegen` を import できないの
で、名前を書き写せば「schema が出した名前を Agent が断る」という組が作れてしまう。ペインの
フォームが `Studio.Samplers` を読んでいるのと同じ理由で、ツールも同じ一覧を読む。列挙が空の
経路＝グラフを組まない経路なので、そこでは引数ごと出さない。

**綴りは ComfyUI のもの**（`euler_ancestral`。利用者が言う `euler_a` は A1111 の綴り）で、
**別名表は作らない**——第二の語彙になり、どちらが正かを次の読み手が決められなくなる。enum が
正で、外れた名前は一覧つきで `bad_params` として断られる。
