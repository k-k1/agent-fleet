...
- What the operator asked for is three rules and one exception — **(1) buy whatever clears the
  ...
  be had, take on-demand; (3) a Spot instance dying suddenly is acceptable, and rule 2 rebuilds it.
...
- Related: [0074-engine-instance-classes.md](0074-engine-instance-classes.md) decision 1 (the
  ...
  means ECS is never called), decision 4 (a change reaches the next instance; wait for the old one to
...
**3. A human picks the instance, and the Control Plane does not know prices.** ADR 0045 decision 21
...
watch an instance **this side set to desired 0**. A Spot interruption takes the instance away with nobody
...
- **A row of the engine table holds exactly one capacity provider** (`engineDef.CapacityProvider`
  ...
  instance is this engine's instance (`box()`) resolve through that single name.
...
- **The parts of an engine table row that can be carried into a running process are the ladder
  ...
  the "where to write" and the "which instance is mine" name, forgetting the instance cache and the
...
| `get-spot-placement-scores` **answers differently per type SET**: 1/10 for one type, 9/10 for three (g6+g5+g6e). An instance arrived **42 seconds** after a 9 was recorded | same |
...
| `describe-instances` **cannot enumerate** an MI instance but **returns it by id** (the only route to `InstanceLifecycle: spot`) | 0074, "Spot であることの証明は" |
| A replacement instance starts with empty storage — re-fetching from S3 measured 6.62 GiB in 66 s (102.7 MiB/s) | 0071 |
| Cold starts: image 165-197 s, llm 527-586 s | 0071 |
| From `mode: on`, a Spot instance was ACTIVE in **42 s**, running at 281 s, warm at 307 s, one image at 403 s | 0074, "実測（すべて `mode: on` の 06:01:16Z から）" |
...
- **A row with no price would become a row that can never be bought.** ADR 0074's convention is
  ...
  at the start (the Spot instance bought in 0074 ran for 403 seconds). Making price the sort key
...
- **When the requirement is `unknown`, nothing is filtered.** The list is tried from the top.
  ...
  about refusing to buy an instance — refusing would leave a deployment holding one unmeasured model
...
- **An engine table row carries two provider names** — `capacityProvider` (on-demand, the
  ...
  one makes an instance bought through the other read as `box: null`, which is exactly what happened
  during the rename in 0074. #542 narrowed the provider name to one field on `engineECS` so
  that "where to write" and "which instance is mine" can never disagree; with two names they stay
...
### 5. Rule 2 — if no instance arrives inside the budget, move to the next offer. **The failure code decides how to wait**

The per-offer budget `<role>OfferBudgetSec` (default **180 s**) measures **from setting desired
to 1 until a container instance is ACTIVE**. Once the instance is there this clock stops and the
...
- **One audit line per move** (`engine.<role>.offer`). "Why are we running on the expensive instance"
...
### 6. Rule 3 — an interruption is "**there is demand and the instance is gone**". Start again from the top of the list
...
replaced (an OOM kill, a health check) the instance is still there, which is what ADR 0071 P0
measured and not an interruption. The CP has not moved the desired count. That is enough for
rule 3, because **the list is restarted from the top** whichever way the instance was lost: a Spot
...
- **An interruption is not counted as a failure.** The controller's `failures` (which doubles the
  ...
  interruption that gets no instance by the deadline IS a failed start, and counts as it always has**.
...
- **The threshold is declared by the operator** (`<role>SpotScoreMin`, default **0 = do not
  ask**). There are two data points (an instance 42 s after a 9; nothing in 17 minutes at 1), which is
...
- 🔴 **Proving it really was Spot needs `describe-instances` by id** (measured, 0074: an MI instance
...
🔁 **What would change this**: a case on real hardware where "the offer the CP chose" and "the instance
...
|---|---|---|
| capacity provider (four fields) | on save and **before every start** (idempotent) | nothing moves; only the spec of the next instance changes |
...
| the service's strategy, **written by CloudFormation** | every update that touches the service resource — every release that changes the task definition | **it reverts to the declared provider and a new deployment starts.** A running Spot instance is replaced by an on-demand one, and the CP has no part in it |

The running-0 limit survives even if (c) is right: it keeps the CP from ever creating, on its
own side, a state where the running instance and the strategy disagree, and passing (b)'s
...
service the window is a BOX, not a state** — a release's deployment replaces a running Spot instance
...
- **Choose an instance per request** (a cheap instance for this generation, a big one for this conversation).
  Rejected: there is one instance per role, and a cold start is 165-197 s for image and 527-586 s for
  llm. Choosing per request means several instances at once, which leaves both the G-family quota
  (8-64 vCPU) and ADR 0071 decision 2 (two roles never share an instance). Kept as open question 4.
- **Act on Spot's two-minute notice and raise an on-demand instance before the interruption.** The
...
- **ADR 0074 decision 4 (a change reaches the next instance; stop → wait for it to leave → start)
  stands.** ⚠️ **The drain wait is needed across purchase types as well** — the silent re-landing
  on the old card has nothing to do with how the instance was bought (measured, 0074 P1).
...
**that gap is what the row is worth.** ⚠️ **1.57 is arithmetic, not a bill** — the Spot instance in
...
on-demand pool is held by a draining instance, the Spot pool is free. ⚠️ **The drain wait (ADR 0074
...
- **The gateway's `AF_ENGINE_WAKE_TIMEOUT` (900 s) is not changed** (ADR 0071 decision 5). What
  ...
  already**, and the next one meets a warm instance. That is the shape ADR 0071 decision 5 already
...
"when there is no stock, an on-demand instance comes up by itself" — i.e. **the class of incident
...
- So **rule 2's fallback is real in quota terms too**: while the on-demand pool is held by a
  draining instance (measured in 0074 P1: `VcpuLimitExceeded` persisted for more than five minutes
...
2. Whether adding `DescribeTasks` (and one IAM action) to conclude that a loss was an interruption
   earns its keep. Detection works on the instance disappearing alone (decision 6); the addition would
...
4. **Choosing an instance per request** (rejected above). Revisit if "one instance per role" ever moves.
...
6. **The placement score threshold.** Two points (9 → an instance in 42 s; 1 → nothing in 17 minutes).
...
8. **The relationship with `--models-max`.** ADR 0074 decision 11 settled that the task
   ...
   offers are chosen by VRAM, "a bigger instance should hold two" becomes the natural expectation — but
   that lives where ADR 0074's open question 6 lives (the task definition and CloudFormation).
   **This ADR does not move it.**
9. **How often interruptions actually happen.** The one Spot instance 0074 bought ran for 403 seconds.
   Whether decision 6's "skip an offer interrupted twice" is needed cannot be known until this is
   filled.
10. 🔴 **The orphan instance.** After the move from the Spot offer to on-demand (decision 4 (b)), is
    the launch the Spot provider had **already started** cancelled, or does the instance arrive late
    and sit with no task to carry until `scaleInAfter` (measured in 0071: a GPU instance drains in
    427-463 s)? An MI provider goes to buy when it sees "task cannot be placed", so an instance from
...
| 3 | Desired 0 → 1 picks the Spot offer and the instance arrives as **`InstanceLifecycle: spot`** | `ec2InstanceId` from `ecs describe-container-instances` → `describe-instances --instance-ids` |
| 4 | Rule 2's fallback | Put a deliberately unbuyable type in the Spot offer (out of stock, or a misspelling) and watch the on-demand instance arrive after the budget. **This is P0's positive control.** Keep watching the old provider's `describe-container-instances` for a few minutes **after** the move and record whether an orphan instance (open question 10) turns up |
| 5 | `box()` accepts an instance from **either** provider | `box` in `GET /api/admin/engines` is not `null` (during the rename in 0074 it was) |
...
1. A test can say desired 1 / running 0 / no instance is treated as an interruption and **does not
...
- **The intended route is AWS FIS's Spot interruption injection**
  ...
  🔴 **Not verified**: whether an MI instance can be an FIS target has not been measured.
  `describe-instances` **cannot enumerate** MI instances (it returns them by id), so **whether FIS,
  which resolves targets by tag or filter, can find this instance is unknown.** Naming the id directly
...
- **The fallback positive control (if FIS cannot be used)**: `ec2 terminate-instances` **by id**,
  ...
  is not an interruption, but **"there is demand and the instance is gone" has the same shape**, which
...
- **Measure one interruption's real cost**: interruption → new instance → models re-fetched → warm, in
...
- **R2. "No word for an interruption" was wrong.** `noteReplacement` in `engine_control.go`
  ...
  task. Decision 6 now builds on that transition plus "the instance is gone too". And since ECS starts
...
- **R7. The service's strategy belongs to CloudFormation.** As long as the template declares it,
  ...
  declared provider back and starts a deployment — the running Spot instance is replaced on-demand
...
- **R8. The orphan instance.** Whether a launch the Spot provider had already begun is cancelled when
...
| 6 | `noteReplacement`'s transition plus the instance gone / relation to ECS's own re-placement / a failed rebuild counts | R2 |
...
| Open questions | 1 rewritten / 10 (the orphan instance) added | R1, R8 |
...
| 18:44:05 / 18:45:58 | `list-container-instances` then `describe-container-instances` | 4 instances. **None carries a `capacityProviderName`** and all registered on 09-06, 09-07 or 10:30 the same day — none belongs to either provider. **Not one new instance arrived** |
...
- **(b) green. The service is unharmed.** The refused call moved nothing (`updatedAt` stayed at
  ...
  `runningCount` and `pendingCount`; no instance ever arrived. The live image provider's
...
2. 🔴 **Decision 4 (a) says "don't pass `forceNewDeployment`" — but if the strategy ends up
   ...
   that sets `desiredCount` to 1 buys an instance, so this $0 run cannot measure it** (it gets measured
...
**six GPU instances, 54 minutes in total**.
...
Spot quota refusal is none of the codes in decision 5's table (🔴2), and every start **buys two instances**
...
| 3 | 🟢 (on the second attempt) | **26 seconds** after `mode: on` a Spot instance (`g6e.xlarge`, `InstanceLifecycle: spot`, 1a), and **5 min 30 s** to `state: running` / `warm: true` / `offer: {id: spot3, buy: spot}`. The first attempt was cut short by the default 180-second budget (🔴1). ⚠️ **No image was generated** — there is no way in from outside to the engine gateway (its token is the Workspace-internal `/internal/engine/token`), and driving somebody else's session was not an option. A generation on a Spot instance is already measured in 0074 (one image in 403 s) |
| 4 | 🟢 (via the budget) | With Spot made **unbuyable** (see "How to build the positive control") the start moved to `l4` (od) and an on-demand instance (`g6.xlarge`) arrived in **28 seconds** and reached `warm: true`. ⚠️ It moved because the budget ran out, not because the failure code was recognised (🔴2). The orphan instance of open question 10 — the old provider's instance arriving **after** the move — was not seen (nothing could be bought there in the first place) |
| 5 | 🟢 | In both 3 and 4 `GET /api/admin/engines` never showed `box: null` (Spot: `i-0fb3e7…` `g6e.xlarge`; on-demand: `i-0a4cdf…` `g6.xlarge`). **Both providers' instances are recognised as its own** |
...
### 🔴 1. The budget ends on "not RUNNING yet", not on "no instance came" — so 180 seconds can never buy Spot
...
**The instance was there after 26 seconds.** What was cut short is the task on top of it, which spends minutes
pulling the container and starting ComfyUI (0071 measured a 527-586-second cold start). At 180 seconds,
therefore, the walk goes to the end of the list **whether or not Spot can be had** — and buys an instance at
every step. That run walked three offers in seven minutes, **bought two instances and started none**. With
...
**The fix (decision 5's text does not change)**: end the budget on "**has an instance arrived for this
...
`waited >= budget` and never reads whether an instance exists.
...
### 🔴 3. A 0 → 1 start that also changes the strategy makes ECS buy **two** instances

Hardware run 0 measured that putting the strategy, `desiredCount: 1` and force into one `UpdateService`
is accepted by the API. It is. **But it does not buy one instance.** Both starts reproduced it:

| | The old strategy's (on-demand) instance | The new strategy's (Spot) instance |
|---|---|---|
...
buys an instance. The new PRIMARY that force created then places it again — **but the instance that was bought
stays**. The second run's spare instance was **billed for 16 min 28 s** ($0.51, 38% of the day's GPU spend),
...
so the provider keeps **its previous requirements** and a real Spot instance arrived while the declaration
...
- Open question 10 (the orphan instance) **did not appear in that shape**. What appeared is 🔴3's "two instances
  at start": the old provider's instance does not arrive after the move, it arrives **together with the start**.

### What the GPU cost

| Instance | Type | Purchase | Alive | Approx. |
|---|---|---|---|---|
| 1 | `g6.xlarge` | on-demand (🔴3's spare) | 10m43s | $0.23 |
| 2 | `g6e.xlarge` | Spot | 6m55s | $0.16 |
| 3 | `g6e.xlarge` | on-demand (🔴3's spare) | 16m28s | $0.51 |
| 4 | `g6e.xlarge` | Spot (the instance run 3 reached warm on) | 9m6s | $0.21 |
...
- `mode: off`, and **the instances were watched until they were gone**: no container instance carries a
...
- **Decision 5 — the budget ends "when an ACTIVE instance appears on that offer's provider".** The
  clock stops the moment the instance arrives; `StartDeadlineSec` takes over from there. ⚠️ With it,
  **the instance lookup asks by provider name** (`boxOn()`) — around a start there is a window with
  **two instances registered**, the draining one and the new one, and a nameless `box()` may return
...
- **Decision 4 (a) — a start is two `UpdateService` calls.** When the strategy changes:
  ...
  call ECS places under the old strategy first and buys two instances (reproduced twice in run 3).
...
`spot3` (spot, three types) / `l4` (od) / `l40s` (od). Two GPU instances, 21 minutes, **about $0.38**.

**Verdict: two of the three are fixed. The third is not, and fixing the others opened one new hole.**
The budget now stops when an instance arrives (🟢) and the Spot quota word is read in 25 seconds (🟢). But a
start **still buys two instances** (🔴, and the cause is one layer deeper than the previous diagnosis), and
...
| (e) does `box` point at the ACTIVE one | 🟢 | While the on-demand instance was `DEREGISTERING`, `box` was `i-05f9f2…` (`g6.xlarge`, `InstanceLifecycle: spot`, `status: ACTIVE`). It is never `null` |
...
| (c) does an orphan instance turn up on the Spot side | 🟢 | Watched for minutes: no container instance carries a capacity provider. Spot never bought anything |
| (d) is there only one instance | 🟢 | **Zero** (nothing could be bought). This re-run cost **$0** |
...
| Instance | Type | Purchase | Alive | Approx. |
|---|---|---|---|---|
| 1 | `g6.xlarge` | Spot (the instance run 3 reached warm on) | 7m34s | $0.07 |
| 2 | `g6.xlarge` | on-demand (🔴(b)'s spare) | 13m8s | $0.28 |

**21 minutes, about $0.35** (plus the 7.80% Managed Instances fee = **about $0.38**). ⚠️ **The spare instance
is 79% of the spend** — worse than last time's 55%: the shorter the successful start, the heavier the
second instance weighs.

- `mode: off`, **both instances `terminated`**, no container instance carries a capacity provider,
...
- **Decision 4 (a) — the gate before the second call is `settled()`**, not "a new PRIMARY
  ...
  buys an instance (reproduced in the re-run). **The price is a start that is at least 2 min 35 s
...
| decision 4, "the strategy only while running is 0" | **goes** (there is no strategy) | no instance swap while running >= 1 (0077 decision 5's drain wait) |
...
| decision 6, "interruption" | detection may use `describe-instances` (the instance is enumerable) | not a failure / skip after two in a row |
| decision 11, "the panel answers from the service's strategy" | **from the instance's tags** (`af-engine-offer` / `af-engine-buy`) | EC2 is not asked for prices (`describe-instances` is read) |
