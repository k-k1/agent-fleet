...
>   エンジンを起動してしまう——この節の 2 つめの箇条書きの再演である；
> - **次の起動の前に段を再適用する**。これは `startGate` の段 2 がすでに冪等にやっていたことなので、
>   代償は `UpdateCapacityProvider` 1 回だけで、新しい仕掛けは要らない。
>
> 行の他の欄（service / url / health / provider / api / apiKeyParam / idle / deadline）はそのままで、
> 今までどおり「再起動が要る」とログに出る。消えたのは運用の 1 手であって、あのログの正直さではない。
