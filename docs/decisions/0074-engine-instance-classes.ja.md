...
6. **重いモデルでタスクの `Memory` が足りるか**（決定 11）。18.5 GB では足りた、で止まっている。
   **image 役は解決（2026-09-19）: 足りない。しかも原因は数字ではない。** ComfyUI はピン留めバッファと
   RAM キャッシュの予算を「読める cgroup の上限」から決めるが、タスクレベルの `Memory` はタスクの
   slice にあってコンテナの cgroup 名前空間の外＝ComfyUI には見えない。32 GiB の箱で 14,336 MiB の
   タスクに対し 15,259 MB をピン留めし、12 GB 級のチェックポイントは読み込み中に必ず OOM kill された
   （2026-09-17/18 に 7 タスク、L40S でも L4 でも同じ）。image のタスク定義からタスクレベルの
   `Memory` を外し、`ImageTaskMemory` は engine コンテナの `MemoryReservation`（配置用）にした
   （`PARAMETERS-60-engines.md`）。llm のタスクも同じ変更で同じ形にした: ホストバッファは小さく
   決定 11 の実測はそのまま成り立つが、`--no-mmap` と 30B が同じ罠を踏むのを待つより両役を
   1 つの形にする方がよい。
