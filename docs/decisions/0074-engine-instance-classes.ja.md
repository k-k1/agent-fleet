...
2. ✅ **解決（2026-09-10・P1 で実測）。ドリフトは 2 通りあり、片方は起きない。**
   ...
   - **残る窓**は「CP が適用してから ECS が箱を買うまでの間に CFN が戻した場合」だけで、これは
...
| 6 | 陽性対照 | **出た**。ただし**古い箱が塞がっている**必要があった（計画の想定は誤り） |
...
空いている前の段の箱が居るあいだ、ECS はそもそも 2 台目を買おうとせず、**タスクを古い箱に
そのまま置く**。つまり決定 4 が防いでいる一番手前の失敗は、クォータ超過ではなく
**静かな再着地**である——サービスは `steady state` と言い、パネルは「稼働中」と言い、
段を上げたつもりの管理者は古いカードの上で走り続ける。陽性対照は、古い箱でタスクが
走っている状態で 2 台目を要求して取った。

そして **「箱が消えるまで待つ」は必要条件だが十分条件ではない**。4 vCPU の箱が ECS から
...
- **P1（実機）**: ✅ **完了（2026-09-10）**。af-sandbox で段を切り替えて実際に箱を買い、
...
- **P2（必要が出たら）**: タスク定義の追随（決定 11・未解決 6）、`--models-max` を段に応じて
  ...
  - 2026-09-10 に上の Console レビューから追加: 決定 6 の要求に KV キャッシュを数え入れること
    （所見 1）と、comfy のエンジンは複数を同時に載せうるとパネルに書くこと（所見 2）。
