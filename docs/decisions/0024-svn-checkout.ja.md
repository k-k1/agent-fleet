...
## 追補（2026-09-10）— 後から認証情報を入れられるようにし、直 `svn` も透過認証にする

利用の中から出た報告 2 件。どちらも上の決定から素直に生じたもの。

**任意保存は片道だった。** チェックアウト時に保存しなければ、パスワードはストアにも
（`--no-auth-cache` により）`~/.subversion` にも残らない。以後の更新は必ず失敗し、入れ直す画面が
無い。→ **既存の作業コピーに対して後から入れられる**ようにした（`POST /repos/{name}/svn-auth`。
行の「再認証」と、更新が `svn_auth_required` で落ちたときの自動提示）。作業コピーがまだ無い場合は
設定 › 接続（`PUT /connections/svn`）。**保存の前にサーバへ当てて確かめる**——「保存した」が
「打ち間違いを保存した」になり得ないこと自体が、利用者をここへ来させた失敗への回答である。

**捨てた選択肢は正しく捨てたが、そこから引いた結論が誤りだった。** `~/.subversion/auth` への
キャッシュは確かに平文であり、これは今も捨てたままでよい。しかし**透過認証にそれは要らない**:
`/usr/local/bin/svn` をシムにして `workspace-agent svn-run` に渡し、**暗号ストア**から引いた creds を
本物の svn に stdin で渡す。`gh` が既に使っている形（`gh-auth-wrapper.sh`）そのもので、ディスクにも
`ps` にも平文は出ない。よって「セッション内の直 `svn` は非透過」は**この設計の限界ではなくなった**。

ラッパーは「注入するか素通しするか」しかせず、コマンドの意味は変えない。stdin を読むコマンド
（`-F -`・`--targets -`・エディタが開く `-m` 無しの `commit`）からは stdin を奪わない。明示指定は
常に勝つ。設計と、その過程で見つかった罠は [docs/41 §9](../log/41-svn-checkout.md)。
