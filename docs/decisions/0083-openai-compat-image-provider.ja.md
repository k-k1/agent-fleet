...
1. **この配備が買って動かすエンジン。** GPU の箱の上の `sd-server` コンテナ、その ECR
...
- `DefaultModel` は「stack が宣言した最初の id。それは sd-server が起動に使った checkpoint でも
  ある」（`:264-272`）。
- サイズはカタログが何も宣言していないとき **model id から Stable Diffusion の族を推測**して
...
- **行がモデルを宣言しているとき `model` を body に入れる。** 値は `sdcppRequestModel` が既に
  ...
  それは今の挙動そのままである。**運用者の宣言がスイッチ**であり、これは ADR 0072 決定 2 が族に
...
- **`response_format: "b64_json"` を明示的に要求する。** sd-server が base64 で答えていたのは
  ...
  理由のない egress である。
- **サイズは行から取り、族の推測は必要としていたエンジンと共に死ぬ。** サイズを宣言していない行
...
この配備は OpenAI 互換の箱を買わないので、`managed` の行がこの provider を名乗ることは無い。
運用者に必要なのは行 1 本である。
...
向こうが自分の重みを持っているので、置くファイルも、ディスパッチする族も無い。
...
上流のパス接頭辞（`engine_gateway.go:949-956`）、ファイル flag の語彙と `base_model` の族語彙
...
`/v1/` の前置を必要とし、ファイル flag も族語彙も持たない — まさに各分岐の「comfy でない側」が
答えているものである。
...
- **ADR 0071 P1 の画像エンジンを覆し**、その実測は維持する: 冷間起動・GPU の段・16 分の予算は
  GPU の箱と S3 の取得の実測で、あのバイナリの実測ではない。
