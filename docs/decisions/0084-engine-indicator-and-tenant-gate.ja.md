...
自前のエンジン（ADR 0071、役は `llm`＝chat と `image`＝images）はオンデマンドで動く。箱は
...
2. **ワークスペースが停止していても出したい。** 「箱は起きている、いま始めれば速い」は、
...
- **「今すぐ起動」ボタンは置かない。** TTS には `POST /api/tts/wake` があり（ADR 0070 決定 4）、
  ...
  GPU の箱は $1.26/h で、押した人が続けて使う保証は無い。メンバーは**使えば起こせる**
...
- **待ち数を C（自分のキュー）だけにする。** 他人が 40 枚積んでいることが見えない。
  共有の箱について知りたいのはまさにそれである。
- **テナントごとのカタログ／モデル単位の許可。** ADR 0072 が実測で却下済み
  （enabled モデルは全部コールドスタートで箱に同期され、~159 MB/s の直列で 10 分の
