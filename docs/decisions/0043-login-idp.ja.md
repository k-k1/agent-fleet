...
- 関連: [61-login-idp.md](../log/61-login-idp.md) /
  [build/07-security.md](../build/07-security.ja.md) §7.3（AUTH 3 モード＝現行契約） /
  [build/06-data.md](../build/06-data.ja.md)（`identity` / `user_key`） /
  [0001-self-host-vs-saas.md](0001-self-host-vs-saas.ja.md)（各社が自社でセルフホスト＝IdP は各社のもの）
...
22. ★ **membership の削除／無効化 API を P3 のスコープに必ず含める。** 運用を書き下して見つかった穴で、
    ...
    **論理削除（`status='inactive'`）で足りる**（workspace / home は残したまま締め出せる）。
    外した後の workspace / home の扱いは [0028-deletion-lock](0028-deletion-lock.ja.md) と
    掃除の段階制に合わせ、即削除しない。
...
26. ★ **workspace / home の削除は `tenant_admin` の責務。** 部署の人員を把握しているのは部署側で、
    ...
    [0028-deletion-lock](0028-deletion-lock.ja.md) と掃除の段階制に合わせる。
...
30. ★ **有効化は `super_admin` の承認を要する。編集と保存は tenant_admin、有効化は別の人。**
    プロジェクト MCP（[0031](0031-mcp-registry.ja.md)）は tenant_admin が単独で登録できるが、
...
33. **`client_secret` は DB にテナント鍵で封印して置く。前例は `mcp_server`。**
    ...
    ない**（[0005](0005-envelope-custodian.ja.md)）／env から DB へ移すと秘密が **`DATA_DIR`＝
