...
- [ADR 0083](0083-openai-compat-image-provider.ja.md) を先に着地させるべき。sdcpp の**エンジン**を
  退役させ、そのクライアントを `openai-compat` に改名するので、下の P0 が動かす語彙は「2 つとも
  意味のある id」になり、決定 3 の「保存された `sdcpp` の別名」は要らなくなり（0083 決定 7）、
  決定 8 の畳み込みは 0083 決定 8 の理由付けのもとで、この ADR の行ごとのリストが置き換えるまで
  生き残る。
...
1. ~~**電源が落ちている LAN ホストへの接続は実際に何秒か。**~~ **測った（2026-09-14）。答えは
   約 3 秒で、否定キャッシュを入れた。**

   運用者の LAN 上の、電源を落とした ComfyUI ホストへ、同じ網の中のコンテナから接続した
   （`engineHealthy` と同じ 5 秒・同じ health パス）。**`No route to host` で 3.05 / 3.05 /
   3.08 / 3.11 秒**（4 回）。この文書が想定していた 2 つの形は、その両側にあった — 閉じた
   ポートへの接続拒否は **0.15 ms**、SYN を黙って捨てるアドレスは上限ちょうどの **5.00 秒**。
   つまり**同じ網にいる電源断の機械は 5 秒ではなく 3 秒**で、カーネルが先に ARP を諦めるためで
   ある。「届かない」と「遮断されている」を取り違えないよう、同じ LAN の別アドレスが
   **HTTP 200 を 2.4 ms** で返すことを先に確かめている（経路はある＝3 秒はその機械の状態）。

   ⚠️ 測ったのは Workspace のコンテナからで、実際にプローブするのは Control Plane である。CP が
   同じ LAN にいる native / docker の配備（ADR 0076 が狙う形）ではこの数字がそのまま効くが、
   ECS の配備から運用者の網を見る場合は経路が別物で、まだ測っていない。
