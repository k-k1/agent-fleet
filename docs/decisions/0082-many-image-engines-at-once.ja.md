...
- [ADR 0083](0083-retire-sdcpp-image-engine.ja.md) を先に着地させるべき。sdcpp を退役させると
  画像の種類が 1 つになるので、下の P0 が相手にするのは 2 つでなく 1 つになり、決定 3 の
  「保存された `comfy` / `sdcpp` の別名」も要らなくなり（0083 決定 5）、決定 8 の畳み込みは
  二度書きではなく削除になる。
