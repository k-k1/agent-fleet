...
## 追記（2026-09-08 v6）: claude 同梱スキルを「//」の奥の 2 段目に

背景: claude 同梱スキル（dataviz / simplify / code-review …）を「/」ボタンから出したいが、
利用者定義を先に、同梱はもう 1 キー押したときだけ、という要望。docs/log/50 §0 では
同梱の列挙を「CLI 版依存の契約になる」として見送っていた。

### 10. 同梱一覧はバイナリでなく SDK の init フレームから取る

CLI は単一バイナリで、スキル定義は minified JS の定数。それを削るのは、まさに拒んだ
版依存の契約になる。一方 SDK の `system/init` フレームは `skills`（名前のみ）を公開
フィールドとして運ぶ。stream-json で `/help` を流すと claude はローカルで応答するので、
プローブはモデル呼び出しゼロ（実測 input_tokens 0・約 0.8 秒）。Agent はバイナリ実体
（パス＋mtime＋size）でキャッシュし、未知の名前を `source:"cli"` として合流させる —
codex の `.system`・cursor の広告リストと同じ扱いで、UI に新しい概念を足さない。

正直な限界（いずれも実測）: `-p` の集合は TUI の近似（モデル・モードでゲート —
keybindings-help は TUI だけ、deep-research は -p だけ）。フレームに説明文は無い
（Console が小さな i18n 表を持つ）。組み込み**コマンド**（/compact /model …）は同じ
経路で取れるが、TUI のダイアログを開くものはミラーに映らないので対象外のまま。

### 11. 2 段: 利用者定義が先、CLI のものはもう 1 キーの奥

空クエリでは `cli` 項目を末尾の「あと N 件表示」行に畳む（↓＋Enter、またはクリック）。
トリガ 2 連打（`//`・`$$`・全角も）でも展開する。クエリがあれば常に両層を横断し、
同順位は利用者定義が勝つ — `/sim` と打つ人は同梱の simplify を期待する。`cli` しか無い
一覧（cursor・自前スキル 0 の claude）は畳まない: 全部を 1 クリックの裏に隠すのは
優先ではなく障害物になる。

却下: 「/」ボタンの 2 回目押しで展開。2 回目は今「閉じる」であり、それを保つ方が
ショートカット 1 つより価値がある。行と `//` で両方の手が足りる。
