...
- 状態: 確定（Phase 2）。旧 `../decisions/0003-ssh-to-connections.md` を置換
- 関連: [HANDOFF §6.6](../HANDOFF.md) / [api-agent §7.0](../reference/api-agent.md#70-現行-api-表面地図--詳細はコード--handoff-610) / [security §4.4](../reference/security.md#44-シークレット管理)
...
- 保存は暗号ストア `secrets.enc`（AES-256-GCM, 0600）。統一 cred helper `workspace-agent cred` が都度復号で
  出力し**平文ファイルを作らない**。鍵の保護は封筒暗号（[0005](0005-envelope-custodian.md)）。
