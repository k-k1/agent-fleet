...
[English](0053-cp-arch-and-availability.md) | 日本語
...
- 関連: [0044-workspace-sizing.md](0044-workspace-sizing.ja.md) 決定 3
  ...
  「既定オフのまま出す」と言うので、理由を明示する必要がある。🔴 この例外は 2026-09-07 に
  終わった: 決定 4 の訂正を参照） /
...
🔴 **訂正（2026-09-07・0.16.0）: `workspace_arm64` も既定 ON にした。非対称は値段だけに
なり、既定には残っていない。** 上の記述は書いた時点では間違っていない——同じ順序の 3 番目が
workspace では遅れて来ただけで、遅れた理由は「確かめたか」の問いが後から片付いたからである。
`workspace_arm64: true` で公開した最初の版は 0.15.1 で、そこで出たイメージは
**実機の Graviton で起動している**（`acrt` の m8g.large スロット・arm64 golden 込み。
[70](../log/70-slot-instance-classes.md)）。これで下の 2 が workspace についても答えられ、
残る反対理由は +593 秒だけになった——**実測で publish 全体が約 14 分 → 約 28 分**、timeout 90 分
に対しての話である。この値段で OFF に据え置くのは
[0044](0044-workspace-sizing.ja.md) 決定 3（**既定オフで出した機能は存在しないのと同じ**）
そのもので、`ecs-ec2` の Graviton スロットが「運用者が自分で焼いたイメージ」に依存し続ける
ことになる。2 つの旗は**別入力のまま**（決定 3 は不変）で、動いたのは既定値だけである。
