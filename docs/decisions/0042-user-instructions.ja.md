...
## 追記（2026-09-08）: フリート層の二分と、トピックファイルのスキル登録

フリート層は 429 行 / 34 KB まで育ち、6 種の全セッションに毎回およそ 8,400 トークンが合成されて
いた（Claude Code が CLAUDE.md に推奨する 200 行を大きく超える）。二つに分けた。

- `workspace/workspace-notes.md` には「必要になる前に知っていなければならないもの」＝禁止事項と
  気づかず踏む罠だけを残し（135 行 / 10 KB）、末尾に索引を置く。
- `workspace/notes/<topic>.md`（コンテナでは `/usr/local/share/agent-fleet/notes/`）に手順を移し、
  エージェントはその場面に来たときに該当ファイルを読む。読まれるまで費用はゼロ。

常時読込部分の配布は本 ADR の決定 4〜9 のまま。トピックファイルには索引に加えて第二の経路を
足した: agent は起動のたびに、CLI の**ユーザー単位のスキル置き場**があり読まれることを実測できた
種に、各ファイルを `af-<topic>` というスキルとして登録する — `$CLAUDE_CONFIG_DIR/skills`
（AF 自身のマウントなので、`/etc` の方針には届かなくても claude にここでは届く）、
`$CODEX_HOME/skills`、`~/.config/opencode/skills`。agy はプロジェクト側の `.agents/skills` しか
読まず、copilot / kiro にはユーザー置き場の痕跡が無いので、索引だけが経路のまま。決定 4 の規則は
保つ: AF が書くのは `af-*` という名前でマーカー行を持つディレクトリだけで、上書きも削除もそれに
限り、トピックが空なら何もしない。`user-invocable: false` で利用者のスラッシュメニューと Console の
ピッカーからは隠しつつ、モデルには見える（実測: モデルは 5 本すべてを列挙し、stream-json の init
一覧には出ない）。途中で却下した案: `@import`（起動時に展開されるので削減ゼロ）、MCP resources
（af サーバは tools-only で CLI 側の対応も不揃い）、tool description への移動（これも起動時に全部
載る）。`scripts/docs-check.py` が索引の指す先・トピックファイル内のガイド参照・frontmatter を検査する。
