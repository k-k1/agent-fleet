...
2. **アイドル停止**: 掃除ループが `AF_ECS_EC2_SLOT_SLEEP_SEC`（既定 15 分）で
   ...
   **止まった Workspace の「インスタンス」を止める**。直列に並ぶので、実際の待ちは
...
> 起こす／建てるは約 40 秒まるごと、**running の空きインスタンスへの載せ替えは待ちが露出して 20〜30 秒**。
...
- ⚠️ **`af-role=slot` で焼かない。** CP はそのタグで世界を作るので、焼いている最中のインスタンスに
...
## 決定 21 — サイズは「メモリ 1 軸」のまま。インスタンスタイプは選ばせず、**結果として乗るインスタンスを表示する**
...
2. **メモリは「上限」ではない。** EC2 のタスクは `memoryReservation: 512`（ソフト）だけで、
   ハード上限を掛けていない（決定 8：インスタンスは 1 人で使う）。`mem_limit` は**インスタンスを選ぶための「必要量」**
...
- **メモリ欄は「必要量」と言い、その下に乗るインスタンスを出す**（例: `→ m7i.large（8 GiB・専有）`）。
...
`StopInstances` を撃たない（`quarantineSlot` は故障インスタンス専用、`sweepGhostInstances` は
...
1. **スロット起点の走査を足す**（`sweepFreeSlots`）。`af-pool` ＋ `af-role=slot` ＋ running の
   ...
   `slotSleepAfter` で停止する。`af-role=slot` で引くので**隔離インスタンスは自然に対象外**になる。
...
4. **温かい空きは 1 台も残さない**（決定 11-4 の「プレウォームは入れない」をそのまま延長する）。
   ...
   プールを増やす 135.4s**。**インスタンスを停止で維持している時点で「135s を払わない」という価値は
...
アダプタは設計上 `TerminateInstances` を持たない——インスタンスを捨てるのは運用者の判断で、
イメージキャッシュを捨てると 110s が 135s に戻る）。~~
→ **撤回（決定 23）。** 「インスタンスを捨てるのは運用者の判断」は、**運用者が気づいて手で消すまで
root ボリュームが課金され続ける**という意味でもあった。実デプロイでそれを踏んだ（docs/64 §64.32）。

## 決定 23 — 停止したスロットは、さらに時間が経ったら **terminate** する（決定 22 の次の段・2026-08-26）

決定 22 で空きスロットも眠るようになったが、**眠った先が無い**。このアダプタにはインスタンスを消す経路が
...
停止から N 経過したインスタンスを terminate する。** 推奨値は 4h。

- **停止と終了は別の課金を止める。** stopped は compute だけ $0 で、root（$3.84/月）は
  インスタンスが消えるまで残る。⚠️ root は `/dev/xvda`・`DeleteOnTermination: true` なので**単独では
  消せない**——「root を消す」＝「terminate する」であり、**分離できない 1 つの決定**である。
- **失うのは 25 秒。** 休眠インスタンスを起こす 110s に対し、新規作成 135s（§64.17.4）。イメージキャッシュが
  救う cold pull 32s を、インスタンス起動 19s ＋ ECS 再登録が食い潰しているため。
  「使えないより遅い方がまし」という運用方針の下では払える。
- ⚠️ **warm 床（最小 warm 台数）は置かない。** home が付いたインスタンスは `occupiedInstances` が
  ...
  **差は 6 秒**であって「渡らない」ではない。それでも親和性を捨てる釣り合いは悪く、休止期間にも
  固定費が残る。買う価値のある 92 秒は **running の空きインスタンス**（$95/月）の側にあり、
...
- ⚠️ **home は `releaseSlot` で先に外してから terminate する。** `DeleteOnTermination: False`
  ...
  **attachment から配置を導出する**ので shutting-down のインスタンスを「自分のスロット」と読み、
...
- ⚠️ **`sweepFreeSlots` の instance-state フィルタを `running, stopped` に広げる。**
  停止がインスタンスに起きる最後の出来事だった間は `running` だけで正しかったが、terminate 段を足すと
  **インスタンスは停止した瞬間に走査から永久に消える**——集めたいインスタンスがちょうど。
- ⚠️ **自分で terminate するときは先に `DeregisterContainerInstance`（`Force: true`）。**
  決定 3-2 のとおり登録は `ACTIVE / agentConnected=false` で残り、**ACTIVE に見える
  ゴーストは配置制約を満たす**。`sweepGhostInstances` は「他の理由で消えたインスタンス」の修復路であって、
  自分が消すときに窓を開けておく理由は無い。

**結果として `Ec2MaxSlots` の役割が 1 つ減る。** それは **A**=同時実行の上限・**B**=保持される
root の本数・**C**=立ち退きの閾値 を兼ねていたが、A と B が一致していたのは「インスタンスを終了しない
から」だった。定常のインスタンス数が「活動量 ＋ 閾値」で決まるようになり、**B が外れる**。
...
立ち退きで得たインスタンスに `attachHomeWithRetry` でそのまま載せる（`freeSlots` を引き直さない）ので
...
**上限に達したときにしか露出しない**ので生き延びた（上限未満なら `growPool` が走り、新しいインスタンスは
...
回収し得る。禁じると、**テナント A が使っていないインスタンスをテナント B が使えない**という規則になり、
上限到達時に「起動できないメンバー」を作る——運用者が絶対に避けると明示した唯一の結果である。
テナントを縛るのは `max_workspaces`（**同時に**動いてよい人数）であって、どの物理のインスタンスに載るかでは
...
~~**やらなかったこと**: 上限に張り付いた状態で休眠中の型違いのインスタンスを terminate して正しい型を
建て直すこと。~~ → **決定 26 で実装した。**「明示的に失敗する」でも**起動できない人は起動
できないまま**であり、それは運用者が唯一「絶対に避ける」と明示した結果だった。

## 決定 26 — 上限が「乗れないインスタンス」で埋まっているなら、片付けて建て直す（決定 24 の続き・2026-08-26）

決定 24 で立ち退きに型検査を入れた結果、**上限到達 ＋ 休眠中のインスタンスが全部型違い**は「黙って詰まる」
...
- **奪うのではなく終了する。** インスタンスタイプは走っているインスタンスが変えられる属性ではない。
  `evictLongestIdle` は**正しい大きさのインスタンスすべてに first refusal を持っている**ので、そこで
  見つからなかった時点で残りは「どう再利用しても乗れないインスタンス」であり、それが 1 枠を握っている。
  **枠を空けるにはインスタンスを消すしかない**（決定 23 と同じで root は単独では消せない）。
- ⚠️ **AZ で絞らない（立ち退きとは逆）。** 立ち退きはインスタンスを**再利用**するので home の AZ に
  縛られるが、こちらは**インスタンスを壊して枠を買う**ので、`Ec2MaxSlots` がプール全体を数える以上
  **別 AZ のインスタンスを空けても価値は同じ**。絞ると、空けられるインスタンスがあるのに要求者が詰まる。
- ⚠️ **`Ec2SlotTerminateAfterSec` では門を作らない。** あれは**アイドルの費用**のつまみで
  0 は「インスタンスを残す」。こちらは**起動できない人**の話で、代わりが「Start の失敗」のときにしか
  走らない。犠牲者が失うのはイメージキャッシュだけ（次回 110s→135s）で home は失わない
  （`releaseSlot` で先に外す・`DeleteOnTermination: False`）。しかも**既定は 0** であり
  **0 の配備こそインスタンスが上限まで育って張り付く**（決定 23）——門を作ると、この経路を最も必要と
  する配備でだけ効かない。
- **空のインスタンスを持ち主付きより先に取る**（誰の親和性も使わないので只）。持ち主付きなら最長休眠から。
- **生きた claim があるインスタンスは取らない**（着地中の Start は attachment をまだ持たず claim だけが証拠）。
  **ECS のタスクが 1 つでもあれば取らない**（home が無くてもタスクは載る——焼きの探針）。
  **自分と同じ型は候補にしない**（立ち退きが first refusal を持っている。壊すと再利用できた
  はずのインスタンスを捨てて建て直すことになる）。隔離インスタンスは両方の走査から外れたまま（決定 20）。
- terminate 直後は `DescribeInstances` がまだ `stopped` と答えうるので **`poolSize` から
  落ちるまで待つ**（`runSlot` が上限を読み直す。待たないと、たった今空けた枠のせいで自分の
  Start が「一杯」で落ちる）。

**片付けられるインスタンスが 1 つも無ければ、立ち退きの元のエラーをそのまま返す**——「プールが一杯」が
...
数えるのは*存在するインスタンス*で、**停止中の Workspace はどちらのテナント枠にも数えられないままインスタンスを掴んで
...
## 決定 27 — 「まだ登録されていない」に時間制限を入れる。**インスタンスの OS が死んだら待たずに捨てて置き直す**（本番事故・2026-08-27）

決定 20 は「home をマウントできなかったインスタンス」を隔離した。今回はその一つ手前で、**インスタンスがタスクを
...
インスタンスの管理系（efs-utils の stunnel → ECS エージェント → SSM エージェント）は**15 秒以内に揃って**
落ちた。EC2 のステータスチェックは 3 つとも `passed` のまま、つまり**外形上そのインスタンスは健全**である。
...
上限があれば追い出し圧力は**そのコンテナの cgroup 内に閉じ**、インスタンスの管理系は生き残った。
⚠️ ただし利用者が選ぶ「サイズ」はそのままインスタンスのサイズなので、`MemBytes` をそのまま入れると
上限＝インスタンス全体になり**管理系の余白が残らない**。**「インスタンスのメモリ − 管理系ぶん」**でなければ意味がない。
（この閉じ込めは決定 27 の範囲外＝別の決定として起こす。）そして 3 つの層がそれぞれ正しく振る舞った結果、
**誰も回復できないデッドロック**になった: ECS はエージェントの居ないタスクを止められず →
task ENI が永久に外れず → スイープは「ENI を持つインスタンスは停止しない」（決定 3-3）で正しく手を引き →
Start は同じインスタンスを親和性で再利用して登録を待ち続け → **ingress が 60 秒で切って利用者は 504。**
...
- **`deferred` の判定に ECS 登録を加える。** homeless 経路は最初から
  ...
  Start は 60 秒の ingress の内側に居る。**健全なプールでは running なインスタンスは登録済みなので、
  この非対称は普段まったく見えない**——見えるのは事故のときだけである。
- **猶予（`AF_ECS_EC2_SLOT_LOST_AFTER_SEC`・既定 5 分）を過ぎ、なお EC2 が `running` と
  言うなら、そのインスタンスは遅いのではなく失われている。** `pending` のインスタンスは対象外にする——
  起動途中のインスタンスを壊すのは「遅い Start」を「壊れた Start」に変えることであり、
  間違えたときの代償が非対称である（待ちすぎても数分遅くなるだけ）。
- **決定 20 のテアダウンは流用できない。** あちらは「まだ何も走っておらず、カーネルは生きている」
  前提で書かれている。OS が死んだインスタンスでは全ステップが静かに間違ったことをする——走っているインスタンスへの
...
- **home を置き直すところまでを自動でやる。予算は 1 回。** 隔離しただけでは、決定 20 と違って
  ...
  無制限だと Start のたびに 1 台ずつ全インスタンスを隔離し、**「劣化したデプロイ」を「空のデプロイ」に
...
インスタンスを停止した時点で dmesg も OOM killer の記録も消える。今回もメモリ枯渇は
**EBS の read が爆発して write が 0 という形からの推定**にとどまった。

## 決定 28 — ワークスペースを**インスタンスから切り離す**。上限は「暴走を止める」ためではなく**巻き添えを止める**ため（2026-08-27）

決定 8 は「EC2 スロットは 1 人で使うのだからタスクは何も予約しない」とし、決定 21 は
サイズを「メモリ 1 軸で、結果として乗るインスタンス」とした。両方とも正しいが、**その 2 つの帰結として
ワークスペースのコンテナはインスタンスを丸ごと取れる**（`Memory: nil`）。docs/64 §64.40 の事故は
そこで起きた——ワークスペースの中の何かが匿名メモリを数 GB 握り、swap がゼロなので
カーネルはページキャッシュを削るしか回収手段が無く、**インスタンスの管理系（dockerd / containerd /
...
- **サイズ表示を正直にする。** 上限が入る前は「8 GiB」がインスタンスとワークスペースの両方を指していて
  1 つの数で正直だった。入った後は違う数になるので、**インスタンスだけを出すのは cgroup が渡さない
  メモリを本人に約束すること**である。`usable_mem_mib` を足し、Console は
  **本人が使える方を先に、インスタンスを括弧で**出す。⚠️ **上限が無い配備では省く**——
  そこではインスタンスが答えそのもので、2 つ出す方が嘘になる
...
## 補遺（2026-09-11）— 決定 21 の 4 つ目を覆す：home は拡張できる

決定 21 は、サイズ編集画面の説明とランタイムの実際との食い違いを 5 つ挙げ、うち 4 つを
「ランタイムに自分を説明させる」ことで直した。4 つ目だけは、説明しただけで終わっていた。

> **ディスクの値は home 作成時にしか効かない。** このアダプタに `ModifyVolume` は無い
> （コメントは「オンラインで拡張できる」と言うが、それは EBS の性質であって実装の性質ではない）。
> 既存ユーザーの値を変えても何も起きない。**そもそも EBS は縮小できない。**

正直ではあった。そして、実際に届く要望——**「home をもっと大きくしてほしい」**——に対する答えを
「ワークスペースを破棄して作り直す」、つまり **大きくするために home を失う** しか残さなかった。
**実装をコメントの言うとおりにした。**

**入れたもの。** `ecs-ec2` アダプタの `ResizeHome`。`MachineProfile` / `SizingProfile` と同じ
任意の能力として探り、クォータ行を書く 2 つの経路（管理 API の `PUT /api/admin/user-limits` と
MCP の `set_user_quota`）が、行を書いた **あとで** 呼ぶ。

**決めた 6 点と、自明でない理由:**

1. **失敗させず、報告する。** どの経路も `HomeResize` と nil の error を返す。クォータ行は
   管理者の **意図** であり、今日 AWS に何ができるかとは無関係に保つ。保存を失敗させれば、
   保存が間違えたわけでもないことを理由に意図を巻き戻すことになる。結果は 5 つ——
   `growing` / `same` / `shrink` / `no_home` / `failed`。
2. **Console は結果で分岐する。数値では決して分岐しない。** `same` と `no_home` は
   どちらも「等しい 2 つの数」または「数が無い」に見え、しかも言うべき文は正反対だ
   （「することが無い」対「home はこの大きさで作られる」）。数から文を導く画面は、必ずどちらかを間違える。
3. **効くのは一方向だけで、その非対称は EBS のものである。** 大きい数は今あるボリュームを拡張し、
   小さい数は保存されるだけで、**次に**この人に作られる home にしか届かない。実装すべき縮小は
   存在しない——だからディスク軸の宣言は `disk_grow_only` になり（`disk_create_only` を置き換えた）、
   入力欄は **打ち込む前に** どちらの向きが何をするのかを言う。
4. **`ModifyVolume` は同期、ファイルシステムは非同期。** API 呼び出しは金がかかる部分であり、
   管理者が目の前で待っている部分でもあるので、断られたことは見えなければならない。
   `xfs_growfs` は変更が `optimizing` に達するまで走れず（数秒、ときに数分）、背景へ回す。
   **`completed` ではなく `optimizing`** ——completed は EBS の背景での再配置で、大きなボリュームでは
   何時間も続く一方、容量自体はもうゲストから読める。
5. **`af-mount` は毎回のマウントで `xfs_growfs` を走らせる。** 背景の手順を落としても安いのはこれのおかげで、
   **切り離されている間に**拡張された home も、growfs が届かなかった home も、次回起動で追いつく。
   これが無いと「ボリュームは大きく、ファイルシステムは元のまま」——**成功と見分けがつかない唯一の壊れ方**
   ——が残る。本人は「増えた」と言われたまま、古い容量を見続けることになる。
6. **拡張先は `resolveWorkspaceSize` を通した **クランプ後** の値**であって、リクエスト本体の数ではない。
   テナント上限（`max_workspace_disk_gb`・設定できるのは super_admin）は、さもなければコンテナ起動時に
   しか効かない。ここでは **金が出る瞬間に** 効いていなければならない。

**こちらの失敗ではない 2 つを、そのまま出す。** EBS は同じボリュームの 2 度目の変更を **6 時間** 断り、
サービスクォータでも断られる。**メッセージでしか見分けられない** ので、`HomeResize.Detail` は
AWS の文面を訳さずに運ぶ。

**IAM 文も機能の一部である。** CP のタスクロール（`20-platform.yaml`）に `ec2:ModifyVolume` と
`ec2:DescribeVolumesModifications` を足した。無いと、保存は数を書き、ボリュームは拡張中だと報告し、
何も拡張しない——決定 4 の注記にあるスナップショット権限の欠落と同じ形の、黙った `AccessDenied` である。
理由も同じで、実機 E2E はこのロールではなくデプロイヤの資格情報で走る。

**あわせて画面を、「どれもボタンだから」ではなく「取り返しがつくかどうか」で並べ直した。**
メンバー詳細の下半分は 1 枚の「操作」の列で、「上限を設定」が強制停止と home の掃除の間に立ち、
サイズ編集フォームは **その破壊的なボタンの列の中で** 開いていた。管理者が週に一度触る操作が、
絶対に誤爆してはいけない 4 つと同じ列にいたことになる。フォーム自体も、セッション上限——単位も
仕組みも効き始める瞬間も違う——を 3 つのサイズ軸と同じ山に積んでいた。理由は「たまたま同じ
クォータ行に保存されるから」でしかない。いまは: **サイズと上限** が独立した 1 枚として、それが
説明しているメーターの直下に来て、中は「ワークスペースのサイズ」と「セッション上限」に分かれ、
保存は 1 リクエストのまま（この API は行を丸ごと書くので、片方だけ保存すればもう片方が消える）。
**操作** には強制停止だけが残り、これは一時停止なので危険色をやめた。home の掃除・メンバーを外す・
Workspace の破棄・完全削除は、罫線の下の **取り消せない操作** に置いた。

コード: `control-plane/internal/runtime/runtime_ecs_ec2.go`（`ResizeHome` / `growHomeFilesystem` /
...
- **タスクは条件にしない。** `abandonLostSlot` が running のまま残したインスタンスは、既に本人の home を
  よそへ移した後のタスク ENI を掴んでいる——それこそ運用者が消しに来るインスタンスである。`terminateSlot` が
