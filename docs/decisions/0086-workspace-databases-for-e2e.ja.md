...
- Status: **提案**（2026-09-16）。実装は何も無い。
  状態の更新（2026-09-24）: P0（Postgres と `af-db` CLI）と P1（MySQL と Console のカード）は実装済みで、2026-09-17 に受け入れた（下の「P0 受け入れ」「P1 受け入れ」の節）。#719 で入り、修正と追加は #721・#726・#729・#735・#741（`workspace/agent/internal/afdb/`）。P2 は書いてあるとおり需要で決める。
...
- **停止をまたいで pid ファイルが残り、`af-db up` がそのまま起動していた——修正済み**。再起動後の
  最初の `up` が `pg_ctl: another server might be running; trying to start server anyway` を出した。
  コンテナごと死んだので `postmaster.pid` が残り、誰も掃除しなかった。古い pid がたまたま死んで
  いたので 0.2 秒で正しく起動したが、区別できなかったのは**番号が再利用されていた場合**で、その
  ときは pg_ctl が拒否し、利用者は Console からどうにもできない。両エンジンとも生存判定を
  `/proc/<pid>/cmdline` で行うようにした（**そのデータディレクトリの、そのエンジンのサーバ**で
  あることまで確かめる。番号が一致するだけの他人は数えない）。それ以外を指す pid ファイルは
  起動前に消し、消したことを 1 行出す。生きている自分のサーバの pid ファイルは決して触らない
  ——1 つのデータディレクトリに postmaster が 2 つ立つ方が、警告より悪い。実測で確認した：
  生きている無関係プロセスを指す pid ファイルを置くと、消して警告なしで起動する。稼働中の
  サーバに対して `up` を打つと、pid ファイルは変わらない。
...
## 決定 4′ を差し替える——ワークスペースの停止でデータベースは消えない（2026-09-18）

決定 4′ が書いている挙動を、利用者が読み返して却下した。**「ワークスペースを止めたらデータベースが
消える」は、配備がやってよいことではない。** 以下が決定 4 と決定 4′ を置き換える。上の節はそのまま
残す。

- **決定 4′ → 4″。データディレクトリは常に `~/.local/state/af-db/<instance>`。** 名指しで頼まれない
  かぎり `AF_WS_SCRATCH` は見ない。ホームはワークスペース自身の状態が置かれる場所であり、停止をまたい
  で残る唯一の場所である。
- **`af-db up --ephemeral` が作業ディスクへのオプトイン**。中身が治具だと分かっていて速さが欲しい
  利用者のためのもの。インスタンスに貼り付く（sticky）：あとから素の `af-db up` を打っても、置いた
  場所のまま使う。黙ってホームに空のデータディレクトリを initdb し直したりはしない。作業ディスクが
  無い環境（`AF_WS_SCRATCH` 未設定）では、無視したと 1 行出してホームを使い、そして**ここが肝心だが、
  インスタンスを ephemeral として記録しない**。そうしておかないと、配備が作業ディスクを注入し始めた
  日にデータディレクトリが動いてしまう。
- **`--persist` は `--durable` になる。古い綴りも動き続ける。** ホームが既定になると `--persist` は
  もう置き場所を選ばない。残るのは `fsync`（Postgres）と `innodb-flush-log-at-trx-commit`（MySQL）を
  on に戻すことだけである。既定になった側を名前にした旗は、初めて見た利用者には必ず「これを付けると
  データが残るのだな」と読まれる。そして本当の答え——「付けなくても残る」——こそ名前が言うべきこと
  である。`--persist` は今も動き、`--durable` になったこと・データディレクトリはどちらにせよ停止を
  またいで残ることを 1 行出す。利用者のシェル履歴にもこのリポジトリの文書にも残っている綴りで、
  そこで失敗させても何も教えないからである。登録簿の鍵は従来どおり `persist` のまま——変えれば、
  今日より前に書かれた登録簿から設定が黙って落ちる。
- **`af-db down --purge` はそのまま。** これは利用者が「これを消せ」と言う操作であり、夜にワーク
  スペースを止めることとは別の行為である。

### これが今日の体験を何も変えないこと（2026-09-18・開発配備で実測）

決定 4 が立っていた根拠は、現に存在する配備には当てはまらない。

- **ecs-ec2 の開発配備では `AF_WS_SCRATCH` がそもそも設定されていない**。2026-09-17 の実機の節が
  すでに記録しているとおりである。つまりそこのデータディレクトリは*いま*全部ホームにある。この訂正は
  今日の利用者の体験を何も変えない——作業ディスクが注入された日に発火する罠を、先に止める話である。
  同時に、**移行すべきものが無い**ことも意味する：作業ディスク上にデータディレクトリを持つ利用者は
  いない。
- **その配備の `/home/dev` は EBS のボリューム**（50 GB・`/dev/nvme1n1`・`df` で確認）**であって
  EFS ではない**。決定 4 の 2 つめの根拠——「小さいファイルを数千個 EFS に置かない。ADR 0044 が
  ファイルあたり約 14.5 ms と実測した」——は、ecs-ec2 のホームにはまったく当てはまらない。そこで
  EFS なのは keep ボリュームの `~/.config` 側で、置かれているのは登録簿とパスワードファイル、
  数えるほどのファイルであってデータディレクトリではない。
- **ホーム上のデータディレクトリは、そもそも遅くなかった。** 2026-09-17 の arm64 の実機（m8g.large）
  が測った Postgres の導入＋`initdb`＋起動 **1.6 秒**、`af-db up mysql` **5.2 秒**は、変数が未設定
  だったのだから全部ホーム上の数字である。`control-plane` の
  `TestPostgres|TestSchemaDialectParity` もその上で 4 PASS / 0 SKIP。速いディスクを待っているもの
  は無い。
- **停止をまたいで残ることは実測済み**（これも 2026-09-17）：停止前に書いた行が、起動し直したあとも
  残っていた。

### 未解決の問い 2 は格上げされ、問い直される

もとの未解決の問い 2——「EFS 上のデータディレクトリはどれだけ遅く、安全か。`--persist`（決定 4）を
勧めるか、勧めないか、MySQL では拒むかを決める」——は差し替える。あれはオプトインの旗について尋ねて
いた。ホームが既定になった以上、問いは**既定そのもの**についてのものになり、しかもホームが EFS で
ある profile に限られる。

> **2″. ホームが EFS である Fargate 配備で、Postgres や InnoDB のデータディレクトリは十分速く、
> 安全か。** 未計測。ホームがブロックストレージである ecs-ec2・docker・native には関係しない。
> 答えが悪ければ、直し方は Fargate 専用のデータディレクトリの置き場所か、そこでは MySQL を出さない
> ことである。**利用者のデータを消す既定に戻すことではない**——そこは決着している。

これが字面ほど危なくない理由は 2 つある。NFS がデータディレクトリの正しさを脅かす経路は主に
ファイルロックだが、ここではどちらのサーバも単一インスタンス・単一書き手で、1 つのワークスペースが
unix socket 越しに触るだけである。そして遅いと感じた利用者には `--ephemeral` がある——旧来の既定
そのものが、旗 1 つの距離にある。

### 変えないと決めたこと：登録簿とデータディレクトリが別の木にある

登録簿は Agent の他の状態とともに `~/.local/state/agent-fleet/af-db/` へ移ったが、データディレクトリ
は `~/.local/state/af-db/` のままである。本来は親を共有すべきところ。だが利用者の足元でデータ
ディレクトリを動かせば、いま存在するデータベースが全部迷子になる。整頓のためにそれは割に合わない
ので、動かさず、また誰かに発見し直されないようここに書いておく。

## 決定 9 にタブが加わる——データベースは名前で作り、名前で消す（2026-09-18）

決定 9 は Console の表面を「Workspace 設定のカード 1 枚」と決め、P1 はそれをツールチェーンタブの
最後のカードとして作った。利用者から、これを独立した**データベース**タブにし、そのタブから名前を
指定してデータベースを作成／削除できるようにと指示があった。どちらも受理する。決定 9 の置き場所を
これが置き換える。

- **データベースはツールチェーンではない。** Java の版は一度選んだら忘れる設定だが、こちらは利用者の
  データを抱え、作られ、つながれ、壊される。加えて、長いタブの一番下にあるカードは、そこにあると
  すでに知っている人にしか見つけられなかった。
- **`POST /env/databases/{engine}/create` と `/drop`。どちらも `db=<name>` を必須にする。** リセット
  と同じ規則で、理由も同じ：名前なしで尋ねられた Agent は `DBNameFor(ResolveDir())`——自分自身の
  ディレクトリ——を答える。作成なら誰も頼んでいないデータベースができ、削除なら誰も名指していない
  データベースが消える。名前が無い／書式に合わなければ 400 `db_required` ／ `bad_db`。`create` は
  冪等（利用者は「そのデータベースがあること」を頼んだのであり、現にある）。`drop` も無い相手に対して
  成功するのは同じ理屈である。
- **Postgres の削除は `WITH (FORCE)`。** 素の `DROP DATABASE` は接続が 1 本でも残っていると拒む。
  そして残っているのはたいてい、別のペインに置き忘れた `psql` である。利用者はデータベース名が
  書かれたボタンを、名前を言う確認を経て押している。そこに「database is being accessed by other
  users」とだけ返して Console から手の打ちようが無い状態にする方が悪い答えである。
- **`af-db create --db=NAME` と `af-db drop --db=NAME`** も用意する。決定 1 は契約が `af-db` だと
  言っている。Console からは届くのにシェルからは届かない動詞があれば、それが崩れる。
- **名前は送る前に Console 側でも検査する**。Agent と同じ `^[a-z_][a-z0-9_]{0,62}$` と、そのエンジン
  にすでにある名前の両方に対して。だめな名前は、トーストの 400 ではなく、無効化されたボタンとその下の
  1 文になる。

**次**：作業ディスク上のデータディレクトリでの ECS 初回は、もう待つ対象ではない——注入する配備は
無く、それを頼むのは `--ephemeral` になった。まだ試していないのは、Agent がイメージのものである
ワークスペースでの Console タブ、タブ越しの arm64 MySQL、そして未解決の問い 2″ である。

## クライアントは契約の一部である（2026-09-18）

利用から報告があった——シェルを開いても `psql` も `mysql` も無い。決定 1 は契約を「`af-db` と
URL」と書いたが、単位として URL は間違いだった。URL が役に立つのは、貼り込む先のクライアントを
すでに持っている人だけである。このコンテナでの実測：

- **`mysql` は最初から届かなかった。** サーバのアーカイブに同梱されており
  （`~/.local/share/agent-fleet/mysql/8.4/bin/mysql`・7.9 MB・P1 からずっと在る）、
  `install_mysql.go` は wrapper を 1 つも書かず、他に PATH へ出すものも無い。MySQL を 1 日
  動かしていたワークスペースで `command -v mysql` は何も答えなかった。
- **`psql` は誰も打たないコマンドを要求していた。** `workspace-agent install-pg-client` を呼ぶ
  場所は 1 つだけ——guide で見つけた利用者が自分で打つ、それだけである。
- `~/.local/bin` は**PATH に入っている**（`entrypoint.sh:10`）。つまり PATH の問題は最初から
  無く、そこに置かれる物が無かっただけである。

**決定 1 に一節を加える：エンジンを起動したら、そのクライアントを PATH に出し、繋がる状態にする。**

- `af-db up`（したがって Console の Start も）が `ensureClientTools` を走らせる。MySQL では
  `~/.local/bin/{mysql,mysqldump}` の wrapper を書き、Postgres では `psql` が無いときだけ
  `install-pg-client` を走らせる。**失敗しても致命傷にしない**——クライアントが取れなくても
  サーバが上がっていればデータベースは使えるのであり、そこで起動ごと失敗させるのは割に合わない。
- **セッション起動時に、標準のクライアント変数を注入する**（`PGHOST`・`PGPORT`・`PGUSER`・
  `PGPASSWORD`・`PGDATABASE`・`MYSQL_UNIX_PORT`・`MYSQL_TCP_PORT`・`MYSQL_PWD`）。稼働中の
  エンジンの分だけを入れ、決定 8′ の `AF_DB_URL_POSTGRES` 1 本を置き換える（ただし同変数も
  引き続き設定する。読んでいるプロジェクトがあるため）。`$HOME` でこれらの変数だけを与えた実測で、
  `psql` は作業コピーのデータベースを開き、`mysql` は `root@localhost` と答えた。
- **MySQL だけもう 1 つ必要だった。** MySQL のクライアントにはユーザ名の環境変数が**無く**、
  OS のユーザに落ちる。そのためソケットとパスワードを環境に入れても、bare な `mysql` は `dev`
  として拒否された。そこで wrapper は `--defaults-extra-file` で導入先の `af-client.cnf` を
  指す。中身は `user=root` の 1 行だけである。`~/.my.cnf` にはしない——あれは利用者のファイル
  である。パスワードも書かない——あれはインスタンスのもので、データディレクトリと一緒に作り直され
  るから、2 つめの写しは黙って古くなる。
- **`af-db connect [engine] [--db=NAME]`** は、ソケット・パスワード・ユーザ・データベースを
  実行時に解決して `execve` でクライアントに入れ替わる。必要なら先に起動も作成もする。これは
  **サーバを起動する前に開いたシェル**——環境注入が届かない場合——で効く綴りであり、Console で
  Start を押してから、すでに開いてあった端末で打つという順序を踏んだ利用者が必ず当たる場面である。
- **データベースタブにデータベースごとの「接続」ボタンを足す。** ホームで `kind=shell` の
  セッションを作り、その中で `af-db connect` を実行する。セッションのプログラムそのものを
  クライアントにはしない——`psql` を抜けたときに、セッションが終わるのではなくシェルが残るように。

### シェルへの `initial_prompt` は 30 秒待たされていた

ボタンを作る途中で見つけた。`deliverInitialPrompt` は打ち込む前に `PaneMode` が composer を
検出するのを待つ（上限 60 × 500 ms）。`PaneMode` に `shell` の場合分けは無く、今後も持たない。
つまりシェルに送られた起動タスクは、毎回 30 秒の上限を使い切り、さらに 2.5 秒の予備を足してから
最初の 1 文字が届いていた。30 秒かかるボタンはボタンではない。

シェルは、ペインができた時点で受け取れるものとして扱うようにした。これは保護の迂回ではない。
あの保護があるのは、起動中のエージェント CLI が起動画面に打ち込まれた文字を**捨てる**からであって、
bash は捨てない——exec した瞬間から行編集付きの tty を持ち、プロンプトが描かれる前に打たれた
文字も端末ドライバが溜めておくからである。

## ポートが動かなくなり、エンジンをワークスペースと一緒に起動できる（2026-09-18）

クライアントの件のあと、利用者から 2 つ。どちらも「データベースは毎セッション見つけ直すものでは
なく、それに向けて**作れる**ものであるべきだ」という話である。

### 決定 6 にポート固定を加える

`pickPort` は起動のたびに `127.0.0.1:0` を bind してカーネルが返した番号を使っていたので、ポートは
再起動のたびに変わっていた。実測：1 回の停止→起動で Postgres 39455 → 37977、MySQL 34935 → 41693。
決定 7 のアイドル停止（接続ゼロ 30 分）が再起動を例外でなく日常にするので、これは隅の話ではない
——**開発中のアプリの接続文字列を、サーバが戻るたびに書き換えることになる**。Postgres では番号
以上の問題で、unix ソケットの名前が `.s.PGSQL.<port>` なのでソケットのパスごと動いていた。

探索の順序をこうした：このインスタンスが前回使ったポート → エンジンの well-known ポートとその上
64 個 → ephemeral。

- **記録した ephemeral ポートではなく 5432 / 3306 から。** `127.0.0.1:0` は 32768〜60999 から
  割り当てるが、これはカーネルが外向きソケットに配る範囲そのものである。つまりそこで覚えた番号は、
  サーバが止まっている間に無関係な接続に取られうる。well-known ポートから始めればその往来の外に
  出られるし、何より利用者のフレームワーク・ORM・あらゆる入門記事が既定にしている番号なので、
  手元にある接続文字列がそのまま正解になる。
- **`PreferredPort` を `Port` とは別の欄にした。** 正常な停止は `Port = 0` に戻すからである。
  これが無いと、一度 5433 へ避けたサーバは、居座っていた相手が消えた次の再起動で 5432 へ戻って
  しまう——「動かないが、動くまでは」という固定になる。
- **固定は「優先」であって「保証」ではない。** このコンテナは利用者の他のセッションと共有である。
  実際に取れた番号は記録され、`af-db status`・`af-db connect`・Console はいつも生の値を読む。

### 決定 7 に、その裏返しを加える——自動起動

アイドル停止は「誰も使っていないデータベースに払わない」に答えていた。「このワークスペースには
常にある」に答えるものが無く、毎朝ボタンかコマンドから始まっていた。

`af-db` のインスタンスに `Autostart` を持たせ、Agent の起動経路で managed セッションの復旧と並べて
`afdb.Autostart` を走らせる。データベースタブではエンジン行のチェックボックスとして出る。

- **既定はオフ。** データベースは問い合わせが無くても、共有でメモリの限られたホスト上でメモリを
  使う。誰も頼んでいないものを起動するのは、このプロジェクトが利用者に代わって決めてよい既定では
  ない。
- **一度起動したエンジンにだけ。** フラグはインスタンスに載り、インスタンスは起動後にしか存在
  しないので、ワークスペースの起動が 450 MB のサーバをダウンロードする経路は生まれない。Console
  も `absent` の間はチェックボックスを出さない。
- **MySQL のメモリ門は同じように効く。** `af-db up` と同じ扱いで、自動起動だけが 2 GiB の
  ワークスペースを上限の向こうへ押し出す経路になってはいけない。
- **失敗は記録するだけで、致命傷にしない。** これは起動中のセッション復旧の隣で走る。起動できない
  データベースが、ワークスペースの起動を道連れにしてはならない。
