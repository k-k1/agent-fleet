...
[English](0061-work-item-inbox.md) | 日本語
...
- 関連: [0031-mcp-registry.md](0031-mcp-registry.ja.md)（MCP は「各 CLI が直接喋り、af は定義を配るだけ」。
  ...
  （温め続けない）/ [0052-tenant-git-oauth.md](0052-tenant-git-oauth.ja.md)（CP は秘密を素通しさせるだけで
  保持しない）/ [0059-repo-import-jobs.md](0059-repo-import-jobs.ja.md)（自走する処理と busy 判定の関係）
...
- 表示のために Workspace を起こせば、[0055](0055-idle-stop-and-carried-interactions.ja.md) が塞いだ
...
これは実装の都合ではなく約束として書く（[0052](0052-tenant-git-oauth.ja.md) が refresh token に
対して守った線と同じ）。
...
リモート MCP は OAuth なので [0031](0031-mcp-registry.ja.md) が非目的にした領域に入る（無人・停止中の
更新に使えない）。
...
**7. 取得ジョブは busy に数えない。** [0059](0059-repo-import-jobs.ja.md) が取り込みジョブを busy に
...
[0036](0036-working-sets.ja.md) の**リポジトリ粒度**の所属と噛み合わない（セッションだけ入れると
...
起票時は「**保存クエリが唯一の絞り込み**」（決定 8・[docs/80](../log/80-work-item-inbox.md) §80.1 /
...
（[docs/80](../log/80-work-item-inbox.md) §80.18.7）。
...
🔴 **訂正（2026-09-11）: 括弧内の「GitHub でも `is:pr` を書かなければ PR は出ない」は誤り。**
`/search/issues` は `is:pr` 無しでも PR を返す（実測: `repo:<owner>/<repo> is:open` の応答に
...
**24（§80.24）. プルリクエストの詳細は開いた時点で取り直す。課題は取り直さないし、取ったものは
保存しない。** 決定 20.1 の唯一の例外で、20.1 が挙げた 3 つの理由がそのまま範囲を決めている。

- **プルリクエストだけ。** レビューを拾うかどうかを決める情報 —— 下書きか / 競合の有無 /
  レビューの状況 / CI の合否 —— は決定 2 の保持範囲に無く、入れれば**キャッシュが provider の
  PR オブジェクトの写しになる**。しかもこの 4 つは 5 分の更新間隔の**中で**動くので、
  キャッシュした方が悪い: 5 分前の読みから「競合なし」と言う画面は、当たらないブランチへ人を
  送る。課題にはこれらの欄が無く、キャッシュの行だけで詳細に出すものは尽きている。
- **本文は取らない**（20.1 の本題はそのまま生きている）。取り直しでも本文は取らないので、
  CP が本文を持てないことは変わらない —— `fields=` の射影がその約束を要求に書いたものである。
- **ワークスペースを起こす理由にはしない。** 停止中は 409 で、詳細は取得済みの行をそのまま出し、
  いまどちらを見ているかを 1 行で言う。これは決定 1 を破るのではなく守る形で、20.1 が退けたのは
  **詳細がその取得に依存する**設計だった。こちらは 1 行で degrade する。

両側とも保存しない（CP は中継して忘れ、レールのキャッシュは自分の周期のまま）。費用が抑えられる
のは**人が詳細を開いたときだけ走る**からで、一覧と違ってタイマーでは動かない。

**24.1. プルリクエストの主ボタンは「GitHub / Bitbucket で開く」にし、セッション起動は畳む。**
差分もレビューのやり取りもコメント本文も af は持っていないので、正直な第一操作はそれらがある場所へ
行くことである（決定 1 の「チケットビュアーは作らない」）。起動は**畳むだけで、消さない**:
PR を一覧に出す理由そのものだった決定 19.1（人の PR のレビューは、こちら側にセッションがまだ
無い作業）には作業コピーが要るし、着手済み台帳（決定 5）も同じレビューを 2 人が拾うのを
止め続ける必要がある。

## 却下した案

- **取り直した PR の情報を CP にキャッシュする**（決定 24 の代案）。停止中でも新しい値が出るが、
  レビュー状況や CI という**新しい保持範囲**を CP に持たせることになり（決定 2 が引いた線はまさに
  そこ）、一覧の更新ごとに PR の本数ぶん provider を叩く。停止中に古い値を出さないことは、
  この機能では失うものではない —— 詳細は「いま拾うか」を決める画面で、その判断に 5 分前の CI は
  使えない。

- **CP が直接取得する（トークンを CP に封筒暗号で保管）。** 停止中も鮮度が保て、チーム共有の
  サービスアカウント運用にも向く。しかし「CP は秘密を保持しない」原則の破棄で、
  [docs/25](../log/25-ops-monitoring.md) §4.3 が ADR 送りにした未決点そのもの。**一覧の鮮度が
