...
39. ★ **サインイン方式は「テナントが持つもの」に一本化し、env の方式は既定テナントに属させる。**
    ...
    テナント定義になると `SUPER_ADMIN_EMAILS` に載っていても**誰も super_admin になれず**、
    承認者が居ないので最初のテナント行も有効化できない（決定 30）＝循環する。加えて
    `AUTH=oauth` は provider ゼロで起動を拒否する。
...
40. ★ **帰属を移すのは表示と規則だけ。provider id の形と identity 層は 1 行も触らない。**
    ...
    自テナントへ固定し（決定 32-3）、`upsertIdentity` は同じ判定で `roleHint` を空にする
    （決定 31）。改名すると **①共有の Google で入った人が既定テナント以外を使えなくなる**
    （兼務も部署テナントも全滅）**②その入口からは新規に super_admin になれない**。
...
41. ★ **「同じプロバイダの別設定」は許すが、ペアワイズ subject の IdP では手当てが要る。**
    ...
    `allowed_providers` を自由入力からピッカーに変えるだけ。**(b) 同じプロバイダの別設定＝
    ...
    - **判定は discovery を読む**（issuer のホスト名で当てない）。読むのは「同じ issuer の行が
      既にある」＝(b) そのものの場合だけでよい。
    - `pairwise` かつ `oid` が使えるなら **`link_claim` を必須**（保存時 400）。現行 UI の初期値は
      「既定（`sub` で見分ける）」なので、そのままだと事故る。
    - 名乗れる安定クレームが無い `pairwise` は**拒否しない**。分裂は起きるが決定 37（本人の同意で
      紐づける）が後から救い、`email_taken` の画面がその導線を案内する。代わりに**保存時と
      承認画面の両方に警告**を出す（誰に着地するかが変わる＝承認者が見るべき情報）。
    - (b) の副作用は残る: `t:` id なのでそのセッションはそのテナントに固定され（決定 32-3）、
      `client_secret` が DB に複製される。
42. **素の `/login` は既定テナントのページとして描画する（描画のみ）。**
    決定 36 の `hidden_providers` が素の `/login` に効かないのは、そこが*どのテナントでもない*
    画面だったから。既定テナントのページとして描けば規則がそのまま効き、§61.15.13 の
    「実装では変えられない」が撤回できる。★ **認可の根拠にはしない** — どのテナントに入れるかは
    membership とテナント規則が決める（決定 13）。未知の slug を汎用ページに落とす挙動も変えない。
    ★ 「全部隠したら無視する」安全弁（決定 36）は**既定テナントでこそ効く**: ここのボタンが 0 に
    なると、どのテナントにも属さない人（新任 super_admin・招待前の人）が入れなくなる。
    ★ 画面側では `allowed_providers` / `hidden_providers` の CSV 2 本が**行ごとの 2 トグル**
    （受け入れる / ボタンに出す）に変わる。**DB 表現は据え置き＝スキーマ変更なし**。
    実装上の罠 2 つは「空＝全部」という既存の意味から出る:
    **最後の 1 つは OFF にできない**（全部落とすと保存結果は「全部 ON」＝絞ったつもりで全開）、
    **未設定のテナントは「全部 ON」と描き、最初の操作で明示リストへ固める**。
    そして**順序を UI が守る**（先に絞ってから管理者を招くとその人が入れない。参照を外せるのは
    active な自前の行が 1 つ以上あるときだけ）。
...
- **デプロイの既定方式を廃止し、全部テナント定義にする（決定 39 の出発点）。** 一様で綺麗に見えるが、
  決定 31 の `roleHint` 空きで**誰も super_admin になれなくなり**、承認者不在で最初のテナント行も
  有効化できない（決定 30）。`AUTH=oauth` は provider ゼロで起動もしない。
...
- **env の方式を各テナントの面に「読み取り専用の行」として並べるだけ（P7-0 の当初案）。**
  ...
  答えない。**参照＝操作**にすれば、同じ一覧が「追加できる方式の選択肢」として意味を持つ（決定 41-a）。
...
- ⏳ P7（決定 39-42・docs/61 §61.17）は **env もスキーマも 1 つも増やさない**。触るのは
  Console のテナント面と `handleLogin` の描画、そして `GET /api/admin/providers` の**意味づけ**
  （「デプロイの env 一覧」→「既定テナントの方式」）だけ。
- ★ P7 で **`GET /api/admin/providers` を tenant_admin にも読ませる**必要が出る（他テナントの
  管理者が「既定テナントの方式から選ぶ」ため）。いまは `withSuperAdmin` 固定で、
...
- ★ P7 の Console 側は、403 を `{error}` で返す `api()` の性質と噛み合わない箇所がある。
  ...
  読める相手を広げる前に、この分岐（読めなかった／本当に 0 件）を分けること。
