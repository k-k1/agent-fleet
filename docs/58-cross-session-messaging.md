...
もうひとつ: **送信側には失敗が返らない**。refuse された送信でも呼び出し側のツール結果は
`success:true` のままだった（5・7 で確認）。つまり**送った側は届いたと信じたまま進む** —
だから遮断は「送信側で見せない」（deny）とセットでなければ、静かな行き違いを増やすだけになる。
...
- **塞ぐ側は「見せない」と「受け取らない」の両方を持つ。** 片方だけだと、送信側は成功したと
  信じたまま進み、事故は静かになるだけで減らない。
