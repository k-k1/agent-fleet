...
- ユーザーの資格情報は DB に入れない: Workspace home の暗号ストア `secrets.enc`（[07 §7.6](07-security.md)）。
...
| テーブル | 役割 / 主なカラム |
|----------|------------------|
| `tenant` | 部署単位（既定 1 テナント=全社）。`slug`(unique)・`limits`(JSON: max_workspaces 等)・`isolation`・`key_ref`・ログイン規則 3 種（0039: `allowed_providers`／`auto_join_domains`／`allowed_domains`・すべて CSV。**`allowed_emails` は無い** — 「誰が入れるか」の名簿は `membership` が持つ。[61](../log/61-login-idp.md) §61.9.5）|
...
| `egress_daily` / `egress_allowlist` / `deployment_setting` | egress 統制（0012/0013・[docs/20](../log/20-container-audit-egress.md)）: 日次観測集計 / 版管理 allowlist（state=active\|proposed\|retired）/ デプロイ全体 KV（egress mode 等） |
...
| `schedule` / `schedule_run` | 定時実行（0022〜0026・[docs/38](../log/38-scheduled-execution.md)）: cron/interval/once の定義と発火台帳（`next_run`/`last_run`）。reuse モードの回転台帳（0024）・run の対象セッションと manual/scheduled 区別（0025/0026）。CP DB に置くのは Workspace 停止中も時計を見られるのが CP だけだから |
| `mcp_server` | テナント配布 MCP サーバ（0028・[docs/48](../log/48-mcp-registry.md)）: remote（http/sse）定義のみで **stdio 用の command/args/env カラムを意図的に持たない**（ADR0031）。`headers_enc` はテナント鍵で封筒暗号、`user_secret`=1 はヘッダ名だけ配布 |
...
- migration を足したら**本書 §6.2 の表を更新**する（更新責務は [README](README.md)）。
