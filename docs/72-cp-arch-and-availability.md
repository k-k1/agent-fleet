...
| **P3** ⏳ | 実機（<dev-deployment>）: arm64 の CP が上がり、**Service Connect で Agent に届く** | 選択肢が本物になる |
...
2. **`control_plane_arm64` を既定 ON にするか。** **ビルド時間の問いは片付いた**——
   増分 **+71 秒**は 90 分の timeout に対して誤差で、OFF にしておく理由にならない。
   通常のリリースを 1 アーキのままにしておく限り `CpArch=arm64` は「専用に焼いた版でしか
   使えない機能」であり続けるので、**倒すべき**である。
   ⚠️ **ただし条件は「税」から「実機」に移った。** いま ON にすると、**一度も起動させて
   いない arm64 の面**を全利用者向けのタグに載せることになる——
   [64](64-ec2-persistent-workspace.md) §64.28 の
