...
- **常設ビルトイン**（5種。※時系列は下記補記参照）: ①「Agent Fleet アシスタント」(af_read, 案内役・観測役, USAGE知識)
  ...
  - **時系列補記（5種↔3種の食い違いの整理）**: 初期実装（`d5707d5b`, 2026-07-05）はビルトイン**3種**
    （AF アシスタント/汎用/翻訳）＝下記実装メモの「3種」はこの時点の記録。同日 `4b2c6e38` で
    オペレーター・整合性チェッカーを追加し設計どおり**5種**に。その後 docs/30 ②（`b66b2a99`, 2026-07-17）で
