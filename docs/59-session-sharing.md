...
**いま出ているモーダル(AskUserQuestion／ExitPlanMode)は転写とは別枠で通す。** 所有者の
Agent は、モーダルが開いているあいだその質問／プランを `messages` から外し、カーソルも
その行の手前で止める(`hidePendingInteraction`) — 転写に残る tool_use と、答えられる保留
カードとで同じ決定が二度描かれるのを避けるため。実体は `pendingQuestions`／`pendingText`／
`pendingPlan` というトップレベルの別枠だけになるので、**allowlist がここを落とすと共有先は
「質問が出ているあいだだけ何も見えない」**(一覧のチップは「質問中」と言うのに、開くと
直前の会話で終わっている)。よって質問文・選択肢・その `preview`・直前の地の文・プラン本文を
通す。`pendingPermission` は通さない — 許可プロンプトは所有者が自分の Workspace について
下す判断で、本文がコマンドと絶対パスを引用する。共有先には**不活性なカードとして**出す
(答えるのは所有者。RO は当然、RW でも回答は提案経路に載せない)。決着すると Agent が止めて
いたカーソルごとその行を出し直すので、転写側の決定済みカードと入れ替わる。

同じ理由で、全転写の回答マップ(`answers`: tool_use id → 回答本文＋却下フラグ)と part の
`declined` も通す。エージェントは質問の tool_use を**訊いた時点**で書き、その回答は数秒〜分後の
別行に来るので、窓がその2行をまたぐと共有先のカードは未回答のまま固定される(ミラーが
`patchAnswers` で直しているのと同じ問題)。`declined`(Escape で閉じられた)を落とすと、
カードが「回答済み」を名乗ってエージェントの定型文を選ばれた答えとして描く。
