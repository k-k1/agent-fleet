...
この inventory 同期は所有者単位で 10 秒間(`shareCatalogTTL`)キャッシュする。同期は所有者
Workspace の Agent へ 2 往復(`/sessions/catalog` + `/repos`)し、さらに catalog の全置換を
所有者ごとの mutex 下で行うため、受信者ごとの履歴ポーリング全てに乗せると共有履歴読み出しの
支配的コストになり、最初の描画が目に見えて遅れる。**間引くのは在庫の突き合わせだけで、認可は
