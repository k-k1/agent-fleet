...
| 価値 | 中身 | 無いとどうなるか |
|---|---|---|
| **文脈の自動注入** | キー・タイトル・URL を最初の指示へ、`feature/PROJ-123-…` をブランチ名へ、1 チケット 1 worktree | 結局コピペになり、ブラウザのタブと変わらない |
...
│   Jira   … workspace-agent から /rest/api/3/search（JQL）＋ API トークン（P1）
...
| ブランチ | `feature/<key の小文字>-<title の slug>`（テンプレは設定で変更可）。⚠️ 既存ブランチとの衝突は `useStartWork` の `branch_exists` 経路が既に扱う |
...
- **その項目名の作業グループを自動作成して所属**させる（`autoAddToActiveWorkingSet` の隣に
  `ensureWorkingSetForItem`）。[52](52-working-sets.md) の「案件別分離」の“案件”がここで実体を得る。
