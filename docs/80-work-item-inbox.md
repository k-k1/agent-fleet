...
## 80.21 取得元は select をやめる／タブ名をレール名に揃える（実装済み）
...
**決定 21: 設定タブ「課題管理」を「作業項目」に改名**（en: *Issue tracker* → *Work items*）。
同じものを指す語がレール（作業項目）と設定タブ（課題管理）で割れていて、どちらの設定なのかが
名前から読めなかった。**接続**グループの中に置いてあるので、「作業項目（に出すための接続）」で
語は足りる。`tenant.git_oauth_intro` の「接続 > 課題管理」も併せて追随させた。
