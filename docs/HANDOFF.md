...
- **残（P3-5 段2）**: 機微状態(`CLAUDE_CONFIG_DIR`/secrets)を browse 範囲外へ退避(D) + ファイルツリー/ビューア（コンテナ全体ブラウズ）。同一 uid shell からの完全不可視は原理的に不可（[17 §17.1](17-p3-5-plan.md#171-d-が前提先にやる-機微状態を-browse-範囲外へ)）。
- 次は **P3-5 段2 / 管理 UI / P3-7（AWS アダプタ）/ P3-10（パッケージング）**。
