...
機能仕様の正は **[dev/](build/README.md)（開発者向け）とコード**、利用者の操作は **[use/](../guide/member/README.ja.md)**、
...
- **操作を知りたい** → [use/](../guide/member/README.ja.md): member / admin / operator / lite の分冊。
...
- **進行中の設計**: egress 統制（[docs/20](log/20-container-audit-egress.md)・enforce 未了）。Go 内部リファクタ
  ...
  **新しく prompt を足すときの判断基準は
  ...
  自前エンジン（[decisions/0072](decisions/0072-engine-model-catalog.ja.md)・
  [0074](decisions/0074-engine-instance-classes.ja.md)）は P2 まで実機検証済み・
  残 = **0072 の P6（seed と残り 4 パラメータの撤去。🔴 `<役>ModelS3Key` はサービス生成条件も
  兼ねるので `<役>Enabled=true` を先に）**・0072 の P3/P5・**0074 の P2（VRAM 見積もりに KV
  キャッシュを入れる）**・そして **どの配備も GPU の梯子を宣言していない**（0074「フェーズ
  ではない」節。宣言するまでクラスの選択欄は出ない）。
...
利用者視点の詳しい手順は [use/01-first-day.ja.md](../guide/member/01-first-day.ja.md)。
