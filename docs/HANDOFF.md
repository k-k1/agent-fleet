...
- **MCP（P3-6 段1 + admin read/write、ともにライブ E2E green）**: `/mcp`（Streamable HTTP）で管理面+作業面を一体公開。PAT（Console 発行・発行者 role を live 継承・scope≤role）認証。**主目的 E=手元 Claude が自分の遠隔 claude セッション群を駆動**。CP（migration 0006/PAT/`/mcp`/member-drive 4 ツール）+Agent（`/sessions/{name}/input|status|output`）+Console（設定→MCP タブ）。**ライブ E2E green**（運用者デプロイで PAT→send_to_session→status→output で遠隔 claude 駆動・2 並行も確認・revoke→401）。**現在 `AF_MCP_ENABLED=true` で稼働中**（`deploy/local/oauth.env` に設定、run-dev.sh が渡す）。⚠️ 駆動の send-keys は **pane id（%N）解決**が必須（`=session` は target-pane でリテラル化し失敗、`sessionPaneID`）。
  - **admin ツール（2026-07-01 実装・ライブ E2E green）**: read=`list_workspaces`/`get_usage`/`list_sessions`、write=`stop_workspace`/`stop_session`/`set_user_quota`。`mcp.go` で `mcpTool.admin` フラグ + `adminPrincipal`（PAT→**live role 再解決**: super_admin / その PAT tenant の tenant_admin で gate、tenant はトークン固定＝クライアント供給を受けない）。read=既存 admin サービス層を wrap、write=`stopWorkspaceByMembership`/Agent `/halt`/`PutUserLimit` を wrap し **`AuditLog`（`actor_kind=mcp`, `actor_id`=PAT id）へ記録**。capability フィルタは UX で、tools/call 時にサービス層で role を再検証。監査の土台 = **migration 0007 `audit_log` + `InsertAudit`/`ListAuditByTenant`**（従来 `AuditLog` はスキーマ未作成だった）。**ライブ検証（運用者デプロイ, CP 再起動後）**: super_admin PAT=全10ツール可視・`get_usage` に host stats・`list_workspaces` 実データ／tenant_admin=admin ツール可視だが host stats 無し（super 限定が正しく効く）／plain member（identity role=user）=member 4ツールのみ・admin ツールは `unknown or unauthorized`／`set_user_quota` の write が `audit_log` に `actor_kind=mcp` で記録。検証は DB へ直挿しした admin PAT（一時・検証後削除）で実施。
  - 残=外部公開時に **ingress で `/mcp` を Bearer 通し**（oauth2-proxy パス除外）。**次段=dangerous（`rotate_key`/`recreate_workspace`/`stop_all_idle`、confirm+dry-run）**＝鍵ローテ実装・idle 検出（P3-9）・`tail_audit` の土台が要る。設計 [decisions/0006](decisions/0006-mcp-unified.md)、実装プラン [history/p3-6-mcp](history/p3-6-mcp.md)。
...
- **recreate / archive / 削除**: `POST /sessions/{name}/recreate`（過去 jsonl を捨て同一スロットで新規会話。opencode は `removeOpencodeSid`）。`POST /sessions/{name}/archive`・`/restore`・`GET /sessions/archived`（meta+jsonl 保持で非表示↔stopped 復帰）。stop（`handleStopSession`）= tmux kill + meta 破棄（jsonl は残）。**halt（`handleHaltSession`）= tmux kill + meta 保持**＝停止中（再開可能）のまま一覧に残す（端末で quit したのと同じ状態）。⋯メニュー「停止する（あとで再開できる）」が稼働中セッションに対し呼ぶ。stop=一覧から消す / archive=隠す / halt=止めるだけ、の3すくみ。
...
- **opencode web（ブラウザ版 opencode）は廃止**（2026-07、`temp/remove-opencode-web-opencode-web-ui`）。pk-opencode-webui の焼き込み・`opencode serve`＋`bun serve-ui.ts`・`/ocweb` プロキシ・Console トグルを撤去。opencode は tmux 内 TUI（CLI）のみ。設計は [decisions/0007](decisions/0007-opencode-web-via-pk-webui.md)（廃止済）。
