...
| やりたいこと | 使う場所 | ざっくり言うと |
|--------------|----------|----------------|
| AI に質問する・翻訳してもらう・文章を整える | **チャット（アシスタント）** | 使い捨ての質問箱。気軽に何度でも |
...
初めて開いたとき、「**Agent Fleet へようこそ / 数ステップで使い始められます**」という案内カードが
出ます。順番にチェックが付いていく、やることリストです。カードには 4 つの項目があります。

1. **ワークスペースを起動** — あなた専用の作業場所を立ち上げます。「起動」ボタンを押して、
   「起動中…」の表示がおさまるのを待つだけ。これは必ずやってください。
2. **git プロバイダを接続** — 開発者がコード置き場（GitHub など）とつなぐための項目です。
   **あなたはここを飛ばして大丈夫**です。チャットや閲覧だけなら不要です。
3. **エージェントを接続** — AI（Claude など）にサインインします。チャットを使うにはここが必要
   なので「接続する」を押し、画面の案内に沿ってサインインしてください。
4. **最初のセッションを作成** — 開発者が AI 作業を始めるための項目です。チャットや閲覧しか
   しないなら、これも飛ばして構いません。

つまり、あなたに必要なのは **1 番（ワークスペースを起動）と 3 番（エージェントを接続）だけ**。
残りは開発者向けの手順です。カードは右下の「あとで」で閉じられ、済んだ項目は自動でチェックが
付きます。ワークスペースは使い終わったら止めておけますが、次に使うときまた「起動」を押せば戻ります。
...
- 何も設定しなくても、いくつかの **アシスタント（AI の性格・役割）** が最初から用意されています。
  たとえば **翻訳**（日本語⇄英語）、**汎用**（なんでも相談）、**整合性チェッカー**（複数の文書を
  見比べて食い違いを指摘）など。用途に合わせて「＋ 新規チャット」のときに選べます。

### チャットとセッションは何が違う？

よくある疑問です。ざっくり言うと:

- **チャット（アシスタント）** = 使い捨ての Q&A・翻訳向け。あなたが気軽に何度でも聞ける相談窓口。
...
**Q. チャットとセッションは何が違う？**
チャットは使い捨ての質問・翻訳向けの相談窓口、セッションは開発者が AI に本格作業をさせる場所です。
