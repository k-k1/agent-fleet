...
**テナントの数字** — 名簿の上に、このテナントの表示名、メンバー数（「◯ 人」）、いま起動中の
ワークスペース数（「◯ 起動中」）、テナント全体の上限（「上限 — ワークスペース: X / セッション: Y」）が
出ます。**読むだけの面**です（上限を決めるのは super_admin・[02-limits.md](02-limits.ja.md) を参照）。
