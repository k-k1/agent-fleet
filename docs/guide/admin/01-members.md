...
Workspace 数（「◯ 起動中」）、そしてテナント全体の上限（「上限 — Workspace: X / Session: Y」）が
...
各メンバーの行には、稼働状態を示す点、user_key、メールアドレス、ロール（`member` / `tenant_admin`）が
並びます。行をクリックするとメンバー詳細へ進みます。

**メンバー詳細** — そのメンバーの Workspace 資源、稼働セッション、各種操作が集まる画面です。
...
1. 「email」欄にそのメンバーの Google ログインに使うメールアドレスを入れます。または「または user_key」欄に
...
3. 「追加」を押すと、そのメンバーがこのテナントに加わります。まだ一度もログインしていない相手でも
   先に追加できます（招待の先出し）。相手が初回ログインしたときに、この所属をもとに Workspace が
   払い出されます。

追加された本人には、このガイドの [member/](../member/README.md) を案内してあげてください。

### メールアドレスと user_key の関係

メンバーは内部的に **user_key**（メールから作られる短いキー）で識別されます。メンバー行やセッション一覧に
出るモノスペースの文字列がそれです。ふだんはメールアドレスを入れれば自動でキーが決まるので意識する必要は
ありませんが、監査ログやセッション俯瞰では user_key が主役になるので、「このキー＝この人」の対応を
つかんでおくと読みやすくなります。同じ人でもテナントが違えば所属（と Workspace）は完全に別扱いです。
...
その人の稼働中の Workspace を止めておく（[02-limits.md](02-limits.md) の「Workspace を強制停止」）ことで、
資源の占有と稼働を抑えられます。
...
- **member（一般メンバー）** — 自分の Workspace でコードを書き、セッションを動かす人。管理画面には入れません。
- **tenant_admin（テナント管理者）** — このテナント内のメンバー管理・資源閲覧・Workspace 強制停止・
