...
- **最大 Workspace** — このテナントで同時に起動できる Workspace の数（`0` = 無制限）。
- **最大 Session** — 同時に動かせるセッションの数。
- **最大 内部リポジトリ / 最大 LFS 容量** — 内蔵 git に置けるリポジトリ数と LFS の総容量。
- **アイドル自動停止** — 放置されたセッションと Workspace を自動で畳む時間（次節）。

テナント一覧のカードに出る「上限 — Workspace: X / Session: Y」が、いま効いている値です。これを
...
- **Session halt まで** — アタッチされていない放置セッションが、この時間を超えると「停止中（再開可）」に
  畳まれます。会話ログは残るので、あとから再開できます。
- **Workspace 停止まで** — 誰も接続しておらず稼働セッションも無い Workspace が、この時間を超えると
  コンテナごと停止します。
...
メンバー詳細を開くと、そのメンバーの Workspace の状態が出ます。上部に「稼働中／停止中」の別、
その下の **「Workspace リソース」** に 3 つのメーターが並びます。
...
「操作」欄の **「Workspace を強制停止」** を使います（稼働中のときだけ押せます）。確認ダイアログの
あと、そのメンバーの Workspace コンテナが停止します。**作業内容（home）は消えません。** メンバーは
...
- **Workspace 数の上限（テナント全体）** — 上限に達している状態で新たな Workspace を起こそうとすると、
  同じく 429 で弾かれます。
...
3. **必要なら手を打つ** — 一時的な暴走はメンバー本人にセッションを止めてもらうのが基本です。連絡が
   つかず放置されている、あるいは明らかに暴走している場合に限り「Workspace を強制停止」でいったん止めます。
