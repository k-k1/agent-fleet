...
**管理 → そのテナント → 「このテナントのサインイン方法」**。issuer・client ID・client secret・
email の信頼方法・受け入れるメールドメインを入力します。

> **登録しただけでは動きません。** 新しいサインイン方法は「承認待ち」で作られ、デプロイ管理者が
> 同じ画面で有効化するまで、そのテナントのログイン画面にボタンは出ず、ログイン URL を手で
> 組み立てた相手にもセッションは発行されません。
>
