...
- **稼働中のセッションを順に見る** — 画面を **← 方向にスワイプ**すると、いま稼働中のセッションへ順番に切り替わります（末尾まで来ると先頭へ戻ります）。切り替わると「2/3 セッション名」のように、何件中の何番目かが一瞬表示されます。作業グループを選んでいるときは、そのグループの中だけを回ります。左ペインが開いている間の ← は、これまでどおり左ペインを閉じる操作です。文字入力欄・ブラウザペイン・横スクロールする場所（コードブロックなど）の上では、そちらの操作が優先され切り替わりません。
