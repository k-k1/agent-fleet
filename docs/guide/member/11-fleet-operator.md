...
- **Launch new sessions** — pick a repository (dir), agent kind (claude / codex / cursor / copilot / agy / opencode / shell), and model, then start. Models are specified after checking the actually selectable list (claude uses tier names fable / opus / sonnet / haiku; codex / cursor / copilot / opencode use a catalog reflecting connection state). With a **worktree** you can carve out an independent working copy plus branch, so parallel work doesn't collide. If you pass an **initial task (initial_prompt)**, work starts right after launch.
