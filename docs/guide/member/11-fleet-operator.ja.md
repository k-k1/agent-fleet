...
- **新しいセッションを起こす** — リポジトリ（dir）、エージェントの種類（claude / codex / cursor / copilot / kiro / agy / opencode / shell）、モデルを選んで起動。モデルは実際に選べる一覧（claude はティア名 fable / opus / sonnet / haiku、codex / cursor / copilot / kiro / opencode は接続状態を反映したカタログ）を確認してから指定します。**worktree 指定**で独立した作業コピー＋ブランチを切り出せるので、並行作業どうしが衝突しません。**最初のタスク（initial_prompt）**を渡しておけば、起動後すぐに作業が始まります。
