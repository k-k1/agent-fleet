# 10. 運用ツール連携 PoC（MCP でインシデント壁打ち）🧪

**実験的な手順です**（[docs/25 サービス運用向け拡張](../../25-ops-monitoring.md) の Phase 0）。製品機能ではなく、現行の Workspace のまま手作業で PagerDuty / Grafana / CloudWatch などを claude セッションに繋ぎ、「インシデント対応・監視運用の壁打ち」が実用になるかを確かめるための PoC 手順です。
