...
- **リポジトリで開発する** — 「開発のセットアップへ」で続きの手順が展開されます。
  1. **git プロバイダを接続**（任意）— GitHub／Bitbucketにサインイン。privateリポジトリをクローン／pushするなら必要です（[04 リポジトリと git](04-git.ja.md)）。
...
[04 リポジトリと git](04-git.ja.md) にまとめています。
...
こまめにコミット・push しておくと安心です（[04](04-git.ja.md)）。
...
クローンに失敗するなど）は [09 困ったとき](09-troubleshooting.ja.md) にまとめてあります。
