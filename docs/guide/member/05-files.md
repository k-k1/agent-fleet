...
左ペインの **Files** に、Workspace 内のファイルがツリー表示されます。リポジトリのノードは
...
「相対パスをコピー」「ダウンロード」（ファイルのみ）「名前を変更」「削除」ができます。
フォルダを削除すると中身ごと消えるので確認が出ます。
...
- **外部 URL** — 新しいタブで開きます。
- **リポジトリ内の相対リンク** — そのファイルをビュアーで開く／フォルダを Files で表示します。
...
エージェントに「このファイルを扱ってほしい」ときは、ビュアーで**テキストを選択**すると
出る **「送る」** から送信できます（範囲を選ばず、ファイル全体を送ることもできます）。
