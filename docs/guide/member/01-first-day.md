...
> 最初のリポジトリのクローン、そして「一日の終わりにWorkspaceを止めていいのか」までを順にたどります。
...
- **リポジトリで開発する** — 「開発のセットアップへ」で続きの手順が展開されます。
  ...
  2. **リポジトリをクローンしてセッション開始** — WSバーの **「はじめる」** からクローンと起動をまとめて行えます（[02 セッション](02-sessions.md)）。
...
| リポジトリをクローンしてセッション開始 | WSバーの「はじめる」（左ペイン「リポジトリ」の「クローン」でもクローンのみ可能） |
