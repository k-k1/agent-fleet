...
> 最初のリポジトリ clone、そして「一日の終わりに Workspace を止めていいのか」までを順にたどります。
...
1. **ワークスペースを起動** — 「起動」ボタンで、あなた専用のコンテナ（Workspace）を立ち上げます。起動中は「起動中…」と表示され、running になるとチェックが付きます。まずこれをしないと後の操作（接続を含む）ができません。
...
- **リポジトリで開発する** — 「開発のセットアップへ」で続きの手順が展開されます。
  1. **git プロバイダを接続**（任意）— GitHub / Bitbucket にサインイン。private リポジトリを clone / push するなら必要です（[04 リポジトリと git](04-git.md)）。
  2. **リポジトリを clone してセッション開始** — WS バーの **「はじめる」** から clone と起動をまとめて行えます（[02 セッション](02-sessions.md)）。
...
| リポジトリを clone してセッション開始 | WS バーの「はじめる」（左ペイン Repos の「クローン」でも clone のみ可能） |
