...
> チャット、走っている claude の会話を読む「チャット」表示、そしてメモキューを扱います。
...
## 走っている claude の会話を読む・返信する

claude のセッションは、端末（ターミナル）表示のほかに **チャット**表示に切り替えられます
（ペイン上部の **チャット ⇄ ターミナル** トグル）。チャット表示では、走っている claude の
やり取りが**ターンごとの Markdown** で読め、そのまま返信もできます。スマホで進捗を
追いながら短く返す、という使い方に向きます。

claude があなたの判断を待っているときは、カードが出ます。
...
で止められます。送った直後は「送信済み。claude が処理を始めると反映されます」と出て、
少し待つと会話に現れます。
...
- **溜める** — 「走り書きメモを追加…（後でまとめて送信）」に書いて「追加」。ファイルの右クリックや [05](05-files.md) の「送る」からも溜められます。リポジトリ・カテゴリでグループ分けされます。
