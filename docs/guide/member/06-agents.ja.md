...
| チャット表示・履歴 | あり | あり | あり | あり | 実行中: あり（ツール出力は簡略）。停止中: マネージドは履歴なし |
...
選び方になります。いずれも会話表示、質問への回答、Planモード、他エージェントへの
会話の引き継ぎに対応します（コンテキストゲージは claude / codex / opencode）。
...
冒頭の表は主要 5 エージェントの比較です。こちらは Antigravity（agy）とエージェント以外の
...
| 機能 | claude | codex | cursor | copilot | agy | opencode | shell | ssm |
|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| マネージド（paneless）実行 | — | ✓ | ✓ | ✓ | — | ✓ | — | — |
| ターミナル（CLI）実行 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| 実行中のチャットミラー | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | — | — |
...
実行した場合は読み取り可能な履歴が残ります）。

WSバーの使用量チップはアカウント単位の残量枠がある場合に表示されます——opencode
（各自のプロバイダ API キー）と cursor にはその枠がありません。**shell** は素のシェル、
...
（Auto）。なお cursor のアシスタントは**読み取り専用**（`--mode ask`）です。
