...
- **Claude の追加モデル** — `claude-opus-4-8` のような完全 ID を登録すると、旧リリースが起動ダイアログ・既定モデル設定・MCP `list_models` の通常の選択肢になります。Claude Code にはアカウント連動のカタログ取得口がないため、利用できるかはセッション起動時に Claude Code が確認します。登録を削除しても既存セッションの設定は書き換えません。
