...
この章は、ターミナル実行のセッションと shell / SSM を扱います。セッションを開くと、
メイン領域に**端末**（黒い画面）が出ます。Codex / opencode をチャット実行で始めた場合は
会話画面だけが開き、端末はありません（実行方法の違いは [02 セッション](02-sessions.md)）。
...
各ペインの内容は独立しています。ターミナル実行の claude / codex / opencode なら上部の
**チャット ⇄ ターミナル**で会話の Markdown 表示と端末表示を切り替えられます。チャット実行の
