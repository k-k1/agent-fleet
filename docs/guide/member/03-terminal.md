...
- **Stepping through the running sessions** — **swipe left** to move to the next running session, wrapping around at the end. Each switch briefly shows where you landed, as in "2/3 session name". While a working set is selected, the rotation stays inside that set. A left swipe with the left pane open still closes the left pane, as before. Over a text field, the browser pane, or anything that scrolls sideways (a code block, say), that surface keeps the gesture and no switch happens.
