...
> 対象: 基本操作に慣れ、もう一歩踏み込みたいメンバー。ワークスペース内サービスのプレビュー、
> 手元の Claude からの遠隔操作（MCP）、社内の別ホストへのログイン（SSM）、環境設定と
> ワークスペースの作り直しを扱います。必要になったところだけ読めば十分です。

## サービスプレビュー — 立ち上げた Web を見る

ワークスペース内で起動した Web サービス（Spring Boot・各種 dev サーバー・任意の Web アプリ）を、
**追加のポート公開もコンテナ作り直しも無し**で新しいタブから開けます。

ワークスペース操作バー右の**ポート入力欄**にポート番号を入れて **「プレビュー」** を押すと、そのポートの
サービスが新しいタブで開きます（ワークスペースが稼働中のときだけ）。
