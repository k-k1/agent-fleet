...
- URL とブランチ名の綴りを確認します。
- submodule は clone 後にベストエフォートで取得され、失敗しても親の clone 自体は成功します（[04](04-git.md)）。

### private リポジトリで認証エラーになる

⚙設定 → 「Git」タブで、該当プロバイダが **「接続済み」** かを確認してください。
未接続なら OAuth かトークンで接続します。接続後は認証が透過するので、以後は
トークン入力なしで clone / push できます（[04](04-git.md)・[06](06-agents.md)）。
...
しばらく操作がないと**自動で停止**します（アイドル自動停止）。WS バーの起動トグルで
Start すれば、セッション一覧も clone も接続もそのまま戻ります（[01](01-first-day.md)）。
...
消えません。clone・未コミットの変更・セッション一覧・接続はすべて残ります。ただし
