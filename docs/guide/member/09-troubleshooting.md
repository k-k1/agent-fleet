...
- **サーバーが `127.0.0.1`（localhost）だけで listen していると届きません。** ブラウザはワークスペース内から
  接続しますが、`0.0.0.0`（全 interface）で待ち受けるよう起動オプションを見直すと確実です（例: Vite は
  `--host`、多くの dev サーバーで `HOST=0.0.0.0`）。
- **真っ白・404** は、絶対パスで asset を読むアプリで起きがちです。Spring Boot なら
  `server.forward-headers-strategy=framework`（または `native`）を設定、その他は base path をアプリ側で調整します。
  （軽量プレビューは特にこの影響を受けやすいので、画面表示は **「ペインで開く」** を使ってください。）（[08](08-advanced.md)）
