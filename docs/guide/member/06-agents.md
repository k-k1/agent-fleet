...
| モデル選択 | あり | なし | なし |
| 状態バッジ | 進行中 / 質問あり / 入力待ち など詳細 | 進行中 / 入力待ち（質問なし） | 進行中 / 入力待ち（質問なし） |
| 再開 | 可（作業フォルダ消失時は不可） | 可 | 可（スロットごと独立） |
| 分岐（fork） | あり | なし | なし |

迷ったら、状態バッジや分岐が充実している **claude** から始めるのが分かりやすいです。
手元の ChatGPT や OpenAI アカウントを使いたいなら codex、複数プロバイダの API キーを
切り替えたいなら opencode、という選び方になります。
...
キーを貼って **「接続」** すると保存され、opencode 起動時に注入されます。複数のキーを
登録でき、セッションのスロットごとに独立して動きます。モデル選択の UI はありません。
