...
- **セッション** — ファイルを実際に読み書きする作業、コードの変更、大きなファイルの一括処理。ファイル出力を伴うならセッションへ（[05 ファイル](05-files.ja.md) の「送る」でファイルを渡せます）。
...
[11 フリート・オペレーター](11-fleet-operator.ja.md) を参照してください。
...
- **溜める** — 「走り書きメモを追加…（後でまとめて送信）」に書いて「追加」。ファイルを開き、[05](05-files.ja.md) の「送る」から溜めることもできます。リポジトリ・カテゴリでグループ分けされます。
