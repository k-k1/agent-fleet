...
`af-sandbox`（アカウント 722507597273・単独アカウント＝自身が payer）の
...
### 67.5.2 ⚠️ タグも有効化も **CloudFormation では入らない**

よくある誤解なので先に潰す。**この機能のうち CFN に入るのは IAM 権限だけである。**

| もの | 誰が入れるか |
|---|---|
| `ce:GetCostAndUsage`（権限） | **CloudFormation**（`20-platform.yaml`）✅ |
| リソースへのタグ（`af-membership` / `af-tenant` …） | **実行中の CP**。CFN ではない |
| コスト配分タグの**有効化** | **人が 1 回**。CFN では**表現できない** |
...
CFN をデプロイ（権限が入る）
  ...
  → AWS がキーを発見（〜24h）
  → 運用者が ce update-cost-allocation-tags-status を 1 回（← CFN では不可）
```

⚠️ **この待ちの分の実費は必ず欠測する。**バックフィルが無いので避けられない。
最小化の実務は「**デプロイしたらすぐワークスペースを 1 つ起動し、翌日に有効化する**」。
...
## 67.13.1 実機に入れた（2026-08-17・af.lazmix.jp）
