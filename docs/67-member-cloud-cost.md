...
| [46](46-usage-accounting.md)（usage accounting） | エージェントの**トークン** | token | 個人設定の「エージェント使用量」 |
...
⚠️ Console には既に **「使用量」/ "Usage" というラベルが 3 か所**あった（個人設定＝トークン、
管理＝稼働時間、テナント設定＝稼働時間）。ここに 4 つ目として金額を足すと、
利用者は「使用量」を開いて何が出るか予測できなくなる。**足す前に名前を分ける。**
（2026-08-31 追記: 個人設定側も **「エージェント使用量」/ "Agent usage"** に改称して、
同名は管理／テナント設定の 2 つだけになった。）
...
`af-sandbox`（開発配備のアカウント・単独アカウント＝自身が payer）の
...
## 67.13.1 実機に入れた（2026-08-17・開発配備）
...
### 67.15.6 実機に入れた（2026-08-18・開発配備）
...
## 67.17 組織のメンバーアカウントでは、有効化状態が**構造的に読めない**（2026-08-25）

本番配備を立て直したときに実物で踏んだ。CP が毎ティックこう言い続ける:

```
cost tags: cannot read activation state: … AccessDeniedException:
Failed to list Cost Allocation Tags: Linked account doesn't have access to cost allocation tags.
```

**この配備のアカウントは AWS Organizations のメンバー（linked）**で、コスト配分タグの
一覧も有効化も **payer（管理）アカウントでしかできない**。CP 側の IAM を足しても解決しない。
実際、payer 側で 5 キー（`af-membership` / `af-tenant` / `af-role` / `af-pool` /
`af-slot-size`）を有効化してもらったが、**その事実をこの CP は読めないまま**である。

⚠️ **費用そのものは読める。** `ce:GetCostAndUsage` はメンバーアカウントでも通り、実測で
当月 $10.84 が引けた。読めないのは**軸の状態だけ**である。つまり「按分できているか」と
「有効化されているか」は別の問いで、**前者は答えられる**。

### 67.17.1 決め: 読めないなら、**実際に按分できているか**で判定する

ポーラーは毎ティック `af-membership` で group by した費用を引いている。**値の入った
membership が返ってくるなら、その軸は効いている** —— これは有効化状態そのものではないが、
**利用者が知りたいことの答えとしてはむしろ正確**である（有効化済みでも AWS の反映は
最大 24 時間かかり、その間は按分されない）。

- 追加の Cost Explorer 呼び出しは**しない**。1 リクエスト $0.01 なので、確認のためだけに
  叩き足さない。既に引いている結果を見るだけ（`noteAttribution`）。
- 証拠が取れたら **`settled()` にして問い合わせをやめる**。linked アカウントである限り
  答えは変わらないので、$0.01 の永久ループを作らない。
- ⚠️ **断定するのは `af-membership` だけ。** ポーラーが group by しているのはその 1 軸で、
  他のキーは「一緒に有効化されたはず」としか言えない。**確かめていないことを確かめたと
  書かない。**
- 生の AWS エラーを出し続けるのをやめ、読み手が取れる行動に置き換える
  （「payer でしか有効化できない。ただし按分は効いている」）。

### 67.17.2 運用側の但し書き

- **有効化は遡らない。** ただし請求コンソールに**タグのバックフィル**があるので、過去分を
  按分したいなら payer 側でそれを回す（対象月を選ぶ・反映に時間がかかる）。
- **キーは「AWS が発見してから」しか有効化できない**（`ValidationException: Tag keys not
  found`）。発見には実際にタグの付いたリソースが要るので、**配備の初日に Workspace を
  1 つ起こす**——その前の期間は永久に按分不能である。
