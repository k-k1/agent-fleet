...
#### 取得元が 2 つある副作用 — 「時々ソート順が乱れる」

この二経路には**並び順の違い**という副作用があった（実測 2026-08-31）:

| 取得元 | 並び |
| --- | --- |
| daemon `GET /api/model` | 上流カタログの生の並び（名前順でもプロバイダ順でもない）。プロセス内では安定 |
| CLI `opencode models` | id の昇順 |

`Catalog()` の Go 先頭寄せは `SliceStable` なので群の中は取得元の並びがそのまま透け、
**同じアカウント・同じ設定でも、serve が動いているかどうかでピッカーの順が入れ替わる**。
利用者からは「起動モーダルを開き直すと並びが乱れる」に見える（報告のスクショの並びは
daemon 実測列と完全一致した）。

対処は取得元の切り替えではなく**整形側の正規化**: `Catalog()` は Go 群 → その他の順に
グループ化したうえで、群の中を id（＝label）昇順に並べ替える（`agents.SortGrouped`）。
他 kind は上流の推奨順が意味を持つ（codex は `priority` 昇順＝ピッカー順、cursor / kiro /
copilot / agy は系列ごと・新しい順）ので**触らない**。取得経路が 1 本ならそれだけで
決定的なので、正規化が要るのは経路が複数ある kind だけ — ポリシーは
`workspace/agent/internal/agents/modelsort.go` に置いた。
