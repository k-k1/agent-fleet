...
- 目視: 自前 headless Chromium（`console/scripts/shots`）でチャットペインを開き、
  ヘッダーのチップと計画パネルを dark/light 両テーマで確認。ChatView を harness で
  開けるよう `/api/chat/conversations/{id}` のスタブを追加した（従来は1会話の GET が
  無く、チャットペインが「読み込み中」で固まっていた）。
