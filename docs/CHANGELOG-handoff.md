...
## 2026-07-15
- **docs/27＋ADR 0015 起草: エージェント制御の Managed Driver 化**。Codex TUI のモデル勝手切替バグ（暫定対処 `ac8c202`）を発端に、3 エージェント横断の設計を確定: Codex/OpenCode は共有 runtime（app-server / serve）＋構造化 RPC の managed を既定・ユーザー選択の CLI ルート常設、Claude は現状 TUI 維持（Session Manager 案は凍結・付録温存）。既存 read 層温存＋Driver/RuntimeSupervisor 増築、記録は「read=ネイティブストア正本／live=イベント／write=API」三層、認証・config 反映は generation＋drain、着手順 P1（Codex 観測拡張）→P1.5（Console managed UI）→P2（OpenCode）→P3（Codex 既定化）。並行設計セッション sol×fable の統合。実装未着手。
- **docs/27 P1 実装: Codex 観測を 5 イベントへ拡張＋observer の thread アタッチ**（`da2917a`＋レビュー修正 `6735801`）。account/rateLimits/updated・model/rerouted・thread/settings/updated・warning・thread/status/changed を構造化ログ化し、rate limits は `/codex/usage` が rollout 読みと鮮度比較。実装中の最重要発見 = **thread スコープ通知は「そのスレッドをロードした接続」にしか配送されない**（0.144.3/0.144.4 実測）→ 既存の圧縮検知（`a6db76c`）は本番不発だった。observer が `thread/resume` で read-only アタッチする `codexObserver` を追加（rollout 不変を sha256 実測）。reroute に rate limit 起因は存在せず発端バグは TUI ナッジでほぼ確定（docs/27 §12.1）。レビュー修正 = rate limits push のスパースマージ・notLoaded で requested を forget。
- **docs/27 P1.5 実装: Console managed セッション UI の受け皿と Driver 層 IF**（`911769e`）。`internal/agents/driver.go`（Driver/ThreadHandle/Capabilities/TurnInput/Interaction/TurnState）、意味論エンドポイント `POST /sessions/{name}/turn`（start/steer/interrupt）・`/respond`（tui は tmux 経路へ委譲・managed は P2 の driver 登録まで 501）、driver 軸のワイヤ化（Meta.Driver→Session.driver、作成 API は managed を P2 まで拒否）、Console は送信 start/steer・停止 interrupt・質問の構造化回答 onRespond・paneless 描画（TerminalView 非マウント/トグル非表示）＋旧 Agent への /input フォールバック。pane 前提機能の棚卸し 12 項目と置き換え設計は docs/27 §10.2。CP 中継 `sessionWire` に driver を追加（無い field は silently drop される中継——なお color/context/exitReason 系は現在も drop されており docs/26 の exit chip が CP 経由で欠ける疑い＝別途要修正）。
- **CP `sessionWire` 中継の silent drop 修正**（`1b745ce`）: title/color/context/branch 系/exitReason 系を中継 struct へ追加（回帰テスト付き）。docs/26 の exit chip・ContextBar・SSM 背景色が CP 経由で復活。
