...
## 4-11. 追補（2026-09-08）— 再認証したのに「再認証する」のまま／その後も止まったまま

利用者報告（スクリーンショット付き）:「再認証のエラーが出たとき、再認証した後、
**再認証済みに変化してほしい**」「また、**自動的に再開してほしい**」。

§4-7 で失敗ブロックに「再認証する」の導線を、§4-8 で事前検知と送信ガードを入れたが、
**その後**を誰も見ていなかった。

1. **カードは永久に「再認証する」を出し続ける。** 転写は追記されるだけで書き換わらない
   ので、利用者が設定 > エージェント で再認証して戻ってきても、会話の末尾には赤いブロックと
   「再認証する」ボタンが残ったまま。**直したのに効いていないように見える**。
2. **セッションは死んだところで止まったまま。** 認証切れは blocked ＝ 再送しても同じ
   なので §4-6 の自動再開は正しく手を出さない。しかし**再認証の後は話が別**で、そこから
   先は「続けて」と打てば進む。つまり §4-6 が消したはずの往復（人が手で「続けて」と打つ）
   が、この経路にだけ残っていた。

### 4-11-1. 判定材料 — 「今のログインがそのターンより後に書かれた」

**`claude.AuthOKAt()`**（`authexpiry.go`）＝ `.credentials.json` の **mtime**。ただし
`Expiry.Known && !Dead(now)` のときだけ返し、それ以外はゼロ（＝判定材料が無い）。

★**「今ログインが有効か」を材料にしてはいけない。** この失敗はサーバ側の失効であることが
多く、**失効しても手元のファイルは健全なまま**（§4-7 / §4-8 の実測どおり `claude auth
status` は手元しか見ない）。だから「有効か」は最初のティックから true で、**あらゆる認証
失敗をその場で「解決済み」と言ってしまう**。偽装できないのは *いつ書かれたか* の方で、
ファイルを書き直すのはサインインし直したときか、リフレッシュが**成功した**とき — 後者も
「ログインは生きている」という同じくらい良い証拠なので、どちらでもよい。

判定はこの一本だけにし、**画面と自動再開の両方が同じ 2 値を比べる**:

```
renewed := AuthOKAt() > 失敗したターンの時刻
```

これを分けると、「カードは直ったと言っているのにセッションは止まったまま」／その逆、
という食い違いが必ず出る。

### 4-11-2. 画面 — 同じカードが「再認証済み」に変わる

- `session.Session.authOkAt`（Agent → CP → Console・表示専用）。**時刻**を運ぶのであって
  真偽ではない。真偽だと過去のターンの失敗が解決済みかを答えられない（上記）。
  停止中のセッションでも送る — 認証切れで死んだセッションのミラーこそ、利用者が
  「効いたか」を見に来る場所。
- `TranscriptCaps.authOkAt` → `authResolved(authOkAt, turn.endTs || turn.ts)`
  （`turnTime.ts`）→ `ErrorBlock resolved`。**片方でも欠けたら false ＝ 導線を出したまま**
  にする。空振りは「直っている設定カードを一度見に行かせる」だけだが、逆は「壊れた
  ログインを直ったと言う」。
- 解決後は**ボタンを出さない**。設定 > エージェント を開いても接続済みのカードがあるだけ
  で、押した人には「これも効かなかった」と読める。失敗そのものは赤いまま（実際に起きた
  こと）で、その下の行だけが緑になる。

### 4-11-3. 自動再開 — `auth_resume.go`（人を待つ側の時計）

`abort_resume.go` と同型だが**待つ相手が違う**: あちらは秒で解ける故障、こちらは人。

- 入口は `claude.AuthAbort(sid)` ＝ **`Abort.Auth`**（`abortFrom` が
  `apiError.isAuth()` で立てる。§4-7 で `cause="auth"` を決めているのと**同じ分類器**）。
  blocked の部分集合で、「待っても解けないが**人の操作**では解ける」のはこれだけ。
- スイープは `abortResumeTick` に相乗り（`authResumeStep`）。転写末尾の読みは
  セッション×スイープで 1 回のままにする。
- **待ちは無期限。TTL は 14 日**。金曜の夜に切れたログインは月曜に更新される — 週末で
  諦めると、この機能が存在する理由そのものの場面を落とす（`abortResumeEpisodeTTL` の
  30 分をそのまま使ってはいけない）。
- 更新を見てから `authResumeSettle`（15 秒）待つ。claude はサインインの途中で
  `.credentials.json` を書くし、ペインがまだ `/login` の最中のこともある。
- 送信可否は `abortResumeReady`（＝`promptBlocker` と ペイン）を再利用。資格情報が死んで
  いる間は `promptBlocker` が `auth` を返して弾くので、**二重に守られている**。
- 再開プロンプトは §4-6-4 と同じ「続けて（自動再開）」— ミラーの 自動再開 バッジに乗る。
- 設定は `claudeAbortAutoResume`（§4-6-6）を共用。**新しいトグルを足していない**: 利用者
  から見て「claude の中断ターンの自動再開」の一種であり、片方だけ切りたいという要望も
  実例も無い。
- 打ち切りは capped（更新後のログインでも同じ認証エラー＝そのログインでは直らない）/
  undeliverable（人がペインを使っている）/ stale。
- **報告は抑止しない**（`AbortResumeHolds` は `Retryable` 限定のまま）。認証切れは何時間も
  何日も続きうるので、その間報告を握るのは §4-6-3 の「遅らせるだけ」の前提から外れる。

### 4-11-4. テスト

| 対象 | 内容 |
|---|---|
| `internal/agents/claude/abort_test.go` | `Abort.Auth` — 実測の 401、文言が未知でも `error` だけで立つこと、上限・プロンプト超過（blocked だが auth でない）、retryable では立たないこと |
| `internal/agents/claude/authexpiry_test.go` | `AuthOKAt` — 資格情報無し／期限切れ／環境トークンでゼロ、書き直しで時刻が進むこと |
| `internal/sessionx/auth_resume_test.go` | 更新前は 24 時間経っても送らないこと、更新＋settle で 1 回だけ送ること、capped / undeliverable、末尾が動けばエピソードが閉じること、retryable には手を出さないこと |
| `console/src/features/mirror/turnTime.test.ts` | `authResolved` — 新しい／古い／同時刻、欠損・壊れた時刻は false、局所オフセットと Z の比較 |
| `console/src/features/mirror/transcript/TranscriptView.dom.test.tsx` | 古いログインではボタンが出て、新しいログインではボタンが消え「再認証済み」が出ること。認証以外の失敗は変わらないこと |
| `control-plane/session_wire_test.go` | 中継で `authOkAt` が落ちないこと |

★**逆側の固定（誤検知）が本体**: `authResumeRenewed` を `true` 固定にすると
...
## 4-13. 追補（2026-09-14 / s4qxfmt）— §4-12 の実機 1 回目で出た 2 つの穴

§4-12 を配備した当日、利用者報告: **codex のバッジが「制限解除待ち · 10:25」なのに、codex 自身の
エラーは "try again at 3:26 PM"**。実測（セッション scpigmc・Agent の `/sessions` と
`/codex/usage` を 1 分間隔で採取）で組んだ時系列:

| 時刻 | 起きたこと |
|---|---|
| ~10:23 | 1 回目の上限。当時の読みは 5h 窓 100% ・reset 10:25:53 → **10:25:53 に予約**（当時のデータとしては正しい） |
| 10:25:53 | 予約が発火 → 再開 → そのターンがまた上限で落ちる |
| 10:26:47 | ここから新しい 5 時間窓（これが 100%・15:26:47 まで＝codex が言う 3:26 PM） |
| 10:26〜10:55 | 🔴 **再予約されず、バッジは 10:25 のまま**（穴 A） |
| 10:55:53 | 猶予切れでエピソード再作成 |
| 10:56〜11:01 | 🔴 **`rateLimitResumeAt` は空のまま**。account view は同じ時間ずっと「100%・15:26:47」と答えているのに予約は 0 件（穴 B） |

つまり §4-12 に書いた「早すぎた再開は撃ち直して正しい窓へ収束する」は**成立していなかった**。

### 4-13-1. 穴 A — 使い終わったエピソードが次の上限を飲む

`rateLimitRecover` の巻き直しは `episodeStale`（＝`ResumeAt` + 掃除猶予 30 分）だけで、その内側では
`scheduleRateLimitResume` が `ScheduleID` を握っている限り即 return する。**発火済みの予約が、その
再開が失敗して生まれた次の上限を 30 分吸う**。表示側も `rateLimitWaiting` が過ぎた `ResumeAt` を
そのまま返すので、バッジは過去の時刻を出し続ける。

直し方:

- `episodeSpent`（`ResumeAt` + `rateLimitRebookSettle` = 3 分）を足し、`rateLimitRecover` の
  巻き直し条件に入れる。**この関数が呼ばれている時点でセッションは「いま上限」なので、送った再開が
  効かなかったことは確定している**。
- settle を 0 にしない: 配達より証拠が遅れる（再開ターンが CLI に届いて 1 行書くまで「上限」は真の
  まま）。0 だと、送ったばかりの再開に対して予約を撃ち直す。
- 巻き直すときは**使い終わった once 予約を消す**（id を忘れる直前なので、残すと定時実行の一覧に
  死んだ行が溜まる）。`episodeStale` 側の巻き直しにも同じ漏れがあったので一緒に塞いだ。
- `episodeStale` に畳み込まない: あちらは follow-up 経路で「ファイルを消してよいか」も答えており、
  そこでは「発火したばかりの予約」は猶予が切れるまで触ってはいけない。
- バッジは過ぎた `ResumeAt` を載せない（`futureInstant`）。時刻なしの「制限解除待ち」は真だが、
  「10:25 まで待ち」を 10:41 に出すのは嘘。

### 4-13-2. 穴 B — `codex.ResetAt` は窓の境界で必ず黙る（本命）

`ResetAt` が見ていたのは `readUsage()` ＝**ローカルの読みだけ**（app-server の push と rollout）。
どちらも「最後に何かが書かれた時点で現在だった窓」を語るので、境界を越えると `adjustWindow` が
率を 0 に潰して reset を次の境界へ送る。**この関数が呼ばれるのはまさにその瞬間**（古い境界で再開が
発火して、また断られた）なので、満杯の窓が 1 つも無くなり、**二度と予約できない**。

本当のことを知っているのは 3 つ目の源 **account view** だけで、これは
「100%・15:26 まで」＝codex が利用者に見せている文言と同じ数字を返し続けていた。

- **読みは「新しい方」ではなく「全部」を見る。** チップ（`pickUsage`）が年齢で選ぶのは
  「いま残りいくつか」を描いているからで、そこでは境界直後の 0% が正解。こちらの問いは
  「出し切っているか、いつまでか」なので、**満杯の窓は、どちらの読みが持っていても証拠**、
  潰れた窓は何も言わない、とする。新しい方を優先すると、境界の直後に push が来ただけで
  潰れた読みが主導権を握る。
- account view は 5 分キャッシュ＋1 エピソードあたり数回なので、叩きすぎの心配は無い
  （claude のチップを殺した 429 の轍は踏まない）。呼び出しは 10 秒で切る（watch の sweep を
  止めないため）。

### 4-13-3. この 2 つを分けて見つけられなかった理由

穴 A だけ直すと「30 分後に巻き直して予約される」と読めてしまうが、実際は穴 B で**巻き直した先でも
予約できない**。実機の 6 サンプル（10:56〜11:01 で `rateLimitResumeAt` が空のまま）を採って初めて
2 つ目が見えた。**「直った」と言う前に、次の予約が入ることまで観測する**。

### 4-13-4. テスト

| 対象 | 内容 |
|---|---|
| `internal/agents/codex/usage_test.go` | 境界をまたいだローカルの読み（潰れている・しかも**より新しい**）より account view の満杯窓を採ること、ChatGPT ログインが無い配備ではローカルだけでも採れること |
| `rate_limit_resume_test.go` | 発火済み＋まだ上限なら新しい時刻で予約し直し、使い終わった予約を消し、試行回数が新品になること／発火直後（settle 内）は撃ち直さないこと／過ぎた `ResumeAt` はチップに載らないこと |
...
- ~~§4-12 は**実機で踏んでいない**~~ → 2026-09-14 に踏んで §4-13 の 2 件が出た。ただし通し
  （「止まる → 予約 → その時刻に再開して**続きが走る**」）はまだ 1 回も成立していない — 実機で
  観測できたのは「止まる → 予約 → 再開 → また上限」までで、最後の 1 歩は上限が解けた窓でしか
  確かめられない。
- §4-11 の対象も **claude TUI のみ**。codex / opencode managed の認証失敗は
  `Abort.Auth` を立てる材料（`isApiErrorMessage` 相当）を持っていないので、
  再認証しても止まったままになる。
- §4-11 は**実機で踏んでいない**（認証切れを起こしてから再認証する、という状況を
  作れていない）。固めてあるのは単体の状態機械と描画で、実際のワークスペースで
  「切れる → 再認証 → 勝手に続く」を通した実測はまだ無い。
- §4-11 の判定は資格情報ファイルの mtime なので、**環境トークン運転（`CLAUDE_CODE_OAUTH_TOKEN`
  / `ANTHROPIC_API_KEY`）では常にゼロ** ＝ カードは「再認証する」のままで自動再開もしない。
  材料が無いので黙って何もしない側に倒してある。
