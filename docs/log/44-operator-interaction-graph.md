...
> 🔴 **2026-09-20 追記 — 下の「状態」行は起票当時のもので、この設計は採らないことになった**。ADR 0027 は
> [ADR 0096](../decisions/0096-fleet-session-graph.ja.md)（レーン＝セッション × 横軸＝時間の
> フリート俯瞰図）が superseded にし、後続は [docs/101](101-fleet-session-graph.md)。
> 理由は本文の前提が 2 つとも崩れたこと: §2.1 の「指示は arm にしか無い」は ADR 0035（報告 v2・
> 2026-07-29）の `instr-ledger` が先に解決しており、§0 の「1 オペレーター会話」という範囲は
> ADR 0041 決定 9（peer は会話に帰属できない）が否定した。**§0 が「別スコープ・別タスク」として
> 送り出したフリート全体の俯瞰図が、そのまま本体になった**。
>
