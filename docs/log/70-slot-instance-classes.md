...
> 状態: **P0〜P5 完了**（2026-08-23）。実デプロイ（開発配備）で arm64 の end-to-end が
> 通り（§70.14）、**難所と言い続けたアーキ跨ぎの home 自己修復**（§70.14.5）も
> **agy の OAuth・実セッション**（§70.14.8）も通った。道中で実機だけが教えてくれた
> バグが 4 件（§70.14.6〜9）。残るは **publish 経路を本物のリリースで 1 度出すこと**だけ。
> arm64 クラスは既定にはまだ載せない。
> ADR は P1 着手時に `0051` として起票する。
> 本書の価格・AMI・ECS API の記述は **ap-northeast-1 の実 AWS（開発配備のアカウント）で
...
1. **もっと安いインスタンスを使えるようにする**（m7g / m6g / m6i）。うち m7g・m6g は **arm64**。
...
**① 「一番安いインスタンスが一番安い」は成り立たない。** 同じ仕事を終えるまでの費用（$/時 × 所要時間）で
並べ直すと、順番がひっくり返る。
...
スロットの課金は **インスタンスが running である壁時計時間**に比例し（アイドル 15 分で stop・
...
2. **メモリ軸と二重管理になる。** 現在は「メモリ要求 → 載るインスタンス」の一方向で、`slotTypeFor` が
   唯一の決定者。タイプを直接持つと「8 GB と書いたのに 2xlarge」が表現でき、
   決定 21 が作った「Console はどのインスタンスに載るか**を言う**」という説明が破綻する。
...
- **`crane` 経路はそのまま通る。** `harness/setup.sh` の `crane copy` はインデックスを
  丸ごと複製するので、実デプロイ手順（[ecs-real-deployment](64-ec2-persistent-workspace.md)）は
  変更不要。
...
🔴 **この帰結は解消した（2026-09-04・実機で確認）。** ベースを Debian 13 (trixie) へ
移した（[ADR 0068](../decisions/0068-debian-13-base.md)）ことで glibc が **2.41** になり、
rtk の `aarch64-unknown-linux-gnu` が要求する `GLIBC_2.39` を満たす。**実機 Graviton
（m8g.large / kernel 6.1 aarch64）の workspace コンテナで、rtk が子プロセスを起こし、
claude の PreToolUse フックが Bash を書き換え、削減が 345,786 → 2,754 バイト
（+107,758 トークン）計上されるところまで測った** —— 数値は amd64 と一致する
（ADR 0068 の ⑥）。**この段落の「第 3 の判断材料」は、いまは要らない。**

なぜ当時こう書いたかまで残しておく: **判断そのものは当時の一次情報に照らして正しかった。**
当時のベースは bookworm（glibc 2.36）で、rtk の arm64 配布は gnu ビルドしか無く、
実際に起動できなかった。誤りは事実ではなく**その事実の寿命の見積り**で、「上流が musl
aarch64 を出すか、ベースイメージの glibc が上がれば直る」と自分で書いておきながら、
**後者が来る日を「いつか」の側に置いていた**。実際に来たのは上流ではなくベースの方で、
しかも rtk とは無関係な理由（chromium の期限・[ADR 0068](../decisions/0068-debian-13-base.md)
決定 2）で来た。⚠️ **「直る条件」を書いたら、その条件が満たされたときに読み直される場所に
置くこと。** ここは凍結ジャーナルなので、その役目は果たさない。
...
🔴 **この表の「Debian 13 … 150 系＝現行より古い」は誤り**（2026-09-04 再測）。**比較の左右が
揃っていなかった**——bookworm 側は **security** 索引の版を、trixie 側は **main** 索引の版を
読んでいる。同じ索引どうしで測ると版差は無い: main は両スイートとも `150.0.7871.100`
（`-1~deb12u1` / `-1~deb13u1`）、security は**両スイート・両アーキとも `152.0.7977.75`**。
AL2023 に chromium が無いことは変わらないので、**E の却下自体は動かない。**
...
- **chromium が 151 → 150 へ「下がる」。** bookworm の pin は `151.0.7922.137-1~deb12u1`、
  ...
  **Debian revision まで固定し**、setuid sandbox の検証込みで採用を決めている。その pin と
  検証を全部やり直すことになる。
  🔴 **この代償は取り下げ（2026-09-04 再測）。** 上の表と同じ左右不揃いで、bookworm の 151 は
  **security** 索引、trixie の 150 は **main** 索引だった。security 索引どうしで測ると
  **両スイート・両アーキとも `152.0.7977.75`**（`-1~deb12u1` / `-1~deb13u1`）で、**版は下がらない**。
  pin と setuid sandbox の検証を取り直すこと自体（Debian revision は変わる）は残るが、
  **「古い chromium を掴む」という代償は存在しない。** 残る主要な代償は次の python 1 つになった。
...
3. **A は独立した作業として、いずれやる。** bookworm の寿命・glibc・各種ツールチェーンの
   ...
   🔴 **「いずれ」には期限が付いた（2026-09-04 に一次情報で確認）。** bookworm は
   **2026-07-12 に通常のセキュリティ支援を終えて LTS 隊へ引き渡され**（[Debian News
   20260712](https://www.debian.org/News/2026/20260712)）、LTS は **2028-06-30 まで**。
   ただし**移行の期限はその日ではない**——ブラウザペインが製品機能である以上、期限は
   **chromium が `bookworm-security` から落ちる日**である。⚠️ bullseye では
   **LTS に入る前に chromium が `non-supported` にされた**前例がある
   （[#1061268](https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1061268)・2024-01・
   `120.0.6099.224-1~deb11u1` で打ち切り）。bookworm では 2026-09-04 時点で
   `debian-security-support` の非支援リストに chromium は**載っていない**が、
   2026-08-21 の DLA 4749-1 で**bookworm のビルドだけが失敗し trixie だけ出た**
   （LTS 側の回答は「bookworm の新しい rustc が原因かもしれない」）。
   **落ちる日は宣言ではなくビルド失敗として来る。**
...
- **言葉は技術識別子でなくドメイン概念**（`console-ui-plain-preview` の線）。
  ...
  `m7g.xlarge` は決定 21 の「載るインスタンス」の行に**副次情報として**出る。
...
- **ガイド**: `docs/admin/02-limits.md` と `.ja.md` の両方（二言語必須）。
...
## 70.14 P5 —— 実デプロイで通した（開発配備・2026-08-22）

0.10.0 を開発配備と本番配備に本デプロイした状態で実施。arm は
**開発配備だけ**に入れた——本番配備には実ユーザーの Workspace が動いており、イメージ参照を
...
## 70.14.5 アーキ跨ぎの home 自己修復 —— 通した（開発配備・2026-08-23）
...
1. §70.14.2 の placeHome バグが task def `:21` に **`RuntimePlatform=ARM64` ＋
   `placementConstraints: ec2InstanceId == <m6i のインスタンス>`** という矛盾を焼き付けた。
...
### 70.14.10 ✅ 「ピン止めできるのか」を実機で最後まで確かめた（2026-08-23）

ここまでの修正は**すべて Dockerfile と entrypoint.sh の中身**なので、焼き直すまで
一切効かない。「直した」と「固定できている」の間には実機 1 回分の距離がある。
`0.10.1-dev-54033c64`（両アーキ・lean・index 済み）を焼いて開発配備に入れ、通した。

**⚠️ 検証の設計がこの節の肝である。** 単に古い版を入れて戻ることを見ても、**旧ロジック
（marker 比較）でも戻ってしまう**ので実体比較の証明にならない。そこで**今回のバグの
鏡像**を人工的に作った——実体を 1.1.17 に落とし、**marker には嘘の 1.1.19（＝新ピンと
同じ値）を書く**。旧ロジックなら「marker == pin」で**必ず素通りする**配置である。

```
ピン = 1.1.19 / marker(嘘) = 1.1.19 / 実体 = 1.1.17
   ↓ 再起動
[entrypoint] boot-install agy 1.1.19
   ↓
ピン=1.1.19 marker=1.1.19 実体=1.1.19 env=true   ← 三者一致
```

**marker を貫通して実体のズレを見つけ、ピンへ戻した。** ②が実証された。

#### ①の実証は事故から生まれた対照群

最初の仕込みは**壊れた**。旧イメージ（`=1`）で動いている間に agy が自己更新して
1.1.17 → 1.1.19 に戻ってしまい、REPIN は正しく「実体==ピン」で skip した。やり直しに
なったが、結果として**同じ Workspace・同じ仕込み・同じ操作で、イメージだけが違う**
対照実験になった:

| イメージ | env | `agy models` を叩いた後 |
|---|---|---|
| 旧 | `=1` | 1.1.17 → **1.1.19 に自己更新**（仕込みが壊れた） |
| 新 | `=true` | 2 分おきに 3 回叩いても **1.1.17 のまま** |

#### 結論: 3 層が揃って初めて固定される

| | 内容 | 実機の証拠 |
|---|---|---|
| ① 自己更新を止める | `AGY_CLI_DISABLE_AUTO_UPDATE=true` | 上の対照表＋`auto_updater.go:218 Auto-update disabled` |
| ② ズレたら戻す | 実体比較 REPIN | `boot-install agy 1.1.19`（嘘の marker を貫通） |
| ③ REPIN が動く設定 | opt-in 実質 OFF | 実機で `REPIN=1` |

❌ **固定されない条件も明記しておく。** **自己更新 opt-in が ON の Workspace は固定
されない**——それは利用者が latest を選んだ結果で、仕様どおりである。また**起動と起動の
間に利用者が明示的に `agy update` を叩けば上がる**（次の起動で戻るが、そのセッション中は
上流版）。

⚠️ **既に流れてしまった home は、焼き直すまで封殺前の世界の産物**である。実体==ピンで
一致していても、それは「封殺が効いていた」証拠ではない。

#### この検証で踏んだ運用上の罠 2 つ

- ⚠️ **vCPU クォータ（上限 16）**。amd64（m7i.2xlarge = 8）と arm64（m8g.2xlarge = 8）を
  同時に走らせ、既存スロット 8 と合わせて超過した。`RunInstances` が
  `VcpuLimitExceeded` で 12 回リトライして失敗する。**Graviton も同じ Standard バケット**。
...
| **P5** ✅ | 実 AWS で端から端まで（§70.14・開発配備）。**arm64 の golden が probe 込みで publish され**、**x86 → arm の切替も home 自己修復ごと通り**（§70.14.5）、**agy の OAuth と実セッションも通った**（§70.14.8）。道中で 4 件のバグを実機だけが教えてくれた（§70.14.6/7/8） | 実運用に出せる |
