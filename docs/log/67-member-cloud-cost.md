...
軸のタグが付いていない。** スロットは「プールのインスタンス」で、誰のものになるかは
...
- **空きプールのスロットも共有に落ちる。** `af-role=slot` かつ `af-membership` 無しの
  インスタンス時間＝「誰も使っていないのに回っているインスタンス」。
...
スイーパーの「タグが抜けているインスタンスをボリュームから写す」側が動いた:
...
## 67.18 推論エンジンと TTS を「別枠」にする——共有の内訳に `af-role` の軸を足す（2026-09-08・ADR 0048 決定 15）

利用者の問い:「推論エンジンや TTS 用インスタンスは共通のクラウド費用に計上されると思うが、
別枠を設けられないか」。**結論から言うと軸は既に在って、CP が読んでいなかっただけだった。**

**分かったこと（実測が先）**

`grep EC2TagRole control-plane/*.go` は `cost_tags.go` の 1 件しか返さない——**許可リストに
入れて有効化までしているのに、どこでも読んでいない**。CE への唯一のリクエストは
`GroupBy=(af-membership, SERVICE)` で、GroupBy は CE の上限が 2 軸。だから共有バケットの
内訳は AWS のサービス名だけになり、エンジンの GPU は `Amazon EC2 - Compute` として
**空きスロットプールと混ざり**、TTS は `Amazon Elastic Container Service` として **CP 自身の
タスクと混ざる**。

タグ自体は打たれていた（`60-engines.yaml` の capacity provider と service、`50-tts.yaml` の
service）。ADR 0071 決定 9 も「コンポーネント費用として表示し、按分しない」と書いていた。
**書いていないのは読み方だけ**だった。

**実測（af-sandbox・2026-09-01〜08・CE 4 リクエスト＝$0.04）**

| af-role | 7 日の額 | 内訳 |
|---|---|---|
| `engine-llm` | **$5.18** | EC2 $4.73／**ECS $0.37**／EC2-Other $0.08 |
| `engine-image` | **$0.73** | EC2 $0.67／ECS $0.05／EC2-Other $0.01 |
| `tts-engine` | **$0.17** | ECS |
| （役割なし） | $22.25 | NAT $8.59・税・Route53・RDS・ALB… |

エンジン＋TTS で **請求全体の 19%**。人に紐づく上限が 22.3%（§67.3）なのだから、同じ桁の塊が
名前を持たずに共有へ落ちていた。

- ✅ **MI の管理手数料とデータボリュームは、サービスとキャパシティプロバイダのタグを継ぐ。**
  `engine-llm` に ECS $0.37 と EC2-Other $0.08 が返ってきたのがその証拠で、「MI の課金は
  タグが付かないのでは」という懸念は実測で消えた。
- 🔴 **`ModelsBucket` と `LogGroup` にはタグが 1 つも無かった。** 有効化と同じでタグも遡らない
  ので、設計の結論より先に打った（§67.5 と同じ順序）。`engine-models` / `engine-logs`。
- ⚠️ **NAT は相変わらず割れない。** 残差 $22.25 の最大項は NAT（bytes $5.61＋hours $2.98）。
  **取り込みが HF から引く 17GB もここ**に入る。S3 からの再取得はゲートウェイ経由なので
  NAT を通らない（`00-network.yaml`）。

**フィルタが本体だった（ここが一番危ない）**

素直に `GroupBy=(af-role, SERVICE)` を投げると、**利用者が握っているスロットが二重に出る**
——`af-membership` と `af-role=slot` の両方を持つので、per-member の割り当て額にも役割別の
`slot` にも入り、共有カードの合計が共有バケットを超える。`af-membership` が **ABSENT** の
行だけに絞ると、`slot` の EC2-Compute は **$3.33 → $0.12** に落ちた。差の $3.21 は誰かが
握っていた時間で、それは per-member 側が既に答えている。

⚠️ 予約メンバーシップ（§67.16）は `af-membership` を持つが取り込みで SHARED へ畳まれるので、
ABSENT だけだと 2 つの表の合計がその分ずれる。`Or[ABSENT, EQUALS(予約 ID…)]` で足し戻す。
⚠️ **CE は空の `Values` を拒否する**ので、予約 ID が無いデプロイでは Or ごと落とす。
実測で 3 つの形（ABSENT のみ／Or 付き／無フィルタ）すべてが通ることを確認してから実装した。

**採らなかったもの**

- **`engine_hourly`（ADR 0071 決定 13）の稼働秒 × 単価。** §67.2 の型そのもの。しかも表が
  既に在るぶん**書けてしまう**のが罠で、$0.01 で実費が取れるのに見積を並べる理由がない。
- **既存の 1 本を差し替える。** membership 軸を失う。CE の 2 軸上限は動かせない。
- **役割別だけ 12 時間おきにして $0.6/月 に抑える。** 1 つの画面の中で取得の新鮮さが
  2 つに割れる。$0.6/月 はそのわかりにくさに見合わない。

**画面**

共有カードに「何のための費用か」を足した。グループ合計（推論エンジン／音声合成／誰も使って
いないワークスペース資源／基盤）が先で、役割ごとの行はその下の折り畳み。⚠️ **グループ分けは
`runtime.CostRoleGroup` の 1 か所**で決める——画面側でも分けると 2 つの合計が食い違う。
⚠️ **知らない役割は「その他」**で、基盤の残差には混ぜない。新しいタグ値が NAT や RDS に静かに
合流したら、二度と誰も問わない。

⚠️ **目視ではなく実測でラベルが切れているのが出た**（§67.15 と同じ型で、DOM テストは全通）。
`.ur-key` は 220px 固定＋省略記号なので、`getBoundingClientRect` と `scrollWidth` を
突き合わせたら **`役割タグなし（NAT・ロードバランサ・DB・DNS・税）` と
`誰も使っていないワークスペース資源` の 2 つが切れていた**（ja・en とも）。
ラベルを短くし（NAT 等の列挙は既存の「内訳の区分」の行が既に持っている）、
全行に `title` を付けた。**切れているかどうかは目で判定しない**——ハーネスに
`clip = scrollWidth > clientWidth` を出させて、ja/en × 暗色/明色の 4 通りで 0 件を確認した。
残っている `clip=true` は `Amazon Relational Database Service` の 1 行だけで、これは今回
足した行ではなく前からのもの。

**acrt（本番・member アカウント）で手作業は要らない。** `af-role` は payer が有効化済みで、
`GetCostAndUsage` の `GroupBy Type=TAG` は member から通る（実測）。acrt に 60-engines は
無いので `tts-engine` の $0.07/7 日だけが出る。必要なのは CP の入れ替えと、engines を採用して
いる配備での 60-engines のスタック更新（S3/Logs のタグが付く）だけ。
