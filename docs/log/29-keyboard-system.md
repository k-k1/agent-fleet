...
- **コマンドパレット = `Ctrl/⌘+P`**。開くと**セッション一覧**（入力待ちが上・停止中が下、レポ/WT/状態バッジつき）で、
  `Tab` でコマンド・変更ファイル・ファイルへ。どれも曖昧検索して実行/開く（§5.8）。
...
## 5.8 パレットのセッション欄（2026-09 追加）

`Ctrl/⌘+P` を押す動機のほとんどは「あのセッションへ行く」なので、**モードタブの先頭に
「セッション」を置き、パレットはそこで開く**（セッションが 1 件も無いときだけコマンドで開く —
空の欄で出迎えても Tab を 1 回押させるだけ）。タブ順は `セッション / コマンド / 変更ファイル /
ファイル / このセッションの変更`。コマンド欄のセッション行は**残してある**: Tab の先でも 1 つの
検索窓で名前から引ける。

行に出すのは**表示名・レポ名・WT 名・状態バッジ**。バッジはレールと同じ `stateInfo()` ＋
`.session-state`、種のアイコンも同じ `.sess-kic` を使い回す — 新しい種が増えたときに色が
片方だけ落ちるのを避けるため。レポ名/WT 名は `workingCopyLabel()` と `worktreeTag()`
（`lib/project.ts`）で、フォルダ名 `<base>@<slug>` を「プロジェクト＋どの WT か」に割る。

### 並び順（`features/sessions/order.ts` — 純関数）

3 段。段の中は「最後に人待ちに入った時刻」の新しい順:

| 段 | 中身 | 段内の順 |
|----|------|----------|
| 0 入力待ち | 稼働中で question / plan / permission | 待ちに入った時刻の新しい順 |
| 1 稼働中 | それ以外の生きているセッション | 同上（＝直前に答えたものが先頭）→ 作成の新しい順 |
| 2 停止中 | 畳まれた/落ちたもの | carried（未回答を抱えたまま畳まれた）が先 → 作成の新しい順 |

**上限リセット待ち（limited）を段 0 に入れない**のは、待っている相手が人ではないから。停止中は
「下部でよい」ので carried でも段は上げず、段の中でだけ前に出す。アーカイブは `GET /api/sessions`
がそもそも返さないので、除外の実装は要らない。

### 「最後に入力待ちになった時刻」をどこから採るか

★ **API はその時刻を持っていない**（`state` はあるが「いつそうなったか」は無い）。そこで 2 つの
材料の**新しい方**を採る:

1. **通知台帳**（`GET /api/notifications` の `question` / `plan-approval` / `permission-request` の
   `createdAt`）… サーバ側なのでリロードや別端末を跨いでも残る土台。
2. **画面での観測**（`features/sessions/waiting.ts`）… 一覧が届くたびに「人待ちでなかった →
   人待ちになった」の遷移を localStorage に刻む。通知が無い経路（通知を出さない Agent、保持窓から
   溢れた古い質問、**質問→回答→また質問の 2 度目**）を埋める。

★ **初観測が人待ちでも記録しない**。いつ入ったのか分からないのに `now` を焼くと、リロード直後に
全員が同時刻で並び、しかも max を採るので**偽の時刻が通知台帳の本物の順序を上書きする**。

### 開いている間は並びを凍結する

一覧は 4 秒ごとに更新され、誰かが質問状態に変わると本来なら段 0 の先頭へ跳ぶ。選択は**添字**なので、
カーソルの下で行が入れ替わると `Enter` が別のセッションを開く。そこで**開いた瞬間に順序（名前の
配列）を固定**し、開いている間に生まれたセッションは末尾に足す。バッジ・レポ名は生のまま追随する。

### 行き先の規則は 1 箇所（`openSessionFromList`）

稼働中＝ミラー/端末、停止＋転写あり＝読み取り専用の履歴、転写なし＝端末の再生（フォルダ消失や
ワークスペース停止では開けない）。レールの行（`SessionRow`）とパレットが別々に条件を書くと必ず
ドリフトするので `features/sessions/open.ts` に集約し、開けなかったときだけトーストで理由を言う。
...
| パレットのセッション並び（純） | `console/src/features/sessions/order.ts` |
| 「最後に人待ちになった時刻」の台帳 | `console/src/features/sessions/waiting.ts` |
| 一覧行の行き先の規則 | `console/src/features/sessions/open.ts`（`openSessionFromList`） |
