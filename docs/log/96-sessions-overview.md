...
### 96.10.4 実測（headless Chromium × スタブの写し・実バンドル・3 通りの幅）

| 場面 | ペイン幅 | 列 | カード高さ | ゲージ | ラベル | はみ出し |
|---|---|---|---|---|---|---|
| 広い 1 ペイン（1500×900） | 1146px | 4 | **160px**（一言・ゲージ無しは 97px） | 4/10・帯 36px・2 段（上下差 20px） | 短縮形のみ | 0 |
| 細い側の列（窓 1500px） | 318px | 1 | 160px | 同 | 同 | 0 |
| 電話（390×844・`mobile:true`） | 376px | 1 | 160px | 同 | 同 | 0 |

- **はみ出しは 3 要素 × 3 幅 × 4 カードすべて 0**（`scrollWidth > clientWidth` で判定）。
  274px のカードでも `ctx ▓▓▓ 216k / 1M・22%` と `token ～～ 最大 9.9k` が 1 行ずつ収まる。
...
- **97px → 160px**（+65%）。電話の 1 画面に入るカードは 6〜7 枚から **4 枚**に減る。
  利用者の明示の指示による交換であり、ADR 決定 13 にも書いた。
