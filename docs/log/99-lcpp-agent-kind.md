...
## 14. 実機 16 本の実測一覧（段 2 着手前・2026-09-20 追加・2026-09-20 生ログ突合により訂正・2026-09-21 #15/#16 追加）
...
**#15・#16（2026-09-21 追加）はこの限りでない**——2 本とも #811 最終版のハーネスで、番号順どおり #15 →
#16 の順に走っている（同じ複写元から窓だけ変えて連続実行・box は #15 の cold start のまま #16 まで温かかった）。
...
| 15 | qwen3-coder-30b-a3b（4 回目） | 3500 | FAIL＝`ErrCompactionThrashing` で**明示停止**（設計どおり）。task-0 は完走（40 ターン）したが、task-1 突入時の最初の `Run` 呼び出しで圧縮が 3 回連続発火し、間に予算内の往復が 1 度も挟まらなかった | 40（task-0 のみ・task-1 は 1 ターンも進まず） | task-0 で 13／task-1 突入時に 3（連続・ここで停止） | 0 | 0/0/0 | 567.57s（cold start 込み） | #811 最終 |
| 16 | qwen3-coder-30b-a3b（#15 と同一走行の窓のみ変更） | 8000 | **PASS**。想起も自己修正の上で正確 | 164 | 計 10（task0:1／task1:2／task2:4／task3:0／task4:2／recall:1） | 0 | 0/0/0 | 274.68s（#15 で温まった box・cold start 無し） | #811 最終 |
...
- 🔴 **決着（2026-09-21・#15/#16）。** `qwen3-coder-30b-a3b` は #811 最終版のハーネスでは 1 度も測っていない、
  という上の指摘（初出 2026-09-20）は解消した。過去 3 回の測定はそれぞれ別のハーネス版に当たっていた:
  **#3＝段 1 版で 72 ターン連続ループ**／**#9＝段 1 版で実窓超過**（この原因である「`Run` に窓が渡らない」
  欠陥は 52bbda2d と 570d5042 で直っている）／**#14＝#811 初版で非収束**（この版自体が #10 と同じ圧縮
  スラッシング欠陥を持つため、coder の性質の証拠として使えない＝交絡）。**#811 最終版（570d5042 以降・
  現在の develop）で新規に測った #15（窓 3500）・#16（窓 8000）の結果**: 窓 3500 は `ErrCompactionThrashing`
  で設計どおり明示停止（task-0 は 40 ターンで完走・task-1 突入時に圧縮 3 連続）、窓 8000 は 164 ターンで
  PASS。これは gemma-4 の §14 #11→#12（窓 3500 で `ErrCompactionThrashing`・窓 8000 で PASS）と**同じ形**であり、
  「窓 3500 は #811 のハーネスにとって狭すぎる」という §14 の結論を 2 族目で裏づけた。過去 3 回の不調
  （#3 の 72 連投・#9 の実窓超過・#14 の非収束）はいずれも当時のハーネス版側の問題で、coder 自体の欠格事由
  ではなかったと言える。
- ⚠️ **ただし PASS は「正しさ」の話にとどまる。費用は別軸で悪い。** #16（coder・窓 8000）は 164 ターン
  かかっている。**同じ 5 タスク＋想起 1 問を gemma-4 は同じ窓 8000 で 63 ターン（§14 #12）**——同一窓での
  直接比較で **coder は gemma-4 の 2.6 倍**。窓が異なるため直接比較はできないが参考までに、**qwen3.8 は窓
  3500 で 29〜30 ターン（§14 #1/#2）**——coder の 164 ターンはその 5.5 倍にあたる。段 2 の門 (a) が
  「族で 2 倍以上違う＝正しさでなく費用の差」と書いているその軸に、coder は同一窓比較でも乗るうえ、
  最も極端な点として乗る。窓を PASS するまで上げれば動くが、動かすたびに他族の数倍のターン（＝Send 呼び出し
  ＝課金）を払う、という費用差として記録する。
- 🔴 **長い単一ツール連投には落ちなかったが、原因は未確定（#15/#16 実測）。** §14 #3 は `todo_write` が
  **72 ターン連続**だった。#15/#16 では最長の同一ツール連続呼び出しは **5 ターン（`read`）**——質的に別の
  挙動である。⚠️ ただし**原因はここでは特定できない**: 繰り返し検知（PR #794・負債 2）は今回も実機で
  0 回発火しており、#794 がこの差を止めたのではない。#811 のツールループ内圧縮（窓を毎反復チェックする
  ように変えた変更）が影響している可能性はあるが、**条件（ハーネス版・モデル・窓）が 1 本ずつしか無いため
  因果は主張できない**。観測として記録するにとどめる。
- 🔴🔴 **`compacted_at_turn=0` は両走行（#15/#16）とも出ているが、これは「往復ゼロで圧縮した」ことを
  意味しない——`live_manual_test.go` 自身のターン計数の粒度によるログ上の見かけである（コードを読んで確認・
  エンジンは再度叩いていない）。** `turnCount`（この変数自身、`live_manual_test.go:248`）は `logAdded`
  （`:250-281`）でのみ加算され、`logAdded` は `runTurn`（`:286-323`）が `Run` を呼び終えた**後**に 1 タスク
  分まとめて呼ばれる。つまり `Run` 呼び出し 1 回の内部（1 タスクぶんのツールループ全体・`loop.go` の
  `maybeCompact` が毎反復チェックする区間）で何往復進もうと、`turnCount` はそのタスクが完了するまで動かない。
  `compaction fired during %s, after %d assistant turns so far`（`:314`）はこの `turnCount` をそのまま
  出しているだけなので、**「テスト全体で最初に発火した圧縮」が task-0 の内部の何往復目であっても**（1 往復目
  でも 30 往復目でも）、ログは必ず「after 0 assistant turns so far」と出る。実際、#15・#16 とも発火時点の
  summary 本文（本節の引用禁止の範囲内で言えば）は main.go の代入・`Divide` のゼロ割・`Average` の分母・
  `stack.Pop` の複数バグが**既に直った後**の内容で、空の会話から発火したのではないことを裏づけている。
  同じ形は §14 #10（`log-gemma4-newloop.txt`）・§14 #14 系列（`log-coder-rerun2.txt`）の生ログにも同じ文言
  ・同様に内容の詰まった summary で出ており、**#811 以降のハーネス（in-loop 圧縮）に共通する性質**と確認できた。
  ⚠️ **切り分けとして、system prompt＋9 個のツール定義（`BuiltinTools`）だけでどれだけ食うかも見た**:
  Qwen3-coder が実際に使うツール呼び出し用 chat template（`$HOME/lcpp-live/qwen3coder_chat_template.jinja`・
  前セッションの一次資料、今回はこのファイルを読んだだけでエンジンは叩いていない）は各ツール定義を
  `<function><name>…</name><description>…</description><parameters><parameter>…` という XML 風のブロックに
  展開したうえ、固定の指示文（約 700 文字）を末尾に付ける——コードから手で同じ変換を再現すると、system
  prompt＋9 ツール定義＋task-0 の文面で合計およそ 6.8KB、大まかに 1,700〜2,300 トークン程度（文字/トークン
  比 3.0〜4.0 の幅で見積り。実測ではない）。これは窓 3500 の閾値（`window*0.9 - reservedOutput` ≈ 2,638）
  にはそれなりに近いが、窓 8000 の閾値（≈ 6,688）には遠く届かない。**つまり「system prompt だけで閾値を
  超えている」という説明は窓 8000 の発火を説明できず、上記の `turnCount` 計数の粒度で説明する方が両走行と
  整合する。** 🔴 **分からなかったこと**: #15・#16 それぞれで実際に何往復目に最初の圧縮が発火したかの正確な
  ターン数は、既存ログの粒度からは復元できなかった。これを知るには `live_manual_test.go` 側にタスク内の
  往復ごとのカウンタを足すか、エンジンを再度叩いて測り直す必要があるが、後者は本節の制約（エンジン再測定
  禁止）に反するため行っていない。**この差分自体は次に harness を触る誰かへの実装課題として残す**（本節は
  測定であって実装ではないので、ここではコードを変更しない）。
