...
| # | モデル | 窓 | 結果 | ターン | 圧縮 | 並列 | JSON不正/未知名/引数欠落 | 所要 |
|---|---|---|---|---|---|---|---|---|
| 1 | qwen3.8-27b-uncensored-q4_k_m | 3500 | PASS | 30 | turn 13 | 11 | 0/0/0 | 431.58s |
| 2 | qwen3.8-27b-uncensored-q4_k_m | 3500 | PASS | 29 | turn 11 | 9 | 0/0/0 | 348.90s |
| 3 | qwen3-coder-30b-a3b（1 回目） | 3500 | 完走だが 162 ターン（`todo_write` が **72 ターン連続**・turn 50〜121）・圧縮後の想起が 17 ターン迷走 | 162 | turn 19 | 0 | 0/0/0 | 451.28s |
| 4 | ternary-bonsai-2-27b-pq2_0 | — | FAIL（起動せず・ggml 型 142＝フォーク限定・§12.2）**目録から削除済み** | — | — | — | — | 605.48s |
| 5 | llama-3.1-8b-instruct-q4_k_m（並列指示あり） | 3500 | FAIL（tool call の構文解析で失敗） | — | — | — | — | 17.98s |
| 5' | llama-3.1-8b-instruct-q4_k_m（**並列指示なし**） | 3500 | FAIL（同じ症状。**並列指示が原因という仮説は反証された**） | — | — | — | — | 15.58s |
| 6 | qwen3.6-35b-a3b-ud-iq3_s | 3500 | PASS | 33 | turn 10 | 14 | 0/0/0 | 144.14s |
| 7 | **gpt-oss-20b-mxfp4**（新規・MoE） | 3500 | **PASS** | 95 | turn 53 | 0 | 0/0/0 | 480.11s |
| 8 | **gemma-4-12b-it-q4_k_m**（新規・dense） | 3500 | **PASS** | 59 | turn 36 | 0 | 0/0/0 | 251.65s |
| 9 | qwen3-coder-30b-a3b（2 回目・再測） | 3500 | FAIL＝**うちの欠陥**。task-1 を 37 ターンで完走した直後、task-2 の最初の送信が実窓超過（32772 トークン＞窓 32768 トークン）で中断 | 37（task-1 まで） | — | — | — | 305.64s |
| 10 | gemma-4-12b-it-q4_k_m（PR #811 の**初版**） | 3500 | FAIL＝**こちらの変更が原因**。353 ターン超で未完（task-2 進入時点で 350・task-3 進入時点で 353。FAIL のため SUMMARY 無し＝総数不明） | 353+ | 計 95（task0:9／task1:86／task2:0／task3:0） | 0 | — | 2488.38s |
| 11 | gemma-4-12b-it-q4_k_m（#811 最終） | 3500 | task-4 で `ErrCompactionThrashing` により**明示停止**（設計どおり） | 139 | 計 35（task0:7／task1:3／task2:24／task3:1） | 0 | 0/0/0 | 1131.20s |
| 12 | gemma-4-12b-it-q4_k_m（#811 最終） | 8000 | **PASS**。圧縮が正しく発火し想起も正確＝**直近往復の生保持が効いた証拠**（`compacted_at_turn=0`） | 63 | 計 2（task0:1／task4:1） | 0 | 0/0/0 | 304.04s |
| 13 | gemma-4-12b-it-q4_k_m（#811 最終） | 24000 | PASS（圧縮 0 回＝現実的な窓では圧縮自体が要らない。`compacted_at_turn=-1`） | 43 | 0 | 0 | 0/0/0 | 262.40s |
| 14 | qwen3-coder-30b-a3b（3 回目） | 3500 | FAIL＝**非収束**。task-0 だけで 434 ターン（同じ検証を延々繰り返す）。その後 task-1 の最初の送信が接続タイムアウトで打ち切り | 434（task-0 のみ） | task-0 で 1 回 | 0 | — | 3300.0s（テストの上限に張り付き） |
...
- 種プロジェクトは `workspace/agent/internal/harness/testdata/liveproject/`（PR #795）。実行ごとに `cp -r` で
  未使用の複写を作る。
- 🔴 **#9 以降（PR #811 以降）はそれ以前の 9 本と圧縮の形が違う。** #811 より前はタスク境界だけで圧縮していたが、
  #811 でツールループ内でも圧縮するようになった（`workspace/agent/internal/harness/loop.go` の圧縮タイミングの
  変更）。合否の基準は同じなので比較はできるが、**`compacted_at_turn` の意味が変わっている**ことに注意
  （以後の比較でこの値を素朴に突き合わせない）。
- `qwen3-coder-30b-a3b` は都合 3 回測って 3 回とも別々の理由で綺麗に終わっていない: **72 連続ループ（#3）／
  実窓超過（#9）／非収束 434 ターン（#14）**。合格チェックポイントには入っていない。
