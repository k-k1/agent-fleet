...
## 6. P0-C（Console）第 1 巡（2026-09-24）

- 対象: `temp/s533673`（PR #927）の `4245279c0`。develop と P0-A（`temp/sch4mby`、第 2 巡の `3fe67ca53` まで）・P0-B を
  同じ worktree に merge し、レーン間の突き合わせも行った。
- 範囲: 次は自分で読んだ。
  - `attach.ts`
  - `studioSync.ts`
  - `useStudio.ts` の保存と同期
  ...
  ペイン・レイアウト・移行・リダイレクト・i18n・撮影スタブは下請けに読ませ、下の 🟡C1〜C6 はその中から自分で裏を取った。
- 検証: 下請けの実行では `vitest`（imagegen・layout・mirror、57 ファイル・678 件）は緑。
  下請けが入れた変異（合図を先頭に置く）は、どの試験でも緑のままだった（🟡C1）。

### 6.1 新たに見つけた点

#### 🔴C1（レーン横断）人は `params` の摘みを個別に消せない

P0-A の第 2 巡（`3fe67ca53`）で、`params` は 1 段深い merge になった。下位の鍵は、null で消し、無い鍵は変えない
（`ig/studio_validate.go:254-290`、RFC 7386 の merge patch と同じ意味で、人の書き込みにも掛かる）。
一方、ペインの `studioFromForm` は、空の摘みを `params` から**省く**（`con/features/imagegen/studioSync.ts:101-108`）。
`draftPatch` はトップレベルの鍵ごとに比べて、`params` をまるごと送る（`:154-160`）。

このため、人が sampler だけを空にすると、`{"params":{"steps":30,"cfg":7}}` が送られ、sampler は残る。
次の読み込みで欄に戻ってくる。4 つとも空にしたときだけ `params: null` になって消える。

直すのは Console 側。`params` が変わったら、base にあって next に無い下位の鍵を明示の `null` で送る。
試験: sampler だけを空にすると、patch が `params.sampler: null` を持つ。

#### 🟡C1 合図を送る実際の経路に試験が無い

`withSignal`（`studioSync.ts:348`）は試験されているが、本番では使われていない。`MirrorView.tsx` は
`` `${t}\n\n${wireSignal}` `` をその場で組む。合図を先頭に置く変異でも、mirror と imagegen の 51 ファイルがすべて緑のままだった。
直し方:

- MirrorView で `withSignal` を呼ぶ。
- `send()` の DOM 試験を 1 本足し、次の 2 つを固定する。
  - TUI でパスを織り込んだ後も、ワイヤの末尾が `STUDIO_SIGNAL_SUFFIX` で終わる。
  - echo に合図が無い。

#### 🟡C2 保存の失敗と 412 で、人が打った内容を捨てる

`useStudio.ts:150-158` の問題は 3 つある。

- どのエラーでも、読み直して `adopt(fresh, "replace")` する。対象は、エージェントが別の欄を書いたことによる 412 と、Agent 再起動中の 502 を含む。
  人がプロンプトを打つ間にエージェントが cfg を書く、という本来の使い方で、人の入力が消える。
- ネットワーク例外では `dirtyRef` が立ったまま、再送の予約が無い。
- 汚れている間はポーリングが止まる（`:218`）ので、エージェントの編集も見えなくなる。

直し方:

- 412 では、新しい base に対して人の保留中の鍵だけを `draftPatch` で当て直す。同じ鍵のときだけ負けにする。
- 一時的なエラーは保って再送する。
- 失敗後にデバウンスをかけ直す。

#### 🟡C3 スタジオの切替で、同じスタジオが 2 枚のペインに開く

`openStudio` は `setPaneTarget` を使う（`ImagegenView.tsx:341-346`）。これは重複を見ない。
決定 10 が防ぎたかった「同じセッションを 2 枚で開いて下書きとエコーが潰し合う」状態を、画面から作れてしまう。
さらに 2 つの `useStudio` が互いの If-Match を古くし、🟡C2 と重なって入力が消える。
既に開いているペインがあれば、そちらを選ぶこと。

#### 🟡C4 再送が成功しても「再送」が消えない

再送は `/input` で送る。Agent は `pending` からしか状態を動かさない（`sx/locks.go:283`）。このため、メタは
`failed`／`unknown` のまま、バナーとボタンが残り続け、押すたびに人格がもう一度送られる（`StudioAgent.tsx:78-121`）。
最小の直しは、成功した再送の後にペイン側でバナーを畳むこと。

#### 🟡C5「エージェントを替える」は停止の完了を待たない

`attach.ts:35-37` は、旧セッションに `stop` を投げて結果を見ずに作成へ進む。Agent 側で旧セッションが
まだ生きていると判定されれば、作成は 409 `studio_bound` になる（`ig/studio_store.go:223-226`）。
その時点で旧セッションは止まっているので、利用者には、エージェントが外れてしまった状態が残る。
作成の前に `POST …/bind {session:""}` で明示的に外すこと（§5 🔵A3）。stop はその後か、止めずに残すかを選ぶ。

#### 🟡C6 ペインを開き直すと、エージェントの縁取りが消える／エージェント列が読み込み中のまま止まる

- 縁取り: 最初の読みを基準にし、縁取りはローカルの state にある（`useStudio.ts:103-104`）。
  タブの切替・再読み込み・開き直しで縁取りが消える。ペインを閉じている間にエージェントが動かした欄も縁取られない
  （決定 6「次に人が触るまで」）。スタジオごとの最後に見た seq を保つこと。
- 読み込み中のまま: `StudioAgent.tsx:40` は `sessions.length > 0` を「読み込み済み」とみなす。
  結ばれていた唯一のセッションが消えると、ずっと読み込み中になり、「付ける」が出ない。

#### 🔵C1 その他（下請けの報告から、実害の小さいもの）

- 最初の読みに失敗したとき（502）の再試行が無い。404 のスタジオ id を忘れない。
- `studioFromForm` は既定値の `op:"generate"`／既定の strength を null で送り返す。このため、人の編集行と「下書きが変わった」の合図が出る。
- localStorage の下書きを「移す」（決定 2）と言いながら、消していない。
- スタジオを消した後に `refreshSessions()` を呼んでいない。
- `wire.ts:5` のコメントに prompthelp が残っている。
- `signalled` がモジュールスコープにある（dom 試験間で漏れる。[module-scope-cache-leaks-across-dom-tests]）。

#### 確認して問題が無かった点

- 合図: `[studio ` で始まり、`→ get_image_studio]` で終わる（Go と同じ定数）。`buildImagePrompt` の後に付き、ワイヤにだけ載って echo と履歴には載らない。
  composer の `send()` 以外には付かない。
- ペイン種別は `{kind:"imagegen"; studioId}`。`sameTarget` は null 同士を同じ的とみなす。旧レイアウトは null に移る。
- セッション一覧・既定・チャット・回転から開くと、スタジオのペインへ寄る。`Pane.tsx` は 2 枚目のミラーを出さない。
- 初回ターン: `pending` は「送っています」で再送を出さず、`failed`／`unknown` で再送を出す。
- 付けるときの kind（決定 8）:
  - マネージドは claude を出さず、opencode・muse・copilot・cursor・kiro を理由付きで塞ぐ。
  - ターミナル（CLI）は muse と lcpp を出さない。
  - worktree を OFF にできるのはターミナル（CLI）と lcpp だけ。
- レーン A と揃っている: 412 `studio_changed`、`dropped[].reason` の 3 値、`recent_log`、`draft-log` の頁、
  決定 7 の 5 鍵（`ImagegenModel`）。
- 層 B（`PromptHelpModal`・`prompthelp.ts`）の撤去は完了し、i18n の鍵も両言語で揃っている。

### 6.2 総評

新規の指摘は 🔴 1（レーン横断）・🟡 6・🔵 1 件で、**マージ不可**。🔴C1 は P0-A の merge の意味（RFC 7386 どおり）に
ペインを合わせる修正で、Console 側で直す。🟡C2・C3 は、決定 10 と「会話しながら直す」本来の使い方で入力を失う
経路なので、同じ PR で直すのが望ましい。

---

## 7. P0-A 第 2 巡（2026-09-24）

- 対象: `temp/sch4mby` の `3fe67ca53`（§5 への対応）。
- 検証: `go test`（imagegen・mcpx・sessionx）は緑。自分で `no_provider` の断りを外す変異を入れ、
  `TestAgentTrialRefusesWithoutAProvider` が赤になることを確かめた。

### 7.1 新たに見つけた点

**新規 🔴 は 0 件。**

- **press:** 投入を錠の外へ出した。錠の下に残るのは、press 行の予約と結果行だけ
  （`ig/studio_press.go`、削除済みのスタジオには結果行を書かない）。版番号と行の順序は錠の下で決まる。
- **履歴:** 先頭行に世代 id を置き、cursor は「世代:行」にした。別の世代や範囲外の cursor には空の頁を返す。
  since は世代が違えば数えない。追記の前に、無ければ作り直す（`ig/history.go`）。

#### 🔵A4 muse の断りの文言が「ターミナル（CLI）を使って」と言う

`StudioSessionUnsupported`（`ig/studio_store.go:207-208`）は、muse のマネージドにも「Terminal を使って」と
返す。muse にはターミナル（CLI）が無い（§4 🟡B1）。muse だけ「いまは付けられない」にする。

#### レーン横断の注記

`params` の 1 段深い merge は、人の書き込みにも掛かる（RFC 7386 どおりで正しい）。ペインがこれに合わせる必要がある（§6 🔴C1）。
P0-A 側の変更は要らない。
...
| 前回 | 判定 | 根拠 |
|---|---|---|
| 🔴A1 | **反映済み** | 1 段深い merge、エージェントは 4 鍵（clip_skip・weight は `human_only`）、changes は `params.cfg` の単位（`ig/studio_validate.go:254-290`） |
| 🔴A2 | **反映済み** | 各段 1 回、最後は固定の短い応答で抜ける。title・label・out_dir は 200 字、since の error は 300 字 |
| 🟡A1 | **反映済み** | `no_provider`（変異で赤を確かめた） |
| 🟡A2 | **反映済み** | `StudioSessionUnsupported` を、作成時の結びと bind の両方で 409 `studio_kind_unsupported` にした。文言は 🔵A4 |
| 🟡A3 | **反映済み** | 追記の前に作り直し、同じ絵が既に入っていれば二重にしない |
| 🟡A4 | **反映済み** | 世代 id と「世代:行」の cursor。別世代・範囲外は空の頁 |
| 🟡A5 | **反映済み** | 捨てた since を `more` に足し、items は `[]` |
| 🟡A6 | **反映済み** | 最上位に seed（試験は structuredContent を読む） |
| 🔵A1 | **反映済み** | モデルの事実と投入を錠の外へ |
| 🔵A2 | **反映済み** | symlink の先へ書き、mode を保つ |
| 🔵A3 | **申し送り** | Console 側（§6 🟡C5） |

前回の 🔴A1・A2、🟡A1〜A6 は **反映済み 8／不十分 0／未反映 0**。

### 7.3 総評

**新規 🔴 0・マージ可。** 🔵A4 は文言 1 行。

---

## 8. P0-C 第 2 巡（2026-09-24）

- 対象: `temp/s533673` の `bd82ecd28`（§6 への対応）。
- 検証:
  - `vitest`（imagegen・`composerSend.test.ts`、153 件）は緑。
  - 自分で「params の下位鍵に null を送らない」変異を入れ、`studioSync.test.ts` と `useStudio.dom.test.tsx` の 2 本が赤になることを確かめた。

### 8.1 新たに見つけた点

**新規 🔴 は 0 件。**

#### 🔵C2 localStorage の鍵がスタジオの削除で残る

`af.imagegen-seen.<id>` と `af.imagegen-signal.<id>.<session>`（`useStudio.ts`）は、スタジオを削除しても消えない。
量は小さいが、削除のときに一緒に消すのが素直。

#### 🔵C3 ワイヤに `out.wire` を渡さない変異は拾えない（承知の上）

`composerSend` の純関数試験は、順序（織り込みの後に合図、echo には無し）を固定している。
`MirrorView` が `out.wire` を `postInput` に渡すことは固定していない。実装者の申告どおりで、mount の重さとの
引き換えとして受け入れる。次に MirrorView の send まわりを触るときに、DOM 試験を 1 本足す。

### 8.2 §6 の指摘を 1 件ずつ照合

| 前回 | 判定 | 根拠 |
|---|---|---|
| 🔴C1 | **反映済み** | `draftPatch` が、base にあって next に無い `params` の下位鍵を null で送る。base はフォームと同じ読みで正規化した（op／strength の偽の編集も解消）。変異で赤を確かめた |
| 🟡C1 | **反映済み（範囲付き）** | `mirror/composerSend.ts` に出し、`MirrorView.send()` が使う。先頭に置く変異で赤。ワイヤへ渡す所は 🔵C3 |
| 🟡C2 | **反映済み** | 412 では `rebaseForm` で人の鍵だけ載せ直して再送する（2 回まで）。0／5xx は保って 2・5・15・30 秒で再送。他の 4xx は知らせて置き換える。最初の読みも再試行する |
| 🟡C3 | **反映済み** | 他のペインにあれば、そのタブを選ぶ |
| 🟡C4 | **反映済み** | 再送が成功したら、そのセッションでは畳む |
| 🟡C5 | **反映済み** | `bind {session:""}` を待ってから作成し、stop はその後に投げっぱなし |
| 🟡C6 | **反映済み** | 縁取り（seq と鍵）と合図の位置を localStorage に保つ。sessions ストアに `loaded` を足した |
| 🔵C1 | **反映済み** | 404 を忘れる、移した下書きを消す、削除後に refresh、コメント、モジュールスコープの解消 |

前回の 🔴C1 と 🟡C1〜C6 は **反映済み 7／不十分 0／未反映 0**。
