...
社内利用者が実運用している <prod-deployment> で踏んだ。**箱を終了する経路がどこにも無い**ので、
...
`Ec2SlotSleepSec`（§64.17.2 / §64.31）が止めるのは 1 列目だけである。2 列目は箱が
...
§64.17.4 の実測を経路ごとに足し直すと、箱を残しておくことで買えているものが出る。

| 次回 Start | 実測 | 箱を残して浮く時間 |
|---|---|---|
| **home が付いたまま**の停止箱を起こす | **110s**（`StartInstances` → 再登録。attach/mount 不要） | 135s → 110s ＝ **25 秒** |
| **空**の停止箱を起こして attach + mount する | 110s ＋ attach 3s ＋ mount 2〜4s ＝ **115〜117s**（当初は mount を 10〜30s と見積もって 123〜143s としていた・§64.38 で訂正） | **18〜20 秒**（当初「ほぼゼロ」と読んでいた） |
| 箱を新規作成 | **135s**（cold pull 込み） | — |
| **running** の空き箱に載せる | **43s** | **92 秒** |
...
> 自体が §64.17.5 のハーネス実測（各 1〜2 回）なので、**「空の停止箱を残す」に舵を切るなら
> その経路を直接測ってからにすること。**

### 64.32.3 warm 床（最小 warm 台数）を置かない理由

「停止した箱を最低 N 台は残す」は warm pool の定石だが、**ここでは定石が効かない**。

- **温存される箱は汎用のキャッシュにならない。** `occupiedInstances` は home の attachment
  だけで「取られている箱」を決めるので、home が付いた箱は **stopped でも busy** であり
  `freeSlots` に出てこない。触れるのは `evictLongestIdle` だけで、それは上限到達時のみ。
  つまり N が助けるのは**最後に使った N 人**であって、次に来た人ではない。
- **汎用にすると価値が減る。** 先に home を外して「空の停止箱」として残すと、次の人は
...
**本当に買う価値がある 92 秒は「running の空き箱」**（$0.1302/時 ＝ $95/月）であって、
停止箱の温存（$3.84/月）ではない。プレウォームを入れるならそちらで、それはスケジューラを
...
- **`sweepVolume`（home が付いた停止箱）**: `af-idle-since` が閾値を超えたら
  **`releaseSlot` で home を外してから** terminate。
- **`sweepFreeSlots`（空の箱）**: `af-slot-idle-since` が閾値を超えたら terminate。
...
- `placeHome` は **attachment から配置を導出する**ので、shutting-down の箱に付いたままの
...
**B**=存在してよい箱＝保持される root の本数の上限、**C**=回収（立ち退き）が始まる閾値。
A と B が一致していたのは「箱を終了しないから」である。

terminate 段が入ると、定常の箱数は**「活動量 ＋ 閾値」**で決まるようになり、**B が
...
オンなら閾値と「次の人は 110 秒でなく 135 秒」を、オフなら「箱は上限 N 台まで残り、
その root を課金し続ける」を出す。

## 64.33 上限到達時の立ち退きが「乗れない箱」を奪う（コード調査・2026-08-26）

§64.32 の作業中に見つけた**元からある粗**。§64.31.6 と同じ形——**同じ判断の 2 つの写しが
揃っていなかった**。

`freeSlots` は最初から **instance-type で絞っている**（型が違う箱ではタスクが動かないため）。
...
しかも `placeHome` は立ち退きで得た箱に **`attachHomeWithRetry` でそのまま載せる**（`freeSlots`
を引き直さない）ので、`slotTypeMatches` の検査も通らない。

**症状**: xlarge の段に居る人が large の箱を奪って載り、ECS が
...
新しい箱は定義上正しい型になる。ユニットテストで再現を確認した
...
`evictLongestIdle` の両方がそれを通る。犠牲者は「型が合う箱に載っている休眠 home」だけになり、
**全体の最長休眠が必ず犠牲者とは限らなくなる**（xlarge の人は何日眠っている large も素通りする）。

犠牲者が見つからないときのメッセージも分けた。「一杯」と「一杯だが自分の大きさが無い」は
運用者の取るべき手が違う（前者は上限を上げる／後者はその大きさの箱が空くのを待つか、
`Ec2SlotTerminateAfterSec` が 1 台回収して正しい型を建てる余地ができるのを待つ）。

~~**残す宿題**: 上限に張り付いた状態で「休眠中の型違いの箱を terminate して正しい型を
建て直す」ところまではやっていない。~~ → **§64.37 で実装した。** 「明示的に失敗する」でも
**起動できない人は起動できないまま**であり、それは運用者が唯一「絶対に避ける」と明示した
結果だった。

## 64.37 上限が「乗れない箱」で埋まっているなら、片付けて建て直す（2026-08-26）

§64.33 で立ち退きの型検査を入れた結果、**上限到達 ＋ 休眠中の箱が全部型違い**という状態は
...
**インスタンスタイプは走っている箱が変えられる属性ではない。** `evictLongestIdle` は
既に**正しい大きさの箱すべてに first refusal を持っている**ので、そこで見つからなかった時点で
残っているのは「どう再利用してもこの人が乗れない箱」であり、それが `maxSlots` の 1 枠を
握っている。**枠を空けるには箱を消すしかない**（§64.32.1 と同じで、root は単独では消せない）。

順序は `evictLongestIdle` →（失敗したら）`makeRoom` → `growPool`。立ち退きが先なのは、
既存の正しい箱への載せ替えが 109.7s なのに対し、片付けて建て直すと 135s ＋ terminate 分に
なるため。**速い順に試して、最後の 1 つが「失敗」ではなく「遅い成功」になった**、というのが
この変更の全部である。

### 64.37.2 ⚠️ AZ で絞らない（立ち退きとは逆）

`evictLongestIdle` は AZ で絞る——箱を**再利用**するので、home が固定されている AZ に
無ければ意味がない。`makeRoom` は**箱を壊して枠を買う**ので、`Ec2MaxSlots` が
**プール全体を数える**以上、**別 AZ の箱を空けても価値は同じ**である。ここで AZ を絞ると、
空けられる箱があるのに要求者が詰まる——何の得も無く。

### 64.37.3 ⚠️ `Ec2SlotTerminateAfterSec` では門を作らない

あのつまみは**アイドルの費用**の話（誰も使っていない箱にいつまで払うか）で、0 は
「箱を残す」を意味する。こちらは**起動できない人**の話で、**代わりが「Start の失敗」の
...
⚠️ しかも**既定は 0** であり、**0 の配備こそ箱が上限まで育って張り付く**（§64.32）。
門を作ると、この経路を最も必要とする配備でだけ効かないことになる。

### 64.37.4 選び方と守り

- **空の箱を、持ち主付きの箱より先に取る。** 誰の親和性も使わないので、選ぶのが只である。
...
- **生きた claim がある箱は取らない。** 着地中の Start は attachment をまだ持たず、
  claim だけが唯一の証拠である。
- **ECS のタスクが 1 つでもあれば取らない**（home が無くてもタスクは載りうる——焼きの探針）。
- **自分と同じ型の箱は候補にしない。** 立ち退きが既に first refusal を持っている。
  ここで同型を壊すと、再利用できたはずの箱を捨てて建て直すことになる。
...
片付けられる箱が 1 つも無ければ（全部走行中・全部 claim 済み）、**立ち退きの元のエラーを
...
- **禁じると「使えない人」が出る。** プールはデプロイ全体で 1 つの資源で、テナント A が使って
  いない箱をテナント B が使えないという規則は、上限到達時に**起動できないメンバー**を作る。
...
| `Ec2MaxSlots` | **存在してよい箱**の数 | **数える**（遅延返却で箱を掴んだまま） |

比べられるのは「running な WS はちょうど 1 つの箱を要る」からで、**差分は停止中の WS が握っている
箱**である。したがって **Σ ≤ 容量 は必要条件であって十分条件ではない**：枠内でも休眠中の箱が上限を
...
それを 10〜30s として「空の停止箱にはほぼ価値が無い」の根拠の 1 つに使い、§64.31.6 は
...
### 64.38.3 「箱を起こした直後は SSM がまだ繋がっていない」も起きていない
...
リトライした形跡が無い。新規に建てた箱でも同じで、今日の 1 台を秒単位で追うとこうなる
...
> 見えていないのではなく**古い版が既に無い箱を指している**。§64.39 に実証がある。
...
- **デプロイ当日はタスクが 2 回生成される。** 例（k1・2026-08-26）: 10:10:02 に
  ...
  日に全員が払っている。**
- **新規箱の初回配置が 1 サイクル空振りする。** 上の表の +30.3s、および 08-26 10:38 の回で
  ...
  置けなかったのは**古い版のタスク定義**で、それが指す箱がもう無かった。
...
- **§64.32.2 の結論 2 は撤回する（結論 1 は残る）。** 「空の停止箱を起こして attach + mount」を
  ...
  - **ただし「だから空の停止箱を貯めよう」にはならない。** 6 秒差のために home を外す
    （＝持ち主の親和性を捨てる）釣り合いは依然として悪く、$3.84/月 の固定費もそのままで、
    §64.32.3 の「温存される箱は汎用のキャッシュにならない」も無傷である。**変わったのは
...
- **「mount が縮めば停止箱の使い回しが意味を持つ」という見通しは成り立たない。** 縮ませても
  買えるのは 2〜4 秒で、どの経路も **ECS 配置・タスク起動の 90〜120 秒**が支配している。
  他人の箱を流用しても大きくは速くならない、という結論はそのまま残る。
...
差し替わる**件と、**新しい箱への初回配置が `MemberOf` 不成立で空振りする**件。
...
⚠️ **この「最も近い候補」に挙がるのは当該スロットではなく別の箱**である。§64.38.4 では
これを「mount 済みのスロットがまだ配置エンジンから見えていない」と読んだが、**逆だった**——
見えていないのは新しい箱ではなく、**古い版が指している箱がもう存在しない**。本命（新しい版）の
タスクはこのあと約 41 秒で置かれる。
...
「登録直後の箱は配置エンジンにまだ見えていないのではないか」を直接測った。開発配備は撤収済み
...
- ⚠️ **①は `desiredCount` を触らない**。これが早出しを安全にしている——走っていない
  ...
  「隔離された箱に pin されたタスク定義を `desiredCount=0` で指したサービス」が残るだけで、
...
5〜35 分かかったこと**で、健全な箱ならミリ秒である。**検知から復帰まで 47 分**——その間に
...
つまり **stunnel が死んだから箱が壊れたのではなく、箱が飢えたから stunnel が死に、
飢えていたから直せなかった**——順序が逆である。

### 64.40.2.2 対策——閉じ込めと、**既に在る計測への警報**

- ★ **ワークスペースのコンテナにメモリのハードリミットを入れる。** 効くのは「暴走を止める」
  からではなく、**追い出し圧力を cgroup に閉じ込めて箱の管理系を守る**からである。
  **仕組みは既に在る**——`MemBytes`（解決済みのワークスペース単位 RAM 上限・`store.go`）は
  docker ランタイムでは使われていて、**ecs-ec2 だけが適用していない**（`Memory: nil`）。
  ⚠️ **ただし `MemBytes` をそのまま入れても足りない。** 利用者が選ぶ「サイズ」がそのまま箱の
  サイズ（8 GiB＝m7i.large）なので、上限＝箱全体になり**管理系のための余白が残らない**。
  **「箱のメモリ − 管理系ぶん」**を上限にする必要がある
...
### 64.40.7 フォレンジック——隔離した箱をそのまま起動して読む

隔離（`af-role=quarantined`・停止）した箱は **home を外してあるので、起動し直しても
利用者に影響しない**。新しいリソースも増えない。手順:

1. `ec2 start-instances`（**箱は普通に起動した**＝ハードも root FS も無傷で、
...
## 64.41 ワークスペースを箱から切り離す——上限・正直なサイズ表示・在る指標への警報（決定 28・2026-08-27）

§64.40 の事故の対策。**直すのは「暴走を止めること」ではなく「暴走が箱の管理系を道連れに
...
- **天井 2048 MiB**: 管理系の費用は箱の大きさで増えないので、大きい段が比例して差し出す必要は無い
...
上限が入る前は「8 GiB」が箱とワークスペースの両方を指していて、**1 つの数で正直だった**。
入った後は違う数になるので、箱だけを出すと**cgroup が渡さないメモリを本人に約束する**ことになる。

- `workspaceSlot.UsableMemMiB`（`usable_mem_mib`）を足す。**上限が無い配備では省く**——
  そこでは箱が答えそのものなので、2 つ出す方が嘘になる
- Console は **本人が使える方を先に、箱を括弧で**（`admin.ws_slot_usable` = `{n}（箱 {box}）`）。
  名簿の行（`MemberSizeChips`）と「→ 乗る箱」の行（`slotSpec`）の 2 か所
...
同じ取り込みタスク・同じ箱・同じ 1 時間の中での実測である。
...
**箱もディスクも 3 本に対して同じ速さで働いていた**——遅かったのは HF の配信だけである。
...
| 時刻 | 経過 | 何が起きたか |
|---|---|---|
| 18:41:24 | 0 s | ECS がタスクを起動（この前に MI が GPU 箱を確保している） |
...
- **S3 →箱が 102.7 MiB/s** に対し、HF →箱は 41 MiB/s だった（§64.43.2）。
...
⚠️ **課金が止まるのは「サービスを 0 にした時刻」ではない。** 箱が実際に消えるまで
さらに 8 分ある。$1.26/時なので 1 回あたり約 $0.17——気にする額ではないが、
**「止めた」と「消えた」を同じ時刻だと思って観測すると必ず食い違う**。

### 64.43.5 🔴 `ec2 describe-instances` は MI の箱を「一覧しないが、ID で引けば返す」

ADR 0071 の起票時に「`describe-instances` は Managed Instances の箱を 1 台も列挙しない」
と書いたが、**これは半分しか正しくない**。同じ 1 秒の中で 2 通りの呼び方を実測した
（対象は engines の GPU 箱・`g6.xlarge`）。

| 呼び方 | 結果 |
|---|---|
| `describe-instances --instance-ids i-…`（**ID 指定**） | ✅ **返る**（`g6.xlarge` / `terminated`） |
| `describe-instances`（**無指定の一覧**） | ❌ **出てこない**。15 台すべてスロット箱で、GPU クラスは 1 つも無い |

つまり**レコードは存在するのに一覧には載らない**。これが罠として悪質なのは、
運用者が自然にやるのは一覧の方だからである——「GPU 箱が無い＝止まっている」と
読んでしまい、**実際には課金され続けている箱を見落とす**。ID を知っていれば引けるが、
...
`capacityProviderName` が付いているコンテナインスタンスが MI の箱で、
