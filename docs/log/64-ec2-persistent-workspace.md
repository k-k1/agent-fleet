...
- ホスト固定オーバーヘッドの按分（実測: 8 GiB のインスタンスで ECS が登録したのは **7,783 MiB** ＝
...
> **止まった Workspace が使っていた「インスタンス」を止める**。直列に並ぶ:
...
> ⚠️ **訂正（§64.38・2026-08-26）**: この「SSM 往復 10〜20s」は見積もりで、実測すると
> **2〜4 秒**である（本番 n=37・`af-mount` 本体は中央値 0.6s）。食い潰している中身は
> mount ではなく、**ECS のタスク配置とコンテナ起動（90〜120s）**だった。
...
そこが駄目なら失敗させる——動けない先にスロットを立てても、attach できないインスタンスが増えるだけである。
...
（手書きの起動条件で焼くと、焼いたインスタンスと製品が立てるインスタンスが静かにズレる）。
...
2. **`af-role=slot` で焼かない。** CP の `freeSlots` / `poolSize` はそのタグから世界を
   作るので、焼いている最中のインスタンスに誰かの home が載る。スクリプトは `af-role=bake` を使う。
...
**home を剥がして claim を落とし**（本人が待たずに別のスロットへ行ける）、**インスタンスを停止**し、
**画面には残す**（まだ課金されているインスタンスが画面から消えるのは、気づけない形の請求になる）。

実機で確認した: 壊れたインスタンスでもう一度 Start → **隔離され（`af-quarantine-at` 付き）**、停止に入り、
...
| 2 | ワークスペースのメモリ（＝上限） | **上限ではない。** `memoryReservation: 512`（ソフト）のみ。値は**インスタンスを選ぶ必要量**で、実際にはインスタンスの全量が使える |
...
とも、4 GiB を確保したとも読める。実際に起きるのは「8 GiB のインスタンスに 1 人で入る」である。
...
{ "runtime":"ecs-ec2",
  ...
  "mem_meaning": "slot",               // "limit"（上限）か "slot"（インスタンスを選ぶ必要量）か
...
### 64.27.6 実機に入れた（2026-08-17・開発配備）
...
残っているのは、メンバー詳細で **CPU 欄が消えていること・メモリ欄がインスタンスを言うこと・
「実行中」が出て強制停止が押せること**の目視で、人が自分のブラウザで見る分担にした。

## 64.28 golden を実デプロイで焼く（本番配備・2026-08-21）
...
その手段は無い。実デプロイ（本番配備 / 0.9.1）で **README どおりの手順だけ**を踏んだら、
...
### 64.29.5 焼き終わったあとに種が AWS に残った（実デプロイ・2026-08-28）

0.13.1 を <prod-deployment> へ適用したときの実測。**焼き直しは成功して golden も公開されたのに、種の
ECS サービス（desired 0）と 50 GiB の home（`available`）が AWS に残った**。同じ日の 0.13.0 の
焼きは `destroyed the af-golden-seed workspace` と probe の 2 行できれいに終わっており、
**残ったのは競合が起きた回だけ**である。

**起こったこと**（CP のログから）。焼き始めの tick が種を作った直後に別経路の `tidy` が種を
destroy し、次の tick（60 秒後）が同じ名前で作り直した。ECS の削除がまだ終わっておらず、
`CreateService` は `InvalidParameterException: Create service is not idempotent` を返した。
6 分ほど遅れてサービスは作られ、焼き自体は最後まで成功している。しかし最後の `tidy` は
**種について 1 行も出さずに素通りし**、サービスと home はそのまま残った。

**直したのは 3 つ**（＝この事故が教えた 3 つの穴）。

1. **無言の早期 return をやめた。** `destroy` は membership と workspace 行の
   「引けなかった（`!ok`）」と「読めなかった（`err`）」を**どちらも黙って return** していた。
   だから「掃除して何も無かった」と「掃除できなかった」がログ上で同じ——**出ない 1 行**に
   なる。probe の行だけが出て種の不在は出ない、というのが今回の見え方だった。
   毎ティック走る経路なので、繰り返す失敗は**同じ文面のあいだ 1 回だけ**出す。
2. **実体はタグで掃除する（`sweepOrphans`）。** AWS 側の正はタグ（`af-workspace` /
   `af-tenant=af-golden` / `af-role=home`）なのに、後片付けは **DB の membership → workspace 行**
   からしか辿れなかった。行が消えた瞬間、タグの付いた実体は**誰にも見つけられない孤児**になる。
   ⚠️ **ドリフトの sweeper は肩代わりできない**——あれは home ボリュームをタグで歩くが
   **削除は決してしない**（`sweepVolume`）。切り離された home は「停止中のワークスペース」と
   区別がつかないからで、その区別ができるのは予約名を持っている baker だけである。
   安全は 2 段: 呼ぶ側が「予約名 ＋ DB に行が無い」を確かめ、こちら側は
   **タスクを載せているサービス**と**接続中またはスナップショット取得中の home** を断る。
3. **削除中のサービス名で作り直さない。** `DRAINING` のサービスに対して `UpdateService` も
   `CreateService` も通らない。`upsertService` は後者を投げて上の AWS エラーを浴びていたが、
   あの文面は**呼び側のバグに読める**（実際、原因の切り分けを 1 往復遠回りさせた）。
   「前のサービスがまだ消えていない」と言って、既にある再試行（焼きの tick・2 回目の Start）に
   待たせる。

⚠️ **一般則**: 実体が AWS にあり、名前が DB にあるとき、**掃除を DB からしか辿れないと
「行だけ消えた」が恒久的な漏れになる**。しかも漏れているあいだ**ログは正常時と 1 バイトも
変わらない**。§64.28.3 の「起動を確かめていない golden は公開しない」と同じで、
**成功だけを言うログは、失敗を隠す**。
...
保留は実デプロイ（本番配備）で実際に焼きを止めた。歯止めは設計どおり正しく効いていたのに、
...
種      boot-install  home 切離   EBS コピー  起動確認   公開
```
...
呼び出しは製品全体で 2 か所しか無く、もう 1 つは `quarantineSlot`（故障インスタンスの隔離）だった。
...
乗ったインスタンスを止める**ことになる（回復はするが `releaseGrace` を待つ）。
...
**インスタンスを停止で維持している時点で「135s を払わない」という価値は既に取れている**（停止スロットは
...
運用者がこの画面に来る理由が「タスクの無いインスタンスが running なのはなぜか」である以上、
...
その間に mount が入る。この窓は **20〜120 秒**で、**その大半はインスタンス起動と ECS 登録・
配置であって mount ではない**（実測の内訳は §64.38）。この窓にスイープが刺さると:
...
## 64.32 停止したスロットが永久に残る（実デプロイ・2026-08-26）

社内利用者が実運用している <prod-deployment> で踏んだ。**インスタンスを終了する経路がどこにも無い**ので、
プールが保持する root ボリュームの本数は**単調増加して `Ec2MaxSlots` に張り付く**。

事の起こりは容量の調整だった。`Ec2MaxSlots` を 6 → 30 に上げ、`SlotRootVolumeGiB` を
100 → 40 に下げた（実測の root 使用量は 6.4〜9.0 GB：OS 2.3 ＋ インフラ 0.4 ＋
`af-workspace` 1 版 2.63 GB。版間でレイヤをほぼ共有しないので、古い版は
`ECS_IMAGE_MINIMUM_CLEANUP_AGE=24h` の GC 頼み）。ところが **EBS の合計が 900 GiB** ある。
内訳を見ると停止中のスロットが 3 台居座っていて、**手で terminate して 600 GiB に戻した**。

つまり `Ec2MaxSlots` を上げることは、**同時に「その台数ぶんの root を無期限に買う」ことに
署名する**のと同じだった。30 × 40 GiB × $0.096/GB-月 ＝ **約 $115/月**が、誰も働いていない
夜間も週末も掛かり続ける方向に育つ。しかも**それを止める手段が「運用者が気づいて手で消す」
しか無い**。

### 64.32.1 停止と終了は別の課金を止める

| 状態 | compute | root ボリューム(40 GiB) |
|---|---|---|
| running | $0.1302/時（m7i.large） | $3.84/月 |
| **stopped** | **$0** | **$3.84/月**（残る） |
| terminated | $0 | **$0** |

`Ec2SlotSleepSec`（§64.17.2 / §64.31）が止めるのは 1 列目だけである。2 列目はインスタンスが
消えるまで終わらない。⚠️ **root は `/dev/xvda`・`DeleteOnTermination: true` なので、
単独で消すことはできない。「root を消す」＝「インスタンスを terminate する」の一択**であり、
この 2 つは分離できない 1 つの決定である。

### 64.32.2 何を失うのか——25 秒。それだけ

§64.17.4 の実測を経路ごとに足し直すと、インスタンスを残しておくことで買えているものが出る。

| 次回 Start | 実測 | インスタンスを残して浮く時間 |
|---|---|---|
| **home が付いたまま**の停止インスタンスを起こす | **110s**（`StartInstances` → 再登録。attach/mount 不要） | 135s → 110s ＝ **25 秒** |
| **空**の停止インスタンスを起こして attach + mount する | 110s ＋ attach 3s ＋ mount 2〜4s ＝ **115〜117s**（当初は mount を 10〜30s と見積もって 123〜143s としていた・§64.38 で訂正） | **18〜20 秒**（当初「ほぼゼロ」と読んでいた） |
| インスタンスを新規作成 | **135s**（cold pull 込み） | — |
| **running** の空きインスタンスに載せる | **43s** | **92 秒** |

イメージキャッシュが救う cold pull 32s を、**インスタンス起動 19s ＋ ECS 再登録**が
そのまま食い潰している。そこから 2 つ出る。

1. **停止スロットが 25 秒の価値を持つのは、持ち主の home が付いたままのときだけ。**
2. **空の停止スロットはほぼ何も買っていない。** $3.84/月 に対して 0〜12 秒。

> ⚠️ **訂正（§64.38・2026-08-26）**: 2 の根拠だった「mount の SSM 往復 10〜30s」は実測 2〜4s
> だった。同じ算術をやり直すと**空の停止スロットも 18〜20 秒**買っており、「ほぼ何も買って
> いない」は成り立たない。**1 と 2 の差は 25 秒対 0 秒ではなく、25 秒対 19 秒である。**
> ただし ①どちらも 110〜135 秒の Start に対して 15% 前後の差でしかなく、②この 110s / 135s
> 自体が §64.17.5 のハーネス実測（各 1〜2 回）なので、**「空の停止インスタンスを残す」に舵を切るなら
> その経路を直接測ってからにすること。**

### 64.32.3 warm 床（最小 warm 台数）を置かない理由

「停止したインスタンスを最低 N 台は残す」は warm pool の定石だが、**ここでは定石が効かない**。

- **温存されるインスタンスは汎用のキャッシュにならない。** `occupiedInstances` は home の attachment
  だけで「取られているインスタンス」を決めるので、home が付いたインスタンスは **stopped でも busy** であり
  `freeSlots` に出てこない。触れるのは `evictLongestIdle` だけで、それは上限到達時のみ。
  つまり N が助けるのは**最後に使った N 人**であって、次に来た人ではない。
- **汎用にすると価値が減る。** 先に home を外して「空の停止インスタンス」として残すと、次の人は
  wake ＋ attach ＋ mount で **115〜117s**（新規作成 135s に対して 18〜20 秒）。**持ち主から
  25 秒を取り上げて、他人に 19 秒渡す。**
  （⚠️ 当初ここは「他人には 0 秒しか渡らない」と書いていた。mount を 10〜30s と見積もった
  ことによる誤りで、§64.38 の実測で訂正した。差し引き 6 秒のために home を外す価値は依然
  薄いが、**「渡らない」ではない**。）
- **休止期間に固定費が残る。** N=2 でも連休中ずっと $7.7/月。「使えないより遅い方がまし」
  という運用方針の下では、払う理由が薄い。

**本当に買う価値がある 92 秒は「running の空きインスタンス」**（$0.1302/時 ＝ $95/月）であって、
停止インスタンスの温存（$3.84/月）ではない。プレウォームを入れるならそちらで、それはスケジューラを
伴う別の決定である（§64.17.2 の「プレウォームは入れない」は据え置き）。

### 64.32.4 直し方——同じ時計の次の段（`Ec2SlotTerminateAfterSec`）

`AF_ECS_EC2_SLOT_TERMINATE_AFTER_SEC` / CFN `Ec2SlotTerminateAfterSec`。**既定 0 ＝ 無効**で、
既存配備の挙動は変わらない。推奨は **14400（4h）**——「同日中に戻る人は 110s、翌日以降は 135s」。

既存スイーパーの 2 経路にそのまま乗せる。

- **`sweepVolume`（home が付いた停止インスタンス）**: `af-idle-since` が閾値を超えたら
  **`releaseSlot` で home を外してから** terminate。
- **`sweepFreeSlots`（空のインスタンス）**: `af-slot-idle-since` が閾値を超えたら terminate。

⚠️ **home を先に外すのは「状態が素直だから」ではなく、外さないと壊れるからである。**
home は `DeleteOnTermination: False` なのでどちらでも消えないが、terminate には 60 秒ほど
かかり、その窓に持ち主が Start すると:

- `placeHome` は **attachment から配置を導出する**ので、shutting-down のインスタンスに付いたままの
  home は**分岐①（自分のスロットがそこにある）**に見える。型一致検査も通り、claim して
  から `StartInstances` に失敗する。
- `releaseSlot` は **Start 世代（`startGen`）に anchor されている**ので、競合した Start は
  release を中断して**再 mount** に落ちる。`TerminateInstances` にこの取り消しは無い。

`releaseSlot` は停止済みのインスタンスでは umount を飛ばし（SSM は届かない）、タスクが残っていれば
そもそも拒否する——この分岐が自前で書き直すはずだったガードが、既にそこにある。

⚠️ **`sweepFreeSlots` の instance-state フィルタを `running` だけから `running, stopped` へ
広げること。** 停止がインスタンスに起きる最後の出来事だった間は正しかったが、terminate 段を足すと
**インスタンスは停止した瞬間にこの走査から永久に消える**——まさに集めたいインスタンスが。

⚠️ **ECS のコンテナインスタンス登録**は `sweepGhostInstances`（`ghostAfter` 既定 1h）が
既に面倒を見ているが、**自分で terminate するときは先に `DeregisterContainerInstance`
（`Force: true`）を撃つ**。terminate 済みのインスタンスは `ACTIVE / agentConnected=false` で登録が残り、
**ACTIVE に見えるゴーストは配置制約を満たしてしまう**ので、ECS がそこにタスクを置いて
起動しないまま止まる（ADR 0045 決定 3-2。今回 手で terminate した 3 台でも再現）。
deregister を先にしておけば、`TerminateInstances` が失敗しても
`sweepGhostInstances` が既に扱える形に落ちるだけで済む。

### 64.32.5 これで `Ec2MaxSlots` の役割が 1 つ減る

`Ec2MaxSlots` は 1 つの数字で 3 役を兼ねていた：**A**=同時に走れる WS 数の上限、
**B**=存在してよいインスタンス＝保持される root の本数の上限、**C**=回収（立ち退き）が始まる閾値。
A と B が一致していたのは「インスタンスを終了しないから」である。

terminate 段が入ると、定常のインスタンス数は**「活動量 ＋ 閾値」**で決まるようになり、**B が
`Ec2MaxSlots` から外れる**。<prod-deployment>（6 人・同時 4）なら定常 4〜6 台 ＝ root $15〜23/月 で、
夜間・週末は 0 に向かう。`Ec2MaxSlots` を上げることの費用リスクがここで消え、
上限は A と C だけの数字になる。

### 64.32.6 運用画面

**終了しない設定であることは事象ではなく常態**なので、オフのときこそ書く
（`golden が無く、かつ誰も焼いていない` を出しているのと同じ理由）。プール画面は
オンなら閾値と「次の人は 110 秒でなく 135 秒」を、オフなら「インスタンスは上限 N 台まで残り、
その root を課金し続ける」を出す。

## 64.33 上限到達時の立ち退きが「乗れないインスタンス」を奪う（コード調査・2026-08-26）

§64.32 の作業中に見つけた**元からある粗**。§64.31.6 と同じ形——**同じ判断の 2 つの写しが
揃っていなかった**。

`freeSlots` は最初から **instance-type で絞っている**（型が違うインスタンスではタスクが動かないため）。
ところが上限到達時の `evictLongestIdle` は **AZ と自分自身しか絞っていなかった**。

```
placeHome ①〜④ → freeSlots  → 型一致は保証される
placeHome ⑤     → evictLongestIdle → 型を見ない ← ここだけ抜けている
```

しかも `placeHome` は立ち退きで得たインスタンスに **`attachHomeWithRetry` でそのまま載せる**（`freeSlots`
を引き直さない）ので、`slotTypeMatches` の検査も通らない。

**症状**: xlarge の段に居る人が large のインスタンスを奪って載り、ECS が
`no container instance met all of its requirements` で配置を拒否し続ける。サービスは
**desired 1 / running 0 のまま永久に止まる**——「遅い」ではなく「動かない」。アーキ跨ぎで
同じ症状を実機で観測したことが `placeHome` 冒頭のコメントに既に書かれていた（docs/70 §70.14.5）。

**上限に達したときにしか露出しない**ので生き延びた。上限未満なら `growPool` が走り、
新しいインスタンスは定義上正しい型になる。ユニットテストで再現を確認した
（`TestECSEC2NeverEvictsASlotOfTheWrongType`——修正前は large を奪って attach まで進む）。

**直し方**: 2 つの写しを 1 つにする。`slotsOfMyType(ctx, az)` を切り出し、`freeSlots` と
`evictLongestIdle` の両方がそれを通る。犠牲者は「型が合うインスタンスに載っている休眠 home」だけになり、
**全体の最長休眠が必ず犠牲者とは限らなくなる**（xlarge の人は何日眠っている large も素通りする）。

犠牲者が見つからないときのメッセージも分けた。「一杯」と「一杯だが自分の大きさが無い」は
運用者の取るべき手が違う（前者は上限を上げる／後者はその大きさのインスタンスが空くのを待つか、
`Ec2SlotTerminateAfterSec` が 1 台回収して正しい型を建てる余地ができるのを待つ）。

~~**残す宿題**: 上限に張り付いた状態で「休眠中の型違いのインスタンスを terminate して正しい型を
建て直す」ところまではやっていない。~~ → **§64.37 で実装した。** 「明示的に失敗する」でも
**起動できない人は起動できないまま**であり、それは運用者が唯一「絶対に避ける」と明示した
結果だった。

## 64.37 上限が「乗れないインスタンス」で埋まっているなら、片付けて建て直す（2026-08-26）

§64.33 で立ち退きの型検査を入れた結果、**上限到達 ＋ 休眠中のインスタンスが全部型違い**という状態は
「黙って詰まる」から「明示的に失敗する」に変わった。診断はできるようになったが、
**利用者から見ればどちらも起動できない**。運用方針は「遅くなるのは許容、使えないのは不可」
なので、ここは失敗させるべきではない。

### 64.37.1 なぜ「奪う」ではなく「終了する」なのか

**インスタンスタイプは走っているインスタンスが変えられる属性ではない。** `evictLongestIdle` は
既に**正しい大きさのインスタンスすべてに first refusal を持っている**ので、そこで見つからなかった時点で
残っているのは「どう再利用してもこの人が乗れないインスタンス」であり、それが `maxSlots` の 1 枠を
握っている。**枠を空けるにはインスタンスを消すしかない**（§64.32.1 と同じで、root は単独では消せない）。

順序は `evictLongestIdle` →（失敗したら）`makeRoom` → `growPool`。立ち退きが先なのは、
既存の正しいインスタンスへの載せ替えが 109.7s なのに対し、片付けて建て直すと 135s ＋ terminate 分に
なるため。**速い順に試して、最後の 1 つが「失敗」ではなく「遅い成功」になった**、というのが
この変更の全部である。

### 64.37.2 ⚠️ AZ で絞らない（立ち退きとは逆）

`evictLongestIdle` は AZ で絞る——インスタンスを**再利用**するので、home が固定されている AZ に
無ければ意味がない。`makeRoom` は**インスタンスを壊して枠を買う**ので、`Ec2MaxSlots` が
**プール全体を数える**以上、**別 AZ のインスタンスを空けても価値は同じ**である。ここで AZ を絞ると、
空けられるインスタンスがあるのに要求者が詰まる——何の得も無く。

### 64.37.3 ⚠️ `Ec2SlotTerminateAfterSec` では門を作らない

あのつまみは**アイドルの費用**の話（誰も使っていないインスタンスにいつまで払うか）で、0 は
「インスタンスを残す」を意味する。こちらは**起動できない人**の話で、**代わりが「Start の失敗」の
ときにしか走らない**。犠牲者が失うのはイメージキャッシュだけ（次回 110s → 135s）で、
home は失わない（`releaseSlot` で先に外し、`DeleteOnTermination: False`）。
「terminate しない」を「詰まりをそのままにする」と読むのは、あのパラメータが
聞かれてもいない問いに答えることになる。

⚠️ しかも**既定は 0** であり、**0 の配備こそインスタンスが上限まで育って張り付く**（§64.32）。
門を作ると、この経路を最も必要とする配備でだけ効かないことになる。

### 64.37.4 選び方と守り

- **空のインスタンスを、持ち主付きのインスタンスより先に取る。** 誰の親和性も使わないので、選ぶのが只である。
- 持ち主付きなら**最長休眠**から。`releaseSlot` で home を外してから terminate する
  （§64.32.4 と同じ理由——`releaseSlot` は Start 世代に anchor されており、競合した Start は
  再 mount に落ちる。`TerminateInstances` に取り消しは無い）。
- **生きた claim があるインスタンスは取らない。** 着地中の Start は attachment をまだ持たず、
  claim だけが唯一の証拠である。
- **ECS のタスクが 1 つでもあれば取らない**（home が無くてもタスクは載りうる——焼きの探針）。
- **自分と同じ型のインスタンスは候補にしない。** 立ち退きが既に first refusal を持っている。
  ここで同型を壊すと、再利用できたはずのインスタンスを捨てて建て直すことになる。
- `af-role=quarantined` は両方の走査から外れたまま（決定 20・証拠は残す）。
- terminate の直後は `DescribeInstances` がまだ `stopped` と答えうるので、
  **`poolSize` から落ちるまで待つ**（`runSlot` が上限を読み直すため。待たないと、
  たった今空けた枠のせいで自分の Start が「一杯」で落ちる）。

### 64.37.5 それでも失敗する場合

片付けられるインスタンスが 1 つも無ければ（全部走行中・全部 claim 済み）、**立ち退きの元のエラーを
そのまま返す**——「プールが一杯」が運用者の取れる手を指している唯一の文である。
`makeRoom` が「何でもいいから 1 台消す」にならないことは、テストで固定した。

起動ダイアログにも段を足した（`slot: making room`）。**製品でいちばん長い待ち**なので、
ここが generic の「起動しています」に落ちると、**最長の待ちだけが理由を名乗らない**ことになる。


## 64.34 立ち退きはテナントを跨ぐ（設計判断・2026-08-26）

`evictLongestIdle` の絞り込みには**テナントが無い**。テナント A の Start が、テナント B の
休眠スロットを回収し得る。これは**そのままにする**（ADR 0045 決定 24）。

- **禁じると「使えない人」が出る。** プールはデプロイ全体で 1 つの資源で、テナント A が使って
  いないインスタンスをテナント B が使えないという規則は、上限到達時に**起動できないメンバー**を作る。
  運用者が絶対に避けると明示した唯一の結果がそれである。
- **テナントを縛るのは `max_workspaces`** ——「その組織の何人が**同時に**動いてよいか」であって、
  「どの物理のインスタンスに載るか」ではない。
- **犠牲者の物は何も露出しない。** `releaseSlot` が umount → detach を済ませてから次の home が
  載る。root ボリュームは元々「前に使った人と共有」の設計であり（§64.15、CFN の
  `SlotRootVolumeGiB` の説明）、テナントを跨ぐかどうかで変わる性質ではない。

⚠️ ただし**テナント上限の合計がプール上限を超えている配備では、この経路が「他テナントの
スロットを奪う」形で表面化する**。そこを塞ぐのはテナント上限側の検証であって、立ち退きの
制限ではない。

## 64.35 テナント上限が無検査で保存できた（コード調査・2026-08-26）

`setTenantLimits`（`control-plane/tenants.go`）が検証していたのは**端末履歴の保持日数と
時間文字列の書式だけ**で、`max_workspaces` を含む整数クォータは**無検査で保存されていた**。

### 64.35.1 2 つの壊れ方

**(1) 誰も満たせない上限が保存できる。** 0 = 無制限なので、**負の数は「小さい上限」ではない**。
`max_workspaces = -1` は `countRunningInTenant() >= limit` を誰も起動する前に真にし、その
テナントは二度と Workspace を開けない。**これは拒否する**（400・フィールド名を含める）。
数値欄の打ち間違いを、メンバーの起動失敗で気づくものにしてはいけない。

**(2) プール上限を超える配分が保存できる。** `Ec2MaxSlots` は CP の env、テナント上限は DB で、
**互いを知らない**。全テナントの合計がプール上限を超えていても誰も警告しない。超過は設定画面から
最も遠いところ——「枠内なのに⑥（`placeHome` の上限到達）で落ちる」か「他テナントのスロットが
立ち退きになる」——でしか表に出ない。**これは警告にする**（後述）。

容量は **`Ec2MaxSlots` − `bakeReservedSlots`(=2)**。golden の焼き直しは種と探針で 2 枠を同時に
要るので（`bakeCapacityBlocked`）、全部配ると**二度と焼けない**。症状は「新しいメンバーの初回
起動が遅い」で、数週間後に気づく類の失敗である。定数は焼き側と共有する——`Ec2SlotSleepSec` と
同じで、2 つの写しは必ずずれる。

### 64.35.2 ⚠️ 0 は「無制限」であって「0 台」ではない

1 テナントでも `max_workspaces = 0` が居れば、**合計はもう上限を縛らない**。これは「超過」とは
**別の問題**なので、別の言い方で出す（`unbounded_tenants`）。合計だけを見て「28/28 なので大丈夫」
と読ませると、そこに無制限のテナントが 1 つ混ざっている配備で嘘になる。

### 64.35.3 ⚠️ 分母が違う——「同時利用」と「占有スロット」を混ぜない

| | 数えるもの | 停止中の Workspace は |
|---|---|---|
| `max_workspaces` | そのテナントの **running / starting** な Workspace（`countRunningInTenant`） | **数えない** |
| `Ec2MaxSlots` | **存在してよいインスタンス**の数 | **数える**（遅延返却でインスタンスを掴んだまま） |

比べられるのは「running な WS はちょうど 1 つのインスタンスを要る」からで、**差分は停止中の WS が握っている
インスタンス**である。したがって **Σ ≤ 容量 は必要条件であって十分条件ではない**：枠内でも休眠中のインスタンスが上限を
埋めて立ち退きが起きうる。§64.32 の `Ec2SlotTerminateAfterSec` を入れると差分は「最後の N 時間ぶん」
に縮むが、0 にはならない。

**画面でも 2 つを 1 つの言葉にしない。** テナントの `max_workspaces` 欄には「同時に動く数」と
添え、警告の文には必ずこの但し書きを付ける。
...
3. **既に超過している配備が、この画面の他の欄すべてを編集できなくなる。** 端末履歴の保持日数を
   直したいだけの管理者が、自分が作ったのでもない条件で止められる——最悪の関門である。

実装は `manager.poolBudget`（`control-plane/limits.go`）1 か所で、**プール上限を持つランタイムの
ときだけ**成立する（`slotCapacityReporter`。他のランタイムでは `ok=false` ＝「大丈夫」ではなく
「そういう問いが無い」）。出口は 2 つ:

- **保存の応答**（`pool_budget`）——いま打った数字について、その場で。
- **プール画面**（`ec2PoolStatus.budget`）——常態として。`manager.poolStatus` が入れる。
  アダプタには DB が無い（ADR 0012）ので、`AutoBake` と同じ継ぎ目に置く。

どちらも**問題があるときだけ**載せる。毎回の保存に「大丈夫です」が付くと、本当に出たときに
読まれなくなる。

## 64.36 「home を外して削除」は既に在る（調査・2026-08-26）

運用者の言葉は「home ボリュームを外して削除」だった。**スナップショット無しの削除は利用者の
データ消失**なのでそのままは実装しない。そして調べた結果、**求められているものは既に実装済み**
だった——`home_hibernate_after`（決定 4 / 決定 13-2・§64.18.2）である。

### 64.36.1 経路は全部繋がっている（実装は要らなかった）

```
tenantLimits.HomeHibernateAfter（DB・テナント毎）
  └→ reaper tier-3 `hibernateHome`（reaper.go）
       ├ homeIdleFor(last_active_at)          … 「誰も開いていない」の判定
       ├ startLock + lifecycle lease + fence  … tier-2 と同じ 3 重の柵
       ├ 柵を取った後にもう一度 State()=="stopped" と last_active_at を読み直す
       └→ BeginHibernate → hibernate()（runtime_ecs_ec2.go）
            ├ CreateSnapshot
            ├ **completed を確認してから** DeleteVolume
            └ 失敗したら snapshot を捨てて次のスイープでやり直す
次回 Start:
  createHomeVolume → restoreSnapshot() → CreateVolume(SnapshotId) → 使えたら snapshot を削除
```

**消えるのは入れ物だけで、中身は S3 に移る。** 利用者から見えるのは「久しぶりに開いた日だけ
起動が +122 秒」と「その日 1 日 I/O が 2.3 倍遅い」（遅延ハイドレート・触ったブロックだけ）。
費用は $0.096/GB-月（プロビジョン量）→ $0.05/GB-月（使用ブロック）で、実使用 20 / プロビジョン 50
なら **$4.80 → $1.00**。

Console の面も既にある（テナント → **使われない home の退避** → 「退避するまで」・`hasPool` で
ecs-ec2 のときだけ出る）。**空＝デプロイ既定、`0`＝そのテナントは退避しない。**

### 64.36.2 唯一の欠け——ECS ではデプロイ既定を設定できなかった

`AF_ECS_EC2_HIBERNATE_AFTER_SEC` に **CFN パラメータが無かった**。ECS 配備では手で
タスク定義を書き換えるしかなく、それは次の `deploy.sh` で上書きされる。つまり
**「既定 0（off）」は既定ではなく、その配備が持ちうる唯一の値だった。**

`Ec2HibernateAfterSec` を足した（既定 0 のまま。既存配備の挙動は変わらない）。

### 64.36.3 <prod-deployment> の状態と、有効化の手順

`AF_ECS_EC2_HIBERNATE_AFTER_SEC` 未設定（＝0）、テナント側も未設定。**一度も走っていない。**

有効化は 2 通りあり、**テナント側はデプロイ不要**である。

1. **テナント毎（推奨・即時）**: Console → 設定 → 管理 → テナント → **使われない home の退避** →
   「退避するまで」に `2160h`（90 日）など。保存した時点から reaper が見る。
   保持期間は本来その組織の答えなので、こちらが筋が良い。
2. **デプロイ既定**: `30-ingress.yaml` の `Ec2HibernateAfterSec`。テナントが何も選ばなかった
   ときの床。**必ず `--no-execute-changeset` で変更セットを確認してから**適用する。

⚠️ **引き金は reaper なので `AF_IDLE_SWEEP_INTERVAL=0` だと退避も止まる**（決定 14）。
⚠️ 45 GiB の snapshot は 30〜40 分かかるが、**1 スイープ 1 手**で進み状態は AWS のタグにしか
無いので、CP が途中で落ちても再開する。**持ち主が先に戻ってきたら退避は放棄され**、
ボリュームはそのまま付け直される。

### 64.36.4 「スナップショットも残さない完全削除」は別機能

退職者アカウントの後始末であって、時間で自動的に走ってよいものではない。既に `Destroy`
（管理者の明示操作・§64.18）がその形で存在し、home ボリューム・退避 snapshot・バックアップを
まとめて消す。**自動退避とは別の入口のままにする。**

## 64.38 mount の SSM 往復は 10〜30 秒ではなかった（実測・2026-08-26）

§64.17.5 は Start の内訳に **「CP の mount（SSM 往復 10〜20s）」** を計上し、§64.32.2 は
それを 10〜30s として「空の停止インスタンスにはほぼ価値が無い」の根拠の 1 つに使い、§64.31.6 は
「その 10〜30 秒の窓にスイープが刺さる」と書いた。**3 か所とも、この数字は一度も測って
いない見積もりだった。** 本番配備（スロット 4 台・全員 m7i.large）で
実測したところ、**mount は 2〜4 秒**である。**この節は §64.17.5 / §64.32.2 / §64.31.6 の
その部分を訂正する。**

⚠️ 計測は本番で行ったが、走行中のワークスペースには触っていない。使ったのは
`ssm list-commands` / `list-command-invocations`（過去 30 日の履歴）、CloudWatch の CP ログ、
ECS / EC2 の describe、および read-only コマンド（`lsblk` / `docker images` / `df`）だけである。

### 64.38.1 `af-mount` 本体は 0.6 秒（中央値）

SSM に残っていた本番の全 `af-mount` / `af-umount`（2026-08-21〜08-26）を、
`RequestedDateTime` → `ResponseStartDateTime` → `ResponseFinishDateTime` で分解した。

| コマンド | n | SSM のキュー（要求→実行開始） | 実行 | 合計（中央値 / 最大） |
|---|---|---|---|---|
| `af-mount … --mkfs` | 37 | 0.2〜0.7s | 0.0〜3.0s | **0.6s / 3.0s** |
| `af-umount …` | 12 | 0.2〜0.3s | 0.0〜0.1s | **0.3s / 0.4s** |

`af-mount` の最大 3.0s は user-data のデバイス探索ループ（`lsblk` を 1 秒おきに最大 30 回）が
2〜3 周した回で、attach 直後にデバイスが見えるまでの待ちである。**`mkfs` で数十秒かかった回は
1 件も無い**（新規 home も含め、記録された 37 回すべてが 3.0s 以内）。

ホットなスロットへ read-only コマンドを投げる往復も直接測った（3 回）: `SendCommand` 受理まで
1.3〜1.6s、`Success` を読むまで 2.8〜3.1s。ただしこれは AWS CLI のプロセス起動を 2 回含む
数字で、SDK で叩く CP はもっと速い。

### 64.38.2 CP が払っているのは、自分のポーリング待ちだった

`runOnSlot` は `SendCommand` の直後に **まず 2 秒寝てから** `GetCommandInvocation` を呼び、
以後も 2 秒間隔で回す。コマンドが 0.6 秒で終わっている以上、**CP から見た mount の所要は
ほぼ「先頭の 2 秒」そのもの**である。

| | 実測 |
|---|---|
| コマンドが実際に終わるまで | 0.6s（中央値） |
| CP が最初に結果を尋ねるまで | **2.0s（固定 sleep）** |
| CP から見た mount 1 回 | **約 2.1s**（実行が 2s を超えた回のみ 4.1s） |

**只で縮む分はここだけで、約 1.5 秒／回。** 載せ替え経路は `umount`（旧スロット）＋ `mount`
（新スロット）で 2 回通るので約 3 秒。ポーリングを 300ms から始めて 2 秒まで倍々に広げる形に
変更した（末尾を 2 秒に収束させるので、`mkfs` が本当に長引く回の API 呼び出し回数は増えない）。

### 64.38.3 「インスタンスを起こした直後は SSM がまだ繋がっていない」も起きていない

`runOnSlot` は `SendCommand` が失敗した最初の 1 回で必ずログを出す（`attempt%5 == 1`）。
**本番 7 日間の CP ログに `not answering SSM yet` は 0 件**であり、`InvalidInstanceId` で
リトライした形跡が無い。新規に建てたインスタンスでも同じで、今日の 1 台を秒単位で追うとこうなる
（`i-0669c7e234fa3e48d`・2026-08-26 17:36:46 JST 起点）。

| 経過 | 出来事 | 出典 |
|---|---|---|
| +0.0s | `RunInstances` 完了 | CP ログ `grew the pool with slot …` ／ `LaunchTime` |
| **+18.0s** | ECS コンテナインスタンス登録 | `registeredAt` 17:37:04.78 |
| +20.2s | `af-mount` の `SendCommand` 受理 | `RequestedDateTime` 17:37:06.99 |
| +20.7s | `af-mount` 完了（実行 0.1s） | `ResponseFinishDateTime` |
| +30.3s | ECS「`MemberOf placement constraint unsatisfied`」 | サービスイベント |
| **+71.4s** | ECS がタスクを生成 | `has started 1 tasks` ＝ `task.createdAt` |
| +96.6s | イメージ pull 開始 | `pullStartedAt`（createdAt +24.5s） |
| +127.8s | pull 終了（**31.2s**） | `pullStoppedAt` |
| +136.6s | コンテナ起動 | `startedAt`（createdAt +64.5s） |
| **+147s** | Agent が `/healthz` に応答 | CP ログ `Agent healthy 121s after Start` |

**SSM の到達は ECS 登録の 2.2 秒後**で、その 2.2 秒も SSM ではなく `waitSlotRegistered` の
ポーリング粒度（3 秒）である。**「SSM が遅い」も「SSM がまだ繋がっていない」も、本番では
起きていない。**

### 64.38.4 では 110〜147 秒はどこに行っているのか

mount は全体の **2〜3%** でしかない。残りはこの 3 つで、どれも ECS 側にある。

| 区間 | 実測 | 中身 |
|---|---|---|
| インスタンス起動 → ECS 登録 | **18s** | 新規作成／wake のときだけ。§64.12.1 の ~20s と整合 |
| mount 完了 → **ECS がタスクを生成** | 定常 **7〜11s** ／ デプロイ直後 **10〜16s ＋ 40 秒後に差し替え** | スケジューラの配置 |
| タスク生成 → コンテナ RUNNING | **37.6〜76.7s** | ENI 0〜4.3s、pull 開始まで 20.8〜33.7s、**pull 28.0〜31.2s（cold）/ 4.1s（warm）**、pull 後 8.8〜14.2s |

2 つ、目に付いたものを記録しておく（どちらもこの調査の範囲外・**次に見るべき場所**）。

> ✅ **追記（§64.39・同日）**: この 2 つは**別の現象ではなく同じ 1 つの原因**だった——
> `upsertService` が「`desiredCount` 0→1」と「タスク定義の差し替え」を 1 回の
> `UpdateService` で出しているため、ECS が増分を**先に古い版のデプロイで満たす**。
> 下の 2 つ目の読み（「当該スロットが配置エンジンから見えていない」）は**誤り**で、
> 見えていないのではなく**古い版が既に無いインスタンスを指している**。§64.39 に実証がある。

- **デプロイ当日はタスクが 2 回生成される。** 例（k1・2026-08-26）: 10:10:02 に
  `UpdateService(desired=1, forceNewDeployment=true, td:3)` を 1 回だけ呼んでいるのに、
  10:10:12 にタスク A、**その 39.7 秒後**の 10:10:51 にタスク B が生成され、10:12:13 に A が
  停止された。CloudTrail 上 `UpdateService` は 1 回きりなので CP の二重呼び出しではない。
  タスク定義の新版が要るとき（＝イメージタグが変わったデプロイ当日）だけ起きており、
  定常日は 1 回（mount+7〜11s）で済んでいる。**この 40 秒は `lastTaskDef` の再利用が効かない
  日に全員が払っている。**
- **新規インスタンスの初回配置が 1 サイクル空振りする。** 上の表の +30.3s、および 08-26 10:38 の回で
  `MemberOf placement constraint unsatisfied` が出て、そこから約 41 秒後にようやく配置される。
  ECS が「最も近い候補」として挙げたのは**別のインスタンス**（`i-06deedb2…`）で、mount 済みの
  当該スロットは登録済みなのにまだ配置エンジンから見えていない~~ように見える~~。
  ⚠️ **この読みは §64.39 で否定された**——伝播遅れは実測 1.8 秒で存在せず、
  `waitSlotRegistered`（`status=ACTIVE && agentConnected`）の待ち条件は正しい。
  置けなかったのは**古い版のタスク定義**で、それが指すインスタンスがもう無かった。

⚠️ **pull 28〜31s は「キャッシュが壊れている」ではない。** 計測日は 0.12.2 を配った当日で、
`docker inspect` の `Metadata.LastTagTime` が当日の pull 時刻と一致した（＝そのタグの初回 pull）。
ルートは 100 GiB 中 6.0 GiB 使用でディスク逼迫も無い。**イメージ 2.63 GB という大きさと、
デプロイ当日は全員が 30 秒払うという事実だけを記録しておく。**

### 64.38.5 タスク側で mount する／mount と配置を並行させる —— どちらも採らない

| 案 | 買える時間 | 判断 |
|---|---|---|
| ECS タスク定義の bind mount / entrypoint で mount する | **2〜4s** | **採らない。** 40-ec2-pool.yaml の user-data が「data volume に触らない・fstab を書かない」を守っているのは、**誰の home が載るかが日に何度も変わり、stale な fstab で次の人が起動不能になる**からである。2〜4 秒のためにその設計理由を壊す釣り合いにない |
| `upsertService` を mount より先に出して並行させる | **2〜4s** | **採らない。** 危険側に外れる。タスクの host volume は `"/af-home/<mid>/dev"` であり、mount 前に docker がそこを触ると**ルートボリューム上に空ディレクトリが作られ、利用者の home が黙って空で立ち上がる**。順序制約は本物で、2〜4 秒と引き換えにしてよいものではない |

### 64.38.6 §64.32 と §64.31.6 に何が起きるか

- **§64.32.2 の結論 2 は撤回する（結論 1 は残る）。** 「空の停止インスタンスを起こして attach + mount」を
  同じ算術でやり直すと 110s ＋ attach 3s ＋ **mount 2〜4s** ＝ **115〜117s** で、新規作成 135s に
  対して **18〜20 秒**買っている。**「空の停止スロットはほぼ何も買っていない（0〜12 秒）」は
  成り立たない。** home 付きの 25 秒に対して 19 秒——**差は 6 秒**である。
  - **ただし「だから空の停止インスタンスを貯めよう」にはならない。** 6 秒差のために home を外す
    （＝持ち主の親和性を捨てる）釣り合いは依然として悪く、$3.84/月 の固定費もそのままで、
    §64.32.3 の「温存されるインスタンスは汎用のキャッシュにならない」も無傷である。**変わったのは
    『渡らない』が『あまり渡らない』になったこと**だけで、判断は変わらない。
  - **⚠️ そして 115〜117s も 135s も、実測ではなく §64.17.5 のハーネス値（各 1〜2 回）に
    今回の mount を差し替えた算術である。** 舵を切るならこの経路を直接測ること。
- **「mount が縮めば停止インスタンスの使い回しが意味を持つ」という見通しは成り立たない。** 縮ませても
  買えるのは 2〜4 秒で、どの経路も **ECS 配置・タスク起動の 90〜120 秒**が支配している。
  他人のインスタンスを流用しても大きくは速くならない、という結論はそのまま残る。
- **§64.31.6 の窓は mount では縮まない。** `clearDormancy`（`placeHome` の中）から
  `upsertService`（`launch` の最後）までの窓は、内訳が `waitInstanceRunning`（0〜19s）＋
  `attachHome` ＋ `waitSlotRegistered`（0〜18s）＋ **mount 2〜4s** ＋ タスク定義登録である。
  **mount はこの窓の 1 割にも満たない。** 窓を閉じたいなら短縮ではなく、順序かマーカーの
  構造で直すしかない（§64.31.6 の対処はそちらで入っている）。

### 64.38.7 直したこと

`runOnSlot` のポーリングを「先に 2 秒寝る」から 300ms 始まりの倍々（上限 2 秒）に変更した。
**買えるのは 1 回あたり約 1.5 秒・載せ替え経路で約 3 秒**で、Start が体感で速くなるものでは
ない。**入れた理由は「只だから」であって、これで起動が速くなると読まないこと。**

## 64.39 起動のたびに 40 秒——1 回の `UpdateService` が 2 つのデプロイを走らせていた（実測・2026-08-26）

§64.38.4 で「次に見るべき場所」として 2 つ記録した——**タスクが 2 回生成されて 40 秒後に
差し替わる**件と、**新しいインスタンスへの初回配置が `MemberOf` 不成立で空振りする**件。
**この 2 つは別の現象ではなく、同じ 1 つの原因の 2 つの見え方**だった。原因は CP 側にあり、
**直した**（§64.39.6）。

原因は `upsertService` が **「`desiredCount` 0→1」と「タスク定義の差し替え」を 1 回の
`UpdateService` で出している**こと。ECS はタスク定義が変わると現 PRIMARY を ACTIVE に降ろして
新しい版を PRIMARY にするが、**`desiredCount` の増分は先に ACTIVE（＝古い版）で満たしにいく。**

### 64.39.1 本番配備で実際に起きていること

CloudTrail（管理イベント）で 1 回の起動を秒単位に開くと、CP の呼び出しは 1 回きりである。

| 時刻 (UTC) | 出来事 |
|---|---|
| 01:09:57 | `RegisterContainerInstance`（起こしたスロットが ECS に戻る） |
| 01:10:00 | `RegisterTaskDefinition` → **rev 3** |
| 01:10:02 | **`UpdateService`（`desiredCount=1` ＋ `td=rev3` ＋ `forceNewDeployment` — これ 1 回だけ）** |
| 01:10:12 | `TaskCreated` — **`taskDefinitionArn` は rev 2**（＝古い版） |
| 01:10:51 | `TaskCreated` — rev 3（本命。**39.7 秒後**） |
| 01:12:13 | サービスイベント「has stopped 1 running tasks」＝ rev 2 のタスクを退役 |

`RunTask` も 2 回目の `UpdateService` も無い。**古い版のタスクは ECS が自分で立てている。**

⚠️ **古い版のタスクは「立って消える」のではなく、2 分ほど本当に走る。** イベントが
`has stopped 1 **running** tasks` と言っているとおり RUNNING に達しており、その間
**古いイメージ・古い env のタスクが Service Connect に本命と並んで載る**。これは
`lastTaskDef` のコメントが `ForceNewDeployment` の代償として書いていた窓
（CP がメモリに持っていた OAuth `flow_id` が落ちた・2026-08-19）と同じものだが、
**`ForceNewDeployment` だけの話ではなく、タスク定義が変わる起動すべてで開く。**

### 64.39.2 もう 1 つの見え方——`MemberOf` の空振り

タスク定義の配置制約は `ec2InstanceId == i-…` で、**古い版は前回のスロットを指している**。
そのスロットが既に無い（terminate 済み／別の型に建て替え）ときは、古い版のタスクは
どこにも置けない:

```
was unable to place a task because no container instance met all of its requirements.
The closest matching (container-instance a8f1…) encountered error
"MemberOf placement constraint unsatisfied."
```

⚠️ **この「最も近い候補」に挙がるのは当該スロットではなく別のインスタンス**である。§64.38.4 では
これを「mount 済みのスロットがまだ配置エンジンから見えていない」と読んだが、**逆だった**——
見えていないのは新しいインスタンスではなく、**古い版が指しているインスタンスがもう存在しない**。本命（新しい版）の
タスクはこのあと約 41 秒で置かれる。

⚠️ **しかもこれは利用者に見えていた。** `ecsPlacementBlocked` は PRIMARY の作成時刻より新しい
イベントだけを拾う設計だが、この空振りは **PRIMARY を作った直後に起きる**ので条件を満たし、
起動ダイアログに `blocked: … was unable to place a task …` が出る（CP ログにも
「cannot be placed and will stay `starting` until this is fixed」が残っている）。
**普通の起動で「配置できません」という深刻な診断が出ていた**わけで、原因は自分の古いデプロイ
だった。分割するとこの誤診も消える。

### 64.39.3 否定した仮説——コンテナインスタンスの伝播遅れは無い

「登録直後のインスタンスは配置エンジンにまだ見えていないのではないか」を直接測った。開発配備は撤収済み
だったので、同じアカウントに**最小の ECS 検証台**（クラスタ 1・t3.small 1 台・busybox）を建てて、
`RegisterContainerInstance` の直後から `RunTask`（`memberOf ec2InstanceId == …` 付き）を
2 秒おきに叩いた。

- 起動から **28.5 秒**で ECS 登録（`status=ACTIVE`・`agentConnected=true`・属性 75 個。
  `task-eni` も `service-connect-v1` も登録の時点で揃っている）
- **登録から 1.8 秒後の 1 発目の `RunTask` が成功**

**伝播遅れは無い。** `waitSlotRegistered` の待ち条件（`ACTIVE && agentConnected`）は正しく、
ここは疑うところではなかった。

### 64.39.4 A/B——分ければ 1 本になる

同じ検証台で、CP と同じ形（サービスは `desiredCount 0` で存在・タスク定義を新版に差し替え）を
作って 2 通り測った。

| やり方 | 生成されたタスク |
|---|---|
| **A. 現行 CP と同じ**——`UpdateService(desiredCount=1 + td=新 + force)` 1 回 | **2 本**（+8.7s に旧版・+15.8s に新版） |
| **B. 2 回に分ける**——①`UpdateService(td=新)`（`desiredCount` は触らない）→ 収束を待つ → ②`UpdateService(desiredCount=1)` | **1 本**（②から +9.4s に新版） |

さらに①と②の**間隔**を振ると、必要な条件がはっきりする。

| ①と②の間隔 | タスク | ②の直前の deployments |
|---|---|---|
| 0 秒 | **2 本** | `PRIMARY:新(IN_PROGRESS)` ＋ **`ACTIVE:旧`** |
| 20 秒 | **1 本** | `PRIMARY:新(IN_PROGRESS)` ＋ `DRAINING:旧` |
| 60 秒 | **1 本** | `PRIMARY:新` のみ |

★**条件は「旧デプロイが `ACTIVE` を抜けていること」だけ**である。`DRAINING` は新しいタスクを
取らないので、そこまで待てば足りる。**完全に消えるのを待つ必要も、`rolloutState=COMPLETED` に
なるのを待つ必要も無い**——この 2 つは高くつく。

| 何を待つか | 実測（`desiredCount=0`・タスク 0） |
|---|---|
| **旧デプロイが `ACTIVE` を抜ける** | **9.5s / 13.2s / 22.9s** |
| 旧デプロイが `deployments` から消える | 58.6s / 59.0s |
| `rolloutState=COMPLETED` | 91.6s / 89.1s |

### 64.39.5 どれくらいの頻度で起きているか

本番配備のサービスイベント（ECS が保持している範囲）を数えると、**起動 15 回のうち 6 回（40%）**
がこれを踏んでいた。

- タスク 2 本 ＝ **4 回**。差はいずれも **+39.7s / +39.7s / +40.1s / +40.7s** ——**ばらつかない**
- 配置の空振り ＝ **2 件**（本命まで約 41 秒）

タスク定義が変わる条件は `reuseOrRegisterTaskDef` の指紋で決まるので、踏む場面は次のとおり:

1. **スロットが前回と違う**（配置制約に `ec2InstanceId` が入っているため）＝**載せ替えは必ず踏む**
2. **イメージが変わった**（デプロイ当日は全員）
3. 設定・テナント上限など env が変わった
4. ⚠️ **CP が再起動した**——`lastTaskDef` はプロセスローカルなので、**中身が同一でも**
   再起動後の初回 Start は新リビジョンを登録する。**デプロイのたびに全員が 1 回踏む**
   （→ §64.39.6.2 で直した）

★ これは §64.16.3 の未解決だった割れ方——**同じ「ホットスロットへ載せる」なのに 13.2s と
51.8s / 94.3s に割れる**——の説明にもなっている。1 行目（13.2s）は 1 のケースに当たらず
タスク定義が再利用された回、2 行目は**別の人が使っていたスロットへ載せ替え＝必ず 1 に当たる**回である。

### 64.39.6 直したこと

**①`UpdateService(taskDefinition=新)` を `launch()` の先頭で先に出し、②`UpdateService(desiredCount=1)`
を最後に出す。**間の待ちは「`ACTIVE` な旧デプロイが無くなるまで」で、上限
`ec2DeploymentRetireBudget`（**90 秒**）。実装は `pointServiceAt` /
`waitOldDeploymentRetired` / `scaleUpService` の 3 つ。

配置制約の `ec2InstanceId` もネットワーク設定の AZ も `launch()` に入った時点で確定している
ので、①を先に出すのに足りない情報は無い。

- ⚠️ **①は `desiredCount` を触らない**。これが早出しを安全にしている——走っていない
  ワークスペースを起こしてしまうことがなく、mount に失敗して隔離に落ちても
  「隔離されたインスタンスに pin されたタスク定義を `desiredCount=0` で指したサービス」が残るだけで、
  **何も走らない**。次の Start が上書きする
- ⚠️ **②で `ForceNewDeployment` を送ってはいけない**——分割して退役させたばかりの ACTIVE
  デプロイをそのまま作り直すので、元の木阿弥になる。②はタスク定義も送らず `desiredCount` だけ
- ⚠️ ①が失敗したら**分割をやめて従来の 1 回呼び出しに落ちる**（遅い Start であって壊れた
  Start ではない）。サービスがまだ無い初回起動も従来どおり `CreateService`（旧デプロイが
  存在しないので、そもそもこの問題は起きない）
- ⚠️ 上限を超えたら**そのまま ②を出す**。今より悪くはならない——**が、超えること自体は
  避けねばならない**。実機では退役に 53〜55 秒かかり、上限 25 秒版はそこで打ち切って
  scale up し、**その場で 2 本立った**（§64.39.8）
- ⚠️ したがって**待ちはリクエストの外に出す**。同期経路（同じ running スロットへの再 attach）
  では `launch()` の後半を `bg` に渡す。背景に出す以上、**上げる直前に「まだ自分の起動か」を
  確かめる**（起動世代の一致と home がまだそのスロットに載っていること）

### 64.39.6.1 これで起動は速くなるのか——⚠️ 分割そのものは「速くする」変更ではない

⚠️ **ここは最初「経路によっては 40 秒まるごと縮む」と書いていた。実機で測ったあと、その
読みは維持できない。**理由は 2 つで、どちらも計測の後で分かった。

1. **退役待ちは只ではない。** 実機では **53〜55 秒**かかる（§64.39.8）。`pointServiceAt` は
   `launch()` の先頭なので、隠れるのはスロット作業と重なるぶんだけ——**起こす/建てるで
   20〜40 秒、同じ running スロットへの再 attach では数秒**。残りは素の待ちである
   （→ ✅ **§64.39.10 でこの待ちごと削除した**）
2. **消える 40 秒は確率的である**（§64.39.7）。本番で踏むのは起動の 40% で、残り 60% は
   もともと 1 本で済んでいた。**その 60% にとっては待ちが増えるだけ**

したがって**期待値としての短縮は主張できない**。実機の「タスク定義が変わる Start」の収束は
**175s（上限 25s・打ち切り）/ 138s（同）/ 104s（上限 90s・待ち切り）**で、**長く待った回が
一番速かった**——が n=1 ずつで、タスク起動自体のばらつき（本番実測 37.6〜76.7s）に埋もれる。
**どちらの向きにも読んではいけない。**

⚠️ **ただし「待ちを削れば速い」は成り立たない。** `State()` は `serviceRolledOut` を見るので、
2 本目が立った時点で「running」はロールアウトが片付くまで出ない。**待ちを打ち切って 2 本
立てると、節約したぶんは向こう側で取り返される**（上の 138s 対 104s はその形に見える）。

**では何が確実に速くなるのか——§64.39.6.2 の方である。**

| 変更 | 速さへの効き |
|---|---|
| **CP 再起動をまたぐ再利用**（§64.39.6.2） | ✅ **確実。** 毎デプロイ後の全員の初回 Start が「新リビジョン登録＋ロール＋退役待ち」を丸ごと回避する。実機の再 wake は **35 秒で収束**（変更ありの 104〜175 秒に対して） |
| **`UpdateService` の分割**（§64.39.6） | ⚠️ 単独では速さを主張しない。**ただし §64.39.10 で退役待ちが消えたので、いまは「確定の損」も無い** |
| **`deploymentConfiguration` を 100%**（§64.39.10） | ✅ **退役待ち 53〜55 秒がまるごと消える。** そのうえで 2 本目は構造的に不可能になる |
| mount のポーリング（§64.38） | 1 回 1.5 秒。誤差 |

そして**秒数より効くかもしれない方**が 2 つある。どちらも時間の話ではない。

1. 旧イメージ・旧 env のタスクが Service Connect に本命と並ぶ 1〜2 分の窓が**消える**。
   これは `lastTaskDef` が「OAuth `flow_id` が落ちた」と書いていた窓そのもの。
2. 起動ダイアログの **`blocked: … unable to place …` という誤診が消える**（§64.39.2）。

⚠️ **「同じ running スロットへ再 attach する」経路は `launch()` が Start の HTTP リクエスト内で
同期実行される**（`Start` の `deferred` 分岐）ので、**そこに長い待ちを置いてはいけない**。CP の
前段の ALB は既定 60 秒で切れ、それが `watchReady` の「90 秒待って 504」の正体である。
最初は退役待ちを 25 秒に切って収めようとし（実機では 53〜55 秒かかって収まらず）、次に待ちごと
背景に逃がした。**いまは §64.39.10 で待ち自体が無くなったので、この制約は効かない。**

### 64.39.6.2 CP 再起動でも再利用が効くようにした（§64.39.5 の 4）

`lastTaskDef` は**プロセスローカル**なので、CP 再起動（＝毎デプロイ）と 2 レプリカ目では必ず
外れる。外れると**中身が同一でも新リビジョンを登録**し、サービスをロールする——分割前は
そのまま 40 秒、分割後も 10〜23 秒の待ちを払う。**全員が、毎デプロイ、1 回ずつ**である。

直しは、**指紋をリビジョン自身の docker ラベル** `af.taskdef.fingerprint` に焼いておき、
プロセスキャッシュを外したときに**「サービスがいま指しているリビジョン」を読んで突き合わせる**
（`serviceTaskDefIfFingerprint`）。一致すればそれを再利用し、登録もロールも起きない。

- **なぜタグでなく docker ラベルか**——`afImageStampLabel`（runtime_ecs_stale.go）と同じ理由で、
  読み戻しに `include=TAGS` も `ecs:TagResource` も要らない。新しい IAM を足さずに済む
- ⚠️ **ラベルはハッシュを取った「後」に付ける。`buildTaskDef` の中に入れてはいけない。**
  ラベルの中身がハッシュそのものなので、ハッシュに含めてしまうと**次の Start が組み立てる
  （ラベルの無い）入力とは永久に一致しなくなる**。症状は「起動が遅い」だけなので気づけない
- ⚠️ **INACTIVE なリビジョンは再利用しない。** deregister 済みのリビジョンへ `UpdateService`
  すると失敗する。**再利用が Start を壊すのは、冗長な登録より遥かに悪い**
- コストはキャッシュミス時の AWS 読み 2 回だけで、どちらも別の理由で既に叩いている呼び出し。
  **`/api/workspace` のポーリング経路には入れていない**

### 64.39.6.3 ⚠️ Fargate 側（`runtime_ecs`）は同じ形のまま（実機で 1 回再現）

`ecsRuntime.upsertService` も **`UpdateService(desiredCount=1 ＋ taskDefinition=新 ＋
ForceNewDeployment=true)` を 1 回**で出しており、しかも `registerTaskDef` は Start のたびに
無条件で新リビジョンを登録する（再利用の仕組みが無い）。

**実機ハーネスで測った**（§64.39.8）: 収束済みのサービスに対する Start 2 回で、**1 回目は
タスク 2 本・2 回目は 1 本**。つまり**同じ欠陥は Fargate にも在る**（推測ではなくなった）。
Fargate は**イメージキャッシュを持たない**（`watchReady` の注記）ので、無駄に立つタスクは
フル pull を払う。

**それでも触っていない。** ADR 0045 のファイル一覧が「**`runtime_ecs.go`（Fargate）は変更
しない**」と明示している面であり、実利用者の居る配備は `ecs-ec2` である。**直すかどうかは
別の判断**として残す。

### 64.39.7 ⚠️ これはレースであって、必ず起きることではない（測って分かった／§64.39.10 で精緻化）

§64.39.1〜4 は「1 回の呼び出しで両方送ると **必ず** 2 本になる」と読める書き方をしていたが、
**実機で測ると毎回は起きない。**

| substrate | 旧の形（1 回の呼び出し） | 結果 |
|---|---|---|
| 使い捨てクラスタ（Fargate・§64.39.4） | 3 回 | **2 本 / 1 本 / 1 本** |
| 実機ハーネス（Fargate アダプタ） | 2 回 | **2 本 / 1 本** |
| 実機ハーネス（EC2 プール） | 2 回 | **2 本 / 1 本** |
| 本番配備 | 15 起動 | **2 本が 4 回・配置空振りが 2 件 ＝ 40%** |

⚠️ **「ACTIVE な旧デプロイが在る状態で desiredCount を上げる」を意図的に作っても、実機では
1 本で済んだ回がある。** 決定論的な再現手段は見つかっていない。したがって:

- **`desiredCount` を上げる時点で ACTIVE な旧デプロイが在ること**は、観測されたすべての
  2 本立ちに共通する**必要条件**である。無い状態で 2 本になった観測は 1 件も無い
- しかし**十分条件ではない**。ECS のスケジューラの中で、カウントの適用とデプロイの降格の
  どちらが先に効くかのレースだと見るのが実測に一番合う
- ⇒ **修正は「必要条件を作らない」ことで効く**。効果の大きさは本番の 40% で読むべきで、
  ハーネスの 1 回の緑を根拠にしてはいけない

★**この節が在る理由**: 実機テストの最初の版は「修正後は 1 本」を 3 ラウンド緑で報告した。
**substrate がそもそもバグを再現していなかった**（Stop 直後で旧デプロイが未収束だった）ので、
**「直った」と「そもそも起きない」が見分けられていなかった。** 実機テストには**バグ側を
撃つラウンド（witness）を必ず置く**こと——ただし今回のように相手がレースなら、
**witness の緑で落とすとテストが flaky になる**ので、落とさず「今回は何も示していない」と
言わせる。

### 64.39.8 実機で確かめたこと（ハーネス・2026-08-26）

`deploy/aws/ecs/harness/setup.sh`（NAT 版＝本番相当）で基盤を立て、
`TestECSEC2LiveStartDeployments` / `TestECSFargateLiveStartDeployments` を回した。**残存 0 まで
撤収済み。** ⚠️ 開発配備そのものは撤収済みで存在しないので、立て直したのは**このハーネスだけ**
（CP も ALB も RDS も要らない——測りたいのは ECS のスケジューラとアダプタの呼び出し方である）。

⚠️ **ハーネス側で 3 つ直した**（どれも「動かない」ではなく「黙って嘘をつく」型）:
- `setup.sh` の `REPO_DIR` が**特定の worktree への絶対パス**で固定されていた。その worktree は
  とっくに無く、誰が動かしても動かない。`AF_HARNESS_REPO_DIR` で渡す形にし、repo 内で直接
  実行したときだけ自動解決する
- `teardown.sh` が**タスク定義を消していなかった**。課金はしないが、**残存確認が「0 件」と
  言い切っているのに 13 個残っていた**。確認が数えない物は消えない
- その掃除を `--family-prefix` で書いたら 0 件だった。⚠️ **あれは前方一致ではなく family の
  完全一致**で、`af-ec2c` を渡すと `af-ec2c-dv1` は引けない

| 測ったこと | 結果 |
|---|---|
| 修正後・**タスク定義が変わる** Start | **タスク 1 本・配置空振り 0** |
| 修正後・**CP 再起動を模した** Start（キャッシュを消す） | **タスク 1 本・新リビジョン 0 本**（§64.39.6.2 が効いている） |
| 旧デプロイの退役待ち（実測） | **53 秒 / 55 秒** |
| Fargate アダプタの Start 2 回 | **2 本 / 1 本**（§64.39.6.3） |

⚠️ **最大の収穫は `ec2DeploymentRetireBudget` が短すぎたこと。** 最初は 25 秒にしていた
（Start の HTTP リクエスト内に収めるため）。実機では **53〜55 秒**かかり、**25 秒版は待ちを
打ち切って scale up し、その場で 2 本立った**（10 秒差）——**修正が自分で作った再現**である。
使い捨てクラスタでの 10〜23 秒（§64.39.4）は**分布の胴体であって裾ではなかった。**

直しは待ちを**リクエストの外に出す**こと: `launch()` の後半（退役待ち → `desiredCount=1` →
claim 解除）を、同期経路では `bg` に渡す。⚠️ **背景に出す以上、上げる直前に「まだ自分の
起動か」を確かめる**（起動世代の一致と、home がまだそのスロットに載っていること）——
そうしないと、直後に Stop した人のワークスペースを背景処理が起こし直す。

### 64.39.9 サンドボックス検証台の後始末

クラスタ 2・サービス 2・EC2 1 台・タスク定義 15・IAM ロール/インスタンスプロファイル 1 組を
作って全部消した。**残存 0**（`list-clusters` / `describe-instances` / `list-roles` /
`list-task-definitions --status ACTIVE` がいずれも空）。走行中のワークスペースには触っていない。

### 64.39.10 ✅ 本当の直し——`deploymentConfiguration` で 2 本目を作れなくする（2026-08-26）

サービスの `deploymentConfiguration` は既定のまま **`maximumPercent: 200` /
`minimumHealthyPercent: 100`** だった（`UpdateService` の応答で確認）。
**200% だからこそ、desired 1 のサービスがタスクを 2 本走らせられる。**

`maximumPercent: 100` / `minimumHealthyPercent: 0`（`ec2SingleTaskDeployment`）に変えた。
**ECS はこのサービスに 2 本目を立てられなくなる**ので、§64.39 の仕掛けそのものが消える。

#### 同じ substrate・同じ手順・同じ瞬間の A/B（決定論的に再現した）

「タスク定義を差し替えた直後、`ACTIVE` な旧デプロイが在る状態で `desiredCount` を上げる」を
2 つの設定で各 2 回:

| `deploymentConfiguration` | 結果 |
|---|---|
| `maximumPercent 200 / minimumHealthyPercent 100`（既定） | **2 本**（+12.2s に**旧**リビジョン → +23.8s に新）／**2 本**（+8.0s **旧** → +18.9s 新）——**2 回とも** |
| `maximumPercent 100 / minimumHealthyPercent 0` | **1 本・rev は必ず新**（+17.4s）／**1 本・新**（+59.3s）——**2 回とも** |

★**この調査で初めて決定論的に再現できた。** §64.39.7 で「レース」と書いたのは正しかったが、
**何がレースなのかの切り分けが足りなかった**——正確にはこうである:

- **条件（`ACTIVE` が在る状態で count を上げる）＋ `maximumPercent 200` ⇒ 2 本立つのは決定論的**
- **レースなのは「1 回の結合呼び出しがその条件を作るかどうか」**（降格とカウント適用の順序）。
  本番で 40% だったのはこの部分

#### これで退役待ちは要らなくなった

`waitOldDeploymentRetired` と `ec2DeploymentRetireBudget` は**削除した**。実機で **53〜55 秒**
払っていた待ちがまるごと消える。同期経路を `bg` に逃がしていたのも**元に戻した**（待たないなら
リクエストの中で終わる 1 呼び出しでしかない）。

- ⚠️ **設定は 4 か所すべての呼び出しに乗せる**（`CreateService` ／ `pointServiceAt` ／
  `scaleUpService` ／ `upsertService`）。**既定は AWS 側の 200 なので、この変更より前に作られた
  サービスは誰かが送るまで 200 のまま**である。作成時 1 回では届かない
- ⚠️ `minimumHealthyPercent: 0` は「デプロイ中に running 0 になってよい」を明示的に認めること
  である。**max 100% では新しい方を先に立てる余地が無い**ので、これが対になる。
  ワークスペースにとっては**そちらが正しい意味論**——2 本同時は Service Connect が両方に振る
  ので**常に害**であって、可用性ではない
- **`pointServiceAt` / `scaleUpService` の分割は残す。** 待ちが無くなった今も、リビジョンを
  先に流しておけばスロットの起動と重なる。costs は API 呼び出し 1 回

✅ **実機（ハーネス）で通した——§64.39.11。** ここまでの A/B は最小構成（Fargate・busybox）で
取った ECS の挙動であり、**CP がこの設定を実際に配れているかはユニットテストまで**だった
（`TestECSEC2EveryServiceCallPinsOneTaskPerWorkspace` が 4 経路すべてを固定している）。

### 64.39.11 ✅ 実機ハーネスで `deploymentConfiguration` を通した（2026-08-27）

`TestECSEC2LiveDeploymentConfig`（`control-plane/runtime_ecs_ec2_live_deploy_test.go`）。
NAT 版ハーネスを立て、**deploy → 検証 → teardown を 1 セッションで閉じた**（残存 0・§64.39.12）。

| 測ったこと | 結果 |
|---|---|
| 新規サービス（`CreateService`） | **`100/0`**・タスク 1 本・空振り 0 |
| A/B `maximumPercent 200`（＝この変更より前に作られた**全**サービス） | **同時 2 本**（`PRIMARY rev2 r=1` と `ACTIVE rev1 r=1` が並ぶ） |
| A/B `maximumPercent 100 / minimumHealthyPercent 0` | **同時 1 本**（`ACTIVE` が 0 になってから `PRIMARY` が立つ） |
| ★**200/100 に戻したサービス**＋タスク定義が変わる Start（**2 回**） | **`100/0` に戻る**・タスク **1 本**・空振り **0**・**降格デプロイのタスク 0** |
| ★同・設定と count の**順序** | **`max=100` は +4s、`desiredCount=1` は +6s**（2 秒間隔）＝**配備直後の初回 Start から安全** |
| `!prepared` 経路（再利用 →`upsertService` の 1 回呼び出し） | `100/0`・タスク 1 本・設定と count は**同一呼び出し**（+6s / +6s） |

★の 2 行が「配備直後の初回 Start は旧挙動が残るのか」への答えである。**残らない**——
`pointServiceAt` がタスク定義の差し替えと同じ `UpdateService` で 100% を送り、`scaleUpService`
はその**後**に count を上げるので、**count が上がる時点でサービスはもう 200% ではない**。

#### ⚠️ 残っている挙動——「2 本」は構造的に消えたが「40 秒」は消えていない

★の 2 回とも、**count はいったん降格した `ACTIVE` デプロイに入る**（`+8s → +50s` ／
`+17s → +46s` で `PRIMARY` に移る）。100% なのでそこにタスクは立たないが、**移るまで 29〜42 秒
待つ**。実測: タスク定義が変わる Start は running まで **164〜176 秒**、再利用の Start は
**95 秒**。§64.39 の**害（旧版のタスクが本命と並ぶ）は消えた**が、**遅さは残っている**。

⚠️ さらに——**100% が上限にしているのは「合計 1 本」であって「`PRIMARY` が 1 本」ではない。**
その 1 本を**降格側が先に取る**とみられる観測が、整地前の 1 回目の走行で 1 度ある
（+88s に立って +128s に停止 → 本命は +200s）。**同時 2 本にはならないが、古い版のタスクが
40 秒だけ単独で走り得る**。整地後の 3 ラウンド（★2 回＋`!prepared`）では再現していない。
`requireNoDemotedTask` がこの形を落とすので、次に出たら緑では通らない。

#### ⚠️ ハーネス側の教訓（どちらも「黙って嘘をつく」型）

- **1 回目の走行の ★ ラウンドは、前ラウンドのロールが終わらないうちに始まっていた。**
  Stop が `desiredCount` を 0 にした瞬間、デプロイは 0/0 で `COMPLETED` になるので
  **`settle` が即返る**——「収束した」ではなく「まだ 1 本も置いていない」状態を次に渡す。
  本番の Start が絶対に置かれない状態で測っていた。**ラウンド間で必ず収束させること。**
- **`countSince` の「2 本」だけでは誰の 2 本か分からない。** `describe-services` の
...
## 64.40 スロットの OS が死ぬと「起動が必ず 504」になる——待つのをやめてインスタンスを捨てる（本番事故・2026-08-27）

<prod-deployment> の利用者 1 名から「昼休憩のあと再開しようとしたら **504 gateway time-out**、
何度試しても起動できない。でもインスタンスは稼働中に見える」という申告。**インスタンスは生きているが
...
### 64.40.2 引き金——**ディスクの中身を 5 分に 5 周、3 時間読み直し続けた**（refault thrash）

⚠️ **ここは 2 度書き直している。** 最初は「メモリ枯渇」、次に「メモリとは無関係な I/O 飽和」、
そして最終形が以下である。**2 度目が行き過ぎで、対策の結論まで逆に振れた**ので、
何をどう測って何が言えるのかを、順を追って残す。

**測ったもの（すべて実測）**

| 指標 | 値 |
|---|---|
| **root** ボリューム (gp3 40GiB) の read | **39.34 GB / 5 分が 3 時間、有効数字 4 桁まで不変** |
| ↑ を秒速に直すと | **125.07 MiB/s ＝ gp3 の既定スループット 125 MiB/s ちょうど** |
| home ボリューム (gp3 50GiB) の read | ピーク 38.3 GB / 5 分。**14:45 に約 1 GB/5 分まで落ちた** |
| インスタンス合計 (`AWS/EC2 EBSReadBytes`・30 秒粒度) | 14:00 の 5 分で 58.90 GB＝**root 39.34 + home 19.56 と 0.0 MB 差で一致** |
| `EBSByteBalance%` | 99% → 70% へ単調減少 / `EBSIOBalance%` は 99% のまま（＝順次読み） |
| **root デバイスに存在するデータ総量** | **8.0 GB**（`df`）。最大のファイルは chromium の **287 MB** |
| RAM / swap | **7.6 GiB / 0** |

★ **これが決め手である**: **8.0 GB しか入っていないデバイスから 39.34 GB/5 分**——
**ディスクの中身を丸ごと 5 分あたり 4.9 周、3 時間読み直していた。**
読むべき大きなファイルは存在しない。**「大量のデータを読んでいた」のではなく
「同じものを読み直し続けていた」**＝ **refault thrash** 以外にあり得ない。

**それを裏づける 3 点**
- **home の read が 14:45 に落ちても root は 16:10（強制停止）まで上限のまま。**
  嵐は利用者のデータ読み出しから独立して自走していた
- ワークスペースコンテナが書き込み層に書いたのは **694 KB** だけ（`docker inspect --size`）。
  root 上でデータを作ってもいない
- ⚠️ **root は `noatime`** なので、どのファイルが読まれたかの追跡は**できない**（この手は使えない）

#### 64.40.2.1-0 「OOM が 0 件」と「メモリが原因」は矛盾しない

ジャーナルには **OOM killer の発火 0 件**・`hung_task` 0 件・`page allocation failure` 0 件、
`docker inspect` も全コンテナ `OOMKilled=false`。それでも**原因はメモリ**である。

**swap がゼロなので、カーネルが回収できるのはページキャッシュだけ**である。何かが匿名メモリ
（swap 不可）を数 GB 握ると、カーネルはキャッシュをほぼゼロまで削り取り、以後あらゆるプロセスが
実行ページをスケジュールされるたびにディスクから読み直す。125 MiB/s ＝ 4 KiB ページで
**毎秒 3 万回超のメジャーフォルト**で、CPU が張り付くのもこれである。

★ **この状態では OOM killer は発火しない。回収は「成功し続けている」からである**
——キャッシュを破壊することによって。**Linux が OOM を出さずにスラッシングで死ぬ**古典的な形で、
「8 GB のディスクを 5 分に 5 周」という数字がその指紋である。

⚠️ **だから「OOM が無い ＝ メモリではない」と読んではいけない。** 2 度目の訂正はまさにそこで
足を踏み外し、「メモリのハードリミットは効かない、むしろ悪化しうる」という**逆の結論**を書いた。
`memory.max` は cgroup のページキャッシュも縛るので、**その cgroup の中だけ見れば**確かに
再読み込みは増える。しかし今回の被害の本体は**巻き添え**であり、上限があれば追い出し圧力は
**そのコンテナの cgroup 内に閉じて**、dockerd / ECS エージェント / SSM / stunnel は
自分のページを保持できた。**閉じ込めこそが対策である**（§64.40.2.2）。

**特定できなかったこと**: 何のプロセスが匿名メモリを握ったかは分からない。プロセス表は残らず、
PSI も残らず、`noatime` でファイル追跡も不可、メモリ履歴も未取得（§64.40.7）。
利用者が 13:20 にブラウザを閉じた**約 5 分後**に始まっているので、
**残っていたセッションのどれかが走らせていた処理**、という以上には絞れない。

### 64.40.2.1 EFS の watchdog は「気づいて直そうとして、間に合わなかった」

`/var/log/amazon/efs/mount-watchdog.log`（UTC）:

```
04:03:32  正常な毎時 SIGHUP（ここまでは平常）
05:14:45  Sending signal SIGKILL(9) to stunnel. PID: 4797
05:20:16  Connection timeout for …-claude… after 30 sec, SIGKILL has been sent to the
          potential unhealthy stunnel 4797, restarting a new stunnel process.
05:26:07  Starting TLS tunnel: …
06:01:18  Started efs-proxy, pid: 330952
```

★ **watchdog の判断は全部正しい。** 異常を検知し、殺し、入れ直した。問題は**自分自身の各段が
5〜35 分かかったこと**で、健全なインスタンスならミリ秒である。**検知から復帰まで 47 分**——その間に
NFS のリース期限（`error 110`）が切れ、`/var/lib/af/claude` を触るプロセスは全部 D 状態に落ちた。
⚠️ **カーネルが `nfs: server 127.0.0.1 not responding` を出し始めたのは 03:29(UTC) 頃で、
watchdog が気づいたのは 05:14。**30 秒のヘルスチェックが跳ねるまで 1 時間45分かかっている。

つまり **stunnel が死んだからインスタンスが壊れたのではなく、インスタンスが飢えたから stunnel が死に、
飢えていたから直せなかった**——順序が逆である。

### 64.40.2.2 対策——閉じ込めと、**既に在る計測への警報**

- ★ **ワークスペースのコンテナにメモリのハードリミットを入れる。** 効くのは「暴走を止める」
  からではなく、**追い出し圧力を cgroup に閉じ込めてインスタンスの管理系を守る**からである。
  **仕組みは既に在る**——`MemBytes`（解決済みのワークスペース単位 RAM 上限・`store.go`）は
  docker ランタイムでは使われていて、**ecs-ec2 だけが適用していない**（`Memory: nil`）。
  ⚠️ **ただし `MemBytes` をそのまま入れても足りない。** 利用者が選ぶ「サイズ」がそのままインスタンスの
  サイズ（8 GiB＝m7i.large）なので、上限＝インスタンス全体になり**管理系のための余白が残らない**。
  **「インスタンスのメモリ − 管理系ぶん」**を上限にする必要がある
- **在る計測に警報が無い。** `VolumeReadBytes` / `VolumeQueueLength` / `EBSByteBalance%` は
  最初から CloudWatch に在り、今回の異常は**どれを見ても一目で分かる**（次点の利用者の 55 倍、
  3 時間 不変）。誰も見ていなかっただけである。**足りないのは計測ではなく警報**
- ホストのメモリ計測（CWAgent / Container Insights）**も**あれば見えた（キャッシュではなく
  匿名メモリが数 GB に膨らんでいたはず）。ただし**より早く・より確実に鳴るのは I/O 側**である
...
5. インスタンスを残すなら **`af-role=quarantined` に付け替える**。両方のスイープ walk が
...
猶予を過ぎたとき **EC2 が `running` と言っている**なら、そのインスタンスは遅いのではなく失われている。
`pending` のインスタンスは対象外——起動途中のインスタンスを壊すのは「遅い Start」を「壊れた Start」に変える。
⚠️ 猶予は**時計ではなくポーリング回数**で数える。`sleep` は注入されており、
壁時計の期限にするとスタブしたテストが実時間だけ待つことになる。

**③ `abandonLostSlot`——OS が死んだインスタンス専用のテアダウン**

`quarantineSlot`（mount 失敗用）は流用できない。あちらはまだ何も走っておらずカーネルも
生きている前提で、**このインスタンスに対しては全ステップが静かに間違ったことをする**：

| `quarantineSlot` | 固まったインスタンスで実際に起きること |
|---|---|
| 走っているインスタンスに `DetachVolume` | `detaching` のまま永久に返らない（unmount できない） |
| 通常の `StopInstances` | 誰も聞いていない ACPI shutdown を待ち続ける |
| task ENI が付いたまま停止 | インスタンスが **multi-ENI** で戻り egress を失う（決定 3-3） |
...
無制限だと Start のたびに 1 台ずつ全インスタンスを隔離して「劣化したデプロイ」を「空のデプロイ」に変える。**
...
- **ワークスペースのコンテナに上限が無い**（`Memory: nil`）。`MemBytes` という仕組みは在るのに
  ecs-ec2 だけ使っていない。§64.40.2.2 のとおり、**余白を引いた値**で入れる必要がある
- **在る CloudWatch 指標に警報が無い**（`VolumeReadBytes` / `VolumeQueueLength` /
  `EBSByteBalance%`）
- **swap もゼロ**（zram は「メモリが多すぎる」で自分を無効化している）。swap があれば
  匿名メモリの一部が退避でき、キャッシュ全滅までは行かなかった可能性がある——ただし
  スラッシング先が EBS に変わるだけとも言えるので、上限の方が本筋である
- root は 40 GiB gp3 の既定スループット（125 MiB/s）のまま。§64.31 の「gp3 はサイズ非依存
  なので縮めても遅くならない」は正しいが、**その 125 MiB/s は 1 台の暴走で使い切れる量**でもある
- **原因プロセスは特定できていない**（プロセス表・PSI・メモリ履歴が残らず、root は `noatime`）

### 64.40.7 フォレンジック——隔離したインスタンスをそのまま起動して読む

隔離（`af-role=quarantined`・停止）したインスタンスは **home を外してあるので、起動し直しても
利用者に影響しない**。新しいリソースも増えない。手順:

1. `ec2 start-instances`（**インスタンスは普通に起動した**＝ハードも root FS も無傷で、
   あの固着は完全にランタイム状態だったことの証明でもある）
2. SSM が Online になったら `ssm send-command` で読む
3. 読み終わったら `ec2 stop-instances`

⚠️ **ECS エージェントは起動後に `level=critical … Agent will terminally exit, unable to
register container instance` で終了する。これは正しい**——先に強制 deregister してあり、
インスタンスは隔離済みなので、クラスタに戻ってこられては困る。

⚠️ **`--parameters commands=[…]` の shorthand は改行を食う。** 複数行のスクリプトを送ると
`tail -n 5` が `tail -n 5necho` になって落ちる。**JSON を書いて `file://` で渡すこと。**

**残っていたもの / 残っていなかったもの**

| | |
|---|---|
| ✅ `/var/log/journal` | **存在した**（AL2023 の ECS 最適化 AMI は永続ジャーナル）。事故当時の boot が丸ごと残っていた——**「journal は tmpfs だから停止で消える」は誤り** |
| ✅ `/var/log/amazon/efs/mount-watchdog.log` | 一番価値のあった 1 本（§64.40.2.1） |
| ✅ `/var/log/ecs/ecs-agent.log` | 30 秒周期の warn の**欠落が飢餓の目盛り**になった |
| ✅ `docker inspect`（コンテナは exited で残る） | `Memory=0` / `MemoryReservation=536870912` / `OOMKilled=false` を実測で確認 |
| ❌ プロセス表 | 残らない。**何が読み続けていたかは特定できない** |
| ❌ メモリ使用量の履歴 | `sysstat` も CWAgent も未導入 |

★ **「証拠は消えているはず」と決めつけて読まなければ、誤った原因のまま対策を設計していた。**
実際、この 1 回の起動が「メモリ枯渇」という当初の見立てを覆した（§64.40.2）。

## 64.41 ワークスペースをインスタンスから切り離す——上限・正直なサイズ表示・在る指標への警報（決定 28・2026-08-27）

§64.40 の事故の対策。**直すのは「暴走を止めること」ではなく「暴走がインスタンスの管理系を道連れに
しないこと」**である。利用者自身が自分のワークスペースを重くするのは、この製品では正常な使い方で、
止めるべきものではない。止めるべきは**巻き添え**だけである。

### 64.41.1 三つの層のうち、どれが土台か（実機で判定した）

| | 効くか | 理由 |
|---|---|---|
| **A. 管理系を `MemoryMin` で守る** | ⚠️ 補助まで | **docker の cgroup driver は systemd**（実機確認）なので**コンテナも `system.slice` の下に入る**＝スライスへの `MemoryMin` はワークスペースごと守ってしまい無意味。ユニット個別のドロップインは可能だが、**ECS エージェント自身がコンテナ**（`HostConfig.Memory=0`・`CgroupParent` 空）で transient scope に落ち、systemd のドロップインを当てられない |
| **B. コンテナに `Memory`（cgroup 上限）** | ✅ **土台** | タスク定義の 1 フィールドで**構造的に**成立する。上限超過の回収は**その cgroup の中で**起きるので、管理系は何があってもページを保持する。暴走は cgroup 内で OOM kill され `OOMKilled=true` として**外から見える** |
| **C. 気づく** | B が入って初めて成立 | ⚠️ **監視は監視対象の外に置く必要がある。**事故中エージェントは凍っていたので自己申告は役に立たなかった。B が入れば凍らないので、自己申告が信頼できる情報になる |

★ **B が #1（気づく仕組み）をほぼ丸ごと点灯させる**というのが、この設計で一番おいしいところである。
エージェントは既に `mem_used` / `mem_max` / `cpu_pct` / `oom_kill_total` を返していて、
**`mem_max` は「cgroup が無制限なら意図的に落とす」実装**（`TestUnlimitedMemoryMaxIsOmittedRatherThanHuge`・
docs/63 §63.9 の「測れない軸を 0 で埋めるな」）。だから ecs-ec2 では**メモリだけ分母が無かった**
——CPU は「1 コア = 100%」、ディスクは「/ 49.9G · 19%」なのに、メモリは「3.23G」だけ。
**上限を入れると、この経路が配線済みのまま点灯する。**

### 64.41.2 上限の値——なぜ「1 GiB 引く」では足りないのか

実機で測った数字:

| | |
|---|---|
| ラダーの段（m7i.large）の**公称** | 8192 MiB |
| **実際の `MemTotal`** | **7784 MiB**（公称より 408 MiB＝約 5% 少ない） |
| ホスト管理系・**待機時**（`system.slice` 合計） | **869 MB**（ssm-agent 170M / docker 114M / containerd 84M が大半） |
| ワークスペース稼働時に上乗せ | ECS エージェント本体＋Service Connect の envoy 3 本＋efs stunnel 2 本 ⇒ **合計 1.2〜1.4 GB** |

⚠️ **CP はタスク定義を組む時点で公称しか知らない。** 実数（`DescribeContainerInstances` の
`registeredResources[MEMORY]`＝7783）は取れるが、**タスク定義はスロットの登録確認より前に組まれる**
ので、使うには `launch()` の順序を変えることになる。予備を厚めに取れば吸収できる精度なので採らない。

だから予備は**公称の 1/5**（上下でクランプ）にする:

```
公称 8192 → 予備 1638 → 上限 6554 MiB   実headroom 7784-6554 = 1230 MiB  ✅
公称 8192 → 予備 1024 → 上限 7168 MiB   実headroom 7784-7168 =  616 MiB  ✗ 足りない
```

- **床 1024 MiB**: 小さい段で「1/5」だと管理系が入らない
- **天井 2048 MiB**: 管理系の費用はインスタンスの大きさで増えないので、大きい段が比例して差し出す必要は無い
  （32768 の 1/5 は 6.5 GiB＝捨てすぎ）
- 段が小さすぎて分け合えない（残りが 512 MiB 未満）なら**上限を付けない**。
  使えないワークスペースを作るより、その配備を今までどおりにしておく方がましである

`AF_ECS_EC2_HOST_RESERVE_MB` = `auto`（既定）| `off` | `<MiB>`。
⚠️ **読めない値は auto にフォールバックする**——このつまみは「利用者が使えるメモリ量」を決めるので、
...
⚠️ **タスクではなくコンテナに付ける。** タスク単位の上限は ECS が注入する Service Connect の
サイドカーぶんまで含むことになり、その大きさは製品が予測してよいものではない。

### 64.41.3 サイズ表示は正直にする（数が 2 つになったから）

上限が入る前は「8 GiB」がインスタンスとワークスペースの両方を指していて、**1 つの数で正直だった**。
入った後は違う数になるので、インスタンスだけを出すと**cgroup が渡さないメモリを本人に約束する**ことになる。

- `workspaceSlot.UsableMemMiB`（`usable_mem_mib`）を足す。**上限が無い配備では省く**——
  そこではインスタンスが答えそのものなので、2 つ出す方が嘘になる
- Console は **本人が使える方を先に、インスタンスを括弧で**（`admin.ws_slot_usable` = `{n}（インスタンス {box}）`）。
  名簿の行（`MemberSizeChips`）と「→ 乗るインスタンス」の行（`slotSpec`）の 2 か所

### 64.41.4 運用者向けの警報は「在る指標」に付ける

⚠️ **足りないのは計測ではなく警報である。** 今回の異常は
`VolumeReadBytes` / `VolumeQueueLength` / `EBSByteBalance%` の**どれを見ても一目**だった
（次点の利用者の 55 倍・3 時間ぶん同一値）。**誰も見ていなかっただけ**である。

⚠️ **CP のスイープに CloudWatch を読ませる案は採らない。** CP タスクロールに権限を足すことになり、
候補の中で一番重く、しかも**製品の中に「AWS の指標を解釈する層」を新設する**ことになる。
警報は substrate（CFN）の仕事にする。

## 64.42 0.17.0 を本番配備へ適用する（実測・2026-09-09）

`update.sh`（`ImageTag` だけを上書きする経路）で 0.16.0 → 0.17.0。手順そのものは
§64.28 で焼き付けた形から変わっていない。記録しておく価値があるのは**適用が終わった
ことをどう確かめたか**の方である。

| 確かめたこと | 実測 |
|---|---|
| CP が新しい版で走っている | タスク定義 `…-ingress-cp:**27**`・`desired 1 / running 1` |
| タスク定義が指す先 | `af-control-plane:0.17.0`（ECR へ push 16:25:40） |
| その digest | `sha256:d0132c2942…`（ECR 側と一致） |
| ワークスペース像 | `af-workspace:0.17.0` `sha256:342846e0dd…`（push 16:28:09） |
| golden home の焼き直し | **両アーキ**（`arm64` / `x86_64`）を 16:36 に取り直し |

⚠️ **タグではなく digest を突き合わせる。** タグは動く名前で、`update.sh` が
`ImageTag` を書き換えた直後は「タスク定義は新しいタグを指しているが、ECR のそのタグが
まだ古い像を指している」が成立しうる。一致を主張してよいのは digest を見たときだけである。

⚠️ **golden は両アーキ焼く。** スロットのクラス表に Graviton が入っている配備
（§64.43.1）では、片アーキだけ焼き直すと**もう片方の初回利用者だけが前の版で home を作る**。
症状は「特定の人だけ古い」で、原因から一番遠いところに出る。

### 64.42.1 稼働中のワークスペースは次の Stop→Start まで前の版のまま

適用時に走っていた 4 ワークスペースは 0.16.0 のままである。これは**意図した挙動**で、
版を上げるためにタスクを置き換えると作業中の利用者を落とすことになる。
「配備は 0.17.0 なのに本人の画面は 0.16.0」は事故ではないので、
バックエンドのずれを示すバッジ（本人が Stop→Start を選べる導線）以上のことはしない。

## 64.43 推論エンジンを新規に配備する（ADR 0071 / 0072・実測・2026-09-09）

セルフホストの推論エンジンを本番配備へ初めて載せた。設計は
[ADR 0071](../decisions/0071-self-hosted-inference-engines.ja.md)、モデル台帳は
[ADR 0072](../decisions/0072-engine-model-catalog.ja.md)。ここに残すのは**実配備で
初めて測れたこと**だけである。
...
| 時刻 | したこと |
|---|---|
| 16:52:27 | 20-platform に ECR を 2 本追加（`af-llamacpp` / `af-sdcpp`） |
| 16:53:13 | エンジンの API キーを SSM（SecureString）へ |
| （前後） | エンジン像を GHCR から ECR へ `crane copy` |
| 17:09:39 | **60-engines 1 パス目**（スタック作成） |
| 17:17〜17:21 | 画像モデル 2 本を取り込み（§64.43.2） |
| 18:23〜18:28 | LLM モデルを取り込み（§64.43.2） |
| 18:30:19 | **60-engines 2 パス目**（更新） |
| 18:33:13 | エンジン表 `/<prefix>/engines` を書き出し |
| 18:38:41 | Console からモデルを有効化（画像側だけ・§64.43.7） |

**60-engines が 2 パスに分かれるのは設計どおり**である。1 パス目でモデル置き場（S3）と
取り込みタスク定義ができ、そこへモデルを入れてからでないと 2 パス目の
サービス／キャパシティプロバイダが意味を持たない。

⚠️ **`standup.sh` は使っていない。** 使えなかった、というのが正しい。この配備の捕捉
（`capture-env.sh` が書く「積み直すのに要る引数一式」）は **0.10.0 の頃のまま**
取り残されていた——以後の版上げが `ImageTag` だけを上書きする `update.sh` だったからである。
`standup.sh` は捕捉済みの params を `--parameter-overrides` で渡すので、
**素で回すと live を捕捉時点へ巻き戻す**。実測した差は次のとおりだった。

| パラメータ | 捕捉（0.10.0 当時） | live |
|---|---|---|
| `ImageTag` | 0.10.0 | 0.17.0 |
| `Ec2MaxSlots` | 6 | 30 |
| `Ec2SlotTypes` | arm クラス無し | arm（Graviton）クラス有り |
| `Ec2SlotAmiArm64` | 空 | 設定済み |
| `Ec2SlotLaunchTemplate` | 前世代の LT | 現行の LT |

つまり素で回せば **CP を 7 マイナー版ダウングレードし、同時実行枠を 30→6 に絞り、
Graviton スロットを消す**。稼働中の利用者がいる配備で、である。

🔴 **さらに悪い形が隠れていた。** 捕捉には当時のテンプレートにしか無い
`BitbucketOauthKey` が残っていて、逆に現行テンプレートが持つ **14 個のパラメータが
捕捉に無かった**（`EnginesSsmParam` / `VoicevoxUrl` / `CpArch` / `PreviewDomain` /
スケジューラ系など）。**捕捉の古さは値のずれだけではなく、パラメータ集合そのもののずれ**として
現れる。無いパラメータはテンプレート既定値に落ちるので、被害は表のような
「見える巻き戻し」だけでは済まない。

だからこの日は**`aws cloudformation deploy` を対象を絞って呼んだ**。渡さなかった
パラメータが `UsePreviousValue` で保たれることを、毎回 changeset の `Parameters` で
確認してから実行する（`ImageTag` と `Ec2MaxSlots` の 2 つを見れば足りる）。

⚠️ **捕捉を取り直しても `standup.sh` を本番で使う理由にはならない。** 取り直しは
「事故ったときの被害を小さくする」ためであって、解禁のためではない。

### 64.43.2 Hugging Face の配信はファイルによって 1 桁違う（実測）

同じ取り込みタスク・同じインスタンス・同じ 1 時間の中での実測である。

| モデル | 大きさ | HF から取得 | 取得速度 | sha256 検証 | S3 へ書き出し |
|---|---|---|---|---|---|
| SDXL base 1.0 | 6.46 GiB | **172 s** | 38.5 MiB/s | 52 s | 45 s |
| Juggernaut-XL v9 | 6.62 GiB | **165 s** | 41.1 MiB/s | 54 s | 48 s |
| Qwen3-Coder-30B-A3B Q4_K_M | 17.28 GiB | **4,023 s（67 分）** | **4.4 MiB/s** | 139 s | 139 s |

**取得速度で 9.3 倍、壁時計で 24 倍**の差である。

★ **これが「大きいファイルだから遅い」ではないことは、同じログの中で証明できる。**
sha256 の検証速度は 3 本とも **127 MiB/s でぴたりと揃っている**（6.46 GiB/52 s、
6.62 GiB/54 s、17.28 GiB/139 s）。S3 への書き出しも 127〜147 MiB/s で揃う。
**インスタンスもディスクも 3 本に対して同じ速さで働いていた**——遅かったのは HF の配信だけである。
サイズに比例していたのならハッシュも書き出しも同じ比率で遅くなっていなければおかしい。

⚠️ **運用上の意味:** LLM モデルの取り込みは**時間で見積もってはいけない**。同じ
17 GiB でも配信元の都合で 10 分にも 70 分にもなる。取り込みは非同期の使い捨てタスクで
回し、進捗はログで見る（この日は 67 分間、他の作業を止めずに済んだ）。
そして**モデル置き場の S3 は teardown で Retain にしてある**理由がこれである——
捨てれば取り戻すのに 70 分かかる、しかも取り戻せる保証が無い。

### 64.43.3 image エンジンのコールドスタート内訳（実測）

`desired 0` から利用者の 1 枚目までを、サービスイベントとコンテナのログで割った。

| 時刻 | 経過 | 何が起きたか |
|---|---|---|
| 18:41:24 | 0 s | ECS がタスクを起動（この前に MI が GPU インスタンスを確保している） |
| 18:42:22 | +58 s | 取り込み側が有効モデル（`juggernaut-xl-v9`）を読む |
| 18:43:28 | +124 s | **S3 から 6.62 GiB を 66 s で取得**（102.7 MiB/s） |
| 18:43:46 | **+142 s** | `listening on 0.0.0.0:8080`——ここから応答できる |
| 18:44:05 | +161 s | 1 枚目 `generate_image 512x512` |
| 18:44:29 | +185 s | **完了（4 枚で 24.15 s）** |
| 18:44:35 | +191 s | 2 回目は**温まっていて 5.56 s**（1 枚） |

- ✅ **モデルは L4（VRAM 22.5 GiB）に全部載った**——`total params 6624 MB` が
  すべて VRAM、RAM 0.00 MB。ディスクへの退避が起きていないので、
  2 回目以降が 5.56 s まで落ちる。
- **S3 →インスタンスが 102.7 MiB/s** に対し、HF →インスタンスは 41 MiB/s だった（§64.43.2）。
  一度 S3 に入れてしまえば取り回しは 2.5 倍速い、という当たり前の裏付けでもある。

### 64.43.4 自動停止は実測で効いた（idle 900 s）

| 時刻 | |
|---|---|
| 18:44:35 | 最後の生成が完了 |
| 18:59:35 | idle 900 s の満期（計算値） |
| **18:59:49** | ECS がタスクを停止——**満期の 14 秒後** |
| 19:01:45〜19:03:25 | コンテナインスタンスの登録が消える（**ドレイン 3.5〜4 分**） |
| 19:07:36 までに | EC2 が `terminated`（サービス 0 から**約 8 分**） |

✅ **満期 +14 秒**は掃除の周期ぶんで、設計どおりである。
⚠️ **課金が止まるのは「サービスを 0 にした時刻」ではない。** インスタンスが実際に消えるまで
さらに 8 分ある。$1.26/時なので 1 回あたり約 $0.17——気にする額ではないが、
**「止めた」と「消えた」を同じ時刻だと思って観測すると必ず食い違う**。

### 64.43.5 🔴 `ec2 describe-instances` は MI のインスタンスを「一覧しないが、ID で引けば返す」

ADR 0071 の起票時に「`describe-instances` は Managed Instances のインスタンスを 1 台も列挙しない」
と書いたが、**これは半分しか正しくない**。同じ 1 秒の中で 2 通りの呼び方を実測した
（対象は engines の GPU インスタンス・`g6.xlarge`）。

| 呼び方 | 結果 |
|---|---|
| `describe-instances --instance-ids i-…`（**ID 指定**） | ✅ **返る**（`g6.xlarge` / `terminated`） |
| `describe-instances`（**無指定の一覧**） | ❌ **出てこない**。15 台すべてスロットインスタンスで、GPU クラスは 1 つも無い |

つまり**レコードは存在するのに一覧には載らない**。これが罠として悪質なのは、
運用者が自然にやるのは一覧の方だからである——「GPU インスタンスが無い＝止まっている」と
読んでしまい、**実際には課金され続けているインスタンスを見落とす**。ID を知っていれば引けるが、
**見落としているときに限って ID を知らない**。

✅ **稼働を判定する正しい呼び方は ECS 側である。**

```
aws ecs list-container-instances --cluster <cluster>
aws ecs describe-container-instances --cluster <cluster> --container-instances <arns> \
  --query 'containerInstances[?capacityProviderName!=`null`]'
```

`capacityProviderName` が付いているコンテナインスタンスが MI のインスタンスで、
これは一覧に必ず出る。スロットインスタンス（`capacityProviderName` が `null`）と
...
### 64.43.8 🔴 起動メニューが空だったのはエンジンのせいではなかった——Bedrock の列挙が 45 秒（実測・2026-09-09）

§64.43.6.1 で opencode に `llamacpp` の provider が書かれたあとも、**起動メニューには
モデルが 1 つも出なかった**（「既定のモデルのみ利用できます（接続とプランをご確認ください）」）。
llm だけでなく**標準のモデルも全部**出ない。エンジンの配備とは無関係の、独立した不具合だった。

**画面の文言が犯人を隠していた。** 実機のワークスペースで 1 行叩いて決着した:

```
$ time opencode models
… amazon-bedrock/… が大半 …
llamacpp/qwen3-coder-30b-a3b
real 0m45.047s   user 0m18.794s   sys 0m2.132s
```

`Models()`（`workspace/agent/internal/agents/opencode/models.go`）の予算は **10 秒**である。
45 秒かかれば毎回殺され、エラー経路は nil を返し、**画面は「既定のみ」になる**。
ログに残っていた `GET /agents/opencode/models` は **8 回すべて 10.03〜10.08 秒**だった
——この揃い方こそが「タイムアウトが毎回発火している」の証拠である。

**原因は opencode 組み込みの `amazon-bedrock` provider。** AWS SDK の資格情報チェーンが
**解決しさえすれば**有効になり、ECS のタスクには必ず
`AWS_CONTAINER_CREDENTIALS_RELATIVE_URI` が入るので**チェーンは必ず解決する**。
別コンテナで条件を作って確かめた:

| 資格情報チェーン | `opencode models` の件数 | うち `amazon-bedrock/` |
|---|---|---|
| 解決する | **158** | **151** |
| 断つ（＋`AWS_EC2_METADATA_DISABLED`） | **7** | **0** |

🔴 **0.17.0 に入れた `keepInMenu` では直らない。** あれは `Catalog()` が整形する一覧から
`amazon-bedrock/*` を落とす修正で、**この列挙が終わったあと**に効く:

```go
list = opencode.Catalog(opencode.Models(), uiprefs.OpencodeCatalog())
//                      ~~~~~~~~~~~~~~~~ ← ここで 45 秒。10 秒で殺されて nil
//     ~~~~~~~~~~~~~~~ ← keepInMenu はこの中。渡す一覧が無いので到達しない
```

同じ根（Bedrock の自動有効化）から出た**別の症状**で、前の修正は前の症状しか塞いでいなかった。

⚠️ **タイムアウトを 15 秒へ上げても直らない。** 45 秒のうち **18.8 秒が CPU** なので、
通信が速くても 20 秒は下回らない。Console は起動モーダルを開くたびにこれを呼ぶので、
その予算は正当化できない。**列挙を安くする以外に解は無い。**

### 64.43.8.1 直したこと——チェーンを断つ。そして二度と無言で失敗させない

`Models()` が組む子プロセスの環境から資格情報チェーン（`AWS_CONTAINER_CREDENTIALS_*` /
`AWS_ACCESS_KEY_ID` 系 / `AWS_PROFILE` / `AWS_ROLE_ARN` …）を落とし、
**`AWS_EC2_METADATA_DISABLED=true` も足す**——IMDS は環境変数ではないので、ecs-ec2 では
スロットのインスタンスプロファイルが答えてしまい、コンテナ側の変数を消すだけでは足りない。

- **失うものは無い。** `WsTaskRole` は意図的にポリシーゼロで `bedrock:InvokeModel` は
  AccessDenied、AF はセッションにも serve デーモンにも `AWS_PROFILE` を出していない。
  **このワークスペースは元から Bedrock を使えない。**
- `AF_OPENCODE_SHOW_BEDROCK=1` は**ここでも尊重する**。片方だけで見ると、フラグが
  「メニューには戻すが、コマンドはもう見えていない」という噛み合わない状態になる。
- ⚠️ **効果範囲はこの 1 プロセスだけ。** 起動されるセッションの環境は別で組まれるので、
  利用者自身の AWS 操作には触れない。

そして**無言をやめる**。`cmd.Output()` は stderr を捨てていて、エラー経路は nil を返すので、
**「0 件」と「失敗」が画面上まったく同じ**だった。stderr を握り、所要時間つきでログに出す。

★ **この無言のせいで、画面は「接続とプランをご確認ください」と言い続けた。** 利用者は
接続とプランを疑い、私はアーキ・通信・CPU・キャッシュ・API キーを順に潰した——
**どれも外れで、答えは実機で 1 行叩けば 1 分で出た。** リモートから状態を眺めるより、
**その場で 1 回動かす方が速い**という、この文書で何度目かの同じ教訓である。
