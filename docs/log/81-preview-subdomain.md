...
🔥 **【2026-09-01・実機】`destination` をプレビューの URL に向け直す、という間違え方がある。**
「フロントから 8080 が見えないのでは」と考えて `https://{slug}-8080.{domain}` を書くと、
**全 API が 401 `preview requires sign-in` になる**。rewrite は**コンテナの中の dev サーバが
サーバ側から出す通信**で、ブラウザの `af_pv` cookie を持っていないためである。

★ **§2.2 の表と §11 は「直書きするな、proxy に寄せろ」までしか言っておらず、
「寄せた先を書き換える」という間違え方を塞げていなかった。** 正しくは
**`destination` は `127.0.0.1:{port}` のまま**——プレビューの URL は**ブラウザが使う入口**で
あって、コンテナの中から自分自身を呼ぶための名前ではない。案内に追記した。
...
🔥 **【2026-09-01・実機で決着】通らなかった。ただし原因は Upgrade の中継ではなく (d) の
`allowedDevOrigins` である。** CP / Agent の WebSocket 中継は正しく動いており、dev サーバが
**そもそも接続を拒んでいた**。`allowedDevOrigins` を設定すると設定なしの想定どおりに繋がる。
★ **「HMR が繋がらない」を Upgrade の問題だと決めつけて CP 側を疑うと、いくら見ても何も
出てこない。** 最初に見るのは dev サーバのログである（§13）。
...
#### (d) 🔥 Next 15.2 以降の `allowedDevOrigins` —— 確認項目ではなく、必須設定だった
...
🔥 **【2026-09-01・実機（acrt / Next 16.2.10 / Turbopack）で確定】この見込みは外れた。**
判定材料は `Host` ではなく **`Origin`** 側で、`Host` を内部アドレスに書き換えていても
**素通りしない**。設定は必須である。
...
```ts
// next.config.ts —— slug は起動のたびに変わるので、ワイルドカード以外に書きようが無い
allowedDevOrigins: ["127.0.0.1", "*.pv.example.com"],  // ドメインは AF_PREVIEW_DOMAIN
```

★ **本当の問題は、症状が「ブロックされた」ように見えないことである。** HMR が張れず
ハイドレーションが完走しないため、**クライアント側の fetch まで到達しない**。画面は
**レイアウトのシェルだけが描画され、API から取るデータ行が 0 件**——真っ白でもエラーでも
ないので、**利用者には自分のアプリのバグにしか見えない**（`127.0.0.1:{port}` で開くと正常、
という切り分けだけが唯一の足がかりになる）。**§11 の「AF 専用のコードを要求しない」に
真正面からぶつかる 1 行**なので、案内（`guide/member/10`・`11`）に明記した。

⚠️ **再現にプレビュー URL もログインも要らない。** ホスト名だけ偽装すれば dev サーバから見て
別オリジンになる:

```
chromium --headless --no-sandbox \
  --host-resolver-rules="MAP {slug}-3100.{domain} 127.0.0.1" \
  --virtual-time-budget=15000 --dump-dom "http://{slug}-3100.{domain}:3100/"
```
...
- 🔥 **停止中に押すと、成功しても画面は何も変わらない**（発行済みの slug が無いので捨てる先が
  無く、予約を捨てるだけ）。**これは「押しても無反応」として報告された** —— 実際にはサーバは
  200 を返していた。応答に `previewReissued`（その場で引き直したか）を足し、Console は
  **どちらだったかを必ず言う**ようにした。★ 成功したのに何も起きない操作は、**黙っていると
  故障として報告されて返ってくる**。
...
★ **節の先頭に「いまの URL」を置く。** 設定側にこれが無いと、公開ポートの列挙も再発行も
**見えない何かに効く操作**になり、押しても何が変わったのか分からない（実際に再発行が
「無反応」として報告された）。

- 発行済みなら**ポート順に**並べる（キー順に任せると 8080 が 3000 より前に出る）。
- **停止中でもドメインだけは出す** —— 「どのドメインに割り当てられているか」は URL が
  発行されていなくても分かってよい情報で、ここの設定の前提そのものである。ポップオーバー
  側の「発行されていない URL は見せない」（押すと 404 になるリンクを出さない）とは別の話で、
  出しているのは**リンクではなく形**である。
...
| **P4 実機と案内** | ⏸ | `docs/guide` の二言語更新は ✅（member/08・09・README・用語集）。**Next.js の通し確認は ✅**（2026-09-01・acrt 実機。§13.1 —— 2 件の落とし穴が出たので案内に追記した）。⏸ **Vite は残** |
...
### 13.1 決着したもの（2026-09-01・acrt 実機）

**Next.js（16.2.10 / Turbopack）を実際のアプリで通した。** §2.5 の 4 点のうち 3 点が片付き、
**そのうち 1 点は「見込みが外れた」**（§2.5 (d)）。

| | 結果 |
|---|---|
| ① HMR の WebSocket が設定なしで繋がるか | ❌ **繋がらない。** ただし CP / Agent の Upgrade 中継は正常で、**dev サーバが Origin で拒んでいた**（③ と同じ原因） |
| ② Server Actions が 403 にならないか | ✅ ならない（§12.1 (1) の入れ直しが効いている） |
| ③ `allowedDevOrigins` に引っかからないか | ❌ **引っかかる。`Host` を内部アドレスにしても素通りしない＝必須設定**（§2.5 (d)） |
| ④ App Router のストリーミングが白画面にならないか | ✅ ならない（`FlushInterval: -1`） |

★ **この 2 件の教訓は同じ形をしている: 症状が原因を指していない。**
①③ は「HMR が効かない」ではなく **「画面の枠だけ出てデータが 0」** として出るので、
**利用者にはアプリのバグに見え、開発側は CP の WebSocket を疑う**——両方が外れる。
もう 1 件（`rewrites()` の `destination` をプレビュー URL に向けると全 API が 401・§2.5 (a)）も、
**案内が塞いだつもりだった穴の隣**に空いていた。どちらも `guide/member/10`・`11` に追記した。
⚠️ 11 は見出しが「HMR が効かない」だけだったので、**症状名（枠だけ出てデータが出ない）を
見出しに足した**——症状名で引けない案内は、書いてあっても無いのと同じである。
...
- **Vite**: HMR がプレビュー越しにどのホスト / ポートへ繋ぎに行くか（§2.3）。
  ⚠️ **Next で決着したことは Vite の答えにならない**（Vite の HMR は接続先をクライアントに
  埋め込む＝壊れ方の原因が別）
...
- → Vite の HMR・ALB のアイドルタイムアウト・WAF の 3 件は Issue #968 に起票（2026-09-24）
