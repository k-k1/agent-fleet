...
> 🔴 **この判定は誤りだった。§89.10 で「どちらが新しいか」から「どのレコードに対して取ったか」へ
> 変えた**（CI 赤・レビュー指摘）。**クリアしない**という方針と、以下の 3 点の利点はそのまま
> 引き継いでいる。「同時刻は無効側に倒す」だけが逆で、**同時刻は有効でなければならない**——
> 実際の観測は必ず「読んだ直後」に書かれる。
...
## 89.10 🔥 有効判定が mtime の「分解能」に依存していた（CI 赤・修正済み）

- 状態: **修正済み**（2026-09-10、PR #467 の CI 赤を受けて）。指摘は scqwtat。
- 症状: `TestPolledTurnEndStillReportsExactlyOnceAfterTheListingRecordedIt` が
  「poll 0: the listing did not record the end」で落ちる。**手元は緑、CI だけ赤。**

### 気まぐれではない

§89.9 の `ObservedTurnEnd` は「**観測ファイルが status ファイルより新しいときだけ有効**」だった。
これは mtime の**順序**ではなく**分解能**に依存している。

- 手元では 200 回の連続書き込みで tie 0 回・最小差 100.71µs（指摘元の実測）。だから緑。
- 両方の mtime を `Truncate(time.Second)` して 1 秒粒度を模擬すると、**CI と同一のテスト・同一の
  行・同一の文言**で落ちる。こちらでも再現した。

**2 つの書き込みは同じ working→idle 遷移で誘発される**ので、間隔は原理的にミリ秒である。粒度が
その間隔より粗い環境では `!at.After(stAt)` が毎回真になり、観測は**毎回捨てられる**。
`TurnEndUnrecorded` が `""` を見て次のポーリングで撃ち直すので自己修復はするが、**終端直後の
1 回は原理的に記録できない**——一覧しか見ない親にとっては「1 ポーリングぶん遅れる」が既定になる。
§89.8 で塞いだはずの遅れが、粒度の粗い環境でだけ半分戻っていた。

### 「どちらが新しいか」から「どのレコードに対して取ったか」へ

`SessionStatus` に **`Rev`（乱数のノンス）** を足し、`persist` が**書き込みのたびに新しい値を打つ**。
観測ストアは `{rev, at}` を持ち、`ObservedTurnEnd` は **`rec.Rev == st.Rev` のときだけ**有効とする。
時計もファイルシステムも関与しない。

- **クリアしない方針は不変。** レコードを書けば `Rev` が変わるので、次のターンの `working`・
  settle・heal が観測をその場で失効させる。2 ファイルにまたがる原子性は要らないまま。
- **同時刻が有効になる。** 実際の観測は「レコードを読んだ直後」に書かれるので、これが常態である。
  §89.9 が「同時刻は無効側に倒す」と書いたのは、**この常態を誤って捨てる**判定だった。
- **カウンタではなくノンス**である。カウンタは加算＝read-modify-write で、§89.9 がまさに潰した形。
- **`TS` は識別子にならない。** RFC3339 は秒粒度で、2 つの書き込みは秒内に収まる。§89.9.1 で
  自分の陽性対照が同じ粒度に隠れたのを見ているのに、production 側の判定でそれを繰り返していた。
- `Rev` が空のレコード（この変更より前に書かれたもの）には**観測を紐付けない**。読み戻せない観測を
  書くと「記録済み」の判定も満たさないので、**毎ポーリング書き直す**ことになる。

### 89.10.1 検証

`(cd workspace/agent && go test ./...)` 全緑（37 パッケージ、exit code はパイプを通さずに取得）。

**まず再現側を固定した。** `fstore.Store.ModTime` が返す時刻を `Truncate(time.Second)` する変異を
入れると:

| 判定 | 粗い mtime での結果 |
|---|---|
| §89.9 の「どちらが新しいか」 | **CI と同じ赤**（`…StillReportsExactlyOnce…` の poll 0、同じ文言） |
| §89.10 の識別子 | **緑**（mtime を一切読まない） |

試験は**保証の範囲に合わせて緩めたのではなく、保証のほうを環境非依存にした**。「1 回目の
ポーリングで必ず記録される」は、識別子で判定する限りどの環境でも成り立つ。新規 3 本:

| 試験 | 何を固定するか |
|---|---|
| `TestAnObservationIsRetiredByIdentityNotByTiming` | 次のターンの `working` を**sleep なしで**書く。内容も秒も mtime tick も同じでよい——それでも失効すること |
| `TestAnObservationCountsInTheInstantItWasTaken` | 逆側。**同じ瞬間に取った観測は有効**でなければならない（CI 赤の正体） |
| `TestARecordWithNoIdentityIsNeverObserved` | `Rev` が無いレコードには紐付けず、**ファイルも書かない** |

#### 陽性対照

| 壊した箇所 | 落ちたテスト |
|-----------|------------|
| 🔥 §89.9 の mtime 判定を戻す（＋粗い mtime） | `…StillReportsExactlyOnce…`（**CI と同一の行・文言**） |
| `persist` が書き込みごとに `Rev` を打たない | 上記 3 本＋経路試験 3 本ほか計 8 本 |
| `ObservedTurnEnd` が `Rev` を比較しない | `…RetiredByIdentityNotByTiming`・`…AdoptsTheObservedEnd` |
| `RecordTurnEnd` が `Rev` の無いレコードにも紐付ける | `TestARecordWithNoIdentityIsNeverObserved` |
| `RecordTurnEnd` を read-modify-write に戻す | §89.9 の race 試験ほか計 6 本 |

#### 🔴 ここでも陽性対照が「対照になっていない」を 1 件見つけた

「`Rev` の無いレコードにも紐付ける」が**最初は緑だった**。`ObservedTurnEnd` 側も空 `Rev` を弾く
ので、**書いた観測が読み戻されないだけ**で誤った時刻は出ないためである。だが弾かれる観測を書くと
