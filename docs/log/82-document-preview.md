...
## 82.6 🔴 訂正（2026-09-15）: `doc:check` の 1 件目が稀に落ちる（`goto` は着地を待っていなかった）

CI で `console` ジョブだけが赤になり、`sample.docx converts to Markdown — missing: 四半期レポートの
本文, …` の 1 件だけが NG（他 8 件は緑、変更は `translate.ts` のみで無関係、再実行で緑）。
**指紋はステップの所要時間**——正常 **≒1.8 秒**が、こけた回は **16.7 秒**＝`until(..., 150回, 100ms)`
の 15 秒ぶん丸ごと。結果行は `report()` が最後にまとめて出すので**行の時刻では分からない**。

原因側で 2 つ直した。

1. **`goto` が「ナビゲーションを投げた」だけで返っていた**（`Page.navigate` は**発行**で解決する）。
   ブラウザが最初のタブを立ち上げている最中に投げた 1 発目は落ちることがあり、その場合ページは
   `about:blank` のまま＝以降の待ちは**前の文書**に対して満了まで空回りする。1 件目だけが落ちて
   残りが緑、という観測とも合う。`goto` を「**頼んだ URL の文書になるまで待ち、ならなければ 1 回だけ
   投げ直す**（壁時計 5 秒 ×2）」に変更（`scripts/lib/headless.mjs`）。
   - 🔥 実測でわかった落とし穴 2 つ: **`Page.navigate` は、接続だけ受けて何も返さないサーバ相手には
     永遠に応答しない**（2 分観測）。await すると検査ごと止まるので**投げっぱなしにして、拒否だけを
     持ち帰る**形にした。同じ状況で **`Runtime.evaluate` も応答しない**ので、現在地の問い合わせには
     250 ms の上限を付けた（答えない＝「まだ着いていない」であって失敗ではない）。
   - 回帰確認: 応答しないサーバへ `goto` → **10.2 秒で「the page never became …」**、閉じている
     ポートへ `goto` → **199 ms で `net::ERR_UNSAFE_PORT`**。
2. **NG の文言に原因が無かった。** `missing: …` だけでは「変換結果が違う」のか「`[data-md]` が
   そもそも出ていない」のかが読めない。判定の待ちを**終端状態**（本文 or 「ダウンロード」を含む
   失敗表示）に変え、本文が空のときは**画面の文言・`.wasm` の配信記録・例外**を詳細に出すようにした
   （`scripts/doc/check.mjs`）。配信記録は pdf:check と同じ理由で、**ページが何を取りに行ったかは
   DOM に出ない**から必要になる。
   - 陽性対照: 束ね後の `www` から `.wasm` を消すと
     `the pane says "(この文書を変換できませんでした) …" — /anydoc_wasm_bg.wasm 404` と出る。
3. ついでに `serveDir` の `createReadStream` に **error ハンドラ**を付けた（無いと読み取り失敗で
   レスポンスが開いたまま＝ページは永遠に待ち、検査は原因の無いタイムアウトになる）。

`doc:check` 3 連続・`pdf:check`・`viewer:font` いずれも緑。
