...
{"ts":"2026-09-20T10:04:11+09:00","ev":"birth","name":"sage35s","kind":"claude","repo":"agent-fleet",
 "origin":"session","originSession":"sxmzm4b","display":"ADR 0096 起票"}
...
{"ts":"…","ev":"instruct","from":"operator:<conv>","to":"sage35s","source":"operator","excerpt":"…"}
{"ts":"…","ev":"report","from":"sage35s","to":"operator:<conv>","kind":"answer-ready"}
...
## 101.5 決めていないこと（実装前に決める）

1. **`ForkFrom` の逆引き**。会話 id → セッション名の表が要る（101.1）。`Meta` を全部読めば作れるが、
   prune 済みの親は引けない。系譜台帳の birth 行に**その時点の会話 id を焼く**のが素直か。
2. **窓の既定**。24 時間で足りるか、それとも「最後に何かが起きた時刻から遡る」ほうが実用か。
   実際に図を見てから決める（利用者の Q3 の答え「今回は決めない」に対応する枠の話ではなく、
   窓の既定値の話）。
3. **書き込みのバッファリングの要否**（101.3 の ⚠️）。
4. **`operator:<conv>` レーンの見せ方**。オペレーター会話もレーンとして 1 本描くのか、
   図の外（左の帯）に置くのか。0027 の吸収（会話 id で絞り込む）はどちらでも成立する。
5. **`archived` セッションの扱い**。一覧からは消えるが台帳には残る。既定で描くか、トグルか。
