...
## 101.14 実機のスマホでスクロールが完全に止まった（2026-09-22・claude opus 5）

「スマホで拡大縮小スクロール繰り返していたら、全くスクロールができなくなった」。**復帰しない**。

### 真因: 「上がった」と報告されない指が 1 本、帳簿に残る

タッチのポインタは **pointerdown した要素へ暗黙にキャプチャ**される。そして**この図は、その
キャプチャ先を自分で消している**——稼働帯の React キーは**窓で切った開始・終了時刻**を含むので、
ピンチでズームするたびに指の下の `<rect>` が作り直される。pointerup は**文書から外れたノード**へ
配送され、ビューには届かない。

漏れが 1 本あると、以降の **1 本指のドラッグが「2 本指」に見える**。ピンチ枝に入り、片方は
動かない固定点なので比がほぼ 1 のまま＝デッドゾーンで `return`——**何も起きない**。
ズームもスクロールも死に、リロードするまで戻らない。

### 直し方（漏れは防げないので、無害化する）

| 番人 | 何を保証するか |
|---|---|
| `isPrimary` の pointerdown で帳簿を空にする | 主ポインタが降りたら新しいジェスチャ＝残りは定義上すべて古い |
| ピンチ開始で両指をキャンバスにキャプチャ | 以降の配送が、再描画される子要素から外れる |
| `STALE_POINTER_MS` 超過の指を新ジェスチャ開始時に捨てる | 取りこぼしの最後の受け皿 |
| ピンチとして測れなければ**パンに落ちる**（`return` しない） | 「名前が付かないジェスチャ」でも必ず何かが動く |
| `.fgraph-body { touch-action: pan-y }` | 図の中のどこからもページズームが始まらない |

🔥 **`return` が張本人。** 漏れそのものは「片方の指が動かないだけ」だが、`return` がそれを
**「図が完全に無反応」**に変えた。**利用者に出る症状が「壊れている」しか無い分岐を作らない。**

🔥 **ページズームの逃げ道も塞いだ。** `index.html` の viewport meta に `user-scalable=no` は無く、
`app/viewport.ts` は**ページのピンチズームが起きる前提で書かれている**。`touch-action` は祖先方向に
積で効くので `.fgraph-body` に `pan-y` を置く。キャンバスだけ `none` だと、**430px の画面で 168px を
占めるラベル列**に指がかかった 2 本指ジェスチャを図が引き取らず、ブラウザがページを拡大する
——その後は指のドラッグが視覚ビューポートを動かすので、**これも「スクロールが効かない」に見える**。

### 🔥 再現できなかったものと、できたもの（ハーネスの限界を先に書く）

`console/scripts/fleetgraph/gestures.mjs` を新設（実タッチ × 反復 × 電話サイズ 430x860 ×
78 レーン）。最初に書いた「ピンチとスクロールを 6 巡」は**全部緑**だった——**CDP の合成タッチは
暗黙キャプチャの再ターゲットもページズームも再現しない**からである。

そこで**漏れた状態を直接作った**: `PointerEvent("pointerdown")` をキャンバスに dispatch して
pointerup を送らない。これで**一発で赤**になり、以降のチェックも全部赤（＝復帰しない、という
利用者の報告と同じ形）。修正後は全 11 項目 OK。陽性対照として `isPrimary` の clear を外すと
同じ 3 項目が赤に戻る。dom にも同じ筋の試験を 1 本置いた（jsdom に `PointerEvent` はある）。

📌 **ハーネスで再現しないことは「起きない」ではない。** 合成入力が通らない層（compositor の
ジェスチャ認識・暗黙キャプチャの再ターゲット）があるときは、**症状ではなく状態を作って測る**。

### 一般化

- **UI の状態機械は「イベントが来ない」を必ず含める。** ポインタ・WebSocket・子プロセス——
  終了通知が来ない経路がある以上、帳簿は**自己修復する形**（新しい開始で捨てる・時刻で腐らせる）で
  書く。来ることを前提にすると、1 回の取りこぼしが恒久的な故障になる。
- **`return` は症状を増幅する。** 分からない入力に対して何もしないより、**いちばん近い意味に
  落とす**ほうが、利用者にとっても調査する側にとっても情報量が多い。
