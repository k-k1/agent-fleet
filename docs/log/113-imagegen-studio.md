...
- **改訂 4（同日・レビュー反映）**: 別セッション `sds6jyw`（Opus）が読み取り専用でレビュー
  ...
  **TUI を P0 に**・worktree 既定 ON・番人の追加・知識の置き場）。§8 の問いはレビューの 5 件に
  差し替えた。
- **改訂 5（同日・5 巡目）**: レビューの問い 5 件が決着（claude は TUI で P0／worktree ON＋opencode
  Managed 除外／知識は `~/imagegen-knowledge/`・将来は ADR 0022 の git スナップショットで差分管理／
...
- **111 の 🔴（クロップ帯の出所が無い）は、クロップ廃止で消える**。ADR 0094 の改訂（PR #904・
  ...
  🟢（改訂 5）**出荷済み**: ADR 改訂の確定 PR #907・実装 PR #913・実機受け入れ log 112 §14（PR #914）。
  **キャンバスの設計は log 111 §10**（PR #915）: 帯も比率表も消え、書き出しは原寸（キャンバスの
  面積上限を超えるときだけ比率を保って縮める）、筆の太さは長辺比、EXIF の向きは実測済み、
  マスクは全ファミリーで「絵の同じ相対領域」。スタジオは 111 §10 をそのまま中列に置く。
...
- **knowledge root**: 🔴（レビュー）初稿の `~/.config/agent-fleet/…` は **Files ペインの拒否リスト
  ...
  **将来（利用者の要望・改訂 5）: claude のメモリと同じく git で差分管理する**——ADR 0022 が
  claude／codex のメモリを bare repo `af-memory.git` へ毎分スナップショットし、差分・復元・
  書き出し（bundle／tar）・取り込みを持つ。knowledge root を**その roots に 1 つ足す**だけで
  「いつ誰が何を書いたか」「あの日の知識に戻す」が手に入る（P2・ADR 0022 の roots は今
  `projects/*/memory` と `~/.codex/memories/` の glob＝1 か所の追加・symlink 不採用と secret 走査の
  規則をそのまま継ぐ）。
...
- **P1**: D7 の絵の履歴（`history.jsonl`・`GET /imagegen/history`・戻す／参照／並べる）、
  ...
  `needs_mask` の番人・キャンバス＝log 111 §10 の設計・前提は出荷済み）。
- **P2**: 会話からのスイープ（「cfg を 3 通り」→ エージェントが**行列の提案**を書き、人が
  「投入」）、D14 層 1 への昇格、**知識の git 差分管理**（ADR 0022 の roots に追加）、
  claude の Managed ドライバ（別 ADR）。
...
**すべて決着（5 巡目・2026-09-23）。**

- 3 巡目: model は人だけ／試走はエージェントにも（引数無し）／別 id／押した 1 回＝版／モーダル撤去／
  ファミリー表を Agent へ／用語はファミリー。
- 5 巡目（レビューの 5 件）: **(b) TUI を P0 に入れて claude を初日から** ✓／worktree 既定 ON・
  opencode Managed を除外 ✓／知識は `~/imagegen-knowledge/`（**将来は git で差分管理**＝ADR 0022 の
  roots・D14）✓／`op`・`inputs` の解放は番人まで保留 ✓／クロップは決着済み（#907・#913・#914・#915）
  ＝D13 の条件は満たされ、キャンバスは 111 §10 で着工可 ✓。

残る技術判断（利用者に問わない）は §9 と ADR 0100 の「未解決」。
