...
**⚠️ 共有していないことはエラーではない。** 宛先候補の取得は、そのセッションが誰にも共有されて
いなくても **200 ＋ 空の候補**で答える。エラーで返すと画面は「取得に失敗した」としか言えず、
利用者は次の一手（先に共有する）に辿り着けない —— 実利用で最初に踏まれたのがこれで、
読み込みが終わらないのと区別が付かなかった。実在しないセッションの 404 とは混ぜない。

**⚠️ 宛先候補の「読み」で在庫同期を毎回走らせない。** `syncCatalogLocked` は所有者 Agent への
往復 2 本で、`/repos` は作業コピーごとに git を回すため worktree が増えるほど重い。共有の
読み取りが `freshCatalog` で間引いているのと同じスロットル（`shareCatalogTTL`）に乗せる。
ただし**同期を省くことはできない**: repo/worktree スコープの共有は後から作られたセッションにも
動的に効くので、catalog に行が無いことは「共有されていない」の証明にならない。差し出しの
瞬間だけは exact に取り直す。
...
**⚠️ 「新しい worktree で開始する」は所有者の Workspace 内の話で、引き継ぎとは基準が違う。**
`git worktree add [-b <new>] <dir> <base>` は親の作業コピーで走り、`<base>` は**ローカル参照**に
解決される（fetch しない）。したがって **push していないコミットも一緒に来る**——同じマシンの
同じリポジトリだからである。持って行けないのは未コミットの変更だけ。一方メンバーへの
引き継ぎは相手のディスクから所有者のリポジトリが見えないので、基準は push 済みかどうかになる。
同じカードに 2 つの行き先が並ぶので、チェックボックスの文言は「このワークスペース内」と
明示して取り違えを防ぐ。
...
## 77.17 表示名の上限は 1 か所で決める（2026-08-28 実障害）

引き継ぎカードから起動した瞬間に
`worktree 起動に失敗: title is too long (max 80) or contains control characters`
で落ちる、という報告があった。worktree は無関係で、真因は**同じ「セッション表示名」を
層ごとに違う規則で検査していた**こと。

| 層 | 当時の上限 |
| --- | --- |
| `propose_session_handoff` の `title`（MCP） | 無制限 |
| 提案の保存 API（`session_handoff_proposal.go`） | 512 **バイト** |
| カードの編集欄 / 起動ダイアログ / 差し出しモーダル | 512 **文字** |
| `POST /sessions`（`cleanTitle`） | 80 **runes**・制御文字禁止 |

だから長い表示名は**保存も表示も編集も通り、起動を押した瞬間にだけ**弾かれる。エージェントは
もう居らず、利用者に出るのは「worktree 起動に失敗」だけ — 一番直しにくい形の失敗である
（日本語は 1 文字 3 バイトなので、512 バイト規則は実質 170 文字まで通していた）。

決めたこと:

- 上限は `cleanTitle`（`sessionTitleMaxRunes = 80`）が**唯一の正**。Console 側の写しは
  `console/src/lib/sessionTitle.ts` の 1 か所だけに置き、入力欄はそこから `maxLength` を取る。
- 提案の保存も MCP ツールも**同じ規則で断る**。長い名前はそれを書いたエージェントがまだ居る
  その場で直せる（ツールのスキーマにも 80 文字と書く）。
- `maxLength` は**打鍵にしか効かない**。引き継ぎ提案・作業項目・メンバー引き継ぎのように
  `value` へ流し込まれる初期値は `clampSessionTitle()` を通す（制御文字を落として 80 文字で
  切る）。既に保存されている長い提案もこれで起動できる。

## 77.18 受け取る口が 1 か所しか無かった（2026-09-01 実利用の報告）

「引き継ぎの通知は届くのに、**開始するボタンが見当たらない**。通知センターにも日本語ラベルが
出ていない」という報告。2 つとも「足りない」ではなく「**繋がっていない**」型の欠けだった。

**① 通知の行き先に受諾の口が無い。** `handoff-offer` の通知を押すと共有ビュー
（`SharedSessionView`）が開く（`notifications/store.ts`）。ところが受諾の面
（`HandoffInboxModal`）を開けるのは**共有セッション節の見出しに出る小さなアイコン 1 つだけ**
で、共有ビューにも、レール行の引き継ぎバッジ（`SharedProjectNode`）にも入口が無かった。
通知 → 共有ビュー → 行き止まり、が利用者の見た景色である。バッジは「届いている」と言うだけで
「ここから受け取れる」とは言っていなかった。

→ 共有ビューのヘッド直下に帯を出す（`.shared-view-handoff`）。押すと受信箱がその 1 件に
絞って開く。行は `HandoffOfferRow` として切り出し、受信箱と帯の両方が同じ面を使う。
帯を**転写の中に置かない**のは、転写が末尾へ追従するので「届いているのに画面外」になるため。

- 在庫のポーリングは共有ビュー側でも張る（レールが無い切り離しタブでも帯が出るように）。
  `startHandoffPolling` を**参照カウント**にして、開いたペインの数だけ GET が増えないようにした。
- 切り離しタブ（popout）には `StartHost` が載っていなかったので、受諾ボタンを押しても
  **起動モーダルが出ない**（target が無いあいだ何も描かない部品なので、載せても最小クロームの
  邪魔にはならない）。メモ送信からの起動も同じ穴だった。
- 作業コピーが 1 つも無い受け手には受諾ボタンが `disabled` になるだけで理由が出ていなかった
  → 「先に取り込んでください」を欄の下に出す。

**② 通知センターの行見出しに訳が無い。** 見出しは `NotificationCenter.tsx` の `labelKeys` を
引き、無ければ **kind をそのまま**出す。`handoff-offer` / `handoff-accepted` /
`handoff-expired` と `carried-interaction` はこの表から漏れていた（文言 `notif.*` は
`wording.ts` に揃っていたので、通知そのものは正しい日本語で読み上げられていた —— 行だけが
生の識別子だった）。表は `Record<string, MsgKey>` で kind は文字列リテラルなので、**型では
永遠に気付けない**。

→ 表を `wording.ts` へ移し（`NOTIFICATION_KIND_LABELS` / `notificationKindLabel`）、
`wording.test.ts` が **`wording.ts` のソースから `n.kind === "…"` を拾って**表と突き合わせる。
分岐を足して訳を忘れたらテストが落ちる。§77.9 の「新しい kind を足すときはここも足す」を
人の注意力ではなく検査に変えたもの。
