...
## 追記（2026-09-11）: 長いメモの畳み方と、1 件送信のスキャフォールド除去

左ペインで実際に使って出た 2 点を直した。

- **展開したメモが閉じられない**。本文クリックで全文、もう一度で編集、という 2 段階に
  「戻る」が無く、長いメモは開いたら最後、ペインを占有し続けた。本文下のメタ行に
  **折りたたむ**（`.memo-less`）を出し、行の ⋯ メニューにも同じ項目を置いた（展開中のみ）。
  ⋯ は行頭にあるので、本文がペインより高くなって `.memo-less` が画面外へ出ても届く。
  併せて `.memo-row.exp .memo-text` に `max-height: 50vh; overflow-y: auto`（`display: block`）
  を入れ、どれだけ長くてもペインの半分までに収める。実測: 2016px の本文が 247px の
  スクロール枠に収まり、直下の「折りたたむ」が見えたまま（headless Chromium の静的ハーネス）。
- **1 件だけの送信にカテゴリ見出しと項番が付く**。`## カテゴリ` と `1. ` は複数件を区別する
  ための足場で、1 件なら読み手が読み飛ばすだけの飾りになる。さらに複数行のメモは
  2 行目以降のインデントが落ちた箇条書きに見えてしまう。`len(memos) == 1` のときは見出し・
  項番・継続行のインデントをすべて落とし、本文そのままを送るようにした。
  **CP（`buildFlushMessage`）とコンソール（`composeMemoMessage`）の両方**を直している
  ——未編集送信が CP の flush とバイト一致する不変条件があり、MCP の `flush_memos` は
  CP 側だけを通るため。コンソール側は `SendMemoModal.tsx` から `compose.ts` に切り出して
  API クライアント無しで試験できるようにした。
