...
- 描画は開発 Workspace の headless Chromium で**確認済み**（2026-09-03）。`scripts/shots` の
  スタブ（実バンドル＋フィクスチャ・CP 不要）を流用した使い捨てスクリプトで、設定モーダルを
  各節で開いて測り、絵も見た。確かめたのは:
  - レールに「AI補助」が アシスタント の次に出て、選択できる（ja / en とも）。
  - **用途で推奨が別に解決されている**——短文生成 Claude=Haiku / 文章生成 Claude=Sonnet。
    これがこの分割の要点なので、絵で見えることに意味がある。
  - 機能トグル 5 行が注記つきで並ぶ。読み上げタブの「読み上げ言語」が音声エンジンの直下に出る。
  - 未解決の i18n キーが 0（カタログ欠落なら生キーが出る）。横あふれ 0。
  - `/api/tts/status` が無い（判定中）ときは `pollyAvailable() === null` で通常の注記のまま
    ——「未取得で決めつけない」経路も踏めている。
  ⚠️ 目視で見つかった 2 点は**どちらも本件の退行ではない**:
  - 「AI返信候補（✨）」の ✨ が豆腐になる。**このコンテナに絵文字フォントが無い**だけ
    （`fc-list | grep -ci emoji` = 0）。✨ は分割前の文言にも 3 箇所あり、UI 側でも
    ChatView / MirrorView が使っている。
  - 「エージェント優先順位」のラベルが 2 行に折れる。設定の `Row` のラベル列が 120px 固定
    だからで、**アシスタントタブの同じラベルでも同じ**（実測）。
- README 用のスクショ集（`docs/img`）は更新していない。設定タブは README に載せていない。
