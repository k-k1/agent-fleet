...
## 対応（2026-09-24、依頼元 sjkmbhc）

7 件とも実在を確かめ、同じブランチで直した。3 は本変更の前からあった競合だが、同じ種類の穴なので一緒に塞いだ。

1. 🔴 アーカイブ ID は `O_EXCL` で作り、取られていれば `-2`, `-3`… を付ける（`writeCleanupArchive`）。
   `trashSession` はセッション名ごとに直列化し、ロック内でメタを読み直す（負けた側は 404、他人のアーカイブを消さない）。
   陽性対照: ID の一意化だけで二重削除は安全になる（名前ロックだけ外してもテストは緑）ので、両方残した。
2. 🔴 退避の直前に claude の会話ログの大きさを控え、消す直前（メタのロック内）に「稼働中か」「ログが増えたか」を見直す。
   どちらかなら何も消さず、書いたアーカイブを取り下げて 409 `session_resumed` を返す。
   **残る窓**: メタを消した後に始まる再開はメタが無いので起動できないが、見直しからメタ削除までの間（メタのロック内）に
   別経路で起動処理が進んでいた場合は検出できない（再開はこのロックを取らない）。
3. 🔴 削除ゲート（`sessionx.WithDeletionGate`）を足した。`HandleDeleteRepo` はガードからセッションの後始末まで
   ゲートの中で走り、`HandleSessionLock`・`HandleRepoLock` もゲートを取る。ロック要求は削除の前に効くか、削除が終わるまで待つ。
4. 🟡 shell / ssm は作業コピーを消す**前に**ごみ箱へ移す（`trashShellSessionsUnder`）。移せなければ作業コピーを消さず
   500 `sessions_trash_failed`。
5. 🟡 `session.WriteMeta` が「このプロセスで人が消したメタ」（パスで記録）への書き込みを拒む。名前は再利用されないので、
   29 か所の書き戻しを一度に塞げる。ごみ箱からの復元（`CreateMetaIfAbsent`）だけが記録を解く。
   ロックの巻き戻しは、指摘の 2 経路（棚からの復帰・題名の提案）を `UpdateSessionMeta`（ロック内の読み書き）に移した。
   **残り**: ほかの書き戻しが古いスナップショットでロックを戻しうる一般の問題は、この変更の範囲外（以前からある）。
   → Issue #950 に起票（2026-09-24）
   → 🟢 解決（2026-09-25・Issue #950）: 読んでから書く経路はすべて `UpdateSessionMeta` を通し、関数はその経路が変える欄だけを
   今のメタに書く。`WriteSessionMetaKeepingLock`（決まった欄だけ今の値に戻して残りは古い値で書く）は廃止した。
   ロックに加えて、アーカイブ・題名・モデルなども巻き戻っていたため。素の `session.WriteMeta` は `locks.go`（ロック内）と
   新しい名前の初回書き込みだけに残し、`meta_write_sites_test.go` がファイルごとの数を固定する。
6. 🟡 Console の `sessionDelete` は、`stop` のとき 409 `session_running` を受けたら `/halt` して 1 回だけやり直す
   （古い Agent は `stop=1` を知らない）。2 の中止は別コード `session_resumed` にしたので、再開されたセッションを止めない。
7. 🟡 完全削除は台帳を消す前に残りのアーカイブを 1 回読み、同じセッションを持つものがあれば台帳を残す
   （一括削除は全件の後に 1 回だけ読む）。
