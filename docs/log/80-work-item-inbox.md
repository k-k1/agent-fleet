...
関連: [21](21-memo-queue.md)（左ペインの CP 永続セクションの雛形）/
...
**接続カードは「作業項目」タブに独立させた**（運用・監視の 1 カテゴリではなく）。あちらのカードは
...
## 80.21 取得元は select をやめる／面の名前を「課題管理」に揃える（実装済み）

**決定 21.1: 保存クエリの「取得元」は select ではなくセグメント（ボタン 3 つ）。** 理由は 2 つ。

1. **暗色テーマで選択肢が読めなかった。** 原因は「文字色が薄い」ではなく、
   `.wi-qform select` の背景が `var(--input-bg, transparent)` だったこと ——
   `--input-bg` はどこにも定義されていない変数で、実際に効いていたのは
   フォールバックの `transparent` である。閉じているときは親の地が透けて問題に見えず、
   **開いた瞬間だけ**ネイティブのドロップダウンが地に沈む。`color-scheme: dark` を敷いても
   背景を透明にした select には効かない。★ 同じ書き方が詳細モーダルの起動先ピッカー
   （`.wi-sfield select`）と報告モーダル（`.wi-rsession select`）にもあったので、
   3 か所とも `var(--panel2)` / `var(--fg)`（ui.css `.ui-field select` と同じ組み）に直した。
   **取得元だけ直しても他の 2 つで同じ苦情が出る**。
2. **3 つで増えない選択肢を畳む理由が無い。** provider は github / jira / bitbucket で、
   増えるとしても種の追加という別の話になる。他のダイアログ（`ShareCreateModal` の
   公開範囲など）が既に `ui-seg` + `seg-btn` でこの形をしているので、合わせただけで
   1 クリック減り、どれが選ばれているかが開かずに読める。
   ※ 「既定の作業コピー」は作業コピーの本数だけ増えるので select のまま。

**決定 21.2: 面の名前は「課題管理」（en: *Issue tracker*）に統一する。** 同じものを指す語が
レール（作業項目）と設定タブ（課題管理）で割れていて、どちらの設定なのかが名前から読めなかった。
**寄せる先はレールではなくタブの語**（利用者の指定）——「作業項目」は af が作った語で、
GitHub / Jira / Bitbucket のどれを触っている人にも通じる語は「課題管理 / Issue tracker」である。
そこで `wi.title`（レール）を改名し、設定タブは元のまま据え置いた。

★ **改名するのは UI から見える語だけ。** コード・API・このドキュメントの「作業項目 / work item」
（`wi.*` キー、`workItem*` の型と経路、docs/80 の題）は内部の呼び名として残す —— 面の語を
揃えるのが目的で、型名まで巻き込む改名に見合う利得が無い。追随させた文言は
`tracker.intro`（「左ペインの『課題管理』」）、`git.bb_token_hint_pr` /
`tenant.git_oauth_bb_scopes` / `ops.jira_hint`（「作業項目レール」→「課題管理レール」）、
`tenant.git_oauth_intro`（「接続 > 課題管理」）。

★ **3 つ目の名前を面に出さない。** 項目 1 件を指していた文言は、名前を言い換えるのではなく
**名前を必要としない形**にした —— 詳細モーダルの見出しは `wi.detail_title` = `{kind} {key}`
（＝「課題 acme/web#45」「プルリクエスト acme/web#45」）で、既にある種別ラベルをそのまま使う。
PR も並ぶ面なので「課題 {key}」に寄せると**嘘になる**ところで、種別で書けば正確かつ短い。
`wi.empty` は「該当する項目はありません」、`wi.load_failed` は「一覧を読み込めませんでした」。
エージェントへ渡すプロンプト（`wi.prompt_*`）は面の語ではないので触っていない。

## 80.22 同じチケットは 1 行（クエリが重なっても重複させない・実装済み）

実機の報告は「Jira と Bitbucket を 1 本ずつ登録したら、同じ Jira のタスクが 2 件ずつ出た」。
調べると保存クエリは **同じ JQL が 2 本**で、片方を消せば直る形だった —— が、直すべきなのは
利用者の登録ではなく**棚の方**である。

### 80.22.1 なぜ「利用者の入力ミス」で片付けないか
...
## 80.23 Bitbucket のクエリは書かせずに組み立てる（実装済み）

実機で 2 回続けて詰まった。1 回目は権限（§80.19.3 の予測どおり、コンシューマに
Pull requests: Read を足しても**既存トークンには古い権限が焼かれたまま**で、接続し直して解決）。
2 回目は**クエリが既定値のまま**だった —— 保存されていたのは
`workspace/repo reviewers.uuid="@me"` で、af は `workspace` というワークスペースの `repo` という
リポジトリを本当に探しに行き、404 を返していた。

### 80.23.1 ★ 「置き換えるべき語を既定値に置く」は実機で成立しなかった

§80.19.1 の目算は「消し忘れたときのエラーが何をし忘れたかを自分で言う」だった。実際には
`bitbucket has no workspace/repo visible to this connection` は**「別のエラーが出た」**と読まれた
——利用者から見れば、権限を直した直後に出た**2 つ目の障害**であって、「あなたの入力がまだ
サンプルのままです」とは読めない。**エラー文で埋め合わせられるのは、そもそも書ける物だけである。**

そして Bitbucket のクエリは、書ける物ではなかった:

- 先頭の `<workspace>[/<repo>]` は **Bitbucket の文法ではなく af の発明**（横断検索の無い API に
  「どこを見るか」を渡すための約束・§80.19.1）。Bitbucket のドキュメントを読んでも出てこない。
- レビュー待ちの式は `reviewers.uuid="{b8ceb65c-…}"` を要求する。`@me` の展開は足してあるが、
  **その書式を知らなければ `@me` に行き着けない**（§80.19.2）。

★ **GitHub の検索構文と JQL は本物の方言**で、利用者が普段 GitHub / Jira の画面で書いている物
だから素通しでよい。**Bitbucket だけが「af しか知らない書式」だった** —— 決定 17（クエリ先頭に
対象を書かせる）は保存形として正しいが、**それを人に手で書かせる**ところまでは含んでいない。

### 80.23.2 決めた形（決定 23）

**Bitbucket のときだけ、クエリ欄の代わりに「何を出すか × 対象」を選ばせて af が組み立てる。**

| 何を出すか | 対象 | 組み上がるクエリ |
|---|---|---|
| レビュー待ち | リポジトリ | `<ws>/<repo> reviewers.uuid="@me"` |
| リポジトリの PR | リポジトリ | `<ws>/<repo>` |
| 自分の PR | ワークスペース | `<ws>` |

- **出せるのはこの 3 つで全部である**（§80.19.1 の API がこれ以上を持っていない）。選択肢が
  API の限界と一致しているので、「4 つ目を選びたい」は起こらない。⚠️ 3 つ目だけ**ワークスペース
  単位**で、しかも**自分が作った PR に限る** —— 説明を 1 行そえるのは、これが af の制限ではなく
  Bitbucket の制限だから（人が悪い書き方をしたのではないと言えるようにしておく）。
- ★ **3 つは排他ではないので、セグメント（1 つ選ぶ）ではなくチェックにした。**「レビュー待ち」と
  「自分の PR」はどちらも見たい物で、1 本ずつ足させると**同じ対象を 2 回選ばせる**ことになる。
  チェックした分だけ 1 回でクエリができる（`bbQueries` が画面の順で並べ、同じ文字列は落とす）。
  対象の粒度は「選んだ中にリポジトリが要る意図が 1 つでもあるか」で決まり、
  ワークスペース単位の意図はそこから `<workspace>` を**畳んで**作る（対象を 2 つ選ばせない）。
  - 説明は**選んだ物だけでなく 3 つとも常に出す**。3 行しかないので、選ぶ前に違いが読める方が短い。
  - **表示名は 1 本のときだけ効く**（複数なら CP の既定＝クエリ文字列そのもの）。同じ名前の行が
    2 本並ぶと、どちらがどちらか行から読めなくなる。入力欄は無効化して 1 行断る。
  - ⚠️ **途中で 1 本弾かれても残りは足す。** 3 本のうち 1 本が断られたときに「何も増えていない」
    より「2 本増えて 1 本だけ断られた」の方が、次に何をすればよいかが分かる。
- **対象は接続から引く。** クローン用のリポジトリ一覧（`/connections/git/bitbucket.org/repos`）と
  同じ経路で、**新しい API も新しい資格情報も増えない**。ワークスペース一覧はその `full_name` を
  割って作る（ワークスペースだけを引く経路を足さない）。
- ★ **候補が 1 つなら選ばせない。** ワークスペースが 1 つの人（＝ほとんど）は、「自分の PR」を
  押した時点で対象が埋まっている。これが「自動解決」の実体で、Bitbucket から**引ける物は
  聞かない**という線である。
- **保存されるのは今までどおり 1 本の文字列。** 列も DTO もマイグレーションも増えず、Agent 側
  （`parseBitbucketWorkItemQuery`）は 1 行も変わらない。組み立てた文字列は**そのまま画面に出す**
  —— 後で行のエラーを読むときに突き合わせる物が要るのと、「af が何を保存したか」を隠さないため。
- ⚠️ **一覧が引けないときは手書きに落ちる。** この経路は Agent（＝ Workspace）に届く必要があり、
  **停止中は取れない**。レール本体は CP のキャッシュなので停止中でも開ける面であり、ここで
  詰まらせると「停止中の Workspace をチケットから起こす」（決定 1）を設定側で塞ぐことになる。
  取れなかったことは黙らず 1 行断る（黙って手書き欄だけ出すと、それが仕様に見える）。
- GitHub と Jira は**変えない**。方言を素通しするという決定 17 以前からの線はそのままで、
  ここで揃えると「af の画面で書ける物しか書けない」に反転してしまう。

### 80.23.3 検証

- `bitbucketQuery.test.ts`（純ロジック）: 3 つの意図の出力文字列そのもの・リポジトリが要る意図に
  ワークスペースを渡させない・未選択は空（＝保存させない）・応答の境界（`null` / `error` /
  形違いは「候補なし」に潰す・§80.17.5 の教訓）。
- `WorkItemsSection.dom.test.tsx`: 組み立て UI が出ると**手書き欄が消える**こと、対象を選ぶと
  `acme/web reviewers.uuid="@me"` が組み上がって**その文字列がそのまま保存に渡る**こと、
  **2 つチェックすると 1 回の追加でクエリが 2 本増える**（＋表示名が無効になる）こと、
  ワークスペースが 1 つなら選ばずに決まること、一覧が取れないときは手書きに落ちて断りが出ること。
- ★ 組み立て関数と自動選択をそれぞれ**壊して赤くなることを先に確認**した（[[test-exit-code-through-pipe]]
  と同じ用心 —— 新しい面のテストは「書いた側の思い込み」を映すだけになりやすい）。
- 実描画（headless Chromium・ja × dark / light）: 3 つのセグメントが折り返さずに収まり、
  組み立て結果と「クエリを自分で書く」導線が読めること。

## 80.24 プルリクエストの詳細は開いた時点で取り直す（実装済み）

利用者の指摘は 2 つだった。**「PR からセッションを開くことは無いと思う」**と、
**「それより、開いたあとに PR の最新情報を取り直して表示し、元のページへのリンクを張る方がよい」**。

### 80.24.1 リンクは既にあった。論点は「主役をどれにするか」

詳細モーダルにも行にも `item.url` への導線は最初からあり、3 provider とも実 URL が入っている
（Bitbucket は `links.html.href`）。だから足すものは無く、決めるのは**主ボタンをどちらにするか**
だった。結論は PR のときだけ **「GitHub / Bitbucket で開く」を主ボタン**にする。差分もレビューの
やり取りもコメント本文も af は持っていないので、正直な第一操作はそれらがある場所へ行くこと。

### 80.24.2 ★ 決定 20.1（詳細で 1 件取得はしない）の例外を、どこまで開けるか

20.1 が 1 件取得を退けた理由は 3 つあり、**そのどれも取り下げていない**：

| 20.1 の理由 | §80.24 でどう保っているか |
|---|---|
| CP は本文を持てない（決定 2） | 取り直しでも**本文は取らない**。`fields=` の射影がその約束を要求に書いたもの |
| 停止中は必ず失敗する＝壊れて見える | 停止中は 409 で**取得済みの行をそのまま出し、1 行でどちらを見ているか言う**。詳細がその取得に**依存しない** |
| チケットビュアーは作らない | 出すのは**判断に要る 6 つ**だけ（下書き・競合・レビュー・CI・base←head・差分量）。本文もコメントも出さない |

★ 課題では例外にしない。**課題にはこの 6 つが無い**ので、キャッシュの行だけで詳細に出すものは
尽きている。逆に PR ではこの 6 つが**5 分の更新間隔の中で動く**。「5 分前の CI」でレビューを
拾うか決めることはできず、キャッシュに載せれば決定 2 の保持範囲に**レビュー状況と CI という
新しい欄**を増やすことになる（＝キャッシュが provider の PR オブジェクトの写しになる）。

費用が抑えられるのは**人が詳細を開いたときだけ走る**からで、一覧と違ってタイマーでは動かない。
GitHub は 3 呼び出し（PR・reviews・head の check-runs）、Bitbucket は 3 呼び出し
（PR・statuses・diffstat）で、後ろの 2 本は**best effort**（権限や CI が無くても PR は出す）。

### 80.24.3 畳みはしたが、消さなかった

「PR からセッションを開くことは無い」はそのとおりの使い方だが、**消すと決定 19.1 の前提が崩れる**
—— PR を一覧に出す理由そのものが「人の PR のレビューは、こちら側にセッションがまだ無い作業」
だった。レビューにも作業コピーは要るし、着手済み台帳（決定 5）が「同じ PR を 2 人が拾う」のを
止める働きも PR でだけ消えることになる。だから `<details>` で畳み、開始ボタンはその中に置いた。

### 80.24.4 ★ 折り畳みの中の値で起動させない

最初の案は「footer の 開始 は残したまま、選択欄だけ畳む」だった。これだと**見えていない
リポジトリ／worktree の既定値で worktree が生える**。畳んだ中にボタンを置き、footer の主ボタンは
外部リンクにしたのは、**押す物と、押したときに効く選択が同じ面にある**ようにするため
（§80.20 で「行のボタン」を外したときと同じ線）。

### 80.24.5 一行に畳むときの規則（4 つの写像すべてに同じ罠がある）

provider の配列を 1 行に畳む箇所が 4 つある。畳んだ先は必ず**人が行動に使う主張**になるので、
規則を揃えた：

- **無いものを緑にしない。** check-run が 0 件／Bitbucket の status が 0 件は `""`（何も言わない）で、
  `success` ではない。何も走っていない PR は何にも通っていない。
- **1 本の赤は 40 本の緑に勝つ。** 優先順位は failure > pending > success。
- **中断は不合格。** Bitbucket の `STOPPED` は failed に数える（誰も完走していない）。
- **判定できないものは「判定できません」と言う。** GitHub の `mergeable` は計算中 null、Bitbucket は
  マージタスクを起こさないと答えない。どちらも `unknown` で、`clean` に寄せない
  （当たらないブランチへ人を送る）。
- **レビューは最後の 1 回だけを数える。** 同じ人が何度でもレビューでき、`COMMENTED` は判断ではなく、
  `DISMISSED` は前の承認を消す。再依頼された人は、過去に承認していても **pending が勝つ**
  （いま待っているのはその人だから）。Bitbucket は participants から **REVIEWER だけ**を採る
  （コメントした人まで数えると「レビュアー 6 人」が行動に使えない数字になる）。

### 80.24.6 検証

- Go: 4 つの写像（GitHub reviews / check-runs、Bitbucket participants / statuses）と、本文が
  1 バイトも出ないことを JSON で固定。★ `parseGitHubCheckRuns` の「0 件は緑ではない」を
  **わざと壊して赤になることを先に確認**した。
- CP: 停止中は 409 で **Agent を叩かず・キャッシュに 1 行も書かない**こと、旧 Agent の 404 は
  「再起動が要る」と言い換えること。
- Console: 取り直した値が出ること／停止中は取得済みの行のままで何も捏造しないこと／失敗時に
  再取得が出ること／**課題では取り直さない**こと。★ 主ボタンを開始に戻す改変で赤になることを確認。
- ⚠️ 実描画（headless Chromium）では見ていない。ここまでは jsdom である。
...
3. **✅ PR を項目として出すか（P2.6 で決着）。** 結論は「**出す。ただし on/off の設定は作らない**」。
   ...
   🔴 **訂正（2026-09-11）: 上の「GitHub でも `is:pr` を書かなければ PR は出ない」は誤り。**
   `/search/issues` は `is:pr` が無くても PR を返す（実測: `repo:<owner>/<repo> is:open` の
   応答に PR 1 件と Issue 4 件が混在）。**PR が出ない真因は既定クエリの `assignee:` で**、
   GitHub は PR の作者を assignee にしないため（実測: 作者本人のアカウントで
   ...
   **なぜ間違えたか**: この行は GitHub の Web UI が Issues と Pull requests を別タブに分けて
   ...
   UI の見た目から推測してはいけない**（§80.19.1 の Bitbucket 404 と同じ型の誤りで、あちらは
   swagger.json で、ここは 1 本の検索クエリで確かめられた）。
