...
## 10. キャンセルの直後だけ「送信できない」— 保留は消したのに**状態**が残る（2026-09-04、利用者報告・修正済み）

**症状（利用者報告）**: 「AUQ をキャンセルしたあと、メッセージが送信できなくなる」。
ミラーの質問カードは §9 のとおり決着済み（「エージェント側で却下されました」）になり、
承認カードも質問カードも**1枚も出ていない**のに、コンポーザから送るたびに

> エージェントが許可の判断待ちです。許可カードから許可・拒否してから送信してください
> （テキストは許可メニューに飲まれ、そのまま許可になります）。

＝ Agent の `permission_pending`（`promptBlocker`）で断られる。**決めるべきカードが
どこにも無いのに「カードから決めろ」と言われる**ので、利用者側に打つ手がない。

**根因**: §9 の掃除は**ペイロードだけ**を消して、`session-status` に残った
`state:"permission"` を消さなかった。

- 表示（カード）は `pending-question` / `pending-perm` を見るが、**送信の可否
  （`promptBlocker`）は生の state を見る**。掃除は前者だけを畳んだので、
  「カードは無いのに state はモーダルを名乗る」という食い違いが残った。
- state を書き直せる者がいない: キャンセルはツールの却下なので PostToolUse は鳴らない
  （§9 と同じ理由）。残る唯一の書き手はペイン由来の自己修復（`WireLive` /
  `DriveState` の `pane.IdleSettled` → `HealIdle`）だが、これは**絵が変わらないまま
  `idleSettleWindow` = 45 秒**続いて初めて成立する。つまり最良でも 45 秒詰まり、
  ペインを描き変え続ける何か（BG 実行など）があれば**永久に解けない**。

**修正**: `sweepSettledState`（`session_transcript.go`）を `sweepSettledPending` の末尾に
追加し、ペイロードと**同じ決着・同じ時計の規則**で state も畳む。歯止めは3つ:

1. モーダル状態（question / plan / permission）のときだけ触る。working / idle は
   ターンの話であってモーダルの話ではない。
2. 決着**より前**に書かれた state だけ（`status.StateAt` = mtime）。決着の後に立った
   モーダルは本物。§9 の 106〜122ms の隙にも同じ歯止めが効く。
3. ペイロードが1つでも残っていれば触らない。「決着より後の許可は本物なので残す」で
   生き残った承認ダイアログは、state に断ってもらわないと自由文がメニューに飲まれる。

消す方向（`status.Remove` ＝ `LiveState` の既定 idle）にしたのは安全だから: 実はまだ
走っていたターンなら、次のポーリングの reverse-heal（`pane.Busy` → working）が1回で
戻す。逆に working を書くと、キャンセルで終わったターンが 進行中 に貼り付き、
Workspace が止まらない（課金）ほうへ倒れる。

回帰テストは `session_transcript_test.go`:
`TestSweepSettledPending` の4件（伏せていた許可状態も掃除される／生きた許可が残っている
なら state も残す／決着より後の state は残す／モーダルでない state は触らない）と、
**層をまたぐ** `TestCancelledInteractionFreesComposer`（掃除を通したあと
`promptBlocker` が実際に開くこと）。後者が本命 — 表示側だけのテストでは、まさに
今回の「カードは消えたのに送信は断られる」食い違いを見逃す。

**教訓**: **保留を消す掃除は、状態も一緒に消さなければ「決めるカードが無いのに断る」を
作る。** 表示（ペイロード）と判定（state）は別の場所に住んでいるので、片方だけを畳むと、
症状は「何も出ていないのに操作を拒否される」という、UI からは原因の見えない形になる。
§9 の教訓（表示の重複除去で状態の後始末を代用するな）の裏返しで、後始末は**その状態を
読むすべての層**まで届いて初めて終わる。
