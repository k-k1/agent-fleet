...
ADR 0073 決定 6 に新しい補遺は追加しない——変えていないので不要である。
...
- ADR 0073（[ja](../decisions/0073-session-spawned-sessions.ja.md) /
  [en](../decisions/0073-session-spawned-sessions.md)）の 2026-09-10 補遺に、根拠として引いていた
  RSS 単純和の数値が本節で実測・訂正されたことを指す 1 行だけ足した（決定の数値・結論は
  変えていない）。
