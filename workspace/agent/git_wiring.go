...
//
// 逆向き（main → gitx）は alias_git.go。**なぜ 2 枚に分かれているか**: エイリアスは
// ウェーブ境界で丸ごと剥がして消えるが、配線は残る（gitx が main の各家系を呼ぶ関係
// そのものは移送で消えない）。同じファイルに置くと、回収のときに「消す行」と
// 「残す行」が混ざる。mcpx が alias_mcp.go → mcp_wiring.go で辿った道と同じ形にした。
//
