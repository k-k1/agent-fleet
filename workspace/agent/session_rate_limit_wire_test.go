...
func atLimitProbe(t *testing.T, atLimit bool) *int {
	t.Helper()
	n := 0
	orig := claudeUsageLimitAbort
	claudeUsageLimitAbort = func(string) (claude.Abort, bool) {
		n++
		return claude.Abort{Msg: "You've hit your session limit."}, atLimit
	}
