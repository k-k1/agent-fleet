...
func TestCodexRateLimits(t *testing.T) {
	// Parse via the same path handleCodexUsage uses.
	u, ok := codexUsageFromRolloutBytes([]byte(codexRateLimitLine))
	if !ok || !u.OK {
		t.Fatalf("expected a usage reading, got ok=%v u=%+v", ok, u)
	}
	if u.FiveHour == nil || u.FiveHour.Pct != 3.0 {
		t.Fatalf("fiveHour = %+v, want pct 3.0", u.FiveHour)
	}
	if u.SevenDay == nil || u.SevenDay.Pct != 0.0 {
		t.Fatalf("sevenDay = %+v, want pct 0.0", u.SevenDay)
	}
	if u.PlanType != "plus" {
		t.Fatalf("planType = %q, want plus", u.PlanType)
	}
	// resets_at (epoch) → RFC3339; just assert non-empty and parseable downstream.
...
func TestReadCodexUsageFromDir(t *testing.T) {
	...
	if !u.OK || u.FiveHour == nil || u.FiveHour.Pct != 3.0 {
		t.Fatalf("readCodexUsage = %+v, want ok with fiveHour 3.0", u)
	}
}
