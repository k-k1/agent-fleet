...
// opencode.json carrying the chat policy: file edits and shell denied (the opencode
// analog of chatToolLimits), the local Agent Fleet MCP server for af grants, and the
// attached registry servers (docs/48 §7).
//
// opencode takes its config from the DIR, not per invocation, so conversations that
// resolve to different configs must not share one — otherwise a concurrent turn's
// rewrite decides which servers this turn gets. The dir key is therefore the tool
// grant plus a digest of the resolved server set (docs/48 §7 制約), not the grant
// alone. Falls back to the shared chat workdir when the dir can't be prepared
// (opencode then runs with its defaults).
