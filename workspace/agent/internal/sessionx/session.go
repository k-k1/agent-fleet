...
func wireSession(m session.Meta, alive bool) session.Session {
	...
	// before LastTurnEndAt is read below so the very poll that observes the end also reports it.
	if alive {
		recordPolledTurnEnd(m, li.State)
	}
	s := session.Session{
		...
		Studio: m.Studio, InitialPromptState: m.InitialPromptState,
