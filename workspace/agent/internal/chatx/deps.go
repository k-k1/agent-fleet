...
type Deps struct {
	...
	// EffectiveHidden / ModelHiddenIn judge candidates against a hidden list (and claude's
	// registered models) a request supplies (sessionx.EffectiveHiddenWith / ModelHiddenIn) —
	// RecommendedModelsWithHidden.
	EffectiveHidden func(kind string, raw, claudeCustom []string) []string
