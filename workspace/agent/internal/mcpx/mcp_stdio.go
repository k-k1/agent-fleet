...
func mcpStdioFleetSpawnTools() []map[string]any {
	return []map[string]any{
		...
		},
		{
			...
			"description": "Agent Fleet: report each agent CLI's subscription usage and rate limits (claude / codex / agy / muse). " +
...
func mcpStdioImageGenTools(offer imageGenOffer) []map[string]any {
	...
	// negative_prompt, on the same rule and for a sharper reason: writing what to avoid INTO the
	// prompt conditions on it instead ("no text" puts text in the picture), so a route without a
	// negative branch cannot be worked around from the caller's side. Offered where at least one
	// model has one; a model that has not says so in warnings.
	if offer.Negative {
...
func mcpStdioCall(req mcpReq) []byte {
	...
	case "create_session":
		...
		// so there is no choice to bias). lcpp and muse are absent for the mirror-image reason —
		// neither has a tui route at all (ADR 0093 決定 2, ADR 0095 decision 2), so there is
		// equally no choice to bias for them: the
