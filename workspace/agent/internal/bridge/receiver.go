...
// with its own lifecycle (bridge.go contract 1 note).
//
// Security (ADR0020 contract 5, the ONLY defense): route ONLY the bound user's messages, and
...
// operator-injected turns (docs/log/37 additional requirements). Matches package main's turnSourceDiscord.
...
type ReceiverDeps struct {
	...
	// decision) to the session, structurally — never via free-text (contract 6). It returns a
	...
	// the built-in operator assistant conversation (docs/log/37, P3 pulled forward) and returns the
...
// Slack Socket Mode), sharing the same provider-neutral deps (docs/log/37 Slack follow-on). No-op
...
// a bound user (contract 5) — the mention target (channel mode, auto-filled owner) or the DM user.
...
//
// stop waits for the goroutine to exit, and is idempotent. Returning right after
// close(done) left an in-flight typing POST running past "stopped", so the typing
// indicator lingered for a moment after the reply was posted. In tests it showed up far
// more sharply: the goroutine read a discordAPIBase the NEXT test had swapped in, and
// `-race` reported it as a data race. Same class as the slack post leak, a goroutine that
...
// routeInbound. Same sole defense (contract 5): only the bound user's clicks are ever
