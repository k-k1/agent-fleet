...
// saw, so there is nothing in the injection store to match them against (docs/58 §58.16).
...
type Turn struct {
	...
	// (docs/30 ②), so the mirror can badge operator-driven prompts distinctly. The full
	...
	// （docs/58 §58.16）で、そこでは名前の出どころが `origin.name` しか無い。
