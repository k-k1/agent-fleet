...
// control-plane 側と同じ内容（ADR 0012 決定 3 でモジュール横断の共有パッケージは見送り。
// writeJSON / wire_golden_test.go / routes_golden_test.go と同じく 2 モジュールに写しを置く）。
//
...
// Agent 側は JSON 書き出し 285 地点のうち 226（79%）が map 直返しで、
// `wire.golden` が撮っている型は 5 個しかない。
//
