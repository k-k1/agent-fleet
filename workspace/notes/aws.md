...
| "not defined in …" / "not an SSO profile" / "has no SSO account and role" / "the AWS CLI cannot read …" / "sso_session is empty" / "it sets <sso field> … but its sso-session …" / "… are only in [sso-session …]" | The profile or the file is not usable as is. | Report the message; the user fixes Settings or the file. |
