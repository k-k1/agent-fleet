...
| "not exported: [DEFAULT] <key> = … in ~/.aws/config differs from this profile" (in `--list`) | A `[DEFAULT]` line makes the AWS CLI refuse that Settings profile. | Report it; the user removes that line from `[DEFAULT]`. |
