...
| "not exported: no account and role in Settings" (in `--list`) / "has no account and role in Settings, so it is not exported" | A Settings profile without both; `aws --profile` would run as the workspace's own role, so it is not exported. | Report it; the user sets the account and role in Settings > SSM. Never use that name with bare `aws`. |
